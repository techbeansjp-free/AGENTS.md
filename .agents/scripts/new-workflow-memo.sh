#!/usr/bin/env bash
# new-workflow-memo.sh — .workflow/{issue}/memo/ の単一作成経路
# 目的:
...
# - .workflow/{issue}/memo/ ディレクトリを作成（存在しなければ）。
...
# - .workflow/**/memo/ 以下のファイルを、Write/Edit で直接手入力のプレフィックス付きファイル名として作成してはならない。
