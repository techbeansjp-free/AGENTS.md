...
- ここは宣言のみ。**いつ・どう移動するか（完了判定との接続を含むライフサイクル詳細）は [workflow/PHASES.md](../workflow/PHASES.md) §完了 issue の close 移動に委譲する。** 自己拡張固有の配置先は [.agents-project/自己拡張ワークフロー.md](../../.agents-project/自己拡張ワークフロー.md) を参照（1 ファイル 1 責務・重複禁止）。
