...
- **MEMORY は「参照」であって「規則」ではない**。共通規則は CORE / AGENTS / WORKFLOW / RULES に置く。プロジェクト固有規則は `.agents-project/` に置き、`.agents/` より優先される（存在する場合は `.agents-project/` を先に参照する）。memory に書いた内容で振る舞いを強制しない。
