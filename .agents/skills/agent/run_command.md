...
- **実装前のドキュメントレビュー**: ユーザーから「ドキュメントレビュー」「00/01/02/03 のレビュー」を依頼された場合、**実装（implement-feature の成果物）がまだ完了していなければ**、verify-and-close を委譲して 04_review を作成してはならない。成果物は **.workflow/{issue}/memo/** に YYYYMMDD_HHMMSS_ プレフィックスの memo として記録させること（PHASES §レビュー成果物の配置ルール）。実装完了後にのみ verify-and-close を委譲し 04_review を成果物に含める。
