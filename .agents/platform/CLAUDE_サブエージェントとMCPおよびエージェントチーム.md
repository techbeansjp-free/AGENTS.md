...
**サブエージェント運用（MVP）**: メインの入口は [.agents/boot/CORE.md](../boot/CORE.md) と [.agents/boot/LOAD_POLICY.md](../boot/LOAD_POLICY.md)。フェーズ→worker の対応は [boot/LOAD_POLICY.md](../boot/LOAD_POLICY.md) の表を参照。6 人格の定義は [workers/README.md](../workers/README.md)（01〜06）。**トレーサビリティは必須**。各サブ実行後に書記へログを委譲する。詳細は [書記役とログ委譲.md](../scribe/書記役とログ委譲.md)。
...
**MVP 運用時**: フェーズごとの委譲先は [workers/README.md](../workers/README.md) の 6 人格（01 要件BDDリード 〜 06 書記）と [boot/LOAD_POLICY.md](../boot/LOAD_POLICY.md) の「フェーズ→worker」に従う。以下 4 種は役割別の例であり、MVP の 6 人格と対応づけて利用する。
...
**抜かし防止**: サブエージェントが工程を飛ばさないよう、定義に**提出物・証跡のルール**を 1 行で参照させることを推奨する。例: 「各 Phase 終了時に提出物を列挙し根拠を示す。詳細は .agents/サブエージェント抜かし防止.md を参照。」（[サブエージェント抜かし防止.md](../rules/サブエージェント抜かし防止.md) セクション 4 参照）
...
- **AGENTS での利用**: ワークフロー手順・ドキュメントルール・レビュー観点などをスキルにまとめ、プロジェクトまたは個人スキルとして配置することを推奨。**重要**: **スキル内に AGENTS-spec の詳細をコピーせず、AGENTS-spec のファイル（`AGENTS.md`, `.agents/boot/CORE.md`, `.agents/boot/LOAD_POLICY.md`, `.agents/rules/` 等）を参照する形にする**。これにより、AGENTS-spec を更新すれば自動的にスキルにも反映される。
...
## 6. AGENTS-spec 刷新時のグローバル設定の扱い

**AGENTS-spec を刷新した場合**（boot / LOAD_POLICY / delegate_to_sub 一本化・6 人格など構成変更を含む場合）、グローバルに置いた **AGENTS 用**のサブエージェント・スキルは、古いパスや旧 worker 名を参照したままになっている可能性が高い。**一度削除し、刷新後の spec に合わせて作り直すことを推奨する。**

### 6.1 削除対象（AGENTS 用のみ）

**削除してよいもの**（AGENTS 規約用に置いたものだけ。他用途のスキル・サブは残す）:

| 種別 | Claude Code | Cursor |
|------|-------------|--------|
| スキル | `~/.claude/skills/agents-follow/` | `~/.cursor/skills/agents-follow/` |
| サブエージェント | `~/.claude/agents/` 配下の agents-explore, agents-docs, agents-review, agents-implement 等（AGENTS 用のもの） | `.cursor/agents/` または `~/.claude/agents/` の AGENTS 用サブ |

**残すもの**: MCP 設定、AGENTS と無関係のスキル（例: create-rule, update-cursor-settings 等）、他プロジェクト用のサブエージェント。

### 6.2 作り直すときの参照先（刷新後）

- **メインの入口**: [boot/CORE.md](../boot/CORE.md)、[boot/LOAD_POLICY.md](../boot/LOAD_POLICY.md)
- **委譲の唯一の入口**: [skills/agent/delegate_to_sub.md](../skills/agent/delegate_to_sub.md)
- **6 人格の定義**: [workers/README.md](../workers/README.md)（01_要件BDDリード 〜 06_書記）
- **サブに渡すコンテキスト**: [boot/SUBAGENT_PACK.md](../boot/SUBAGENT_PACK.md)、[boot/SUBAGENT_MINIMUM.md](../boot/SUBAGENT_MINIMUM.md)

グローバルに再配置する場合は、上記パスを**参照する**形で定義し、AGENTS-spec 内に詳細をコピーしない。
