...
**現状の実装について**: PreToolUse.sh と PostToolUse.sh は、案内メッセージの出力のみで **exit 0** で終了しており、**違反時の exit 1 による物理的拒否は実装されていない**。その場で違反を止めるのではなく、**CI（audit.sh）で事後検知する構成**である。物理強制を実現するには、プラットフォームがツール名・対象パス等をフックに渡す仕様が必要であり、その前提設計は **DESIGN.md および本 README §現状の強制レベル・矯正するもの** を参照すること。
...
| 4 | **docs 更新要否未記載** | 実装・設計変更に伴うシステム仕様書（docs/）の更新要否が判定・記載されていない。04_review §9 および DOCS_RULES で求められる記載が欠落している。 | 04_review に追記 または 03/該当 issue |
