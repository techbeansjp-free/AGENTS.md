...
**現状の実装について**: PreToolUse.sh と PostToolUse.sh は、案内メッセージの出力のみで **exit 0** で終了しており、**違反時の exit 1 による物理的拒否は実装されていない**。その場で違反を止めるのではなく、**CI（audit.sh）で事後検知する構成**である。物理強制を実現するには、プラットフォームがツール名・対象パス等をフックに渡す仕様が必要であり、その前提設計は **DESIGN.md および本 README の「矯正するもの（物理強制の例）」** を参照すること。
