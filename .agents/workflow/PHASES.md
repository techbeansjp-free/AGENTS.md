...
- **概要**: ユーザーが「この要件で issue を作成して」「issueを作成して」等と依頼した場合、メインエージェントは **issue 作成をサブに自動委譲**する（サブへの指示文案だけを返して終了してはならない）。サブは `.workflow/<timestamp>_<title>/` 配下に 00_要求定義.md 等を作成し、タイトル・概要・保存場所を返却する。
