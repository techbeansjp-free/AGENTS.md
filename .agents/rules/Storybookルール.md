...
> **サブエージェント運用時**: Storybook 関連タスクを委譲された worker が本ルールを参照する。
...
> **汎用版**: 本セクションはどのプロジェクトにも適用できる一般的な手順です。各プロジェクトでは、title の形式（ルート名・階層名）をプロジェクトの論理構造に合わせて定義してください。

サイドバーは **Docs（ドキュメント）を常に最優先**し、そのうえで**連番順（01 → 02 → 03 → …）**で表示する。実装は次のとおり。

1. **`.storybook/main.ts` の `stories`**
   - **glob で一括指定**する（例: `../src/**/*.mdx`, `../src/**/*.stories.@(js|jsx|mjs|ts|tsx)`）。
...
3. **storySort を書くときの注意（Storybook 7 以降）**
   ...
   - **型注釈を付けない**。シリアライズ時に `:` が SyntaxError の原因になる。

- **新規ストーリー追加時**: 該当コンポーネントに `*.stories.tsx` を追加し、**meta.title を上記形式で付与するだけ**。main.ts の編集は不要。
