...
3. **配布**: setup 脚本（scripts/setup.sh の sync_skills）で、`.agents/skills/{domain}/{capability}/` を対象プラットフォームの `skills/{domain}__{capability}/` に**コピー**する。例: Claude 用なら `.claude/skills/requirements__write-bdd/`。同名 capability の domain 間衝突を防ぐためプレフィックスを採用済み。
