...
1. **正本の場所**: `.agents/skills/{domain}/{capability}/` に **SKILL.md** を置く。Agent Skills 形式（YAML frontmatter: name, description ＋ 本文に手順・制約・成果物）。**README.md** は索引・詳細用として残す。
...
3. **配布**: setup 脚本（scripts/setup-agents-spec.sh の sync_skills）で、`.agents/skills/{domain}/{capability}/` を対象プラットフォームの `skills/{domain}__{capability}/` に**コピー**する。例: Claude 用なら `.claude/skills/requirements__write-bdd/`。同名 capability の domain 間衝突を防ぐためプレフィックスを採用済み。
