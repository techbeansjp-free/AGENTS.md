...
**責務**: サブ委譲時の reasoning effort（推論深度）の役割別静的割当の**抽象原則のみ**を規定する。具体的な role×effort 対応表・モデル名・閾値は**コアに置かず** `.agents-project/` に委ねる（PF 中立性・CORE.md §ルールの優先順位）。model ティア選定（[MODEL_SELECTION.md](MODEL_SELECTION.md)）とは**独立した別次元**の軸であることを本ファイルで明記する。
...
- **`.agents-project/`（採用先で定義）**: 具体 role×effort 対応表・モデル名・閾値。**対応表はコアに置かない（PF 中立性）。**
