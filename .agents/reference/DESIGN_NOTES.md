...
**壊れない設計の流れ**: メイン → delegate_to_sub（入力を正規化）→ SUBAGENT_PACK（最小コンテキスト注入）→ サブ実行（readonly or スコープ限定）→ 結果を親へ返却 → 書記サブへ委譲 → **workflow.db に 1 件記録**（`.workflow/**/logs/` は廃止・使用禁止）。**B と C で「毎回同じ形に整形してから渡す」**ことが鍵。
