...
**ログは SQLite（workflow.db）を用いる**。書記（write-workflow-log）のみが書き込む。**workflow.db が無ければ作成する**（初回実行時に schema.md のスキーマで作成）。

---

## 配置

- **配置先**: プロジェクトルートの **.workflow/workflow.db** を推奨。または .agents/ledger/ 配下など、書記のみが書き込む一意のパスをプロジェクトで定める。
- **スキーマ定義ファイル**: 本ディレクトリの [schema.md](schema.md) を参照する。write-workflow-log capability はこのスキーマに従って記録する。**DB が存在しなければ、schema.md の CREATE TABLE を実行して作成すること。**
- **Git 管理・配布**: **workflow.db の実体および SQLite WAL モードの sidecar（workflow.db-wal, workflow.db-shm）は Git 管理対象外とする。** .workflow/.gitignore に `workflow.db`, `workflow.db-wal`, `workflow.db-shm` を記載すること。テンプレート・spec の配布物に DB 実体を含めない。証跡の正本はローカルまたは CI で生成される DB であり、リポジトリにはスキーマ定義のみを管理する。
...
- **workflow.db に必ず書くもの**: schema.md で定義した必須カラム（ts_utc, command, issue_path, summary, changed_files, dod_met 等）。1 実行 = 1 行を基本とする。
