...
test('gate workflow: 認証情報未設定はneutral Check Runとexit 78で終了し、実行後エラーのfailure経路を残す', () => {
  ...
  assert.match(workflow, /name: End gate job neutrally when reviewer credentials are unavailable[\s\S]*?run: exit 78/);
