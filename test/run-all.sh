...
#   | test-model-tier-gate.sh             | bash・sqlite3（audit.sh #38 check_model_tier_recorded の判定・grandfather・fail-open を隔離 DB で検証。sqlite3 不在は exit 2 で SKIP） |
...
#   | test-worktree-record-guard.sh       | bash・git（記録 commit・push 漏れ検知 R9/#41・共有 lib worktree_record_guard.sh を直接 source／bash 実行＋正本 hook を stdin JSON で駆動・隔離 git リポ／ローカル bare origin・tmp 隔離） |
...
#   | test-release-workflow-trigger.sh    | bash のみ（release.yml の push トリガー・paths・RELEASE_ENABLED 静的シミュレーション。python3/PyYAML 不在時は該当シナリオのみ内部でインライン SKIP） |
#
...
test-worktree-record-guard|test-worktree-record-guard.sh|bash git
...
test-release-workflow-trigger|test-release-workflow-trigger.sh|bash
EOF
}
