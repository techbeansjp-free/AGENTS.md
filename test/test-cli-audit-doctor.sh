...
#
# ユースケース（このテストファイル全体）:
#   (1) agents-md audit が .agent-skill-chain/source/enforcement/ci/audit.sh の終了コードを透過する（薄ラッパー）。
...
#     scripts をオーバーレイする。本開発リポの .agent-skill-chain/source/ .agent-skill-chain/runtime/ workflow.db を変更しない。
#
# 使い方:
...
cp "$REPO_ROOT/.agent-skill-chain/source/enforcement/ci/audit.sh" "$TMP/.agent-skill-chain/source/enforcement/ci/audit.sh"
...
#   スクリプトは隔離環境の正本（$TMP/.agent-skill-chain/source/scripts/write-workflow-log.sh）を絶対パスで呼ぶ。
#   スキーマは同スクリプトの AGENTS_ROOT（$TMP/.agents）から解決され、DB は cwd（$root）の .workflow に作られる。
build_healthy_db() {
  local root="$1"
  local wwl="$TMP/.agent-skill-chain/source/scripts/write-workflow-log.sh"
  mkdir -p "$root/.agent-skill-chain/runtime"
...
doctor_tamper_hash() {
  ...
  local T="$TMP/tamper"
  rm -rf "$T"; mkdir -p "$T/.agent-skill-chain/runtime"; cp "$H/.agent-skill-chain/runtime/workflow.db" "$T/.agent-skill-chain/runtime/workflow.db"; touch "$T/AGENTS.md"
  cp -r "$H/.agents" "$T/.agents" 2>/dev/null || true
  local before after out
  before="$(sha256sum "$T/.agent-skill-chain/runtime/workflow.db" | awk '{print $1}')"
  sqlite3 "$T/.agent-skill-chain/runtime/workflow.db" "UPDATE workflow_log SET entry_hash='deadbeefdeadbeef' WHERE rowid=(SELECT rowid FROM workflow_log ORDER BY rowid LIMIT 1);"
  # When: doctor 実行
  ...
  after="$(sha256sum "$T/.agent-skill-chain/runtime/workflow.db" | awk '{print $1}')"
  local post_doctor; post_doctor="$(sha256sum "$T/.agent-skill-chain/runtime/workflow.db" | awk '{print $1}')"
...
doctor_delete_row_dangling() {
  ...
  local D="$TMP/del"
  rm -rf "$D"; mkdir -p "$D/.agent-skill-chain/runtime"; cp "$H/.agent-skill-chain/runtime/workflow.db" "$D/.agent-skill-chain/runtime/workflow.db"; touch "$D/AGENTS.md"
  local victim out
  victim="$(sqlite3 "$D/.agent-skill-chain/runtime/workflow.db" "SELECT entry_id FROM workflow_log WHERE entry_hash IN (SELECT prev_hash FROM workflow_log WHERE prev_hash IS NOT NULL AND prev_hash<>'') LIMIT 1;")"
  if [[ -n "$victim" ]]; then
    sqlite3 "$D/.agent-skill-chain/runtime/workflow.db" "DELETE FROM workflow_log WHERE entry_id='$victim';"
...
audit_passthrough() {
  ...
  ( cd "$TMP" && bash .agent-skill-chain/source/enforcement/ci/audit.sh . >/dev/null 2>&1 ) || direct=$?
