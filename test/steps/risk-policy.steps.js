...
When('explicit trusted commitでpolicy validate CLIを実行する', function () {
  ...
  const runObserved = (overrides, args = matching) => {
    ...
    fs.writeFileSync(stub, `#!/usr/bin/env node\nconst fs=require('node:fs');const a=process.argv.slice(2);const countFile=${JSON.stringify(countFile)};if(a[0]==='auth'){if(${JSON.stringify(unavailable)})process.exit(2);fs.writeFileSync(countFile,String(Number(fs.readFileSync(countFile,'utf8'))+1));process.exit(0);}const index=Number(fs.readFileSync(countFile,'utf8'));const o=index>=2?${JSON.stringify(second)}:${JSON.stringify(observed)};if(${JSON.stringify(malformed)}){process.stdout.write('{');process.exit(0);}if(a[0]==='repo')process.stdout.write(JSON.stringify({nameWithOwner:o.repository,viewerPermission:'READ',defaultBranchRef:{name:o.defaultBranch}}));if(a[0]==='pr')process.stdout.write(JSON.stringify({number:o.pr,baseRefName:o.baseRefName,baseRefOid:o.baseRefOid,headRefOid:o.headRefOid}));\n`); fs.chmodSync(stub, 0o755);
