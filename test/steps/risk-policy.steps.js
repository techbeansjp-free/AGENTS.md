...
const prepareReviewGhStub = (world, variant) => {
  ...
  const prAuthor = variant === 'bot-pr-implementation-self-review' ? 'actor-bot' : 'actor-pr-author';
...
Given('rollback済みmigration stateと変更済みcandidateまたはrevision改竄がある', function () {
  ...
  this.rolledTamper = rollbackFileMigration(applyFileMigration(plan, this.trusted, this.candidate, { approvedPlanHash: this.approvedPlanHash, expectedRevision: 0, persist: persistJournal }), this.trusted, this.candidate, { approvedPlanHash: this.approvedPlanHash, expectedRevision: 1, persist: persistJournal });
...
When('trustedとcandidateを再検証してretryする', function () { this.tamperResults = [retryFileMigration(this.rolledTamper, this.trusted, this.changedCandidate, { approvedPlanHash: this.approvedPlanHash, expectedRevision: 2, persist: persistJournal }), retryFileMigration(this.revisionTamper, this.trusted, this.candidate, { approvedPlanHash: this.approvedPlanHash, expectedRevision: 2, persist: persistJournal })]; });
...
Given('trusted ruleのevidence remediation rollbackとauthority choiceをcandidateが変更する', function () {
  const choices = { language: 'a', testRunner: 'b', gherkinDialect: 'en', testLayers: ['c'], forbiddenTestFileSuffixes: ['.test.js'], naming: 'd', packageManager: 'e', runtime: 'f', ci: 'g', modelMapping: 'h', release: 'i' };
...
When('不正writeを注入してmigration applyを実行する', function () { this.result = applyFileMigration(this.plan, this.trusted, this.candidate, { approvedPlanHash: this.plan.planFingerprint, expectedRevision: 0, persist: persistJournal, write: (file) => fs.writeFileSync(file, 'corrupt\n') }); });
...
Given('durable persistを省略できる単一file migrationがある', function () {
  this.root = this.temp('asc-missing-journal-'); this.trusted = basePolicy();
  this.candidate = basePolicy([...this.trusted.rules, baseRule({ ruleId: 'ASC-MISSING-JOURNAL-001', enforcement: 'assist', activation: 'staged', riskClass: 'quality', overridePolicy: 'bound' })]);
  this.migrationFile = path.join(this.root, 'src/value.js'); fs.mkdirSync(path.dirname(this.migrationFile), { recursive: true }); fs.writeFileSync(this.migrationFile, 'before\n');
  this.plan = planFileMigration(this.root, this.trusted, this.candidate, [{ kind: 'runtime', path: 'src/value.js', after: 'export const value = 2;\n' }]);
});
When('durable persistなしでmigration applyを実行する', function () { this.result = applyFileMigration(this.plan, this.trusted, this.candidate, { approvedPlanHash: this.plan.planFingerprint, expectedRevision: 0 }); });
Then('applyはjournal不足で拒否され対象fileは変更されない', function () { assert.equal(this.result.allowed, false); assert.match(JSON.stringify(this.result), /durable journal persist/u); assert.equal(fs.readFileSync(this.migrationFile, 'utf8'), 'before\n'); });
...
Given('unknown、duplicate、dead referenceを持つconformance bindingがある', function () { this.root = this.temp('asc-conformance-cli-'); const contract = JSON.parse(fs.readFileSync('.agent-skill-chain/policy/conformance.json', 'utf8')); contract.invariants[11].id = 'I13'; contract.invariants.push(structuredClone(contract.invariants[0])); const binding = JSON.parse(fs.readFileSync('.agent-skill-chain/project/conformance/bindings.json', 'utf8')); fs.writeFileSync(path.join(this.root, 'fake-export.js'), "// export function missing() {}\nconst documentation = 'export function missing';\n`export function missing`;\n"); binding.bindings[0].enforcement[0] = { path: 'fake-export.js', export: 'missing' }; this.contractFile = path.join(this.root, 'contract.json'); this.bindingFile = path.join(this.root, 'binding.json'); this.evidenceFile = path.join(this.root, 'evidence.json'); writeJson(this.contractFile, contract); writeJson(this.bindingFile, binding); writeJson(this.evidenceFile, { tool: 'cucumber-js', passedScenarioIds: [] }); });
When('repository conformance CLIを実行する', function () { this.cliResult = executeBin(['conformance', 'validate', `--root=${this.root}`, `--contract=${this.contractFile}`, `--binding=${this.bindingFile}`, `--evidence=${this.evidenceFile}`]); });
Then('exact I1からI12と実在export、SCN、成功証拠の不足を拒否する', function () { assert.notEqual(this.cliResult.status, 0); for (const fragment of ['exact 12', '未知invariant', 'fake-export.js#missing', '成功証拠']) assert.ok(this.cliResult.stdout.includes(fragment), fragment); });
...
When('外部approvalまたはexpected revisionなしで全state changing APIを呼ぶ', function () {
  ...
  const applied = applyFileMigration(this.plan, this.trusted, this.candidate, { approvedPlanHash: this.approvedPlanHash, expectedRevision: 0, persist: persistJournal });
...
When('monolith afterとcandidate raw inventoryのmigrationを計画する', function () {
  ...
  if (this.fragmentPlan.allowed) this.fragmentApplied = applyFileMigration(this.fragmentPlan, this.trustedSet, this.candidateSet, { approvedPlanHash: this.fragmentPlan.planFingerprint, expectedRevision: 0, persist: persistJournal });
});
...
Given('origin HEADのないPR checkoutとtrusted base commitとcandidate policy setがある', function () { this.root = this.initRepo(); fs.mkdirSync(path.join(this.root, '.agent-skill-chain'), { recursive: true }); fs.copyFileSync('.agent-skill-chain/project-policy.json', path.join(this.root, '.agent-skill-chain/project-policy.json')); fs.cpSync('.agent-skill-chain/project', path.join(this.root, '.agent-skill-chain/project'), { recursive: true }); fs.mkdirSync(path.join(this.root, '.agent-skill-chain/policy'), { recursive: true }); fs.copyFileSync('.agent-skill-chain/policy/default.json', path.join(this.root, '.agent-skill-chain/policy/default.json')); spawnSync('git', ['add', '.agent-skill-chain'], { cwd: this.root }); spawnSync('git', ['commit', '-q', '-m', 'trusted base'], { cwd: this.root }); this.baseSha = spawnSync('git', ['rev-parse', 'HEAD'], { cwd: this.root, encoding: 'utf8' }).stdout.trim(); spawnSync('git', ['update-ref', 'refs/remotes/origin/main', this.baseSha], { cwd: this.root }); const manifestFile = path.join(this.root, '.agent-skill-chain/project-policy.json'); const manifest = JSON.parse(fs.readFileSync(manifestFile, 'utf8')); const ref = 'project/rules/pr-candidate.json'; manifest.ruleFiles.push(ref); manifest.ruleFiles.sort(); writeJson(manifestFile, manifest); writeJson(path.join(this.root, '.agent-skill-chain', ref), baseRule({ ruleId: 'ASC-PR-CANDIDATE-001', enforcement: 'assist', activation: 'staged', riskClass: 'quality', overridePolicy: 'bound', targetLayer: 'project' })); spawnSync('git', ['add', '.agent-skill-chain'], { cwd: this.root }); spawnSync('git', ['commit', '-q', '-m', 'candidate feature'], { cwd: this.root }); this.featureSha = spawnSync('git', ['rev-parse', 'HEAD'], { cwd: this.root, encoding: 'utf8' }).stdout.trim(); this.committedCandidateSetHash = loadTrustedProjectPolicySet(this.root, this.featureSha).setHash; });
When('explicit trusted commitでpolicy validate CLIを実行する', function () {
  const policy = '.agent-skill-chain/project-policy.json'; const common = ['policy', 'validate', policy, `--root=${this.root}`]; const missingSha = 'b'.repeat(40); const branchBinding = ['--base-ref=main', '--default-branch=main']; const providerBinding = ['--repo=o/r', '--pr=835', `--candidate-head-sha=${this.featureSha}`]; const matching = [`--trusted-commit=${this.baseSha}`, `--expected-base-sha=${this.baseSha}`, ...branchBinding, ...providerBinding];
  const ghDirectory = this.temp('asc-policy-gh-'); const stub = path.join(ghDirectory, 'gh');
  /** @param {Record<string, any>} overrides @param {string[]} [args] */
  const runObserved = (overrides, args = matching) => {
    const { second: secondOverrides = {}, unavailable = false, malformed = false, ...firstOverrides } = overrides;
    const observed = { repository: 'o/r', pr: 835, baseRefName: 'main', baseRefOid: this.baseSha, headRefOid: this.featureSha, defaultBranch: 'main', defaultBranchTipOid: this.baseSha, ...firstOverrides }; const second = { ...observed, ...secondOverrides }; const countFile = path.join(ghDirectory, 'observation-count'); fs.writeFileSync(countFile, '0');
    ...
    return executeBinIn(this.root, [...common, ...args], { ...process.env, PATH: `${ghDirectory}${path.delimiter}${process.env.PATH ?? ''}` });
  };
  this.explicitTrustedResults = [runObserved({}), executeBinIn(this.root, common), runObserved({}, ['--trusted-commit=not-a-sha', `--expected-base-sha=${this.baseSha}`, ...branchBinding, ...providerBinding]), runObserved({}, [`--trusted-commit=${this.baseSha}`, `--expected-base-sha=${missingSha}`, ...branchBinding, ...providerBinding]), runObserved({}, [`--trusted-commit=${missingSha}`, `--expected-base-sha=${missingSha}`, ...branchBinding, ...providerBinding]), runObserved({ baseRefOid: this.featureSha }, [`--trusted-commit=${this.featureSha}`, `--expected-base-sha=${this.featureSha}`, ...branchBinding, ...providerBinding]), runObserved({}, [`--trusted-commit=${this.baseSha}`, `--expected-base-sha=${this.baseSha}`, '--base-ref=feature/x', '--default-branch=main', ...providerBinding]), runObserved({}, [`--trusted-commit=${this.baseSha}`, `--expected-base-sha=${this.baseSha}`, '--base-ref=main', '--default-branch=main^{commit}', ...providerBinding]), runObserved({}, [`--trusted-commit=${this.baseSha}`, `--expected-base-sha=${this.baseSha}`, ...branchBinding]), runObserved({ headRefOid: missingSha }, [`--trusted-commit=${this.baseSha}`, `--expected-base-sha=${this.baseSha}`, ...branchBinding, '--repo=o/r', '--pr=835', `--candidate-head-sha=${missingSha}`])];
  this.providerIdentityResults = [runObserved({ repository: 'x/r' }), runObserved({ pr: 999 }), runObserved({ baseRefName: 'feature/x' }), runObserved({ defaultBranch: 'develop' }), runObserved({ defaultBranchTipOid: this.featureSha }), runObserved({ headRefOid: this.baseSha }), runObserved({ headRefOid: 'f'.repeat(40) })];
  this.providerRaceResults = [runObserved({ second: { headRefOid: 'e'.repeat(40) } }), runObserved({ second: { baseRefOid: 'e'.repeat(40) } }), runObserved({ second: { defaultBranchTipOid: 'e'.repeat(40) } }), runObserved({ second: { baseRefName: 'develop', defaultBranch: 'develop' } })];
  this.providerUnavailableResults = [runObserved({ unavailable: true }), runObserved({ malformed: true })];
  const manifestFile = path.join(this.root, '.agent-skill-chain/project-policy.json'); const manifestRaw = fs.readFileSync(manifestFile, 'utf8');
  const dirtyManifest = JSON.parse(manifestRaw); const dirtyRef = 'project/rules/dirty-uncommitted.json'; dirtyManifest.ruleFiles.push(dirtyRef); dirtyManifest.ruleFiles.sort(); writeJson(manifestFile, dirtyManifest); writeJson(path.join(this.root, '.agent-skill-chain', dirtyRef), baseRule({ ruleId: 'ASC-DIRTY-UNCOMMITTED-001', enforcement: 'assist', activation: 'staged', riskClass: 'quality', overridePolicy: 'bound', targetLayer: 'project' })); this.dirtyCandidateResult = runObserved({}); fs.rmSync(path.join(this.root, '.agent-skill-chain', dirtyRef)); fs.writeFileSync(manifestFile, manifestRaw);
  this.gitValidBranchResults = ['_main', 'release_'].map((branch) => { spawnSync('git', ['update-ref', `refs/remotes/origin/${branch}`, this.baseSha], { cwd: this.root }); return runObserved({ baseRefName: branch, defaultBranch: branch }, [`--trusted-commit=${this.baseSha}`, `--expected-base-sha=${this.baseSha}`, `--base-ref=${branch}`, `--default-branch=${branch}`, ...providerBinding]); });
});
Then('default branchとprovider head commitに拘束したpolicyだけ成功しdirty filesystem・自己申告・非default・branch injectionはauthorityにならない', function () { assert.equal(this.explicitTrustedResults[0].status, 0, this.explicitTrustedResults[0].stderr || this.explicitTrustedResults[0].stdout); for (const result of this.explicitTrustedResults.slice(1)) assert.notEqual(result.status, 0); for (const result of this.gitValidBranchResults) assert.equal(result.status, 0, result.stderr || result.stdout); for (const result of this.providerIdentityResults) { assert.notEqual(result.status, 0); assert.equal(JSON.parse(result.stdout).result.status, 'rejected'); } for (const result of [...this.providerRaceResults, ...this.providerUnavailableResults]) { assert.notEqual(result.status, 0); assert.equal(JSON.parse(result.stdout).result.status, 'pending'); } assert.equal(this.dirtyCandidateResult.status, 0, this.dirtyCandidateResult.stderr || this.dirtyCandidateResult.stdout); const dirtyObserved = JSON.parse(this.dirtyCandidateResult.stdout); assert.equal(dirtyObserved.candidateSetHash, this.committedCandidateSetHash); assert.equal(dirtyObserved.candidateProvenance.commitSha, this.featureSha); });

Given('canonical化するとpathとexportが重複するproject conformance bindingがある', function () { this.binding = { schemaVersion: 'agent-skill-chain/project-conformance/v1', bindings: Array.from({ length: 12 }, (_, index) => ({ id: `I${index + 1}`, sourcePaths: ['src/domain/conformance.js'], enforcement: [{ path: 'src/domain/conformance.js', export: 'validateProjectConformanceBinding' }], counterexampleScenarios: ['SCN-UNIT-CONFORMANCE-001'] })) }; this.binding.bindings[0].enforcement.push({ path: './src/domain/conformance.js', export: 'validateProjectConformanceBinding' }, { path: 'a/../src/domain/conformance.js', export: 'validateProjectConformanceBinding' }); });
Given('enforcementが配列でないproject conformance bindingがある', function () { this.binding = { schemaVersion: 'agent-skill-chain/project-conformance/v1', bindings: Array.from({ length: 12 }, (_, index) => ({ id: `I${index + 1}`, sourcePaths: ['src/domain/conformance.js'], enforcement: [{ path: 'src/domain/conformance.js', export: 'validateProjectConformanceBinding' }], counterexampleScenarios: ['SCN-UNIT-CONFORMANCE-001'] })) }; this.binding.bindings[0].enforcement = {}; });
Given('末尾slashを持つenforcement pathがある', function () { this.binding = { schemaVersion: 'agent-skill-chain/project-conformance/v1', bindings: Array.from({ length: 12 }, (_, index) => ({ id: `I${index + 1}`, sourcePaths: ['src/domain/conformance.js'], enforcement: [{ path: 'src/domain/conformance.js', export: 'validateProjectConformanceBinding' }], counterexampleScenarios: ['SCN-UNIT-CONFORMANCE-001'] })) }; this.binding.bindings[0].enforcement[0].path = 'src/domain/'; });
When('project conformance bindingを検証する', function () { this.bindingResult = validateProjectConformanceBinding(this.binding); this.bindingSchema = JSON.parse(fs.readFileSync('.agent-skill-chain/schemas/project-conformance-binding.schema.json', 'utf8')); });
Then('runtimeとschemaは重複tupleを拒否する', function () { assert.equal(this.bindingResult.valid, false); assert.ok(this.bindingResult.errors.some((/** @type {string} */ error) => error.includes('重複'))); const enforcementSchema = this.bindingSchema.properties.bindings.items.properties.enforcement; assert.equal(enforcementSchema.uniqueItems, true); const pathPattern = new RegExp(enforcementSchema.items.properties.path.pattern, 'u'); assert.equal(pathPattern.test('src/domain/conformance.js'), true); for (const alias of ['./src/domain/conformance.js', 'a/../src/domain/conformance.js', 'src//domain/conformance.js', 'src\\domain\\conformance.js']) assert.equal(pathPattern.test(alias), false, alias); });
Then('validatorは例外終了せずenforcement不正を返す', function () { assert.equal(this.bindingResult.valid, false); assert.ok(this.bindingResult.errors.some((/** @type {string} */ error) => error.includes('enforcement'))); });
Then('runtimeとschemaは末尾slashを拒否する', function () { assert.equal(this.bindingResult.valid, false); assert.match(this.bindingResult.errors.join(' '), /path/u); const pattern = new RegExp(this.bindingSchema.properties.bindings.items.properties.enforcement.items.properties.path.pattern, 'u'); assert.equal(pattern.test('src/domain/'), false); });

Given('trusted boundaryに同じrisk classのruleが2件と1件分の観測だけがある', function () { const first = baseRule({ ruleId: 'ASC-SAME-RISK-001', scope: ['pull_request'] }); const second = baseRule({ ruleId: 'ASC-SAME-RISK-002', scope: ['pull_request'] }); this.boundaryInput = { policy: basePolicy([first, second]), boundary: 'pull_request', observations: [{ ruleId: first.ruleId, violated: false, checks: ['first ruleだけを観測した'] }] }; });
Then('未観測のruleは同じrisk classでも拒否される', function () { assert.equal(this.result.allowed, false); assert.equal(this.result.diagnostic.ruleId, 'ASC-SAME-RISK-002'); assert.match(this.result.diagnostic.reasons.join(' '), /actual observation/u); });
Given('trusted boundaryに適用ruleがないpolicyがある', function () { this.boundaryInput = { policy: basePolicy([baseRule({ scope: ['policy'] })]), boundary: 'pull_request', observations: [] }; });
Then('scope未統治のoperationを拒否する', function () { assert.equal(this.result.allowed, false); assert.equal(this.result.diagnostic.ruleId, 'ASC-SCOPE-001'); });

Given('Object prototypeと同名のpolicy schema versionがある', function () { this.prototypeVersionPolicies = ['toString', 'constructor'].map((schemaVersion) => ({ ...basePolicy(), schemaVersion })); });
When('prototype名のpolicy versionを検証する', function () { this.prototypeVersionResults = this.prototypeVersionPolicies.map(validatePolicy); });
Then('prototype名は未対応versionとして拒否される', function () { for (const result of this.prototypeVersionResults) { assert.equal(result.valid, false); assert.match(result.errors.join(' '), /schemaVersionが未対応/u); } });
