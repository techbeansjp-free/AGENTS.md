...
When('explicit trusted commitでpolicy validate CLIを実行する', function () {
  ...
  const runObserved = (overrides, args = matching) => {
    ...
    const observed = { repository: 'o/r', pr: 835, baseRefName: 'main', baseRefOid: this.baseSha, headRefOid: this.featureSha, defaultBranch: 'main', ...firstOverrides }; const second = { ...observed, ...secondOverrides }; const countFile = path.join(ghDirectory, 'observation-count'); fs.writeFileSync(countFile, '0');
  ...
  this.providerIdentityResults = [runObserved({ repository: 'x/r' }), runObserved({ pr: 999 }), runObserved({ baseRefName: 'feature/x' }), runObserved({ defaultBranch: 'develop' }), runObserved({ headRefOid: this.baseSha }), runObserved({ headRefOid: 'f'.repeat(40) })];
  this.providerRaceResults = [runObserved({ second: { headRefOid: 'e'.repeat(40) } }), runObserved({ second: { baseRefOid: 'e'.repeat(40) } }), runObserved({ second: { baseRefName: 'develop', defaultBranch: 'develop' } })];
...
Then('runtimeとschemaは重複tupleを拒否する', function () { assert.equal(this.bindingResult.valid, false); assert.ok(this.bindingResult.errors.some((/** @type {string} */ error) => error.includes('重複'))); assert.equal(this.bindingSchema.properties.bindings.items.properties.enforcement.uniqueItems, true); });
