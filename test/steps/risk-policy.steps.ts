...
const baseRule = (changes: Partial<Rule> = {}): Rule => ({
  ...
  targetLayer: "package",
  evidence: "検証証拠",
  remediation: "信頼済み条件を維持して再実行する",
  overridePolicy: "never",
  rollback: "trusted policyへ戻す",
  ...changes,
});
...
const requireMigrationState = (value: unknown): MigrationState => {
  if (
    typeof value !== "object" ||
    value === null ||
    !("revision" in value) ||
...
const migrationEntries = (
  ...
  const entries: MigrationEntry[] = [
    {
      kind: "policy",
      path: ".agent-skill-chain/project-policy.json",
      after: `${JSON.stringify(candidate, null, 2)}\n`,
    },
    {
      kind: "schema",
      path: ".agent-skill-chain/schemas/example.schema.json",
      after: `${JSON.stringify({ $schema: "https://json-schema.org/draft/2020-12/schema", type: "object" }, null, 2)}\n`,
    },
    {
      kind: "runtime",
      path: "src/domain/example.ts",
      after: "export const migrated = true;\n",
    },
    {
      kind: "CI",
    ...
    {
      kind: "template",
      path: ".agent-skill-chain/templates/example.md",
      after: "# Generic hook\n",
    },
  ];
  for (const entry of entries) {
    ...
    fs.writeFileSync(file, `before-${entry.kind}\n`);
  }
  return entries;
};
...
const prepareReviewGhStub = (world: ReviewStubWorld, variant: string) => {
  const directory = world.temp("asc-review-gh-");
  world.ghLog = path.join(directory, "operations.log");
  const observedRepo = variant === "wrong-repository" ? "x/r" : "o/r";
  const observedPr = variant === "wrong-pr" ? 999 : 835;
  const observedRun = variant === "wrong-run" ? 999 : 32635972969;
  const observedReview = variant === "wrong-review" ? 999 : 9001;
  const observedHead =
    variant === "wrong-head" ? "f".repeat(40) : world.finalCommitSha;
  const prAuthor =
    variant === "bot-pr-implementation-self-review"
      ? "actor-bot"
      : "actor-pr-author";
  const implementationAuthor =
    variant === "null-implementation-author" ? null : "actor-implementer";
  const reviewer = [
    "self-review",
    ...
    ? "actor-implementer"
    : "actor-reviewer";
  const state = variant === "commented" ? "COMMENTED" : "APPROVED";
  const pullRequests =
    variant === "empty-run-pr"
      ? []
      : [{ number: variant === "wrong-run-pr" ? 999 : 835 }];
...
Given("secret保護のactive deny ruleがある", function () {
  this.rule = baseRule({
    ruleId: "ASC-SECRET-001",
    purpose: "秘密情報の混入を防ぐ",
    riskClass: "secret",
    scope: ["artifact"],
  });
...
Given("表記統一のactive assist ruleがある", function () {
  this.rule = baseRule({
    ruleId: "ASC-STYLE-001",
    purpose: "表記を統一する",
    riskClass: "quality",
    scope: ["docs"],
    enforcement: "assist",
    overridePolicy: "bound",
    remediation: "dry-run差分を確認して修正する",
  });
});
When("違反を検出してruleを評価する", function () {
  this.result = evaluateRule(this.rule, {
    ...
    autoFixes: [{ description: "表記を修正する", dryRunDiff: "- old\n+ new" }],
  });
});
...
Given(
  "trusted policyのactive deny ruleをcandidateがwarnへ緩和する",
  function () {
    this.trusted = basePolicy();
    this.candidate = basePolicy([
      baseRule({ enforcement: "warn", overridePolicy: "bound" }),
    ]);
  },
);
...
Given("local gateと外部service必須gateがある", function () {
  this.gates = [
    { id: "local-test", requiresExternal: false },
    { id: "github-check", requiresExternal: true },
  ];
});
...
Then("外部service必須gateだけがpendingである", function () {
  assert.deepEqual(
    this.result
      ...
      .map((gate: { id: string; status: string }) => gate.id),
    ["github-check"],
  );
});
...
Given("同じ差分とriskと合格証跡がある", function () {
  this.validationInput = {
    changedFiles: ["src/domain/enforcement.ts"],
    risk: ["authority"],
    evidence: {
      sha: SHA,
...
When("targeted gateとfinal gateを計画する", function () {
  const input = {
    ...this.validationInput,
    successfulFingerprints: [this.fingerprint],
    successfulEvidence: [
      {
        fingerprint: this.fingerprint,
        passed: true,
        sha: this.validationInput.evidence.sha,
        policyHash: this.validationInput.evidence.policyHash,
        tool: this.validationInput.evidence.tool,
        scope: this.validationInput.evidence.scope,
      },
    ],
  };
  this.targeted = planValidation({ ...input, kind: "targeted" });
  this.final = planValidation({ ...input, kind: "final" });
});
Then("targeted gateはdeduplicatedである", function () {
  assert.equal(this.targeted.status, "deduplicated");
  assert.deepEqual(this.targeted.checks, []);
});
Then(
  "final gateはsecurity、受け入れ条件、独立reviewを含むfullである",
  function () {
    assert.equal(this.final.scope, "full");
    assert.deepEqual(this.final.checks, [
      "security",
      "acceptance",
      "independentReview",
      "fullTest",
...
Given(
  "wait、duplicate、false block、override、rollback、missのeventがある",
  function () {
    this.events = [
      { kind: "gateWaitMs", value: 150, secret: "TOKEN=never-log-this" },
      { kind: "duplicate" },
      { kind: "falseBlock" },
      { kind: "override" },
      { kind: "rollback" },
...
Given("v0.3.0のtrusted policyとv0.3.1のcandidate policyがある", function () {
  ...
  this.beforeFiles = this.entries.map((entry: MigrationEntry) =>
...
Then("書き込みは行われない", function () {
  assert.deepEqual(this.trusted, this.before);
  for (const [index, entry] of this.entries.entries())
    assert.equal(
      fs.readFileSync(path.join(this.root, entry.path), "utf8"),
      this.beforeFiles[index],
    );
});
...
Given("適用済みmigrationと適用前snapshotがある", function () {
  ...
  const plan = planMigration(this.trusted, this.candidate);
  this.approvedPlanHash = plan.planFingerprint;
  this.applied = applyMigration(plan, {
...
When("migrationをrollbackしてretryする", function () {
  this.result = retryMigration(
    ...
    this.trusted,
    this.candidate,
    { approvedPlanHash: this.approvedPlanHash, expectedRevision: 2 },
  );
});
...
Given(
  "dogfooding policy、role log、metrics、test fixture、秘密fixtureがある",
  function () {
    this.assets = [
      ".agent-skill-chain/project-policy.json",
      ".agent-skill-chain/role-log/implementer.jsonl",
      ".agent-skill-chain/metrics/run.json",
      "test/fixtures/policy.json",
      "secret-fixtures/token.txt",
    ];
  },
);
...
Given("dogfooding用のtrusted policyと通常拡張candidateがある", function () {
  this.root = this.temp("asc-risk-cli-");
  this.trusted = basePolicy();
  this.candidate = basePolicy([
    ...this.trusted.rules,
    baseRule({
      ruleId: "ASC-NORMAL-001",
      enforcement: "assist",
      activation: "staged",
      riskClass: "quality",
      overridePolicy: "bound",
  ...
  this.candidateFile = path.join(this.root, "candidate.json");
  writeJson(this.trustedFile, this.trusted);
  writeJson(this.candidateFile, this.candidate);
});
When("policy migrate CLIをdry-runする", async function () {
  this.cliResult = await execute([
    "policy",
    "migrate",
    `--trusted=${this.trustedFile}`,
    `--candidate=${this.candidateFile}`,
    "--dry-run",
  ]);
});
Then("CLIは書き込まずstaged migrationを表示する", function () {
...
When("policy evaluate CLIを実行する", async function () {
  this.cliResult = await execute([
    "policy",
    "evaluate",
    `--trusted=${this.trustedFile}`,
...
Then("CLIは非0で終了する", function () {
  assert.notEqual(this.cliResult.status, 0);
});
Then("stdoutにASC-TRUST-001と日本語解決経路がある", function () {
  assert.ok(this.cliResult.stdout.includes("ASC-TRUST-001"));
  assert.ok(this.cliResult.stdout.includes("独立review"));
  assert.ok(this.cliResult.stdout.includes("candidateの緩和差分を取り消す"));
});
...
Given("一時projectにmigration入力とsnapshotがある", function () {
  ...
  writeJson(this.trustedFile, this.trusted);
  writeJson(this.candidateFile, this.candidate);
  this.repositoryHead = spawnSync("git", ["rev-parse", "HEAD"], {
    cwd: process.cwd(),
    encoding: "utf8",
  }).stdout;
  this.repositoryRemotes = spawnSync("git", ["remote", "-v"], {
    cwd: process.cwd(),
    encoding: "utf8",
  }).stdout;
});
...
Then("実repositoryとremoteは変更されない", function () {
  assert.equal(
  ...
  assert.equal(
    spawnSync("git", ["remote", "-v"], { cwd: process.cwd(), encoding: "utf8" })
      .stdout,
    this.repositoryRemotes,
  );
});

Given("delivery証拠をrequireするactive ruleがある", function () {
  this.rule = baseRule({
    ruleId: "ASC-DELIVERY-REQUIRE-001",
    purpose: "delivery証拠を要求する",
    riskClass: "quality",
    scope: ["delivery"],
    enforcement: "require",
    overridePolicy: "bound",
...
Then("有効なoverrideまたは証拠があれば許可される", function () {
  const overridden = enforceOperation({
    ...
    boundary: "delivery",
    violated: true,
    override: this.validOverride,
    expectedOverride: this.overrideExpected,
  });
  const passed = enforceOperation({
    policy: this.policy,
    ruleId: this.rule.ruleId,
    boundary: "delivery",
    violated: false,
  });
  const malformedEvidence = enforceOperation({
    policy: this.policy,
    ruleId: this.rule.ruleId,
    boundary: "delivery",
    violated: false,
  ...
  assert.ok("overrideAudit" in overridden && overridden.overrideAudit);
  assert.equal(passed.allowed, true);
  assert.equal(malformedEvidence.allowed, false);
  assert.ok(malformedEvidence.diagnostic);
  assert.equal(malformedEvidence.diagnostic.ruleId, "ASC-EVIDENCE-001");
});

Given(
  "trusted policyのmerge、review、check、branch、method、scope、意味をcandidateが弱化する",
  function () {
    const trustedRule = baseRule({
      enforcement: "require",
      overridePolicy: "bound",
      scope: ["delivery", "package"],
      purpose: "trusted意味",
      targetLayer: "project",
    });
...
Given("tokenとpasswordを含むblock diagnosticがある", function () {
  this.diagnostic = {
    ...
    diagnostic: {
      ...
      purpose: "秘密を守る",
      risk: "secret",
      reasons: ["token=super-secret-value password=never-show"],
      scope: ["package"],
      checks: ["api_key=hidden-value"],
      autoFixes: [],
      next: "credentialを失効する",
      requiredAuthority: "security owner",
      rollback: "artifactを破棄する",
    },
  };
});
...
Then("安全floorを弱化せずproject ruleをstagedで追加する", function () {
  ...
  assert.deepEqual(
    trusted.policy.rules.map((rule: Rule) => rule.ruleId),
    ["ASC-FLOOR-001", "ASC-PROJECT-001"],
  );
});

Given("bound ruleとnon-override ruleと期限付きoverrideがある", function () {
  this.boundRule = baseRule({
    enforcement: "require",
    overridePolicy: "bound",
    scope: ["policy"],
  });
  this.nonOverrideRule = baseRule();
  this.overrideExpected = {
    ruleId: this.boundRule.ruleId,
    issue: 834,
    scope: "policy",
    actor: "maintainer",
    sha: SHA,
...
When(
  "overrideの正常、Issue不一致、理由なし、期限切れ、non-overrideを検証する",
  function () {
    this.overrideResults = {
      valid: validateOverride(
        this.boundRule,
        this.validOverride,
        this.overrideExpected,
      ),
      issue: validateOverride(
        this.boundRule,
        { ...this.validOverride, issue: 999 },
        this.overrideExpected,
      ),
      reason: validateOverride(
        this.boundRule,
        { ...this.validOverride, reason: "" },
        this.overrideExpected,
      ),
      expired: validateOverride(
        this.boundRule,
        { ...this.validOverride, expiresAt: "2026-08-22T00:00:00Z" },
        this.overrideExpected,
      ),
      nonOverride: validateOverride(
        this.nonOverrideRule,
        this.validOverride,
        this.overrideExpected,
      ),
    };
...
Given("override対象と異なるrule IDの記録がある", function () {
  this.rule = baseRule({
    ruleId: "ASC-OVERRIDE-TARGET-001",
    enforcement: "require",
    overridePolicy: "bound",
  });
  this.expectedOverride = {
  ...
  this.override = {
    ruleId: "ASC-OTHER-RULE-001",
    issue: 834,
    scope: "policy",
    actor: "maintainer",
    reason: "別ruleの記録を再利用しようとした",
    expiresAt: "2026-08-24T00:00:00Z",
    sha: SHA,
  };
});
...
Given("trusted boundaryに必須属性を欠くruleがある", function () {
  const withoutTarget = Object.fromEntries(
    Object.entries(baseRule({ scope: ["pull_request"] })).filter(
      ([key]) => key !== "targetLayer",
    ),
  );
  const invalid = withoutTarget as unknown as Rule;
  this.boundaryInput = {
    policy: basePolicy([invalid]),
    boundary: "pull_request",
    observations: [{ ruleId: invalid.ruleId, violated: false }],
  };
});
...
Given("trusted policyにstaged project ruleがある", function () {
  this.stagedRule = baseRule({
    ruleId: "ASC-PROMOTION-001",
    riskClass: "quality",
    scope: ["project"],
    enforcement: "assist",
    activation: "staged",
...
Given("candidateは同じruleのactivationだけをactiveへ昇格する", function () {
  this.candidate = basePolicy([{ ...this.stagedRule, activation: "active" }]);
});
When(
  "trusted policyとcandidate policyを比較してeffective policyを解決する",
  function () {
...
Given("trusted boundaryにactive deny ruleと空の観測集合がある", function () {
  this.boundaryInput = {
    policy: basePolicy([baseRule({ scope: ["pull_request"] })]),
    boundary: "pull_request",
    observations: [],
  };
});
...
Given(
  "source、counterexample、成功証拠の配列型が不正なconformance入力がある",
  function () {
...
Given("ownerとtarget layerが誤配置されたasset分類がある", function () {
  ...
  this.evidenceAssets = [
    {
      path: "docs/reviews/result.md",
      owner: "review owner",
      targetLayer: "evidence",
      evidence: "exact-head attestation",
    },
    {
      path: "test/features/unit/x.feature",
      owner: "test owner",
      targetLayer: "evidence",
      evidence: "SCN-X-001",
    },
...
When("local、PR、packageのownership境界を評価する", function () {
  this.ownership = {
    local: validateOwnershipBoundary(this.ownershipAssets, "local"),
    pr: validateOwnershipBoundary(this.ownershipAssets, "pr"),
    package: validateOwnershipBoundary(this.ownershipAssets, "package"),
    evidence: validateOwnershipBoundary(this.evidenceAssets, "pr"),
  };
});
Then(
  "localは移動先とdry-run案を支援しPRは証拠を要求して実配布だけをdenyする",
  function () {
    ...
    assert.equal(this.ownership.package.allowed, false);
    assert.equal(this.ownership.evidence.allowed, true);
  },
);
...
Then(
  ...
  function () {
    assert.equal(
      this.contractResult.valid,
      true,
      this.contractResult.errors.join("; "),
    );
    for (const key of [
      "sourceHook",
      "enforcementHooks",
      ...
      assert.ok(this.invariant[key]?.length > 0, key);
    for (const [file, property] of [
      ...
      assert.ok(
        ...
        `${file} must constrain exact invariant IDs`,
      );
      assert.equal(
        clauses.filter(
          (clause: {
...
Given(
  "policy、schema、runtime、CI、templateの隔離fixtureと変更manifestがある",
  function () {
    this.root = this.temp("asc-manifest-");
    this.trusted = basePolicy();
    this.candidate = basePolicy([
      ...this.trusted.rules,
      baseRule({
        ruleId: "ASC-MANIFEST-001",
        enforcement: "assist",
        activation: "staged",
        riskClass: "quality",
        overridePolicy: "bound",
    ...
    this.entries = migrationEntries(this.root, this.candidate);
    this.beforeContents = Object.fromEntries(
      this.entries.map((entry: MigrationEntry) => [
        entry.path,
        fs.readFileSync(path.join(this.root, entry.path), "utf8"),
      ]),
    );
...
When("実manifestをdry-run、apply、rollbackする", function () {
  ...
  this.appliedManifest = requireMigrationState(
    applyFileMigration(this.plan, this.trusted, this.candidate, {
      approvedPlanHash,
      expectedRevision: 0,
      persist: persistJournal,
    }),
  );
  this.afterContents = Object.fromEntries(
    this.entries.map((entry: MigrationEntry) => [
      entry.path,
      fs.readFileSync(path.join(this.root, entry.path), "utf8"),
    ]),
  );
  this.rolledManifest = requireMigrationState(
    rollbackFileMigration(this.appliedManifest, this.trusted, this.candidate, {
      approvedPlanHash,
      expectedRevision: 1,
      persist: persistJournal,
    }),
...
Then("path、順序、before after hashと状態revisionを記録する", function () {
  assert.equal(this.plan.revision, 0);
  assert.deepEqual(
    this.plan.manifest.map((item: { order: number }) => item.order),
    [0, 1, 2, 3, 4],
  );
  for (const item of this.plan.manifest) {
    assert.ok(item.path);
    assert.ok(item.beforeHash);
    assert.ok(item.afterHash);
  }
  assert.equal(this.appliedManifest.revision, 1);
  assert.equal(this.rolledManifest.revision, 2);
});
Then("apply後検証とrollback後復旧が実fileで一致する", function () {
  assert.equal(this.appliedManifest.readAfterWrite, true);
  for (const entry of this.entries) {
    assert.equal(this.afterContents[entry.path], entry.after);
    assert.equal(
      fs.readFileSync(path.join(this.root, entry.path), "utf8"),
      this.beforeContents[entry.path],
    );
  }
  assert.equal(this.rolledManifest.rollbackVerified, true);
});

Given(
  "rollback済みmigration stateと変更済みcandidateまたはrevision改竄がある",
  function () {
    this.root = this.temp("asc-tamper-");
    this.trusted = basePolicy();
    this.candidate = basePolicy([
      ...this.trusted.rules,
      baseRule({
        ruleId: "ASC-RETRY-TAMPER-001",
        enforcement: "assist",
        activation: "staged",
        riskClass: "quality",
        overridePolicy: "bound",
    ...
    const entries = migrationEntries(this.root, this.candidate).slice(0, 1);
    const plan = requireMigrationState(
      planFileMigration(this.root, this.trusted, this.candidate, entries),
    );
    this.approvedPlanHash = plan.planFingerprint;
    this.rolledTamper = requireMigrationState(
      rollbackFileMigration(
        requireMigrationState(
          applyFileMigration(plan, this.trusted, this.candidate, {
            approvedPlanHash: this.approvedPlanHash,
            expectedRevision: 0,
            persist: persistJournal,
          }),
...
When("trustedとcandidateを再検証してretryする", function () {
  this.tamperResults = [
    retryFileMigration(this.rolledTamper, this.trusted, this.changedCandidate, {
      approvedPlanHash: this.approvedPlanHash,
      expectedRevision: 2,
      persist: persistJournal,
    }),
    retryFileMigration(this.revisionTamper, this.trusted, this.candidate, {
      approvedPlanHash: this.approvedPlanHash,
      expectedRevision: 2,
      persist: persistJournal,
    }),
...
Then("immutable fingerprintとhash不一致をstructured拒否する", function () {
...
Given(
  ...
  function () {
    ...
    this.v031Empty = basePolicy([]);
  },
);
When("schema契約とruntime契約を検証する", function () {
  this.schemaRuntime = [
    validatePolicy(this.v030Unknown),
    validatePolicy(this.v031Empty),
  ];
  this.schemaText = fs.readFileSync(
...
Given("配布fixtureにenv派生fileとmanifest外assetがある", function () {
  this.root = this.temp("asc-pack-abuse-");
  writeJson(path.join(this.root, "package.json"), {
    name: "fixture",
    version: "1.0.0",
    files: ["index.js", ".env.production", "extra.txt"],
  });
  fs.writeFileSync(path.join(this.root, "index.js"), "export {};\n");
  fs.writeFileSync(
    path.join(this.root, ".env.production"),
    "TOKEN=not-a-real-secret\n",
  );
  fs.writeFileSync(path.join(this.root, "extra.txt"), "extra\n");
});
When("実npm pack内容をpackage境界で検証する", function () {
  const output = this.temp("asc-pack-output-");
...
Given("同一policyと空の実manifestがある", function () {
  this.root = this.temp("asc-no-change-");
  this.trusted = basePolicy();
  this.candidate = structuredClone(this.trusted);
  this.emptyManifest = [];
});
...
Given("tracked dogfood policyを持つ隔離Git repositoryがある", function () {
  ...
  this.trusted = basePolicy();
  this.normalCandidate = basePolicy([
    ...this.trusted.rules,
    baseRule({
      ruleId: "ASC-DOGFOOD-NORMAL-001",
      enforcement: "assist",
      activation: "staged",
      riskClass: "quality",
      overridePolicy: "bound",
  ...
  this.weakCandidate = basePolicy([
    baseRule({ enforcement: "warn", overridePolicy: "bound" }),
  ]);
  this.trackedPolicy = path.join(
    this.root,
    ".agent-skill-chain",
    "project-policy.json",
  );
  this.normalFile = path.join(this.root, "normal.json");
  this.weakFile = path.join(this.root, "weak.json");
  writeJson(this.trackedPolicy, this.trusted);
  writeJson(this.normalFile, this.normalCandidate);
  writeJson(this.weakFile, this.weakCandidate);
  spawnSync("git", ["add", ".agent-skill-chain/project-policy.json"], {
    cwd: this.root,
  });
  spawnSync("git", ["commit", "-q", "-m", "dogfood policy"], {
    cwd: this.root,
  });
});
When("実binで通常拡張と自己緩和を評価する", function () {
  this.binResults = [
    executeBin([
      "policy",
      "evaluate",
      `--trusted=${this.trackedPolicy}`,
      `--candidate=${this.normalFile}`,
    ]),
    executeBin([
      "policy",
      "evaluate",
      `--trusted=${this.trackedPolicy}`,
      `--candidate=${this.weakFile}`,
    ]),
  ];
  this.tracked = spawnSync(
    "git",
    ["ls-files", ".agent-skill-chain/project-policy.json"],
    { cwd: this.root, encoding: "utf8" },
  ).stdout.trim();
});
Then("通常拡張は成功し自己緩和はASC-TRUST-001で拒否される", function () {
  assert.equal(this.tracked, ".agent-skill-chain/project-policy.json");
  assert.equal(this.binResults[0].status, 0, this.binResults[0].stderr);
  assert.notEqual(this.binResults[1].status, 0);
...
Given("{word}境界のrequire ruleと未達条件がある", function (boundary) {
  ...
  const rule = baseRule({
    ruleId: `ASC-${boundary.toUpperCase()}-REQUIRE-001`,
    enforcement: "require",
    overridePolicy: "bound",
    riskClass: "quality",
    scope: [boundary],
  ...
  this.enforcePolicy = path.join(this.root, "policy.json");
  this.enforceInput = path.join(this.root, "input.json");
  writeJson(this.enforcePolicy, basePolicy([rule]));
  writeJson(this.enforceInput, {
    ruleId: rule.ruleId,
    boundary,
    violated: true,
    reasons: ["必要証拠がない"],
...
When("実binでoperationをenforceする", function () {
  this.cliResult = executeBin([
    "policy",
    "enforce",
    `--policy=${this.enforcePolicy}`,
    `--input=${this.enforceInput}`,
  ]);
});
Then("operationは日本語diagnostic付きで非許可になる", function () {
  assert.notEqual(this.cliResult.status, 0);
  for (const fragment of ["ルールID", "安全な次の操作", "rollback方法"])
    assert.ok(this.cliResult.stdout.includes(fragment));
});

Given(
  "policy、schema、runtime、CI、templateのCLI migration fixtureがある",
  function () {
    this.root = this.temp("asc-cli-manifest-");
    this.trusted = basePolicy();
    this.candidate = basePolicy([
      ...this.trusted.rules,
      baseRule({
        ruleId: "ASC-CLI-MANIFEST-001",
        enforcement: "assist",
        activation: "staged",
        riskClass: "quality",
        overridePolicy: "bound",
    ...
    writeJson(this.trustedFile, this.trusted);
    writeJson(this.candidateFile, this.candidate);
    const entries = migrationEntries(this.root, this.candidate);
    this.manifestFile = path.join(this.root, "manifest.json");
    writeJson(this.manifestFile, { root: this.root, entries });
    this.cliManifestEntries = entries;
  },
);
When(
  "実binでdry-run、apply、rollback、retry、改竄retryを実行する",
  function () {
    const common = [
      `--trusted=${this.trustedFile}`,
      `--candidate=${this.candidateFile}`,
...
Then("実fileは復旧再適用され改竄retryだけがstructured拒否される", function () {
  ...
  assert.notEqual(this.tamperedCliResult.status, 0);
  assert.ok(this.tamperedCliResult.stdout.includes("ASC-MIGRATION-TOCTOU-001"));
  assert.ok(this.tamperedCliResult.stdout.includes("安全な次の操作"));
});
...
Given(
  "current evidenceがpassed falseで同fingerprintの成功cacheがある",
  function () {
    this.failedValidation = {
      changedFiles: ["src/x.ts"],
      risk: ["quality"],
    ...
    const fingerprint = evidenceFingerprint(this.failedValidation);
    this.failedValidation.successfulEvidence = [{ fingerprint, passed: true }];
  },
);

Given(
  "token key、Bearer、URL credential、PEMを含む全結果種別がある",
  function () {
    const secrets = {
      token: "token-value-never-show",
      nested: { apiKey: "api-value-never-show" },
      text: "Bearer bearer-never-show https://user:pass@example.test -----BEGIN PRIVATE KEY-----\nprivate-never-show\n-----END PRIVATE KEY-----",
    };
    this.outputKinds = [
      "passed",
      "warned",
      ...
      "autofix",
      "exception",
    ].map((status) => ({ status, ...secrets }));
  },
);
When("全結果をsafe serializerで出力する", function () {
  this.serializedKinds = this.outputKinds.map(serializeDiagnostic);
});
Then("秘密が残らず全結果に完全diagnosticがある", function () {
  const output = JSON.stringify(this.serializedKinds);
  for (const secret of [
    "token-value-never-show",
    "api-value-never-show",
    "bearer-never-show",
    "user:pass",
    "private-never-show",
  ])
...
Given(
  "Git内部、unrelated file、symlink、制御文字、Unicode case衝突を含むmanifestがある",
  function () {
    this.root = this.initRepo();
    this.trusted = basePolicy();
    this.candidate = basePolicy([
    ...
    this.badManifests = [
      ...
      [
        {
          kind: "schema",
          path: ".agent-skill-chain/schemas/x.json",
          after: '{"$schema":"x","type":"object"}\n',
        },
      ],
      [
        {
      ...
      [
        {
          kind: "schema",
          path: ".agent-skill-chain/schemas/Å.json",
          after: '{"$schema":"x","type":"object"}\n',
        },
        {
          kind: "schema",
      ...
      [
        {
          kind: "runtime",
          path: path.join(this.root, "absolute.js"),
          after: "export {};\n",
        },
      ],
    ];
...
When("authority付き実manifestをdry-runする", function () {
  if (this.badManifests)
    this.authorityPlans = this.badManifests.map((entries: MigrationEntry[]) =>
      planFileMigration(this.root, this.trusted, this.candidate, entries),
    );
  else {
    this.authorityPlan = planFileMigration(
      this.root,
      this.trusted,
      this.candidate,
      this.entries,
    );
    this.duplicatePlan = planFileMigration(
      this.root,
      this.trusted,
      this.candidate,
...
Given("runtime kindの異なるowned pathを2件持つmanifestがある", function () {
  this.root = this.temp("asc-multi-kind-");
  this.trusted = basePolicy();
  this.candidate = basePolicy([
    ...this.trusted.rules,
    baseRule({
      ruleId: "ASC-MULTI-001",
      enforcement: "assist",
      activation: "staged",
      riskClass: "quality",
      overridePolicy: "bound",
  ...
  this.entries = [
    { kind: "runtime", path: "src/a.ts", after: "export const a = 1;\n" },
    { kind: "runtime", path: "src/b.ts", after: "export const b = 1;\n" },
  ];
  for (const entry of this.entries) {
    fs.mkdirSync(path.dirname(path.join(this.root, entry.path)), {
      recursive: true,
    });
    fs.writeFileSync(path.join(this.root, entry.path), "before\n");
  }
});
Then("kind重複は許可しpath重複だけを拒否する", function () {
  assert.equal(
    this.authorityPlan.allowed,
    true,
...
Given(
  "candidateと異なるpolicy afterと不正なschema runtime CI templateがある",
  function () {
    this.root = this.temp("asc-invalid-artifacts-");
    this.trusted = basePolicy();
    this.candidate = basePolicy([
      ...this.trusted.rules,
      baseRule({
        ruleId: "ASC-ARTIFACT-NEW-001",
        enforcement: "assist",
        activation: "staged",
        riskClass: "quality",
        overridePolicy: "bound",
    ...
    this.badManifests = [
      ...
      [{ kind: "runtime", path: "src/x.ts", after: "\u0000" }],
      [{ kind: "CI", path: ".github/workflows/x.yml", after: "name: x\n" }],
      [
        {
          kind: "template",
          path: ".agent-skill-chain/templates/x.md",
          after: "",
        },
      ],
    ];
...
Then("kind別validatorが全ての不正artifactを拒否する", function () {
  assert.equal(this.authorityPlans.length, 5);
  for (const result of this.authorityPlans) assert.equal(result.allowed, false);
});

Given("durable journalを持つ複数file migrationがある", function () {
  this.root = this.temp("asc-journal-");
  this.trusted = basePolicy();
  this.candidate = basePolicy([
    ...this.trusted.rules,
    baseRule({
      ruleId: "ASC-JOURNAL-001",
      enforcement: "assist",
      activation: "staged",
      riskClass: "quality",
      overridePolicy: "bound",
  ...
  this.entries = [
    { kind: "runtime", path: "src/a.ts", after: "export const a = 2;\n" },
    { kind: "runtime", path: "src/b.ts", after: "export const b = 2;\n" },
  ];
  for (const entry of this.entries) {
    fs.mkdirSync(path.dirname(path.join(this.root, entry.path)), {
      recursive: true,
...
When(
  "state書込直後、partial apply、rollback途中のcrashを注入する",
  function () {
    const journals: MigrationState[] = [];
    try {
      applyFileMigration(this.plan, this.trusted, this.candidate, {
        approvedPlanHash: this.plan.planFingerprint,
        expectedRevision: 0,
        persist: (value) =>
          journals.push(requireMigrationState(structuredClone(value))),
        interruptAfterStep: 0,
      });
    } catch {
      /* simulated process interruption */
    }
    this.initialJournal = journals[0];
    this.partialRecovery = recoverFileMigration(
      requireMigrationState(journals.at(-1)),
      this.trusted,
      this.candidate,
      {
        approvedPlanHash: this.plan.planFingerprint,
    ...
    const applied = requireMigrationState(
      applyFileMigration(
        { ...this.plan, revision: 0 },
        this.trusted,
        this.candidate,
        {
          approvedPlanHash: this.plan.planFingerprint,
    ...
    const rollbackJournals: MigrationState[] = [];
    try {
      rollbackFileMigration(applied, this.trusted, this.candidate, {
        approvedPlanHash: this.plan.planFingerprint,
        expectedRevision: 1,
        persist: (value) =>
          rollbackJournals.push(requireMigrationState(structuredClone(value))),
        interruptAfterStep: 0,
      });
    } catch {
      /* simulated process interruption */
    }
    this.rollbackRecovery = recoverFileMigration(
      requireMigrationState(rollbackJournals.at(-1)),
      this.trusted,
      this.candidate,
      {
        approvedPlanHash: this.plan.planFingerprint,
...
Given(
  "read-after-write不一致を注入できる単一file migrationがある",
  function () {
    this.root = this.temp("asc-read-after-write-");
    this.trusted = basePolicy();
    this.candidate = basePolicy([
      ...this.trusted.rules,
      baseRule({
        ruleId: "ASC-READ-AFTER-WRITE-001",
        enforcement: "assist",
        activation: "staged",
        riskClass: "quality",
        overridePolicy: "bound",
...
Given("durable persistを省略できる単一file migrationがある", function () {
  this.root = this.temp("asc-missing-journal-");
  this.trusted = basePolicy();
  this.candidate = basePolicy([
    ...this.trusted.rules,
    baseRule({
      ruleId: "ASC-MISSING-JOURNAL-001",
      enforcement: "assist",
      activation: "staged",
      riskClass: "quality",
      overridePolicy: "bound",
...
Given(
  "allowlisted srcにcredential境界、oauth、reauth及びtoken内容がある",
  function () {
    this.root = this.temp("asc-credential-pack-");
    ...
    for (const name of [
      ...
      fs.writeFileSync(path.join(this.root, "src", name), '{"username":"x"}\n');
    fs.writeFileSync(
      path.join(this.root, "src/data.json"),
      '{"token":"token-never-ship"}\n',
    );
  },
);
When("実npm packの名前とcontentを検査する", function () {
  const output = this.temp("asc-pack-output-");
  const packed = spawnSync(
    "npm",
    ["pack", "--json", "--ignore-scripts", `--pack-destination=${output}`],
    {
      cwd: this.root,
      encoding: "utf8",
      env: { ...process.env, npm_config_cache: path.join(output, "npm-cache") },
    },
  );
  assert.equal(packed.status, 0, packed.stderr);
  ...
  assert.ok(archiveName, "npm pack archive");
  ...
  const contents = Object.fromEntries(
    files
      .filter((file: string) => file !== "package.json")
      .map((file: string) => {
        const extracted = spawnSync(
          "tar",
          ["-xOf", archive, `package/${file}`],
          { encoding: "utf8" },
        );
...
Given(
  ...
  function () {
    ...
    this.contractFile = path.join(this.root, "contract.json");
    this.bindingFile = path.join(this.root, "binding.json");
    this.evidenceFile = path.join(this.root, "evidence.json");
    writeJson(this.contractFile, contract);
    writeJson(this.bindingFile, binding);
    writeJson(this.evidenceFile, {
      tool: "cucumber-js",
      passedScenarioIds: [],
    });
  },
);
When("repository conformance CLIを実行する", function () {
  this.cliResult = executeBin([
    "conformance",
    "validate",
    `--root=${this.root}`,
    `--contract=${this.contractFile}`,
    `--binding=${this.bindingFile}`,
    `--evidence=${this.evidenceFile}`,
  ]);
});
Then("exact I1からI12と実在export、SCN、成功証拠の不足を拒否する", function () {
  assert.notEqual(this.cliResult.status, 0);
  for (const fragment of [
    "exact 12",
    "未知invariant",
    "fake-export.js#missing",
    "成功証拠",
  ])
    assert.ok(this.cliResult.stdout.includes(fragment), fragment);
});

Given(
  "trusted origin policyを持つ隔離repositoryと不正なactual operationがある",
  function () {
    this.root = this.initRepo();
    ...
    spawnSync("git", ["commit", "-q", "-m", "trusted floor"], {
      cwd: this.root,
    });
...
When(
  "preflight省略、別rule、violated falseを指定してactual CLIを実行する",
  function () {
    this.actualBypasses = ["omit", "other", "false"].map((name) =>
      executeBin([
        "worktree",
        "create",
        `--root=${this.root}`,
        `--path=${path.join(this.root, ".git", name)}`,
        "--branch=feature/bypass",
        "--base=main",
        "--rule=ASC-OFFLINE-001",
        "--violated=false",
      ]),
    );
  },
...
Then(
  "actual stateから導出したtrusted enforcementが全てを拒否する",
  function () {
    for (const result of this.actualBypasses) {
      assert.notEqual(result.status, 0);
      assert.ok(result.stdout.includes("ASC-GIT-INTERNAL-001"));
    }
  },
);

Given("secretを含む存在しないpath入力がある", function () {
  this.secretPath = path.join(
    this.temp("asc-secret-error-"),
    "token=token-never-show.json",
  );
});
When("actual binでpath例外を発生させる", function () {
  this.cliResult = executeBin([
    "policy",
    "validate",
    `--file=${this.secretPath}`,
  ]);
});
...
Given(
  "valid project policy manifestと悪用fragment variantsがある",
  function () {
    const create = () => {
      const root = this.temp("asc-project-set-");
    ...
    this.projectSetVariants = [];
    {
      const root = create();
      fs.writeFileSync(
        path.join(root, ".agent-skill-chain/project/rules/orphan.json"),
        "{}\n",
      );
    ...
    {
      const root = create();
      fs.rmSync(path.join(root, ".agent-skill-chain/project/rules/docs.json"));
      this.projectSetVariants.push(root);
    }
    {
      const root = create();
      ...
      fs.rmSync(file);
      fs.symlinkSync(
        path.join(process.cwd(), ".agent-skill-chain/project/rules/docs.json"),
        file,
      );
      this.projectSetVariants.push(root);
    }
    {
      ...
      fs.writeFileSync(file, '{"ruleId":"A","ruleId":"B"}\n');
      this.projectSetVariants.push(root);
    }
    {
      const root = create();
    ...
    {
      const root = create();
      fs.mkdirSync(path.join(root, ".agent-skill-chain/project/unknown"));
      fs.writeFileSync(
        path.join(root, ".agent-skill-chain/project/unknown/value.json"),
        "{}\n",
      );
      this.projectSetVariants.push(root);
    }
    ...
    }
    {
      ...
      fs.writeFileSync(
        file,
        '{"schemaVersion":"agent-skill-chain/project-policy-manifest/v1","schemaVersion":"duplicate"}\n',
      );
      this.projectSetVariants.push(root);
    }
  },
);
When("filesystem policy setを厳密にloadする", function () {
  this.projectSetErrors = this.projectSetVariants.map((root: string) => {
    try {
      loadProjectPolicySet(root);
      return undefined;
    } catch (error) {
...
Given("originにmanifestと全fragmentを持つtrusted commitがある", function () {
  ...
  spawnSync("git", ["add", ".agent-skill-chain"], { cwd: this.root });
  spawnSync("git", ["commit", "-q", "-m", "trusted project set"], {
    cwd: this.root,
  });
  spawnSync("git", ["update-ref", "refs/remotes/origin/main", "HEAD"], {
    cwd: this.root,
...
When("trusted project policy setをloadする", function () {
  this.trustedSet = loadTrustedProjectPolicySet(this.root, "origin/main");
  this.trustedHead = spawnSync("git", ["rev-parse", "HEAD"], {
    cwd: this.root,
    encoding: "utf8",
  }).stdout.trim();
  try {
    loadEffectiveTrustedPolicySet(this.root, "main");
  } catch (error) {
    this.missingFloorError =
      error instanceof Error ? error.message : String(error);
  }
});
Then(
  "provenance commit、set hash、semantic policy hashが固定される",
  function () {
    assert.equal(this.trustedSet.provenance.commitSha, this.trustedHead);
    assert.match(this.trustedSet.setHash, /^[a-f0-9]{64}$/u);
    assert.match(this.trustedSet.semanticPolicyHash, /^[a-f0-9]{64}$/u);
    assert.notEqual(
      this.trustedSet.setHash,
      this.trustedSet.semanticPolicyHash,
    );
    assert.match(this.missingFloorError, /default safety floor/u);
  },
);

Given(
  "self asserted approved hashを持つforged migration stateがある",
  function () {
    this.root = this.temp("asc-self-approved-");
    this.trusted = basePolicy();
    this.candidate = basePolicy([
      ...this.trusted.rules,
      baseRule({
        ruleId: "ASC-SELF-APPROVAL-001",
        enforcement: "assist",
        activation: "staged",
        riskClass: "quality",
        overridePolicy: "bound",
...
When(
  "外部approvalまたはexpected revisionなしで全state changing APIを呼ぶ",
  function () {
    this.authorityResults = [];
    this.authorityResults.push(
      applyFileMigration(this.plan, this.trusted, this.candidate, {
        expectedRevision: 0,
      }),
    );
      ...
      retryFileMigration(
        {
          ...
          revision: 2,
          approvedPlanHash: this.approvedPlanHash,
          history: ["staged", "applied", "rolled_back"],
        },
        this.trusted,
        this.candidate,
        { expectedRevision: 2 },
    ...
    const conceptualApplied = applyMigration(conceptualPlan, {
      approvedPlanHash: conceptualPlan.planFingerprint,
      expectedRevision: 0,
    });
...
Then("全APIはauthority不足をstructured拒否しfileを変更しない", function () {
...
Given("trustedとcandidateのfragmented project policy setがある", function () {
  const copySet = (prefix: string) => {
    const root = this.temp(prefix);
    fs.mkdirSync(path.join(root, ".agent-skill-chain"), { recursive: true });
    fs.copyFileSync(
      ".agent-skill-chain/project-policy.json",
      path.join(root, ".agent-skill-chain/project-policy.json"),
  ...
  this.root = copySet("asc-fragment-target-");
  this.candidateRoot = copySet("asc-fragment-candidate-");
  ...
  writeJson(
    path.join(this.candidateRoot, ".agent-skill-chain", newRef),
    baseRule({
      ruleId: "ASC-FRAGMENT-MIGRATION-001",
      enforcement: "assist",
      activation: "staged",
      riskClass: "quality",
      overridePolicy: "bound",
  ...
  this.trustedSet = loadProjectPolicySet(this.root);
  this.candidateSet = loadProjectPolicySet(this.candidateRoot);
  this.fragmentEntries = [
    ...
    after: fs.readFileSync(
      path.join(this.candidateRoot, ".agent-skill-chain", relative),
      "utf8",
    ),
  }));
});
When(
  "monolith afterとcandidate raw inventoryのmigrationを計画する",
  function () {
    this.monolithPlan = planFileMigration(
      this.root,
      this.trustedSet,
      this.candidateSet,
      [
        {
          kind: "policy",
          path: ".agent-skill-chain/project-policy.json",
          after: `${JSON.stringify(this.candidateSet.policy, null, 2)}\n`,
        },
      ],
    );
    this.mixedFormPlan = planFileMigration(
      this.root,
      this.trustedSet,
      this.candidateSet.policy,
      [
        {
          kind: "policy",
          path: ".agent-skill-chain/project-policy.json",
    ...
    this.fragmentPlan = planFileMigration(
      this.root,
      this.trustedSet,
      this.candidateSet,
      this.fragmentEntries,
...
Then(
  "monolithは拒否されreal inventoryはapply後set hashまで一致する",
  function () {
    assert.equal(this.monolithPlan.allowed, false);
    assert.equal(this.mixedFormPlan.allowed, false);
    assert.equal(
      this.fragmentPlan.allowed,
      true,
      JSON.stringify(this.fragmentPlan),
    );
    assert.equal(
      this.fragmentApplied.allowed,
      true,
      JSON.stringify(this.fragmentApplied),
    );
    const loaded = loadProjectPolicySet(this.root);
    assert.equal(loaded.setHash, this.candidateSet.setHash);
    assert.equal(
      loaded.semanticPolicyHash,
      this.candidateSet.semanticPolicyHash,
    );
  },
);
...
Given(
  ...
  function () {
    ...
    spawnSync("git", ["add", ".agent-skill-chain"], { cwd: this.root });
    spawnSync("git", ["commit", "-q", "-m", "trusted deny"], {
      cwd: this.root,
    });
    spawnSync("git", ["update-ref", "refs/remotes/origin/main", "HEAD"], {
      cwd: this.root,
...
When("authority operation policyをloadする", function () {
  try {
...
Then("trusted ref不明をfail closedにする", function () {
  assert.equal(this.operationPolicy, undefined);
  assert.match(this.operationPolicyError ?? "", /trusted|origin|HEAD|SHA/u);
});

Given("recover可能journalとunknown hashのartifactがある", function () {
  this.root = this.temp("asc-recovery-cli-");
  this.trusted = basePolicy();
  this.candidate = basePolicy([
    ...this.trusted.rules,
    baseRule({
      ruleId: "ASC-RECOVERY-001",
      enforcement: "assist",
      activation: "staged",
      riskClass: "quality",
      overridePolicy: "bound",
  ...
  fs.writeFileSync(path.join(this.root, "src/value.ts"), "unknown\n");
  this.journalBefore = fs.readFileSync(this.stateFile, "utf8");
});
When("CLI recoveryが失敗する", function () {
  this.recoveryCli = executeBin([
    "policy",
    "migrate",
    "--operation=recover",
    `--trusted=${this.trustedFile}`,
    `--candidate=${this.candidateFile}`,
    `--state=${this.stateFile}`,
    `--approved-plan-hash=${this.plan.planFingerprint}`,
    "--expected-revision=0",
    "--apply",
  ]);
});
...
Given(
  ...
  function () {
    ...
    spawnSync("git", ["add", ".agent-skill-chain"], { cwd: this.root });
    spawnSync("git", ["commit", "-q", "-m", "trusted base"], {
      cwd: this.root,
    });
    this.baseSha = spawnSync("git", ["rev-parse", "HEAD"], {
      cwd: this.root,
      encoding: "utf8",
    }).stdout.trim();
    spawnSync("git", ["update-ref", "refs/remotes/origin/main", this.baseSha], {
      cwd: this.root,
    });
    const manifestFile = path.join(
      this.root,
    ...
    writeJson(manifestFile, manifest);
    writeJson(
      path.join(this.root, ".agent-skill-chain", ref),
      baseRule({
        ruleId: "ASC-PR-CANDIDATE-001",
        enforcement: "assist",
        activation: "staged",
        riskClass: "quality",
        overridePolicy: "bound",
    ...
    spawnSync("git", ["add", ".agent-skill-chain"], { cwd: this.root });
    spawnSync("git", ["commit", "-q", "-m", "candidate feature"], {
      cwd: this.root,
    });
    ...
    this.committedCandidateSetHash = loadTrustedProjectPolicySet(
      this.root,
      this.featureSha,
    ).setHash;
  },
...
When("explicit trusted commitでpolicy validate CLIを実行する", function () {
  const policy = ".agent-skill-chain/project-policy.json";
  const common = ["policy", "validate", policy, `--root=${this.root}`];
  const missingSha = "b".repeat(40);
  const branchBinding = ["--base-ref=main", "--default-branch=main"];
  const providerBinding = [
    "--repo=o/r",
    "--pr=835",
    `--candidate-head-sha=${this.featureSha}`,
  ];
  const matching = [
    `--trusted-commit=${this.baseSha}`,
    `--expected-base-sha=${this.baseSha}`,
    ...branchBinding,
    ...providerBinding,
  ];
  const ghDirectory = this.temp("asc-policy-gh-");
  const stub = path.join(ghDirectory, "gh");
  const runObserved = (
    ...
    args: string[] = matching,
  ) => {
    const {
      second: secondOverrides = {},
      unavailable = false,
      malformed = false,
      ...firstOverrides
    } = overrides;
    const observed = {
      ...
      pr: 835,
      baseRefName: "main",
      baseRefOid: this.baseSha,
      headRefOid: this.featureSha,
      defaultBranch: "main",
      defaultBranchTipOid: this.baseSha,
      ...firstOverrides,
    };
    const second = { ...observed, ...secondOverrides };
    const countFile = path.join(ghDirectory, "observation-count");
    fs.writeFileSync(countFile, "0");
    fs.writeFileSync(
      stub,
      `#!/usr/bin/env node\nconst fs=require('node:fs');const a=process.argv.slice(2);const countFile=${JSON.stringify(countFile)};if(a[0]==='auth'){if(${JSON.stringify(unavailable)})process.exit(2);fs.writeFileSync(countFile,String(Number(fs.readFileSync(countFile,'utf8'))+1));process.exit(0);}const index=Number(fs.readFileSync(countFile,'utf8'));const o=index>=2?${JSON.stringify(second)}:${JSON.stringify(observed)};if(${JSON.stringify(malformed)}){process.stdout.write('{');process.exit(0);}if(a[0]==='repo')process.stdout.write(JSON.stringify({nameWithOwner:o.repository,defaultBranchRef:{name:o.defaultBranch}}));if(a[0]==='pr')process.stdout.write(JSON.stringify({number:o.pr,baseRefName:o.baseRefName,baseRefOid:o.baseRefOid,headRefOid:o.headRefOid}));if(a[0]==='api')process.stdout.write(o.defaultBranchTipOid+'\\n');\n`,
    );
    fs.chmodSync(stub, 0o755);
    return executeBinIn(this.root, [...common, ...args], {
      ...process.env,
      PATH: `${ghDirectory}${path.delimiter}${process.env.PATH ?? ""}`,
    });
  };
  this.explicitTrustedResults = [
    runObserved({}),
    executeBinIn(this.root, common),
    runObserved({}, [
      "--trusted-commit=not-a-sha",
      `--expected-base-sha=${this.baseSha}`,
      ...branchBinding,
      ...providerBinding,
    ]),
    runObserved({}, [
      `--trusted-commit=${this.baseSha}`,
      `--expected-base-sha=${missingSha}`,
      ...branchBinding,
      ...providerBinding,
    ]),
    runObserved({}, [
      `--trusted-commit=${missingSha}`,
      `--expected-base-sha=${missingSha}`,
      ...branchBinding,
      ...providerBinding,
    ]),
    runObserved({ baseRefOid: this.featureSha }, [
      `--trusted-commit=${this.featureSha}`,
      `--expected-base-sha=${this.featureSha}`,
      ...branchBinding,
      ...providerBinding,
    ]),
    runObserved({}, [
    ...
    runObserved({ headRefOid: missingSha }, [
      `--trusted-commit=${this.baseSha}`,
      `--expected-base-sha=${this.baseSha}`,
      ...branchBinding,
      "--repo=o/r",
      "--pr=835",
      `--candidate-head-sha=${missingSha}`,
    ]),
  ];
  this.providerIdentityResults = [
    runObserved({ repository: "x/r" }),
    runObserved({ pr: 999 }),
    runObserved({ baseRefName: "feature/x" }),
    runObserved({ defaultBranch: "develop" }),
    runObserved({ defaultBranchTipOid: this.featureSha }),
    runObserved({ headRefOid: this.baseSha }),
    runObserved({ headRefOid: "f".repeat(40) }),
  ];
  this.providerRaceResults = [
    runObserved({ second: { headRefOid: "e".repeat(40) } }),
    runObserved({ second: { baseRefOid: "e".repeat(40) } }),
    runObserved({ second: { defaultBranchTipOid: "e".repeat(40) } }),
    runObserved({
      second: { baseRefName: "develop", defaultBranch: "develop" },
    }),
  ];
  this.providerUnavailableResults = [
    runObserved({ unavailable: true }),
    runObserved({ malformed: true }),
  ];
  const manifestFile = path.join(
    this.root,
    ".agent-skill-chain/project-policy.json",
  ...
  writeJson(manifestFile, dirtyManifest);
  writeJson(
    path.join(this.root, ".agent-skill-chain", dirtyRef),
    baseRule({
      ruleId: "ASC-DIRTY-UNCOMMITTED-001",
      enforcement: "assist",
      activation: "staged",
      riskClass: "quality",
      overridePolicy: "bound",
  ...
  this.dirtyCandidateResult = runObserved({});
  fs.rmSync(path.join(this.root, ".agent-skill-chain", dirtyRef));
  fs.writeFileSync(manifestFile, manifestRaw);
  this.gitValidBranchResults = ["_main", "release_"].map((branch) => {
    spawnSync(
      "git",
      ["update-ref", `refs/remotes/origin/${branch}`, this.baseSha],
      { cwd: this.root },
    );
    return runObserved({ baseRefName: branch, defaultBranch: branch }, [
      `--trusted-commit=${this.baseSha}`,
      `--expected-base-sha=${this.baseSha}`,
      `--base-ref=${branch}`,
...
Then(
  "default branchとprovider head commitに拘束したpolicyだけ成功しdirty filesystem・自己申告・非default・branch injectionはauthorityにならない",
  function () {
    assert.equal(
      this.explicitTrustedResults[0].status,
      0,
      this.explicitTrustedResults[0].stderr ||
        this.explicitTrustedResults[0].stdout,
    );
    for (const result of this.explicitTrustedResults.slice(1))
      assert.notEqual(result.status, 0);
    for (const result of this.gitValidBranchResults)
    ...
    assert.equal(dirtyObserved.candidateProvenance.commitSha, this.featureSha);
  },
);

Given(
  "canonical化するとpathとexportが重複するproject conformance bindingがある",
  function () {
    this.binding = {
    ...
    this.binding.bindings[0].enforcement.push(
      ...
      {
        path: "a/../src/domain/conformance.js",
        export: "validateProjectConformanceBinding",
      },
    );
  },
);
Given("enforcementが配列でないproject conformance bindingがある", function () {
  this.binding = {
    schemaVersion: "agent-skill-chain/project-conformance/v1",
    bindings: Array.from({ length: 12 }, (_, index) => ({
      id: `I${index + 1}`,
...
Then("runtimeとschemaは重複tupleを拒否する", function () {
  ...
  assert.ok(
    this.bindingResult.errors.some((error: string) => error.includes("重複")),
  );
  ...
  for (const alias of [
    "./src/domain/conformance.js",
    "a/../src/domain/conformance.js",
    "src//domain/conformance.ts",
    "src\\domain\\conformance.js",
  ])
    assert.equal(pathPattern.test(alias), false, alias);
});
Then("validatorは例外終了せずenforcement不正を返す", function () {
  assert.equal(this.bindingResult.valid, false);
  assert.ok(
    this.bindingResult.errors.some((error: string) =>
      error.includes("enforcement"),
    ),
  );
});
...
Given(
  "trusted boundaryに同じrisk classのruleが2件と1件分の観測だけがある",
  function () {
    const first = baseRule({
      ruleId: "ASC-SAME-RISK-001",
      scope: ["pull_request"],
    });
    const second = baseRule({
      ruleId: "ASC-SAME-RISK-002",
      scope: ["pull_request"],
    });
    this.boundaryInput = {
      policy: basePolicy([first, second]),
      boundary: "pull_request",
      observations: [
        {
          ruleId: first.ruleId,
          violated: false,
          checks: ["first ruleだけを観測した"],
        },
      ],
    };
  },
...
Given("trusted boundaryに適用ruleがないpolicyがある", function () {
  this.boundaryInput = {
    policy: basePolicy([baseRule({ scope: ["policy"] })]),
    boundary: "pull_request",
    observations: [],
  };
});
...
Given("Object prototypeと同名のpolicy schema versionがある", function () {
  this.prototypeVersionPolicies = ["toString", "constructor"].map(
    (schemaVersion) => ({ ...basePolicy(), schemaVersion }),
  );
});
When("prototype名のpolicy versionを検証する", function () {
  this.prototypeVersionResults =
    this.prototypeVersionPolicies.map(validatePolicy);
});
Then("prototype名は未対応versionとして拒否される", function () {
  for (const result of this.prototypeVersionResults) {
    assert.equal(result.valid, false);
    assert.match(result.errors.join(" "), /schemaVersionが未対応/u);
  }
});
...
Given(
  ...
  function () {
    ...
    spawnSync("git", ["add", "product.js"], { cwd: this.root });
    spawnSync("git", ["commit", "-q", "-m", "implementation"], {
      cwd: this.root,
    });
    this.implementationCommitSha = spawnSync("git", ["rev-parse", "HEAD"], {
      cwd: this.root,
      encoding: "utf8",
    }).stdout.trim();
    this.artifactPath = "docs/reviews/phase-a.json";
    fs.mkdirSync(path.dirname(path.join(this.root, this.artifactPath)), {
      recursive: true,
    });
    writeJson(path.join(this.root, this.artifactPath), {
      phase: "A",
      status: "pending-external-attestation",
    });
    spawnSync("git", ["add", this.artifactPath], { cwd: this.root });
    spawnSync("git", ["commit", "-q", "-m", "phase A evidence"], {
      cwd: this.root,
    });
    this.finalCommitSha = spawnSync("git", ["rev-parse", "HEAD"], {
      cwd: this.root,
      encoding: "utf8",
    }).stdout.trim();
    this.externalFile = path.join(
      this.temp("asc-review-external-"),
      "external.json",
    );
    writeJson(this.externalFile, {
      pr: { headSha: this.finalCommitSha },
      ci: {
        headSha: this.finalCommitSha,
        runId: "run-123",
        conclusion: "success",
      },
      review: {
        headSha: this.finalCommitSha,
        commitSha: this.finalCommitSha,
        actorId: "reviewer-123",
        submittedAt: "2026-08-23T12:00:00Z",
        verdict: "approved",
        artifactId: "review-123",
      },
    });
  },
);
...
When("review evidence CLIでGitとGitHub providerを結合する", function () {
  const common = [
    ...
    "evidence",
    `--root=${this.root}`,
    `--implementation-commit=${this.implementationCommitSha}`,
    `--final-commit=${this.finalCommitSha}`,
  ...
  if (this.reviewVariant === "forged-review-file") {
    const forged = path.join(this.temp("asc-forged-review-"), "review.json");
    writeJson(
      forged,
      completeReview(this.finalCommitSha, {
        candidateEvidence: {
          implementationCommitSha: this.implementationCommitSha,
          finalCommitSha: this.finalCommitSha,
          implementationTreeSha: "a".repeat(40),
          implementationIsAncestor: true,
          changedPaths: [this.artifactPath],
          artifact: {
        ...
        externalEvidence: {
          ...
          implementation: {
            repository: "o/r",
            commitSha: this.implementationCommitSha,
    ...
    this.reviewEvidenceCli = executeBinIn(
      this.root,
      ["review", "validate", `--file=${forged}`],
      this.reviewCliEnv,
    );
    return;
  }
  const args =
    this.reviewVariant === "forged-file"
      ? [...common, `--external=${this.externalFile}`]
      : this.reviewVariant === "forged-implementer-option"
        ? [...common, ...provider, "--implementer-actor-id=forged-actor"]
        : [...common, ...provider];
...
Then(
  "実tree、diff、artifact hash、blobとH_finalのtrusted review gateが一致する",
  function () {
    assert.equal(
      this.reviewEvidenceCli.status,
      0,
      this.reviewEvidenceCli.stderr || this.reviewEvidenceCli.stdout,
    );
    ...
    assert.deepEqual(operations, [
      ...
      "repo view",
      "pr view",
      `api repos/o/r/commits/${this.implementationCommitSha}`,
      "api repos/o/r/actions/runs/32635972969",
      "api repos/o/r/pulls/835/reviews/9001",
    ]);
  },
);
Then("review evidenceは承認不能でgh呼出境界も守られる", function () {
  assert.notEqual(this.reviewEvidenceCli.status, 0);
  if (this.reviewEvidence?.candidateEvidence)
    assert.equal(
      evaluateReview(completeReview(this.finalCommitSha, this.reviewEvidence))
        .approved,
      false,
    );
  const called = fs.existsSync(this.ghLog)
    ? fs.readFileSync(this.ghLog, "utf8")
    : "";
  if (this.reviewVariant.startsWith("forged-")) assert.equal(called, "");
  else assert.match(called, /repo view/u);
});

Given(
  "passed current evidenceとlegacy fingerprint及び矛盾structured cacheがある",
  function () {
    this.boundValidation = {
      changedFiles: ["src/x.ts"],
      risk: ["quality"],
      evidence: {
        sha: SHA,
    ...
    const fingerprint = evidenceFingerprint(this.boundValidation);
    this.boundValidation.successfulFingerprints = [fingerprint];
