...
import { init, upgrade, uninstall, doctor } from '../../src/domain/lifecycle.js';
...
When('doctorを実行する', function () { this.result = doctor(this.root); });
Then('legacy directoryを2件報告する', function () { assert.deepEqual(this.result.legacyDetected.sort(), ['.agents', '.workflow']); });
Then('legacy runtime enabledはfalseである', function () { assert.equal(this.result.legacyRuntimeEnabled, false); });
...
Given('dirty fileを持つ一時Git repositoryがある', function () {
  ...
  this.statusBefore = execFileSync('git', ['status', '--porcelain=v1', '--untracked-files=all'], { cwd: this.root, encoding: 'utf8' });
});
When('新しいbranchと専用pathでworktreeを作成する', function () { this.result = createWorktree({ repoRoot: this.root, worktreePath: this.worktree, branch: 'feature/gherkin-worktree', base: 'main' }); });
Then('source dirty statusは作成前後で同一である', function () { const after = execFileSync('git', ['status', '--porcelain=v1', '--untracked-files=all'], { cwd: this.root, encoding: 'utf8' }); assert.equal(after, this.statusBefore); });
Then('専用worktreeに指定branchがある', function () { const branch = execFileSync('git', ['branch', '--show-current'], { cwd: this.worktree, encoding: 'utf8' }).trim(); assert.equal(branch, 'feature/gherkin-worktree'); });

Given('異なるorigin URLを持つ一時Git repositoryがある', function () {
  this.root = this.initRepo(); this.worktree = `${this.root}-wrong-remote`; this.temporaryDirectories.push(this.worktree);
  execFileSync('git', ['remote', 'add', 'origin', 'https://github.com/wrong/repository.git'], { cwd: this.root });
});
Given('期待repository文字列を一部に含む別originの一時Git repositoryがある', function () {
  this.root = this.initRepo(); this.worktree = `${this.root}-substring-remote`; this.temporaryDirectories.push(this.worktree);
  execFileSync('git', ['remote', 'add', 'origin', 'https://github.com/evil/expected/repository.git'], { cwd: this.root });
});
When('期待repositoryを指定してworktreeを作成する', function () { try { createWorktree({ repoRoot: this.root, worktreePath: this.worktree, branch: 'feature/must-not-exist', base: 'main', expectedRepository: 'expected/repository' }); } catch (error) { this.error = error; } });
...
Given('remoteへpush済みのcleanな専用worktreeがある', function () {
  ...
  this.worktree = `${this.root}-pushed-worktree`; this.temporaryDirectories.push(this.worktree);
  createWorktree({ repoRoot: this.root, worktreePath: this.worktree, branch: 'feature/recovery', base: 'main' });
  fs.writeFileSync(path.join(this.worktree, 'implemented.txt'), '実装済み\n');
  execFileSync('git', ['add', 'implemented.txt'], { cwd: this.worktree });
  execFileSync('git', ['commit', '-m', 'test: recovery fixture'], { cwd: this.worktree });
  execFileSync('git', ['push', '-u', 'origin', 'feature/recovery'], { cwd: this.worktree });
});
When('finalize stateをread-onlyで検査する', function () {
  this.result = inspectFinalizeState(this.root, this.worktree, { repository: 'o/r', base: 'main', specConsistent: true, testsPassed: true, reviewApproved: true, prMerged: true });
});
