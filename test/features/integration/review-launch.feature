...
Feature: reviewer役割のローカルLLM実行と信頼境界
  ...
  Scenario: SCN-INTEGRATION-REVIEW-1425-004b 正しいtrusted policyではdispatchされる
  ...
  Scenario: SCN-INTEGRATION-REVIEW-1425-005b timeoutはunknownとして扱う
    Given 応答しないfake Ollamaサーバーがある
    When executeLocalLlmを実行する
    Then 実行結果はunknownである

  Scenario: SCN-INTEGRATION-REVIEW-1425-005c 容量上限超過はunknownとして扱う
    Given 容量上限を超える応答をするfake Ollamaサーバーがある
    When executeLocalLlmを実行する
    Then 実行結果はunknownである

  Scenario: SCN-INTEGRATION-REVIEW-1425-006 supplementモードはローカルLLM未設定でも既存レビューを妨げない
    Given ローカルLLM未設定のtrusted policy fixtureがある
    When DIしたexecutorでlaunchReviewを実行する
    Then launchReviewはrejected状態を返す

  Scenario: SCN-INTEGRATION-REVIEW-1425-007 replaceはgit管理下trusted policyで明示宣言した場合だけ成立する
