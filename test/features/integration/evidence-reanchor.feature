...
Feature: 証跡再固定がCLIと診断経路で機能する
  ...
  @issue-1493
  Scenario: SCN-1484-01 既定branch追随を伴う前進commitをreviewed-forwardで受理する
  ...
  @issue-1493
  Scenario: SCN-1484-02 oldBaseShaのancestorでないnewBaseShaを再固定しない
    Given pr-bound後に既定branchが前進した前進実装と、oldBaseShaのancestorではない無関係commitをnewBaseShaに指定した再固定入力がある
    When 同じ再固定入力でpreviewとapplyをCLIから実行する
    Then reviewed-forwardのpreviewとapplyは拒否され追記しない

  @issue-1493
  Scenario: SCN-1484-03 解決できないnewBaseShaを再固定しない
