...
Feature: モード判定質問の正本
  ...
  Scenario: SCN-UNIT-MODEQ-023 Q-01が対象を外部提供へ限定している
    Given 公開されたモード判定質問がある
    When Q-01の質問文を読む
    Then 外部提供への限定と内部仕様の除外と確認できない場合のfalseが含まれる

  Scenario: SCN-UNIT-MODEQ-024 正本にQ-01の判定例がある
    Given 開発ワークフローの正本がある
    When モード判定質問の判定例を検査する
    Then 真の例と公開生成物へ影響する偽の例と確認できない場合の偽が各1行ある

  Scenario: SCN-UNIT-MODEQ-025 正本がQ-01の確認対象と条件付きの表記修正文を持つ
    Given 公開されたモード判定質問がある
    When Q-01の確認対象の記述を表示本文で検査する
    Then 確認対象4項目と条件付きの表記修正文と既存の注意文が存在する
