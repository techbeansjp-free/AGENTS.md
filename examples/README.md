...
**使い方**: 各ディレクトリの README に **コピペで実行できるコマンド** を記載しています。プロジェクトルートで `cp` 等を実行するだけで、そのレベルまで導入できます。実際のファイルは本リポジトリ（AGENTS-spec）のルートからコピーするため、examples 内に実ファイルの二重配置はありません。
