...
| 5 | 回帰テスト追加 | worktreeファイルシステムに無いがtarget_sha上に存在/digest一致/digest不一致/target_sha上にも無い/implementation gateでsentinel許容/implementation以外でsentinel拒否、の6パターンを検証するテストを追加 | `AC-1`〜`AC-6` | `#2, #3, #4` |
| 6 | 検証 | `npm run build`・`npm test` | `AC-1`〜`AC-6` | `#1〜#5` |
