...
| 4 | 分類後の判定集約 | 有効sub-verdictの4条件導出（4条件目の裏付けはレビュア単位で判定し、findingへ観点識別子を追加しない）と、判定値の順序評価を集約の1箇所へ実装。最終ラウンドは`approved`条件を満たさなければ`rejected`ではなく`human_required`とし、それ以外のラウンドは従来どおり`rejected`とする優先関係を同じ箇所へ結線する。raw値は書き換えず`subverdict_reclassification`へ併記し、gate-report schemaへ任意フィールドを追加 | AC-3〜AC-6, AC-8 | #2, #3 |
...
- 判定集約: 有効sub-verdictの4条件（宣言成立・raw `inconclusive` false・未分類blockingの不在・当該レビュアによるblocking findingの提出）をそれぞれ単独で崩した入力で `approved` にならないこと、崩した入力が最終ラウンドでは `rejected` ではなく `human_required` へ収束し最終ラウンド以外では `rejected` になること、4条件成立時のみ最終ラウンドが `approved` になること、raw値が現行記録に残ること、有効sub-verdictを記録した `approved` が既存のpublish整合検査を通ることを検証する。
- 反証ケース: round取得不能、宣言なし・レビュー開始後/結果後の追加・上書き、直前attempt/digest不一致、非trustedな投稿者による宣言・分類記録、非trusted記録単独でのゲート停止、current recordの元severity欠落・raw evidence変更、判定不能を表明したattempt、未分類blockingの残存、blocking findingを伴わないraw `fail`、follow-up永続化失敗、データ喪失・セキュリティ低下、理由なし必要追加、worker 1ロールだけの契約欠落をそれぞれ失敗または安全側停止として検証する。
