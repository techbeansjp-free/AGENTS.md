...
| 2 | `agent-skill-chain-gate.yml: pending時action_required Check Run発行 + Publish Check Runステップのpendingスキップ` | `verify-and-publish`ジョブの`Verify gate report schema`ステップ（D2）へ`id: schema`を付与し、`GH_TOKEN`/`GATE_ID`(`matrix.gate`)/`HEAD_SHA`(`needs.detect-segments.outputs.target_sha`)環境変数を追加する。`set +e`で`verify-gate-report.sh`のexit codeを捕捉し、`CODE -eq 2`の場合のみ`Verify local-review evidence`ステップと同型の`node -e`＋`gh api -X POST repos/{owner}/{repo}/check-runs`で`name: agent-skill-chain/${GATE_ID}-gate`, `conclusion: action_required`のCheck Runを発行したうえで`echo "pending=true" >> "$GITHUB_OUTPUT"`してから`exit 0`する。`CODE -eq 0`の場合は`echo "pending=false" >> "$GITHUB_OUTPUT"`してからそのまま終了（既存挙動維持）。`CODE -eq 1`はそのまま`exit "$CODE"`（既存挙動維持、pending出力は書かずジョブが停止する）。続く`Publish Check Run`ステップ（`gate-publish.sh`経由で`src/commands/gate.ts`の`publish()`を呼ぶ）の現状無条件実行（if条件なし）を`if: steps.schema.outputs.pending != 'true'`へ変更する。`publish()`関数（`report.gate.final === 'pending'`を拒否するガード含む）自体は変更しない。 | `AC-1, AC-2, AC-3` | `#1` |
