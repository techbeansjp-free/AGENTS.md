...
| 12 | 自動テスト整備 | AC-2〜AC-6・AC-9・AC-10に対応する自動テスト（init/upgradeの新規プロファイル分岐、スキル配置・同期、config schema、GLOSSARY/CONFIGURATION見出し検査）を実装セグメントで追加・実行する。AC-9のテストは出力メッセージではなくpre-flight方式（対象集合拡大後も1件でも衝突があれば1件も書き込まずに停止する既存の衝突時非破壊方針が維持されること）を検証対象とする | AC-2, AC-3, AC-4, AC-6, AC-9, AC-10 | 1〜9, 11 |
