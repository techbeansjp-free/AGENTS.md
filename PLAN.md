...
| 4 | `src/commands/upgrade.ts` 修正 | 所有権記録読込 → `stale-assets.ts` で候補算出・（非dry-runなら）削除実行 → 結果を `summary` へ追記 → 削除失敗があれば成功結果を先に出力してから非ゼロ終了 → 非dry-run時のみ所有権記録を更新（削除成功エントリのみ除去、削除失敗・読み取り失敗エントリは保持） | `AC-2, AC-3, AC-4, AC-5, AC-6, AC-7, AC-8, AC-9, AC-10, AC-11` | `#1, #2` |
