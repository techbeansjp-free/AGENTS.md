...
| 6 | claude.sh launch_worker分岐 | `launch_worker()`冒頭に分岐追加、新関数`_dispatch_via_agent_tool()`を実装（lease取得→segment start→`mktemp -d`一時ディレクトリへcontract.md書き出し→`_dispatch_lease_renew_daemon()`を`setsid`で起動しrenew.pid書き出し→定型文＋contract.mdパス＋`DISPATCH_TEMP_DIR=<一時ディレクトリの絶対パス>`行を標準出力→exit 4）。contract本文を標準出力へ書かない点、workerサブプロセスは起動しないがrenewalデーモンは起動する点、`DISPATCH_TEMP_DIR=`行が`#7`のverifyスクリプトへの引数受け渡し手段である点に注意。既存フルフロー本体は無変更のままelse節に残す | `AC-1, AC-2, AC-4` | `#4, #5` |
| 7 | worker-launch-verify.sh新規作成 | `worker-launch.sh`のworktree自己解決ロジックを再利用し、第1位置引数として一時ディレクトリの絶対パスを必須で受け取る（`#6`が出力する`DISPATCH_TEMP_DIR=`行の値、未指定・存在しないパスはexit 1即時失敗）。有効な場合はまず当該ディレクトリの`renew.pid`があれば対応プロセスをkillしディレクトリごと削除、続けて`report latest`とHEAD SHA照合→`release_lease`（一致）または`report_status blocked`+`release_lease`（不一致・未報告）を実装 | `AC-1, AC-3, AC-5` | `#6` |
