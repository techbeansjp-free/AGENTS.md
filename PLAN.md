...
| 3 | 複数clone間の出力完全一致テスト | 新規テストファイル（例: `test/integration/gate-reviewer-prompt-determinism.test.ts`）を追加し、DESIGN.md「AC-2テスト構築方針」節のとおり (a) `core.abbrev`を明示的に異なる値（`7`・`12`・auto相当の未設定）に設定した複数clone、(b) 総オブジェクト数を実際に大きく変えた複数clone（新規clone vs 追加blob投入clone）の両方で`gate reviewer-prompt`を実行し、出力バイト列が全clone間で完全一致すること・`evidencePromptDigest()`によるdigestも一致することを検証する | `AC-2` | `#1` |
