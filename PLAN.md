...
| 3 | 配布不在・本体存在の単体テスト、およびAC-1/AC-2 integrationテスト追加 | (a) `.agent-skill-chain/templates/github/.github/workflows/`配下に`agent-skill-chain-release.yml`が存在しないことを検査するテスト、(b) `.github/workflows/agent-skill-chain-release.yml`が存在し、既存ジョブ名`release`・トリガ`push: branches: [main]`・`secrets.RELEASE_MAIN_PAT`参照ステップが保持されていることを検査するテストを`test/unit/`に追加する。(c) `test/integration/setup.test.ts`の既存テスト（`mkScratch`で一時ディレクトリを作成し`runCli(['setup', 'github', targetDir], ...)`をsubprocess実行、`fs.existsSync`で展開先を検査する構造）に倣い、新規の一時target_dirへ`setup github`を実際に実行し、展開された`<target_dir>/.github/workflows/`に`agent-skill-chain-release.yml`が含まれないことを検証するintegrationテストを追加する（DESIGN.md D4(e)、AC-1のGiven/Whenを直接自動化する。単体テスト(a)は配布元テンプレートの静的確認に留まりCLI実行を伴わないため、この(c)がAC-1の自動検証を担う）。(d) `test/integration/upgrade.test.ts`の既存テスト（`runCli(['init', targetDir])`で導入した状態を起点に`runCli(['upgrade', targetDir])`を実行しファイルシステムの変化を検証する構造）に倣い、`init`実行済み・`setup github`実行済みで`agent-skill-chain-release.yml`を保持しない既存consumer相当の一時ディレクトリを用意したうえで`upgrade`実行後に`setup github`を再実行し、`<target_dir>/.github/workflows/agent-skill-chain-release.yml`が作成されないことを検証するintegrationテストを追加する（DESIGN.md D4(f)、AC-2のGiven/Whenを直接自動化する）。(c)(d)はいずれも`#1`（テンプレート側削除）の完了後でなければ「除外されている」というGivenを再現できないため、`#1`完了後に実装する。 | `AC-1, AC-2, AC-3` | `#1, #2` |
...
- 常時必須: lint/format・型検査・単体テスト（`#3`(a)(b)・`#4`・`#7`で追加・実行）・変更範囲の結合テスト（`#3`(c)(d)がAC-1/AC-2それぞれのGiven/When/Thenを`setup github`/`upgrade`の実CLI実行で自動検証するintegrationテストとして担う）・secretスキャン（`RELEASE_MAIN_PAT`という文字列自体は既存のADR-0005/0007で承認済みの識別子であり新規secret値を導入しないため、スキャン対象は「値」ではなく既存の文字列一致のみ）。
