...
artifact集合、nonce recovery、Strict slot adapter、配布同期と自動テストを出力する。実装前に#277の
純粋reducerと#283のdedicated App backendをmainへ取り込み、ADR-0010/0013のacceptedを確認する。
依存未成立時は実装を開始せず、類似reducer、別App、旧publisher fallbackを作らない。

## 実装順序

| # | 変更単位 | 内容 | AC-ID | 依存 |
|---|---|---|---|---|
| 1 | accepted依存統合 | #277/#283をrebaseしAPI・App/environment/ruleset契約を固定 | AC-2, AC-6 | なし |
| 2 | ADR参照確定 | DESIGNの`related_adrs`へADR-0010/0013を`id/relation: adopts` objectで追加し再gate | 全AC | #1 |
...
| 8 | Strict seam | durable envelopeを#277 pure reducerへ写像しreplayを外部化 | AC-6 | #5〜#7 |
...
| Strict別actor/invocation 2 approve | reduce | pure reducerがapprovedを返しparent successへ写像する | AC-6 |
| Strict不足/重複/replay/混合 | reduce | reducer外でreplay拒否し、reducerはfailure/action_requiredを返す | AC-6 |
...
- unit: gate順、一意key、classifier、tombstone、canonical digest、state mapper、nonce reducer、pure reducer seam。
