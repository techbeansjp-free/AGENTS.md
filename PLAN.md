...
| 2 | package.json書込み・mainコミッタ | `needCommit` 時に `package.json.version` を `target` へ書換え、`chore(release): v<target> [skip ci]` でcommitし `RELEASE_MAIN_PAT` で main へpush | `AC-1`, `AC-6` | `#1` |
