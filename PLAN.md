...
変更単位#1・変更単位#2（恒久検査2件・AC-5自己検証テストケースの単一fixture方式・複数fixture化・Issue #332除外コメント・テストコメント内の陳腐化参照の是正を含む）はいずれも実装済みである（変更単位#2の備考欄参照）。下記手順0〜10はいずれも本Issue完了後を含めいつでも再実行可能な検証として位置づける。手順0（negative control）は`git show`によるbase commit参照方式であり作業ツリーの状態に依存しないため、手順1〜10との固定の実行順序を持たない。全手順が成功することを確認する。
...
10. **テストコメント内の陳腐化参照の是正確認（automated + manual）**: `grep -n 'DESIGN\.md' test/unit/worker-selection-reference.test.ts` を実行し、出力が0件（終了コード1）であることを確認する。これにより、DESIGN.md「テストコードコメントにおける陳腐化参照の禁止（設計制約）」が是正を指示した「（DESIGN.md「回帰防止テストの拡張設計」参照）。」という一文（Issue単位の一時成果物であるDESIGN.mdの見出しへの陳腐化参照）が削除されていることを確認する。あわせて、置き換え後の文言を目視レビューし、元のコメントが伝えていた設計判断の要旨（単一実例のみでは正規表現の縮退〔vacuous pass〕に気付けないため、桁数・空白パターンの異なる複数独立fixtureで検証するという趣旨）が参照を用いず直接書き下された形で引き続き読み取れることを確認する。

## 実装順序の見直しについて

変更単位#1（コメント是正）は実装済み（commit `f56cde1`）。変更単位#2（テスト拡張）のうち恒久検査2件はcommit `f56cde1`、AC-5自己検証テストケース（単一fixture方式）はcommit `1758e21`、複数fixture化・Issue #332除外コメントの付与はcommit `2c7f1bfd1`として、いずれも実装済みである。commit `2c7f1bfd1` で追加されたコメント自体に含まれていた陳腐化参照（「（DESIGN.md「回帰防止テストの拡張設計」参照）。」）についても、commit `7160b5b`（ISSUE-325: 回帰テストコメントのDESIGN.md見出し参照を除去し設計判断を直接書き下し）で参照句を削除し設計判断の要旨を直接書き下す是正が実装済みであり、検証手順10で確認できる。実装順序としては、変更単位#2（テスト拡張）が変更単位#1（コメント是正）の完了後に着手する順序を採った（変更単位#2のテストが検査するパターンの不在は、変更単位#1の完了を前提とするため）。手順0（negative control）は`git show`によるbase commit参照方式へ変更したため、現在の作業ツリーの状態（変更単位#1・#2の完了前後）に関わらずいつでも独立して実行可能であり、他の手順との固定の実行順序を持たない——本Issue完了後もいつでも再実行できる恒久的な検証として位置づける。両単位内での検証手順3〜5・8・10の実行順序を入れ替える必要が生じた場合も、DESIGN.md の更新・設計ゲートの再通過は不要であり、本ファイルのみを更新すればよい。
