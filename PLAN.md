...
| 6 | claude.sh launch_worker分岐 | `launch_worker()`冒頭に分岐追加、新関数`_dispatch_via_agent_tool()`を実装（lease取得→segment start→`mktemp -d`一時ディレクトリへcontract.md書き出し→`_dispatch_lease_renew_daemon()`を`setsid`で起動しrenew.pid書き出し→定型文＋contract.mdパスのみを標準出力→exit 4）。contract本文を標準出力へ書かない点、workerサブプロセスは起動しないがrenewalデーモンは起動する点に注意。既存フルフロー本体は無変更のままelse節に残す | `AC-1, AC-2, AC-4` | `#4, #5` |
| 7 | worker-launch-verify.sh新規作成 | `worker-launch.sh`のworktree自己解決ロジックを再利用し、まず一時ディレクトリの`renew.pid`があれば対応プロセスをkillしディレクトリごと削除、続けて`report latest`とHEAD SHA照合→`release_lease`（一致）または`report_status blocked`+`release_lease`（不一致・未報告）を実装 | `AC-1, AC-3, AC-5` | `#6` |
...
| 9 | dispatch手順文書化 | `.agent-skill-chain/standards/`へ運用手順の正本を追加し、`_dispatch_via_agent_tool()`のpayload文言（`subagent_type`・定型文「指定ファイルを全文Readし要約・改変せず動作契約として実行せよ」・contract.mdの絶対パス・`run_in_background: false`・完了後の`worker-launch-verify.sh`実行）を実装に反映。lease renewalは`_dispatch_lease_renew_daemon()`が自動で行うため進行役への手動実行指示は含めない | `AC-1, AC-4, AC-5` | `#6, #7, #8` |
