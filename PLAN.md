...
| 2 | bumpブランチ・PR作成／admin merge器 | `needCommit` 時に `package.json.version` を `target` へ書換え、短命ブランチ `release/bump-v<target>` 上に `chore(release): v<target> [skip ci]` でcommitして `GITHUB_TOKEN` でpush、`gh pr create` で機械生成の版数台帳更新PRを作成、`gh pr merge --admin --squash`（bypass_actor登録済み `RELEASE_MAIN_PAT`）で main へマージ。ブランチ名の `target` 埋め込みで重複防止、既存ブランチ・PRは再利用（冪等）。生pushではなくPR経由でありI4を満たす | `AC-1`, `AC-6` | `#1` |
