...
export interface StepDefinitionSource {
  ...
  readonly patterns: readonly StepPattern[];
  /** patternを字面から取り出せない登録（動的文字列など）を含む場合false */
  readonly complete: boolean;
}

export interface FeatureStepBinding {
  /** featureごとに、そのstepへ一致したstep定義file（昇順） */
  readonly byFeature: Readonly<Record<string, readonly string[]>>;
  /** どのstep定義にも一致しないstepを含むfeature（昇順） */
  readonly unmatchedFeatures: readonly string[];
  /** 解釈できなかったfeature（言語指定が英語以外など、昇順） */
  readonly unparsedFeatures: readonly string[];
}

const STEP_KEYWORD = /^(?:Given|When|Then|And|But|\*)\s+(.*)$/u;
const OUTLINE_KEYWORD = /^(?:Scenario Outline|Scenario Template):/u;
const BLOCK_KEYWORD =
  /^(?:Feature|Rule|Background|Scenario|Example|Scenario Outline|Scenario Template):/u;
...
const PARAMETER_SOURCES: Readonly<Record<string, string>> = Object.freeze({
  string: `(?:"[^"]*"|'[^']*')`,
  int: "-?\\d+",
  float: "-?(?:\\d+\\.?\\d*|\\.\\d+)(?:[eE][-+]?\\d+)?",
  word: "[^\\s]+",
...
function convertExpressionFragment(fragment: string): string {
  let output = "";
...
function splitAlternation(word: string): string[] {
  const parts: string[] = [];
  let current = "";
  let depth = 0;
 ...
 */
export function cucumberExpressionSource(expression: string): string {
  let output = "";
  let run = "";
  const flush = (): void => {
    if (run === "") return;
...
function literalPrefix(pattern: StepPattern): string {
  if (pattern.kind === "regex") return "";
...
function compilePattern(path: string, pattern: StepPattern): CompiledPattern {
 ...
 */
export function parseFeatureSteps(text: string): readonly string[] | undefined {
  const lines = text.split(/\r?\n/u);
  const steps: string[] = [];
  let docString: string | undefined;
  let outlineSteps: string[] | undefined;
  let exampleHeader: string[] | undefined;
  ...
  for (const raw of lines) {
    ...
    if (language !== null && language[1] !== "en") return undefined;
    ...
    if (line.startsWith("|")) {
      if (!inExamples || outlineSteps === undefined) continue;
      ...
      for (const step of outlineSteps)
        steps.push(
          exampleHeader.reduce(
            (current, name, index) =>
              current.split(`<${name}>`).join(values[index] ?? ""),
            step,
          ),
        );
      continue;
  ...
  if (outlineSteps !== undefined && !inExamples) steps.push(...outlineSteps);
  return steps;
}
...
export function bindFeaturesToStepDefinitions(input: {
  ...
  readonly definitions: readonly StepDefinitionSource[];
}): FeatureStepBinding {
  const compiled = input.definitions.flatMap((definition) =>
    definition.patterns.map((pattern) =>
      compilePattern(definition.path, pattern),
    ),
  );
  const byFirstCharacter = new Map<string, CompiledPattern[]>();
  const unprefixed: CompiledPattern[] = [];
  ...
  const matchStep = (step: string): readonly string[] => {
    ...
    for (const candidate of candidates) {
      if (!step.startsWith(candidate.prefix)) continue;
      candidate.regex.lastIndex = 0;
      if (candidate.regex.test(step)) paths.add(candidate.path);
    }
    const result = [...paths].sort();
    cache.set(step, result);
    return result;
  };
  const byFeature: Record<string, readonly string[]> = {};
  const unmatchedFeatures: string[] = [];
  const unparsedFeatures: string[] = [];
  for (const feature of [...input.features].sort((left, right) =>
