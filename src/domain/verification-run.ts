 ...
 */
export const VERIFICATION_RUN_SCHEMA_VERSION =
  "agent-skill-chain/verification-run/v1" as const;
...
export const VERIFICATION_SCOPES = ["targeted", "full"] as const;
...
export interface VerificationRunRecord {
  readonly schemaVersion: typeof VERIFICATION_RUN_SCHEMA_VERSION;
...
export type VerificationRunBody = Omit<VerificationRunRecord, "recordDigest">;

/** review証跡へ埋め込む検証欄。記録から写すだけで、値を足さない。 */
 ...
 */
export function validateVerificationArgv(
  value: unknown,
  label = "検証command",
): readonly string[] {
  if (
    ...
    throw new Error(
      `${label}は1〜${MAX_VERIFICATION_ARGV}要素のargvが必要です`,
    );
  return Object.freeze(
    value.map((argument, index) => {
      if (
        ...
        throw new Error(
          `${label}[${index}]は制御文字を含まない空でない${MAX_ARGUMENT_BYTES} byte以下のNFC文字列が必要です`,
        );
      /** argvは記録とcommitされる証跡へ残るため、秘密情報らしき値を拒否する */
      if (redactSecrets(argument) !== argument)
        throw new Error(
          `${label}[${index}]に秘密情報らしき値があります。argvは証跡へ残るため環境変数で渡してください`,
        );
      return argument;
    }),
  );
...
export function verificationRunDigest(body: VerificationRunBody): string {
  const withoutDigest = Object.fromEntries(
    Object.entries(body).filter(([field]) => field !== "recordDigest"),
  );
  return crypto
    .createHash("sha256")
...
function parseRecordValue(
  ...
  if (
    typeof scope !== "string" ||
    !(VERIFICATION_SCOPES as readonly string[]).includes(scope)
  )
  ...
  if (record.scope === "targeted" && record.impactMode === "full")
    throw new Error(
      `${label}.scope=targetedは影響集合がfullのとき記録できません`,
    );
  const body: VerificationRunBody = {
    ...
    scope: scope as VerificationScope,
    impactDigest: pattern(
      record.impactDigest,
      SHA256,
      `${label}.impactDigest`,
      "sha256",
    ...
    stdoutDigest: pattern(
      record.stdoutDigest,
      SHA256,
      `${label}.stdoutDigest`,
      "sha256",
    ),
    stderrDigest: pattern(
      record.stderrDigest,
      SHA256,
      `${label}.stderrDigest`,
      "sha256",
...
export function sealVerificationRun(
  body: VerificationRunBody,
): VerificationRunRecord {
  return parseRecordValue(
    { ...body, recordDigest: verificationRunDigest(body) },
    "検証記録",
  );
}

export function renderVerificationRun(record: VerificationRunRecord): string {
 ...
 */
export function parseVerificationRuns(
  source: string,
  ...
  if (lines.length > MAX_VERIFICATION_RUNS)
    throw new Error(
      `検証記録は${MAX_VERIFICATION_RUNS}件以下が必要です。stagingを整理してください`,
    );
...
export function renderVerificationRuns(
 ...
 */
export function missingTargetedFeatures(
...
export function selectObservedVerification(
...
): readonly ObservedVerification[] {
  const matching = records.filter(
    (record) =>
      record.headSha === target.headSha &&
      record.impactDigest === target.impactDigest,
  );
  if (matching.length === 0) {
    const atHead = records.filter(
      (record) => record.headSha === target.headSha,
    ).length;
    throw new Error(
      atHead > 0
  ...
  for (const record of matching) {
    const index = latest.findIndex((item) =>
      sameArgv(item.command, record.command),
    );
    if (index >= 0) latest.splice(index, 1);
    latest.push(record);
  }
  const failed = latest.filter(
    (record) => record.exitCode !== 0 || record.signal !== null,
  );
  if (failed.length > 0)
    throw new Error(
      `最新の実行が不合格の検証commandがあります: ${failed
        .map(
          (record) =>
            `${JSON.stringify(record.command)}（exitCode=${String(record.exitCode)}, signal=${String(record.signal)}）`,
        )
        .join("; ")}`,
    );
 ...
 * いないことだけを受理する。**
 */
export function verificationRecordErrors(
  entries: readonly ObservedVerification[],
  records: readonly VerificationRunRecord[],
): string[] {
  const errors: string[] = [];
  for (const entry of entries) {
    const index = records.findIndex(
      (record) => record.recordDigest === entry.recordDigest,
    );
    const record = records[index];
    if (record === undefined) {
      errors.push(
        `review証跡の検証記録${entry.recordDigest}がstagingの観測記録にありません`,
      );
      continue;
    }
    if (
      !sameArgv(record.command, entry.command) ||
      ...
      errors.push(
        `review証跡の検証記録${entry.recordDigest}がstagingの観測記録と一致しません`,
      );
      continue;
    }
    const later = records
    ...
    if (later !== undefined && (later.exitCode !== 0 || later.signal !== null))
      errors.push(
        `review証跡の検証command ${JSON.stringify(record.command)} は同じH_implでの後続実行が不合格です`,
      );
  }
  return errors;
}
