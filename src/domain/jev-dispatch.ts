...
export interface JevDispatchRequestBody {
...
export function buildJevChoiceRequestBody(input: {
  ...
  const missingCriteria = input.question.options.filter(
    (option) => !(option in input.question.criteria),
  );
  if (missingCriteria.length > 0)
    throw new Error(
      `Jev choice questionのcriteriaに未記載のoptionがあります: ${missingCriteria.join(", ")}`,
    );
  return {
    model: input.model,
    questions: {
 ...
 * 「そもそも送らない」）。
 */
const EGRESS_DENIED_KEY_PATTERN =
  /^(?:token|password|secret|api[_-]?key|apikey|databaseurl|connectionstring|privatekey|authorization|cookie|credential)$/iu;
...
export type JevEgressValidation =
 ...
 * 「疑わしければ止める」設計を踏襲する）。
 */
export function validateJevEgressPayload(
  value: unknown,
  keyPath = "$",
): JevEgressValidation {
  if (Array.isArray(value)) {
    for (let index = 0; index < value.length; index += 1) {
      const result = validateJevEgressPayload(
        value[index],
        `${keyPath}[${index}]`,
      );
...
export type JevDispatchOutcome =
  | {
      readonly kind: "ok";
      readonly resolvedModel: string;
      readonly answer: JevChoiceAnswer;
      readonly usage: JevUsage;
    }
  | { readonly kind: "schema-error"; readonly detail: string }
...
function extractPydanticDetail(body: unknown): string {
...
export function parseJevChoiceResponse(
  ...
  if (status === 200) {
    ...
    const answerRaw = body.answers[questionKey] as Record<string, unknown>;
    if (
      answerRaw.type !== "choice" ||
      ...
      return {
        kind: "schema-error",
        detail:
          "answers[key]の形状が期待（type/choice/confidence/probabilities）と不一致です",
      };
    ...
    return {
      ...
      answer: {
        ...
        probabilities: answerRaw.probabilities as Record<string, number>,
      },
  ...
  if (status === 429) return { kind: "rate-limited", retryAfterMs };
  return {
