 ...
 */
function changeLogTermCitations(file: string): {
  ids: string[];
  ranges: string[];
} {
  const ids: string[] = [];
  const ranges: string[] = [];
  if (!fs.existsSync(file)) return { ids, ranges };
  for (const line of fs.readFileSync(file, "utf8").split(/\r?\n/u)) {
    const cells = tableCells(line);
    const column = cells[CHANGE_LOG_TERM_COLUMN];
    if (column === undefined) continue;
    const cited = [...column.matchAll(/TERM-[A-Z0-9][A-Z0-9-]*/gu)].map(
      (match) => match[0],
    );
    if (cited.length === 0) continue;
    ids.push(...cited);
    if (/TERM-[A-Z0-9][A-Z0-9-]*\s*[〜~]/u.test(column)) ranges.push(column);
  }
  return { ids, ranges };
}
...
function validateDomainGlossary(file: string): string[] {
   ...
   */
  const citations = changeLogTermCitations(
    path.join(path.dirname(path.dirname(file)), SPEC_CHANGE_LOG),
  );
 ...
 */
export function requiresSpecUpdate(changedFiles: readonly string[]): boolean {
