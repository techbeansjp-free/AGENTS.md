...
export function parseMetricsEventLine(
  ...
  if (!nonEmptyString(value.label))
    ...
    errors.push(
      `${label}.labelはkind=roleのときROLES列挙値（${ROLES.join("・")}）のいずれかが必要です`,
    );
...
export function openIntervalsByKind(
  entries: readonly MetricsEvent[],
): Partial<Record<MetricsEventKind, OpenInterval>> {
  const open: Partial<Record<MetricsEventKind, OpenInterval>> = {};
...
export function validateNextMetricsEvent(
  ...
  candidate: Pick<MetricsEvent, "kind" | "phase" | "label"> &
    Partial<Pick<MetricsEvent, "recordedAt">>,
...
export interface StepDuration {
  /** 対象entryのStep番号。journalが同じStepを複数回記録する場合（reconfirm・再記録・
   * post-pr/post-terminal intake）は複数entryを持つため、Step番号だけをkeyにした
   * Recordへ縮約しない（独立reviewのH2指摘。以前の実装は後勝ちで上書きしていた）。 */
  step: number;
...
export interface EventDurations {
  ...
  /** 現在open状態のkind一覧。開いたままの区間は合計へ含まれず、この一覧で明示する。 */
  openKinds: MetricsEventKind[];
  warnings: string[];
}
...
export function computeEventDurationsMs(
  ...
  for (const entry of entries) {
    ...
    if (!Number.isFinite(fromMs) || !Number.isFinite(toMs)) {
      warnings.push(
        `kind=${entry.kind}のlabel="${started.label}"区間は時刻を解釈できないため除外しました`,
      );
    ...
    if (toMs < fromMs) {
      warnings.push(
        `kind=${entry.kind}のlabel="${started.label}"区間はend（${entry.recordedAt}）がstart（${started.recordedAt}）より前のため除外しました`,
      );
      delete open[entry.kind];
      continue;
    }
  ...
  const openKinds: MetricsEventKind[] = [];
  for (const [kind, interval] of Object.entries(open) as Array<
    [MetricsEventKind, OpenInterval | undefined]
  >) {
    if (interval) {
      openKinds.push(kind);
      warnings.push(
...
export function computeSupportArtifactSplit(input: {
  ...
  if (input.roleOpen) return { artifact_build_ms: null, support_ms: null };
