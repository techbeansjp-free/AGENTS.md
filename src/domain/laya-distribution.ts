...
export const LAYA_DECISION_BUNDLE_SCHEMA_VERSION =
  "asc/laya-decision-bundle/v1" as const;
...
export interface DistributionAssessment {
  classification: DistributionClass;
  reasons: string[];
}

export interface LayaDecisionBundle {
  schemaVersion: typeof LAYA_DECISION_BUNDLE_SCHEMA_VERSION;
  bundleId: string;
  model: {
    id: string;
    path: string;
...
function assertDigest(value: string, label: string): void {
  if (!SHA256.test(value)) throw new Error(`${label}はSHA-256が必要です`);
}
...
export function assessDistribution(
  evidence: readonly DistributionEvidence[],
  ...
  const requiredKinds = new Set<DistributionArtifactKind>([
    "source",
    "dist",
    "schema",
    "skill",
  ...
  for (const item of evidence) {
    ...
    assertExactKeys(
      item,
      ["kind", "path", "expectedSha256", "observedSha256", "succeeded"],
  ...
  if (consumerBreak) return { classification: "consumer-break", reasons };
  if (packageDrift) return { classification: "package-drift", reasons };
  if (unknown) return { classification: "unknown", reasons };
  return { classification: "no-impact", reasons: [] };
}

export function validateDecisionBundle(
  value: LayaDecisionBundle,
  observedWeightSha256: string,
  currentAscVersion: string,
): LayaDecisionBundle {
  assertExactKeys(
    value,
    [
  ...
  if (!SAFE_ID.test(value.bundleId)) throw new Error("bundleIdが不正です");
  assertExactKeys(
    value.model,
    ["id", "path", "weightSha256"],
    "DecisionBundle.model",
  );
  if (!SAFE_ID.test(value.model.id)) throw new Error("model.idが不正です");
  for (const [label, artifactPath] of [
    ...
    if (!safeRelativePath(artifactPath)) throw new Error(`${label}が不正です`);
  ...
  if (
    value.compatibleAscVersions.length === 0 ||
  ...
  if (value.eligibility !== "eligible")
    throw new Error(
      "eligibleではないcandidateをDecision Bundleへ利用できません",
    );
  return value;
}

export function decisionBundleDigest(value: LayaDecisionBundle): string {
