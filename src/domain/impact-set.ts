...
import type { FeatureStepBinding } from "./cucumber-binding.js";
import type { ReviewAdjacentScope } from "./review-convergence.js";
import {
 ...
 */
export const IMPACT_SET_SCHEMA_VERSION =
  "agent-skill-chain/impact-set/v1" as const;
...
export type ImpactMode = "targeted" | "full";

export interface ImpactSet {
  readonly schemaVersion: typeof IMPACT_SET_SCHEMA_VERSION;
  readonly baseSha: string;
  readonly headSha: string;
  readonly changeDigest: string;
  readonly changedPaths: readonly string[];
  readonly graphContentHash: string | null;
  /** 変更source fileの直接import元と直接import先（変更pathを除く、path昇順） */
  readonly adjacent: readonly ReviewAdjacentScope[];
  readonly securitySensitive: boolean;
  readonly securityPaths: readonly string[];
  /** 実行するfeature file（targetedのときだけ非空になり得る） */
  readonly features: readonly string[];
  /** 追跡表経由で到達したscenario ID */
   ...
   * `adjacent[].graphEvidence`はこの値そのものである（循環を避けるため除外する）。
   */
  readonly digest: string;
}

export interface StepDefinitionFileSummary {
  readonly path: string;
  readonly complete: boolean;
  readonly global: boolean;
}

export interface ImpactDerivationInput {
  readonly baseSha: string;
  readonly headSha: string;
  readonly changeDigest: string;
  readonly changedPaths: readonly string[];
  ...
  readonly literalReferences: Readonly<Record<string, readonly string[]>>;
  /**
   * Cucumberへ登録するfile。`complete: false`は字面から取り出せないstep登録を含み、
   * `global: true`はhook・World等の全scenarioへ効く登録を含む。
   */
  readonly stepDefinitionFiles: readonly StepDefinitionFileSummary[];
 ...
 */
const INFRASTRUCTURE_PATTERNS: readonly RegExp[] = Object.freeze([
  /^package\.json$/u,
  /^package-lock\.json$/u,
...
const SECURITY_NAME =
  /auth|trust|secret|permission|policy|merge|delivery|release|reanchor|evidence|review-convergence|jev-/u;

export function isInfrastructurePath(changedPath: string): boolean {
 ...
 */
export function isSecuritySensitivePath(candidate: string): boolean {
  if (SECURITY_EXACT.has(candidate)) return true;
  if (candidate.startsWith(".github/")) return true;
...
export function referenceNames(candidate: string): string[] {
  const name = basename(candidate);
  const typed = TYPESCRIPT_EXTENSION.exec(name);
  if (typed === null) return [name];
...
interface GraphIndex {
  ...
  /** file → そのfileをsatisfied-byに持つscenario ID */
  readonly scenariosBySatisfyingFile: ReadonlyMap<string, readonly string[]>;
  /** scenario ID → verified-byのfeature file */
  readonly featuresByScenario: ReadonlyMap<string, readonly string[]>;
}
...
function indexGraph(snapshot: SemanticGraphSnapshot): GraphIndex {
  const files = new Set<string>();
  for (const node of snapshot.nodes)
    if (node.kind === "file") files.add(node.id.slice("file:".length));
  const importers = new Map<string, string[]>();
  const imports = new Map<string, string[]>();
  const scenariosBySatisfyingFile = new Map<string, string[]>();
  const featuresByScenario = new Map<string, string[]>();
  for (const edge of snapshot.edges) {
    if (edge.certainty !== "deterministic") continue;
    if (
      edge.kind === "imports" &&
    ...
    if (
      ...
      pushTo(
        ...
        edge.to.slice(5),
        edge.from.slice("scenario:".length),
      );
    if (
...
function classifyChangedPath(
  ...
  if (changedPath.endsWith(".feature")) return { kind: "feature" };
  if (changedPath.endsWith(".md")) return { kind: "document" };
  if (ECMASCRIPT_EXTENSION.test(changedPath)) {
    if (context.index?.files.has(changedPath)) return { kind: "source" };
    return {
      kind: "fallback",
...
function affectedClosure(
  ...
  index: GraphIndex,
  literalReferences: Readonly<Record<string, readonly string[]>>,
): Closure {
  const affected = new Set<string>();
  const literalExpanded = new Set<string>();
  let frontier = [...seeds];
  ...
  for (let round = 0; round < IMPACT_EXPANSION_LIMIT; round += 1) {
    ...
    for (const file of affected) {
      if (literalExpanded.has(file)) continue;
      const entry =
        seedSet.has(file) ||
    ...
    if (next.length === 0) return { affected, complete: true };
    frontier = next;
  }
  return { affected, complete: false };
}
...
function selectFromClosure(
  closure: Closure,
  index: GraphIndex,
  input: ImpactDerivationInput,
  stepDefinitionFiles: ReadonlyMap<string, StepDefinitionFileSummary>,
): Selection {
  ...
  for (const file of closure.affected) {
    ...
    if (!summary.complete)
      reasons.push(
        `影響を受けるstep定義fileが字面から取り出せない登録を含みます: ${file}`,
      );
    if (summary.global)
      reasons.push(
        `影響を受けるfileがhookまたはWorldとして全scenarioへ効きます: ${file}`,
      );
    affectedSteps.add(file);
  }
  if (affectedSteps.size > 0) {
    for (const [feature, steps] of Object.entries(
      input.featureBinding.byFeature,
    ))
      if (steps.some((step) => affectedSteps.has(step))) features.add(feature);
    ...
    if (input.featureBinding.unparsedFeatures.length > 0)
      reasons.push(
        `step本文を解釈できないfeatureがあります: ${input.featureBinding.unparsedFeatures.join(", ")}`,
      );
  }
  return { features, scenarios, reasons };
}

function scriptsReferencing(
  ...
  for (const [name, command] of Object.entries(scripts))
    if (candidates.some((file) => command.includes(file))) names.push(name);
  return names;
}

function withDigest(
  body: Omit<ImpactSet, "digest" | "adjacent"> & {
  ...
  return Object.freeze({
    ...
    adjacent: Object.freeze(
      body.adjacent.map((path) =>
        Object.freeze({ path, graphEvidence: digest }),
      ),
    ),
    digest,
  });
 ...
 */
export function deriveImpactSet(input: ImpactDerivationInput): ImpactSet {
  ...
  const stepDefinitionFiles = new Map(
    input.stepDefinitionFiles.map((summary) => [summary.path, summary]),
  );
  ...
  for (const changedPath of changedPaths) {
    ...
    if (classified.kind === "document") documents.push(changedPath);
    if (classified.kind === "feature") {
      if (index?.files.has(changedPath)) features.add(changedPath);
      if (input.scripts["test:format"] !== undefined) checks.add("test:format");
    }
  }
  if (documents.length > 0 && input.scripts["docs:format"] !== undefined)
    checks.add("docs:format");
  if (
    documents.some((file) => file.startsWith("docs/specs/")) &&
  ...
  if (index !== undefined) {
    for (const source of sources) {
      for (const importer of index.importers.get(source) ?? [])
        if (!changed.has(importer)) adjacent.add(importer);
      for (const imported of index.imports.get(source) ?? [])
        if (!changed.has(imported)) adjacent.add(imported);
    }
    if (adjacent.size > IMPACT_ADJACENT_LIMIT)
      reasons.push(
        `隣接範囲が上限${IMPACT_ADJACENT_LIMIT}件を超えました: ${adjacent.size}件`,
      );
    /**
    ...
    for (const source of sources) {
      ...
      const selection = selectFromClosure(
        closure,
        index,
        input,
        stepDefinitionFiles,
    ...
    if (documents.length > 0) {
      ...
      for (const document of documents) {
        const readBySource = referenceNames(document).some((name) =>
          (input.literalReferences[name] ?? []).some((referrer) =>
            referrer.startsWith("src/"),
          ),
        );
        if (!readBySource) continue;
        const own = selectFromClosure(
          affectedClosure([document], index, input.literalReferences),
          index,
          input,
          stepDefinitionFiles,
        );
        if (own.features.size === 0)
          reasons.push(
            `src/が字面で読む文書から検証featureへ到達できません: ${document}`,
          );
      }
    }
  }
  const adjacentPaths = sortedUnique(adjacent);
  const securityPaths = sortedUnique(
    [...changedPaths, ...adjacentPaths].filter(isSecuritySensitivePath),
  );
