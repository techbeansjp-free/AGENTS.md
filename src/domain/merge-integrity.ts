...
/** 内容観測の三値。`absent`は存在しないことを確認できた状態、`unreadable`は観測に失敗した状態。 */
export type TokenObservation =
  | { readonly kind: "present"; readonly tokens: readonly string[] }
  | { readonly kind: "absent" }
  | { readonly kind: "unreadable"; readonly reason: string };

/**
 * 親ごとのrename追跡結果。
 * `unresolved`を保持するのは、片方の親だけ解決できた場合に残りを黙って捨てないためである。
 */
export type RenameResolution =
  | {
      ...
      readonly observation: TokenObservation;
    }
  | { readonly kind: "unresolved"; readonly parent: string };

export interface MergePathObservation {
  readonly path: string;
  readonly base: TokenObservation;
  readonly firstParent: TokenObservation;
  readonly secondParent: TokenObservation;
  readonly merged: TokenObservation;
  /**
   * `merged`が`absent`のときの、当該pathを`present`で保持していた親ごとの追跡結果。
   * すべて未解決なら損失、一部だけ解決または移動先不一致なら判定不能とする。
   */
  readonly renameTargets?: readonly RenameResolution[];
}
...
export interface MergeObservation {
  ...
  readonly paths: readonly MergePathObservation[];
}

export interface MergeIntegrityResult {
  readonly valid: boolean;
  readonly errors: readonly string[];
  readonly checkedMerges: number;
  readonly inspectedPaths: number;
}

const NEXT_ACTION =
  "mergeでは両親の内容を保持し、消す操作はmergeの後の通常commitで行ってください";
const UNREADABLE_NEXT_ACTION =
  "repositoryのobjectを復旧するか取得し直したうえで再実行してください";
const RENAME_NEXT_ACTION =
  "renameと内容変更を別commitへ分けて取り込み直してください";

/**
 * 文字列から損失検知tokenを重複なく昇順で取り出す。
 */
export function extractLossTokens(text: string): string[] {
...
function tokensOf(observation: TokenObservation): readonly string[] {
 ...
 */
function requiredTokens(observation: MergePathObservation): string[] {
  ...
  for (const token of first)
    if (second.has(token) || !base.has(token)) required.add(token);
  for (const token of second) if (!base.has(token)) required.add(token);
 ...
 * 呼び出し側が判定不能として扱う。
 */
function actualTokens(
  observation: MergePathObservation,
): { tokens: readonly string[] } | { undecidable: string } {
  ...
  const resolved = targets.filter(
    (target): target is Extract<RenameResolution, { kind: "resolved" }> =>
      target.kind === "resolved",
  );
  // どの親からも移動先を特定できない場合だけ、保持必須集合がすべて失われたものとする。
  if (resolved.length === 0) return { tokens: [] };
  ...
  const unreadable = resolved.find(
    (target) => target.observation.kind === "unreadable",
  );
  if (unreadable)
    return {
 ...
 */
function claimsTarget(observation: MergePathObservation): boolean {
 ...
 */
function collidingRenameTargets(
  paths: readonly MergePathObservation[],
): Map<string, string[]> {
  const occupied = new Set(
    paths
      .filter(
        (observation) =>
          observation.merged.kind === "present" && claimsTarget(observation),
      )
      .map((observation) => observation.path),
  );
  const sourcesByTarget = new Map<string, Set<string>>();
  for (const observation of paths) {
    if (!claimsTarget(observation)) continue;
    for (const target of observation.renameTargets ?? []) {
      if (target.kind !== "resolved") continue;
      const sources = sourcesByTarget.get(target.path) ?? new Set<string>();
      sources.add(observation.path);
      if (occupied.has(target.path)) sources.add(target.path);
...
function evaluatePath(
  ...
  if (unreadable.length > 0)
    return unreadable.map(
      (reason) =>
        `${shortSha(commit)} の ${observation.path} を判定できません。${reason}。観測できない内容を空として扱わずに拒否します。${UNREADABLE_NEXT_ACTION}`,
    );
  ...
  const lost = requiredTokens(observation).filter(
    (token) => !present.has(token),
  );
...
function evaluateObservation(observation: MergeObservation): string[] {
  ...
  return observation.paths.flatMap((path) =>
    evaluatePath(observation.commit, path, collisions),
  );
}
...
export function evaluateMergeIntegrity(
  observations: readonly MergeObservation[],
  ...
  return {
    ...
    inspectedPaths: observations.reduce(
      (total, observation) => total + observation.paths.length,
      0,
    ),
  };
}
