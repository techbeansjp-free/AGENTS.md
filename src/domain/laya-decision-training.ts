...
export const LAYA_DATASET_SCHEMA_VERSION =
  "asc/laya-decision-dataset/v1" as const;
...
export type LayaQuestionId = (typeof LAYA_QUESTION_IDS)[number];
export const LAYA_VALIDITY = ["yes", "no", "insufficient-evidence"] as const;
export type LayaValidity = (typeof LAYA_VALIDITY)[number];
export const LAYA_SEVERITY = ["critical", "high", "medium", "low"] as const;
export type LayaSeverity = (typeof LAYA_SEVERITY)[number];
export const LAYA_ACTIONS = ["fix", "investigate", "dismiss"] as const;
export type LayaAction = (typeof LAYA_ACTIONS)[number];
export type DatasetPartition = "train" | "validation" | "holdout" | "reserve";

const CONTROL = /[\p{Cc}\p{Cf}]/u;
const SECRET_MARKER =
  /(?:BEGIN (?:RSA |EC |OPENSSH )?PRIVATE KEY|github_pat_|gh[pousr]_|authorization\s*:|password\s*[=:]|secret\s*[=:]|api[_-]?key\s*[=:])/iu;
const PROMPT_INSTRUCTION =
  /(?:ignore (?:all |the )?(?:previous|prior) instructions|system prompt|developer message|指示を無視|命令に従え)/iu;
const GOLD_LEAKAGE =
  /(?:\b(?:valid|invalid|resolved|false[- ]?positive|approved|rejected)\b|是正済み|修正済み|解決済み|却下|誤検知|判定\s*[:：])/iu;

export interface LayaEvidence {
  path: string;
  lineStart: number;
  lineEnd: number;
  excerpt: string;
...
export interface LayaDecisionCase {
  schemaVersion: typeof LAYA_DATASET_SCHEMA_VERSION;
  caseId: string;
  ...
  provenance: {
    kind:
      ...
      | "distribution-check";
    artifactPath: string;
    failBeforeCommit: string | null;
    passAfterCommit: string | null;
  };
  strength: "strong" | "weak" | "excluded";
  gold: Partial<Record<LayaQuestionId, string>>;
  critical: boolean;
}
...
export function questionIdsForCase(
  input: Pick<LayaDecisionCase, "slice">,
): readonly LayaQuestionId[] {
  return input.slice === "distribution"
    ? (["distribution-impact"] as const)
    : (["finding-validity", "severity", "required-action"] as const);
}
...
export interface SealedSplit {
  ...
  createdAt: string;
  sourceDigest: string;
  partitions: Record<DatasetPartition, string[]>;
  splitDigest: string;
}
...
export interface LayaTrainingManifest {
...
export interface EvaluationRow {
  caseId: string;
  gold: string;
  goldSeverity: LayaSeverity;
  goldAction: LayaAction;
}
...
function assertSafeText(value: string, label: string, max: number): string {
...
export function digestLayaArtifact(value: unknown): string {
  return crypto.createHash("sha256").update(stableJson(value)).digest("hex");
}

export function validateDecisionCase(input: unknown): LayaDecisionCase {
  assertRecord(input, "DecisionCase");
  const value = input as unknown as LayaDecisionCase;
  assertExactKeys(
    value,
    [
      "schemaVersion",
  ...
  assertExactKeys(
    value.provenance,
    ["kind", "artifactPath", "failBeforeCommit", "passAfterCommit"],
    "provenance",
  );
  if (
    ![
  ...
  for (const [label, commit] of [
    ...
    if (commit !== null) assertSha(commit, label);
  if (!(["strong", "weak", "excluded"] as const).includes(value.strength))
    throw new Error("strengthが不正です");
  assertRecord(value.gold, "gold");
  for (const [questionId, answer] of Object.entries(value.gold)) {
    if (!LAYA_QUESTION_IDS.includes(questionId as LayaQuestionId))
  ...
  if (value.strength === "strong") {
    if (value.provenance.kind === "review-artifact")
      throw new Error(
        "review artifact単独をstrong ground truthへ昇格できません",
      );
    if (
      value.provenance.kind === "test-reproduction" &&
      (!value.provenance.failBeforeCommit || !value.provenance.passAfterCommit)
    )
      throw new Error(
        "strong reproductionにはfail-before/pass-afterが必要です",
      );
  }
  return value;
}

export function createTeacherView(
  input: LayaDecisionCase,
  questionId: LayaQuestionId,
): TeacherView {
...
export function groupDecisionCases(
  cases: readonly LayaDecisionCase[],
): SplitGroup[] {
  const groups = new Map<string, LayaDecisionCase[]>();
...
export function createSealedSplit(
  cases: readonly LayaDecisionCase[],
  createdAt: string,
): SealedSplit {
  ...
  const trainCount =
    groups.length - reserveCount - holdoutCount - validationCount;
  if (trainCount < 100) throw new Error("split後のtrain groupが100未満です");
  const toIds = (selected: SplitGroup[]): string[] =>
    selected.flatMap((group) => group.caseIds).sort();
  const partitions = {
    train: toIds(groups.slice(0, trainCount)),
    validation: toIds(groups.slice(trainCount, trainCount + validationCount)),
    holdout: toIds(
      groups.slice(trainCount + validationCount, groups.length - reserveCount),
    ),
    reserve: toIds(groups.slice(groups.length - reserveCount)),
  } satisfies Record<DatasetPartition, string[]>;
  const seen = new Set<string>();
  for (const ids of Object.values(partitions))
    for (const id of ids) {
      if (seen.has(id)) throw new Error(`caseが複数partitionにあります: ${id}`);
      seen.add(id);
    }
  const sourceDigest = digestLayaArtifact(
    cases.map((item) => validateDecisionCase(item)),
  );
  const unsigned = {
...
export function validateTeacherAssessment(
  ...
  if (
    value.splitDigest !== split.splitDigest ||
    value.sealDigest !== sealDigest
  )
    throw new Error("assessmentのsplit/seal digestが一致しません");
  const expectedPurpose =
    value.partition === "holdout" ? "holdout-baseline" : "train-label";
...
export function validateTrainingManifest(
  value: LayaTrainingManifest,
): LayaTrainingManifest {
  assertExactKeys(
    value,
    [
      "schemaVersion",
  ...
  for (const [label, digest] of [
    ...
    if (!/^[0-9a-f]{64}$/u.test(digest)) throw new Error(`${label}が不正です`);
  if (
    value.layaRevision !== "2c6c16baf3ea3149948777937d5005a7c7fba425" ||
...
function argmax(
  ...
  if (selected === undefined) throw new Error("classesが空です");
  let maximum = -1;
  let sum = 0;
...
export function evaluatePredictions(
  ...
  const resolved = rows.map((row) => {
    ...
    if (!prediction) throw new Error(`predictionがありません: ${row.caseId}`);
    ...
    if (!correct) errors.push(row.caseId);
    if (confidence >= 0.9) {
      highConfidence++;
      if (!correct) highConfidenceErrors++;
    }
    ...
    if (row.goldSeverity === "critical") {
      criticalBase++;
      const missed =
        predicted === "no" || prediction.predictedAction === "dismiss";
      if (missed) criticalMisses.push(row.caseId);
      else criticalCorrect++;
    }
    for (const name of classes) {
  ...
  for (const name of classes) {
    const tp = resolved.filter(
      (row) => row.gold === name && row.predicted === name,
    ).length;
    const fp = resolved.filter(
      (row) => row.gold !== name && row.predicted === name,
    ).length;
    const fn = resolved.filter(
      (row) => row.gold === name && row.predicted !== name,
    ).length;
  ...
  return {
    ...
    highConfidenceErrorRate:
      highConfidence === 0 ? null : highConfidenceErrors / highConfidence,
    falseEscalationRate:
      falseEscalationBase === 0 ? null : falseEscalations / falseEscalationBase,
    ...
    meanLatencyMs:
      predictions.reduce((sum, prediction) => sum + prediction.latencyMs, 0) /
      predictions.length,
  };
}
...
): "eligible" | "rejected" | "insufficient-data" {
  if (report.sampleCount === 0 || report.criticalRecall === null)
    return "insufficient-data";
  if (report.criticalMissCaseIds.length > 0) return "rejected";
  return "eligible";
}
