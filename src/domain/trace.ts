...
export function validateDependencyGraph(
  ...
  const nodes =
    ...
    nodesInput.every((node): node is string => typeof node === "string")
      ? nodesInput
      : [];
  ...
  for (const candidate of edges) {
    ...
    const edge: GraphEdge = { from: candidate.from, to: candidate.to };
...
export function validateScenarioTrace(
  ...
  const trace: TraceEvidence =
    traceInput && typeof traceInput === "object" && !Array.isArray(traceInput)
      ? (traceInput as TraceEvidence)
      : {};
  if (trace !== traceInput)
  ...
  const forbiddenFiles =
    Array.isArray(trace.forbiddenFiles) &&
    trace.forbiddenFiles.every(
  ...
  if (
    ...
    errors.push(
      "trace evidence.forbiddenFilesは重複のない文字列配列でなければなりません",
    );
  const ids = new Set<string>();
  const layerCounts = Object.fromEntries(
    Array.isArray(layers) ? layers.map((layer) => [layer, 0]) : [],
  );
  for (const scenario of scenarios) {
    ...
    const item = scenario as ScenarioEvidence;
    ...
    if (typeof layer !== "string" || !layers.includes(layer))
      errors.push(
        `${String(id)}のtest layerがproject choiceにありません: ${String(layer)}`,
      );
    else layerCounts[layer] += 1;
    if (
      !Array.isArray(steps) ||
      unknownArray(steps).some(
        (step) =>
          typeof step !== "string" || !["given", "when", "then"].includes(step),
      )
    )
  ...
  if (forbiddenFiles.length > 0)
    errors.push(
      `project policyが禁止するtest fileが残っています: ${forbiddenFiles.join(", ")}`,
    );
  return {
    valid: errors.length === 0,
    errors,
    ...
    scenarios: scenarios
      .filter(
        (scenario): scenario is ScenarioEvidence =>
          Boolean(scenario) &&
