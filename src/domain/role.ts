...
export function validateProviderSelection(input: {
   ...
   * 拒否される（CodeRabbit指摘）。
   */
  const normalizedAllowed = ceiling.allowed.map((model) =>
    model.normalize("NFC").toLowerCase(),
  );
  if (normalizedAllowed.includes(selection)) return { valid: true, errors };
...
export const CODEX_ADOPTION_SELECTOR =
  "codex:provider_recommended_default:high:default";
/** A project adoption policy for Claude Code's account/provider default. */
export const CLAUDE_ADOPTION_SELECTOR =
