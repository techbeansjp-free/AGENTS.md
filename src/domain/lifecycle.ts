...
export function init(target: string, options: { apply: boolean }) {
  ...
  const conflicts = assets
    .filter(
      ({ src, dest }) => fs.existsSync(dest) && digest(src) !== digest(dest),
    )
    .map(({ dest }) => dest);
...
export function upgrade(target: string, options: { apply: boolean }) {
  ...
  const old = parsed as { version?: unknown; files: Record<string, unknown> };
  const current = mappings(target);
  const retained: string[] = [];
  const planned: Array<{ src: string; dest: string }> = [];
  for (const item of current) {
    const key = path.relative(target, item.dest);
    if (
      fs.existsSync(item.dest) &&
      (!old.files?.[key] || digest(item.dest) !== old.files[key])
    )
      retained.push(key);
  ...
  if (!options.apply)
    return {
      applied: false,
      planned: planned.map((item) => path.relative(target, item.dest)),
      retained,
    };
  const next: { version: string; files: Record<string, unknown> } = {
    version: PACKAGE_VERSION,
    files: { ...old.files },
  };
...
export function uninstall(target: string, options: { apply: boolean }) {
  ...
  if (!isRecord(parsed) || !isRecord(parsed.files))
    throw new Error("managed asset recordが不正です");
  const record = parsed as { files: Record<string, unknown> };
  const removable: string[] = [];
  const retained: string[] = [];
  for (const [relative, expected] of Object.entries(record.files ?? {})) {
    if (
      !isPackageOwnedPath(relative) ||
