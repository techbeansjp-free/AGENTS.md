...
export function createWorktree(input: {
  ...
  baseBranch?: string;
  /**
   * `baseBranch`の取得済みtip。`baseBranch`を指定するとき必須。
   *
   * **照合先はlocalの`refs/remotes/origin/<branch>`であり、provider APIではない。**
   * 既定branchの`remoteDefaultSha`と同じ強度の錨点である。
   */
  baseSha?: string;
  expectedRepository?: string;
  trustedPolicy?: Policy;
   ...
   */
  git(
    ["worktree", "add", "-b", input.branch, destination, verifiedBaseCommit],
    input.repoRoot,
  );
