...
export function validatePolicy(policy: unknown) {
  ...
  if (
    ...
    errors.push(
      "projectRuleRetirementProposalsが不正です（最大16件、一意、ruleId、beforeSha256、制御文字なしのreason・ownerが必要です）",
    );
  rejectUnknownKeys(
    candidate.merge,
    [
...
export interface PolicyManifest {
  ...
  /** 検証command宣言（REQ-WF-040）。trusted commitの値だけをauthorityにする。 */
...
export function validateProjectPolicyManifest(manifest: unknown) {
  ...
  if (
    policy.projectRuleRetirementProposals !== undefined &&
...
export function loadEffectiveTrustedPolicySetAtCommit(
  ...
  if (
    !validateEnforcementPolicy(packageFloor).valid ||
    !validateEnforcementPolicy(committedFloor).valid
  )
    throw new Error(
      "package default safety floorは1件以上の有効ruleが必要です",
    );
  const result = git(
 ...
 */
export function trustedVerificationPolicy(
  policySet: unknown,
): VerificationPolicy {
  const manifest = isRecord(policySet) ? policySet.manifest : undefined;
  const commit =
    isRecord(policySet) && isRecord(policySet.provenance)
      ? policySet.provenance.commitSha
      : undefined;
  const where =
    typeof commit === "string" ? `（trusted commit ${commit}）` : "";
  if (
    !isRecord(manifest) ||
    ...
    throw new Error(
      `trusted project policy${where}に検証command宣言（.agent-skill-chain/project-policy.jsonのverification.fullCommandとverification.targetedRunner）がありません。検証は既定branchのproject policyが宣言したcommandの観測だけを受理します。宣言を既定branchへmergeしてから再実行してください`,
    );
  return parseVerificationPolicy(
    manifest.verification,
    `trusted project policy${where}のverification`,
  );
}
 ...
 */
export function loadTrustedVerificationPolicy(
  root: string,
): VerificationPolicy {
...
export function ruleFragmentSources(policySet: {
  ...
  if (
    ...
    return [];
  const sources: RuleFragmentSource[] = [];
  for (const [index, relative] of manifest.ruleFiles.entries()) {
    ...
    try {
      const parsed: unknown = parseJsonStrict(raw, relative);
      if (
        !isRecord(parsed) ||
  ...
  if (
    new Set(sources.map((source) => source.ruleId)).size !== sources.length ||
