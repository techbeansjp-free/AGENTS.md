...
export function installVerifiedTypeScriptVendorAssets(root: string): void {
  const packageMetadata = JSON.parse(
    fs.readFileSync(
      path.resolve(root, "node_modules/typescript/package.json"),
      "utf8",
    ),
  ) as { version?: unknown };
  ...
  for (const asset of TYPESCRIPT_VENDOR.assets) {
    ...
    if (!fs.existsSync(source) || sha256File(source) !== asset.sha256)
      throw new Error(
        `固定TypeScript compiler資産のSHA-256が一致しません: ${asset.source}`,
      );
    fs.mkdirSync(path.dirname(destination), { recursive: true });
    fs.copyFileSync(source, destination);
    if (sha256File(destination) !== asset.sha256)
      throw new Error(
        `固定TypeScript compiler資産のcopy後SHA-256が一致しません: ${asset.destination}`,
      );
  }
}
