...
export function redactSecrets(input: string): string {
  return String(input)
    ...
    .replace(
      /\b(?:token|password|secret|api[_-]?key|apiKey|databaseUrl|connectionString|privateKey)\s*[=:]\s*[^\s,;]+/gi,
      "[REDACTED_SECRET]",
    );
}
...
export function parseJsonStrict(source: string, label = "JSON"): JsonValue {
  let index = 0;
  ...
  const value = (): JsonValue => {
    ...
    if (source[index] === "{") {
      ...
      while (index < source.length) {
        whitespace();
        const key = string();
        if (key === "__proto__") fail("禁止keyです");
        if (keys.has(key)) fail("重複keyを拒否しました");
        keys.add(key);
        whitespace();
        if (source[index++] !== ":") fail("colonが必要です");
        object[key] = value();
        whitespace();
        const separator = source[index++];
        if (separator === "}") return object;
        if (separator !== ",") fail("commaまたはobject終端が必要です");
      }
      fail("objectが閉じていません");
    }
    if (source[index] === "[") {
      ...
      while (index < source.length) {
        ...
        if (separator !== ",") fail("commaまたはarray終端が必要です");
      }
      fail("arrayが閉じていません");
    }
    ...
    if (token === undefined) return fail("値が不正です");
    index += token.length;
    return JSON.parse(token) as JsonValue;
  };
  const parsed = value();
  whitespace();
  if (index !== source.length) fail("末尾に余分な入力があります");
  return parsed;
}
