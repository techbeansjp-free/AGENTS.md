...
/** @param {unknown} value */
export function isPackageVersion(value) { return typeof value === 'string' && packageVersionPattern.test(value); }

/** @param {unknown} value */
export function isPolicySchemaPatchVersion(value) { return typeof value === 'string' && policyVersionPattern.test(value); }

/** @param {string} value */
export function packageReleaseVersion(value) { return value.split(/[+-]/u, 1)[0]; }

if (!isPackageVersion(packageMetadata.version)) throw new Error('package.json.versionは0.3.x SemVerでなければなりません');
if (!isPolicySchemaPatchVersion(packageMetadata.agentSkillChain?.policySchemaVersion)) throw new Error('package.jsonのpolicySchemaVersionが不正です');
if (!Array.isArray(packageMetadata.agentSkillChain?.compatiblePolicySchemaVersions) || packageMetadata.agentSkillChain.compatiblePolicySchemaVersions.some((/** @type {unknown} */ version) => !isPolicySchemaPatchVersion(version))) throw new Error('package.jsonのcompatiblePolicySchemaVersionsが不正です');
