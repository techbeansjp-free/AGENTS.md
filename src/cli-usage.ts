...
export const COMMAND_USAGE: readonly CommandUsage[] = Object.freeze([
  ...
  },
  {
    command: "impact",
    summary:
      "2 commit間の影響集合（隣接範囲・security注意・検証feature選択）を実Gitから導出する。影響を証明できない場合はmode=fullで全体検証を要求する",
    requiredFlags: [
    ...
    optionalFlags: [
      ...
      optional(
        "format",
        "json|features",
        "json=影響集合全体、features=実行するfeature pathを1行1件（mode=fullは終了値1）",
        "json",
      ),
    ],
    example:
      "npx agent-skill-chain impact --root=. --base=origin/main --head=HEAD --format=features",
  },
  {
    command: "graph",
    subcommand: "path",
  ...
  },
  {
    ...
    example:
      'npx agent-skill-chain routing ceiling --issue=886 --scope=issue-886 --provider=claude --selection=fable --now=2026-09-01T12:00:00Z --override=\'{"provider":"claude","selection":"fable","issue":886,"scope":"issue-886","instructedBy":"owner","instructedAt":"2026-09-01T00:00:00Z","expiresAt":"2026-09-02T00:00:00Z"}\'',
  },
  {
    command: "routing",
    subcommand: "observe",
  ...
  },
  {
    ...
    optionalFlags: [ROOT_FLAG],
    example:
      "npx agent-skill-chain routing delegated-review-staging --root=<対象worktree> --step=3 --staging=.agent-skill-chain/tmp/issues/example",
  },
  {
    command: "routing",
    ...
    optionalFlags: [ROOT_FLAG],
    example:
      "npx agent-skill-chain routing delegated-review-diff --root=<対象worktree> --base=<比較基点SHA> --head=<H_impl SHA> --staging=<対象staging>",
  },
  {
    command: "routing",
  ...
  },
  {
    ...
    requiredFlags: [
      ...
      flag(
        "evidence",
        "text",
        "Stepの証跡。Step 4・8の同期証拠は64桁のhex digestとsync語を含める",
      ),
  ...
  },
  {
    ...
    subcommand: "mark",
    summary:
      "role切替・model呼び出し・deterministic tool呼び出しの境界をmetrics event logへ追記する",
    requiredFlags: [
      flag("staging", "path", "staging directory"),
    ...
    example:
      "npx agent-skill-chain workflow mark --staging=.asc/1482 --kind=role --phase=start --label=implementer",
    acceptsSpaceSeparatedFlags: true,
  },
  {
    command: "workflow",
    subcommand: "metrics",
    summary:
      "step_ms・role_ms・model_ms・deterministic_ms・review_rounds・support_ms・artifact_build_msを算出する",
    ...
    optionalFlags: [
      optional(
        "review-session",
        "path",
        "review_roundsを算出するreview session state file",
        "review_roundsはunavailable(null)",
      ),
      optional(
        "out",
        "",
        `.agent-skill-chain/metrics/<staging名>.jsonへも書き込む`,
        "stdoutだけへ出力",
      ),
    ],
    example:
      "npx agent-skill-chain workflow metrics --staging=.asc/1482 --review-session=.asc/1482/review-session.json --out",
    acceptsSpaceSeparatedFlags: true,
  },
  {
    command: "workflow",
  ...
  },
  {
    ...
    optionalFlags: [
      ...
      optional(
        "staging-root",
        "path",
        "stagingを作る親directory（repository相対）。project policyのstaging.rootに一致させる。rootが*を含むときは必須",
        "project policyのstaging.root（既定 .agent-skill-chain/tmp/issues）",
      ),
      optional(
        "name",
        "text",
        "staging directory名。版管理下のstagingで人が読む名前（例: S1-T02_安定ID）を付けるときに使う",
        "<JST timestamp>_<title slug>",
      ),
    ],
    example:
  ...
  },
  {
    ...
    conditionalFlags: [
      conditional(
        ...
        "path",
        "反映する本文file",
        "--generate-bodyを指定しないとき",
        (provided) => provided["generate-body"] === undefined,
      ),
      conditional(
        "staging-path",
        "path",
        "同期本文を生成するIssue staging",
  ...
  },
  {
    ...
    example:
      "npx agent-skill-chain issue start --issue=886 --repo=owner/name --staging-path=.agent-skill-chain/tmp/issues/example --dry-run",
  },
  {
    command: "issue",
    subcommand: "staging",
  ...
  },
  {
    ...
    requiredFlags: [
      ...
      flag(
        "scope",
        "targeted|full",
        "検証範囲。既定値は無い。fullはargvが既定branchのtrusted policyのverification.fullCommandと完全一致する場合、targetedは影響集合がtargetedでargvがverification.targetedRunnerに選ばれたfeatureを全部並べた形の場合だけ記録できる",
      ),
    ],
    conditionalFlags: [],
    optionalFlags: [
  ...
  },
  {
    ...
    optionalFlags: [
      ...
      optional(
        "base-sha",
        "sha",
        "base branchの取得済みtip。localのrefs/remotes/origin/<branch>と照合する。--base-branchを指定するとき必須",
        "remote既定branchのSHA",
      ),
      ...APPLY_MODE,
    ],
  ...
  },
  {
    ...
    example:
      "npx agent-skill-chain worktree finalize --root=. --path=.worktrees/20260826_111243-886-cli-usage --evidence=./evidence.json --report-hash=<preview digest> --approved-digest=<preview digest> --merge-sha=<merge SHA> --cleanup-authority --complete --authorize=approved --apply",
  },
  {
    command: "pr",
    subcommand: "create",
    ...
    inputContract: {
      description:
        "--evidenceのJSON。headShaは--head-shaと同じexact HEAD。tests.scenarioIdsとspec.trace.scenariosは同じSCN集合。spec.impactはupdated|no-spec-impactで、no-spec-impactではspec.rationaleに対象範囲を限定した12文字以上の根拠が必要。ownershipは変更pathの所有者と層",
      example: {
        headSha: "<40hex>",
  ...
  },
  {
    ...
    conditionalFlags: [
      conditional(
        "reopen-terminal",
        "approved",
        "旧outcome=pull-request終端から新しいowner delivery判断を開始する",
