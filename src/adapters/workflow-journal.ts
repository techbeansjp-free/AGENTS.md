...
import {
  isPlanFrozenCheckStep,
  planResealRejection,
  planSealStep,
  stagingDriftDiagnostic,
} from "../domain/plan-seal.js";
import {
  assertPlanFrozenForEntries,
  changedPlanningSince,
  computePlanSeal,
} from "./plan-seal.js";
 ...
 */
export function assertPlanFrozen(
  staging: string,
  commit = "HEAD",
  ...
  if (journal.errors.length > 0)
    throw new Error(
      `計画凍結検査のworkflow journalが不正です: ${journal.errors.join("; ")}`,
    );
 ...
 * journal追記経路も同じ判定を持つ。
 */
export function assertPlanSealOpen(
  staging: string,
  step: number,
  humanOverride = false,
): void {
 ...
 */
export function describeStagingDigestDrift(staging: string): string {
  let added: string[] = [];
  let removed: string[] = [];
  try {
...
function appendWorkflowJournalEntryLocked(
  ...
  if (entry.reconfirmation)
    throw new Error(
      "上流再確定entry（reconfirmation）は廃止されました。承認済み計画は封印後に編集せず、計画の変更は05_計画変更.mdへ追記してください",
    );
  /**
  ...
  if (resealRejection) throw new Error(resealRejection);
  const sealsPlan =
...
export function resolvePullRequestStaging(input: {
  ...
  const issuesRoots = stagingRootPatterns(layout).flatMap((pattern) =>
    listStagingRoots(root, pattern),
  );
