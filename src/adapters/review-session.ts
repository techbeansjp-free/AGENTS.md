...
function impactNotes(impact: ImpactSet): string[] {
  ...
  if (impact.mode === "targeted")
    ...
    notes.push(
      `影響集合を証明できないため全体reviewを適用する（focus.adjacentScopeは空）: ${impact.reasons.slice(0, 3).join("; ")}${impact.reasons.length > 3 ? ` ほか${impact.reasons.length - 3}件` : ""}`,
    );
