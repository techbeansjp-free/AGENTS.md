...
import type {
  IssueProjectConnection,
  IssueProjectObservation,
} from "../domain/issue-start.js";
...
function repositoryParts(repository: string): [string, string] {
  ...
  if (!match) throw new Error("repositoryはowner/nameで指定してください");
  return [match[1]!, match[2]!];
}

export function inspectIssueProject(
  input: {
    repository: string;
    issue: number;
    connection: IssueProjectConnection;
  },
  cwd: string,
): IssueProjectObservation {
  verifyRepository(input.repository, cwd, "read");
  const [repoOwner, repoName] = repositoryParts(input.repository);
  const result = projectGraphql(
    ISSUE_PROJECT_QUERY,
    {
  ...
  const options = project.field.options.filter(
    (option): option is Record<string, unknown> =>
      isRecord(option) && option.name === input.connection.startedStatus,
  );
  ...
  const items = repository.issue.projectItems.nodes.flatMap((entry) => {
    ...
    if (entry.project.id !== project.id) return [];
    ...
    return [
      {
        id: entry.id,
        statusOptionId:
          isRecord(field) && typeof field.optionId === "string"
            ? field.optionId
            : null,
        statusName:
...
export function addIssueProjectItem(
  input: { repository: string; projectId: string; issueId: string },
  cwd: string,
): string {
  verifyRepository(input.repository, cwd, "write");
  const result = projectGraphql(
    `mutation($projectId:ID!,$contentId:ID!){addProjectV2ItemById(input:{projectId:$projectId,contentId:$contentId}){item{id}}}`,
    { projectId: input.projectId, contentId: input.issueId },
    cwd,
  );
  const item =
    isRecord(result) &&
...
export function updateIssueProjectStatus(
  input: {
    repository: string;
    projectId: string;
    itemId: string;
...
export function github(
  operation: "branch.delivery-policy",
  input: Pick<GitHubInput, "repository" | "branch">,
  cwd: string,
): BranchDeliveryPolicyObservation;
export function github(
...
export function github(
  ...
  if (operation === "branch.delivery-policy") {
    ...
    try {
      ...
      if (classic.status === 0) {
        const value: unknown = JSON.parse(classic.stdout);
        if (!isRecord(value))
        ...
        if (required !== null && required !== undefined) {
          if (!isRecord(required) || typeof required.strict !== "boolean")
            throw new Error(
              "classic protectionのstrictがbooleanではありません",
            );
      ...
      for (const rule of rules) {
        ...
        if (
          ...
          throw new Error(
            "ruleset required status checksのstrictがbooleanではありません",
          );
      ...
      return {
        ...
        mergeQueueConfigured: rules.some(
          (rule) => isRecord(rule) && rule.type === "merge_queue",
        ),
