...
function readPrompt(input: CodexLaunchInput): { root: string; prompt: string } {
...
export async function launchCodex(
  ...
  dependencies: {
    observeExecutor?: ProviderExecutor;
  ...
  if (
    !["full", "quick", "poc"].includes(input.mode) ||
  ...
  if (!["read-only", "workspace-write"].includes(input.sandbox))
    return rejection(
      "sandboxはread-onlyまたはworkspace-writeだけを指定できます",
    );
  if (input.coordinator === input.implementer)
    return rejection(
      "coordinatorとimplementerを別identityへ割り当ててください",
    );
  ...
  const required =
    configured && MODEL_TIERS.indexOf(configured) > MODEL_TIERS.indexOf(minimum)
      ? configured
      : minimum;
  ...
  if (!tier.valid) return rejection(tier.errors.join(" / "));
  const mapping =
    "providerMappings" in trusted ? trusted.providerMappings[0] : undefined;
  if (!mapping)
    return rejection("trusted provider capability mappingが未設定です");
  const observation = await observeProvider(
    "codex",
    dependencies.observeExecutor,
    undefined,
    { cwd: root, official: true },
  );
  ...
  return {
    ...
    modelEvidence: "dispatch_arguments" as const,
