...
} from "../domain/laya-distribution.js";
...
export function validateDecisionBundleArtifacts(
  bundle: LayaDecisionBundle,
  root: string,
  currentAscVersion: string,
): LayaDecisionBundle {
  const weight = resolveContained(root, bundle.model.path);
  const validated = validateDecisionBundle(
    bundle,
    fileDigest(weight),
    currentAscVersion,
  );
  assertArtifact(
    root,
    validated.questionSchemaPath,
    validated.questionSchemaSha256,
    "question schema",
  );
  assertArtifact(
    root,
    validated.thresholdPath,
    validated.thresholdSha256,
    "threshold",
  );
  assertArtifact(
    root,
    validated.modelCardPath,
    validated.modelCardSha256,
    "model card",
  );
  assertArtifact(
    root,
    validated.evaluationPath,
    validated.evaluationSha256,
    "evaluation",
  );
  const evaluation = parseJsonStrict(
    fs.readFileSync(resolveContained(root, validated.evaluationPath), "utf8"),
    "Laya evaluation",
  );
  if (
    !evaluation ||
  ...
  if (
    ...
    throw new Error(
      "Decision Bundleとeligible evaluationのdigest chainが一致しません",
    );
  return validated;
}
