...
import type { DecisionAuthorityMode } from "../domain/decision-types.js";
import type { JevDispatchOutcome } from "../domain/jev-dispatch.js";
import type { JevShadowRecord } from "../domain/jev-shadow.js";

export const JEV_SHADOW_DIRECTORY = ".agent-skill-chain/runtime/decisions";

function shadowDirectory(primaryRoot: string, id: string): string {
  return path.join(primaryRoot, JEV_SHADOW_DIRECTORY, path.basename(id));
}

function shadowPath(primaryRoot: string, id: string): string {
...
const OUTCOME_KINDS: readonly JevDispatchOutcome["kind"][] = [
  "ok",
  "schema-error",
  "auth-error",
  "usage-error",
...
export function parseJevShadowLine(value: unknown): JevShadowRecord {
  if (!isRecord(value)) throw new Error("jev-shadow行はobjectが必要です");
  ...
  if (
    typeof authorityMode !== "string" ||
    !(AUTHORITY_MODES as readonly string[]).includes(authorityMode)
  )
    throw new Error("primaryAuthorityModeが不正です");
  const outcomeKind = value.outcomeKind;
  if (
    typeof outcomeKind !== "string" ||
    !(OUTCOME_KINDS as readonly string[]).includes(outcomeKind)
  )
  ...
  return {
    ...
    primaryProposedValue: requiredString(
      value.primaryProposedValue,
      "primaryProposedValue",
    ),
    primaryAuthorityMode: authorityMode as DecisionAuthorityMode,
    jevModel: requiredString(value.jevModel, "jevModel"),
    jevResolvedModel: nullableString(
      value.jevResolvedModel,
      "jevResolvedModel",
    ),
    jevProposedValue: nullableString(
      value.jevProposedValue,
      "jevProposedValue",
    ),
    jevConfidence: nullableNumber(confidence, "jevConfidence"),
    outcomeKind: outcomeKind as JevDispatchOutcome["kind"],
...
export function readJevShadowJournal(
  primaryRoot: string,
  id: string,
): { records: JevShadowRecord[]; errors: string[] } {
...
export function appendJevShadowRecord(
  ...
  withStagingMutationLock(directory, () => {
    ...
    if (existing.errors.length > 0)
      throw new Error(
        `jev-shadow journalの既存内容が不正です: ${existing.errors.join("; ")}`,
      );
    if (
      ...
      throw new Error(
        `decisionRecordIdが既存jev-shadow journalと重複しています: ${record.decisionRecordId}`,
      );
