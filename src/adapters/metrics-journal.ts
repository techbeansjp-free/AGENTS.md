...
function assertRegularFilePath(rawTarget: string, label: string): void {
...
function byLabelForKind(
  ...
  for (const [key, ms] of Object.entries(byLabel)) {
    if (key.startsWith(prefix)) result[key.slice(prefix.length)] = ms;
  }
  return result;
}
  ...
  if (input.reviewSessionPath) {
    ...
    if (!fs.existsSync(input.reviewSessionPath)) {
      ...
      try {
        ...
        const state = parseReviewSessionState(
          parseJsonStrict(raw, "review session"),
        );
        reviewRounds = computeReviewRounds(state);
      } catch (error) {
        warnings.push(
          `review session fileの解析に失敗しました: ${error instanceof Error ? error.message : String(error)}`,
        );
      }
    }
  }
