...
function readCommittedAmendment(
  ...
  const location = assertIssueStagingLocation(staging);
  if (!location.layout.tracked) return null;
  const result = git(
    [
      "cat-file",
      "blob",
...
export function assertPlanFrozenForEntries(
  staging: string,
  entries: readonly SealedJournalEntry[],
  commit = "HEAD",
  options: { delivery?: boolean } = {},
): PlanGeneration | undefined {
  const latest = latestPlanSeal(entries);
  if (latest === undefined) return undefined;
  const changed = changedSealedArtifacts(
    latest.seal,
    observeSealedArtifacts(staging, latest.seal),
  );
  if (changed.length > 0)
    throw new Error(planFrozenMessage({ sealStep: latest.step, changed }));
  const committed = observeCommittedSealedArtifacts(
    staging,
    latest.seal,
    commit,
  );
  ...
  const amendmentText = readPlanningFile(
    staging,
    PLAN_AMENDMENT_FILE,
  )?.toString("utf8");
  ...
  if (
    ...
    throw new Error(
      `版管理下stagingのcommit ${commit}上の${PLAN_AMENDMENT_FILE}がworktreeと一致しません。計画変更記録はcommitしてreviewの差分へ含めてから記録・配送してください`,
    );
  const sinceSeal = entries.slice(latest.index + 1);
  const chain = latestPlanGeneration(
    latest.seal,
    sinceSeal.flatMap((entry) =>
      entry.planGeneration ? [entry.planGeneration] : [],
    ),
  );
  if (chain.errors.length > 0)
    throw new Error(
      `計画世代chainの検査に失敗しました: ${chain.errors.join("; ")}`,
    );
  ...
  if (options.delivery) {
    ...
    if (
      ...
      throw new Error(
        "配送前の計画変更検査: chain付きjournalの封印後の最後のStep 10にplanGenerationがありません。旧journal互換として扱わず配送を拒否します",
      );
    if (reviewed?.planGeneration !== undefined) {
      const violation = deliveredAmendmentViolation(
