...
function renderEvidenceReanchorChain(
 ...
 */
function recoverPublishedReanchorDigest(
  staging: string,
  chain: readonly EvidenceReanchorRecord[],
): void {
  ...
  const expectedBeforeArtifacts =
    previous.length === 0
      ? currentArtifacts.filter(
          (artifact) => artifact !== EVIDENCE_REANCHOR_FILE,
        )
      : currentArtifacts;
  if (stableJson(stored.artifacts) !== stableJson(expectedBeforeArtifacts))
    throw new Error(
      "再固定再開時のartifact集合が公開前・公開後のどちらとも一致しません",
    );
  ...
  const projectedBefore = crypto
    ...
    .update(
      stableJson(
        expectedBeforeArtifacts.map((relative) => ({
          relative,
          digest:
            relative === EVIDENCE_REANCHOR_FILE
              ? previousDigest
              : crypto
                  .createHash("sha256")
                  .update(
                    fs.readFileSync(path.join(staging, ...relative.split("/"))),
                  )
                  .digest("hex"),
        })),
      ),
  ...
  if (stored.digest !== projectedBefore)
    throw new Error(
      "再固定以外のstaging成果物が変更されているためdigestを復旧できません",
    );
  refreshStoredStagingDigest(staging);
  const refreshed = readStoredStagingRecord(staging);
  if (
    stableJson(refreshed.artifacts) !== stableJson(currentArtifacts) ||
...
interface ArtifactReplacementEvidence {
  oldPath: string;
  newPath: string;
  oldDigest: string;
  newDigest: string;
...
interface ArtifactSupersessionEvidence {
  artifactPath: string;
  oldDigest: string;
  newDigest: string;
}
 ...
 */
function acceptedSessionEvidence(
  staging: string,
  evidence: ReviewEvidence,
  options: { postPrIntake: boolean },
): boolean {
  ...
  const step10 = [...journal.entries]
    ...
    .find(
      (entry) =>
        entry.step === 10 && (!options.postPrIntake || entry.postPrIntake),
    )?.reviewSession;
  return (
    session !== null &&
...
function observeArtifactSupersession(
  ...
  if (
    oldEvidence.baseSha !== input.oldBaseSha ||
  ...
  if (
    evidenceOnlySuffix(
      root,
      newEvidence.implementationHeadSha,
      input.newHeadSha,
    ) !== artifactPath
  )
    return undefined;
 ...
 */
function observeArtifactReplacement(
  staging: string,
  root: string,
  input: ReanchorComparison,
): ArtifactReplacementEvidence | undefined {
  const beforeAll = observeReanchorDiff(
    root,
    "旧base→旧head",
    input,
  ...
  if (
    evidence.baseSha !== input.oldBaseSha ||
...
function observeReviewedForward(
  ...
  if (!("evidence" in parsed)) return undefined;
  const evidence = parsed.evidence;
  if (evidence.baseSha !== input.newBaseSha) return undefined;
  if (finalParent !== evidence.implementationHeadSha) return undefined;
  if (input.oldHeadSha === evidence.implementationHeadSha) return undefined;
  try {
...
function resolveAnchor(
  ...
  if (layer === "delivery") {
    ...
    if (state.state !== "step11-recorded" && state.state !== "pr-bound")
      throw new Error(
        `delivery stateが${state.state}です。pr-boundまたはstep11-recordedだけがpr reanchorを受理します`,
      );
    return {
...
export function evaluateEvidenceReanchor(input: {
  ...
  if (!isContentEquivalent(before, after)) {
    ...
    if (rebase.reason !== "ok") {
      artifactReplacement = observeArtifactReplacement(
        staging,
        input.root,
        comparison,
      );
      if (artifactReplacement !== undefined) method = "artifact-replacement";
        ...
        artifactSupersession = observeArtifactSupersession(
          staging,
          input.root,
          comparison,
        );
