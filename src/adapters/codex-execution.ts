...
export function codexExecutionArguments(input: CodexExecutionInput): string[] {
  if (
    !/^[a-z0-9][a-z0-9.-]{0,127}$/u.test(input.model) ||
...
export async function executeCodex(
  ...
  return new Promise((resolve) => {
    ...
    const timer = setTimeout(
      () =>
        stop(
          "Codex実行が有限時間上限に達しました。作業状態を確認してから明示的に再開してください",
        ),
      timeoutMs,
    );
      ...
      for (const line of lines) {
        ...
        try {
          ...
          if (!isRecord(event) || typeof event.type !== "string")
            malformed = true;
          else if (event.type === "turn.completed") completed += 1;
