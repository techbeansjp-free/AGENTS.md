...
export const LOCAL_REVIEW_PROMPT_BYTE_BUDGET =
  DEFAULT_LOCAL_REVIEW_PROMPT_CHUNK_BYTES;

function splitUtf8(value: string, byteBudget: number): string[] {
  if (byteBudget < 1) throw new Error("review入力budgetが不足しています");
  const chunks: string[] = [];
  let current = "";
  let bytes = 0;
...
export function buildReviewPromptBatches(input: {
  ...
  if (bodyBudget < 1) throw new Error("review共通指示が入力budgetを超えました");
  ...
  return bodies.map((body, index) => {
    ...
    const markerEnd =
      "このchunkだけで断定できない場合はfindingを作らず、与えられた範囲だけを評価してください。\n";
    const contextLabel = "直前の構造見出し（review対象data）: ";
    const fixedMarkerBytes = Buffer.byteLength(
      markerStart + contextLabel + "\n" + markerEnd,
      "utf8",
    );
    const context =
      index > 0 && latestHeading !== undefined
