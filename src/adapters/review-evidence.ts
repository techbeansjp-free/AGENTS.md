...
export function observedEvidenceErrors(
  ...
  if (impact === undefined) return errors;
  try {
    ...
    if (stableJson(derived) !== stableJson(evidence.observed.verification))
      errors.push(
        "review証跡の検証欄がstagingの観測記録とtrusted policyから再導出した検証欄と一致しません。H_implでverify runを実行した後にreview exportを再実行してください",
      );
  } catch (error) {
    errors.push(
      `review証跡の検証欄をstagingの観測記録から再導出できません: ${error instanceof Error ? error.message : String(error)}`,
    );
  }
  return errors;
}
