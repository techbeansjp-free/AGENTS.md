...
export function configureJevProviderConfig(
  ...
  if (!ENV_VAR_NAME_PATTERN.test(input.apiKeyEnvVar))
    errors.push(
      "apiKeyEnvVarは英大文字・数字・_のみで先頭は英字か_が必要です（例: JEV_API_KEY）",
    );
...
export function appendJevApiKeyToShellRc(
  ...
  if (!ENV_VAR_NAME_PATTERN.test(input.apiKeyEnvVar))
    throw new Error(
      "apiKeyEnvVarは英大文字・数字・_のみで先頭は英字か_が必要です",
    );
  ...
  const existingContent = fs.existsSync(input.rcPath)
    ...
    : "";
  const alreadyPresent = exportLinePattern(input.apiKeyEnvVar).test(
    existingContent,
  );
