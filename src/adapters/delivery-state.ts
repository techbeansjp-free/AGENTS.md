...
import {
  ...
  resumePullRequestCreationAfterConfirmedAbsence,
...
export const DELIVERY_STATE_TRANSACTION_SCHEMA =
  "agent-skill-chain/delivery-state-transaction/v1" as const;
...
interface DeliveryStateTransaction {
  schemaVersion: typeof DELIVERY_STATE_TRANSACTION_SCHEMA;
  deliveryBeforeDigest: string | null;
  deliveryAfterDigest: string;
  deliveryBeforeFileDigest: string | null;
  deliveryAfterFileDigest: string;
  stagingRecordBeforeFileDigest: string;
  stagingRecordAfterFileDigest: string;
  stagingDigestBefore: string;
  stagingDigestAfter: string;
...
type DeliveryTransactionState =
  "before-publish" | "delivery-published" | "record-refreshed";

interface InspectedDeliveryTransaction {
  marker: DeliveryStateTransaction;
...
export function deliveryStateTransactionPath(stagingInput: string): string {
  const staging = assertWorkflowStaging(stagingInput);
  return path.join(
    path.dirname(staging),
    `.${path.basename(staging)}.delivery-state.transaction.json`,
  );
}
...
function readRegularFileBounded(
  ...
  if (
    ...
    throw new Error(
      `${label}はsymlink・hardlinkでない${maximumBytes} byte以下の通常fileが必要です`,
    );
  ...
  try {
    ...
    while (offset < buffer.length) {
      ...
      if (count <= 0) throw new Error(`${label}を完全に読み取れません`);
      offset += count;
    }
...
function parseArtifactPaths(value: unknown, label: string): string[] {
  if (
    !Array.isArray(value) ||
...
function requiredDigest(value: unknown): value is string {
  return typeof value === "string" && SHA256.test(value);
}

function parseDeliveryStateTransaction(
  source: string,
): DeliveryStateTransaction {
  ...
  const artifactsBefore = parseArtifactPaths(
    value.artifactsBefore,
    "delivery state transaction artifactsBefore",
  );
  const artifactsAfter = parseArtifactPaths(
    value.artifactsAfter,
    "delivery state transaction artifactsAfter",
  );
  const expectedAfter = artifactsBefore.includes(DELIVERY_STATE_FILE)
    ? artifactsBefore
    : [...artifactsBefore, DELIVERY_STATE_FILE].sort((left, right) =>
        left.localeCompare(right),
      );
  if (
    stableJson(artifactsAfter) !== stableJson(expectedAfter) ||
...
function readDeliverySource(staging: string): string | undefined {
  const file = deliveryFile(staging);
  if (!fs.existsSync(file)) return undefined;
  return readRegularFileBounded(file, 1024 * 1024, "delivery state").toString(
    "utf8",
  );
}

function projectedStagingDigest(
  staging: string,
  artifacts: readonly string[],
  deliveryFileDigest: string | null,
): string {
  const values = artifacts.map((relative) => {
    if (relative === DELIVERY_STATE_FILE) {
      if (!deliveryFileDigest)
        throw new Error(
          "delivery stateを含む成果物集合にbefore digestがありません",
        );
...
function recordSource(record: StoredStagingRecord): string {
  return `${JSON.stringify(record, null, 2)}\n`;
}

function transactionMarkerPathLocked(staging: string): string {
  return path.join(
    path.dirname(staging),
    `.${path.basename(staging)}.delivery-state.transaction.json`,
  );
...
function assertRegularTransactionMarker(marker: string): void {
  readRegularFileBounded(
    marker,
    DELIVERY_STATE_TRANSACTION_LIMITS.bytes,
    "delivery state transaction marker",
  );
}

function clearDeliveryStateTransactionLocked(staging: string): void {
...
function inspectPendingDeliveryStateTransactionLocked(
  staging: string,
): InspectedDeliveryTransaction | null {
  const markerFile = transactionMarkerPathLocked(staging);
  if (!fs.existsSync(markerFile)) return null;
  const marker = parseDeliveryStateTransaction(
    readRegularFileBounded(
      markerFile,
      DELIVERY_STATE_TRANSACTION_LIMITS.bytes,
      "delivery state transaction marker",
  ...
  const otherArtifacts = marker.artifactsAfter.filter(
    (artifact) => artifact !== DELIVERY_STATE_FILE,
  );
  if (
    stableJson(
      marker.artifactsBefore.filter(
        (artifact) => artifact !== DELIVERY_STATE_FILE,
      ),
    ) !== stableJson(otherArtifacts) ||
    calculateStagingDigest(staging, otherArtifacts) !==
      marker.otherArtifactsDigest ||
    projectedStagingDigest(
      staging,
      marker.artifactsBefore,
      marker.deliveryBeforeFileDigest,
    ) !== marker.stagingDigestBefore ||
    projectedStagingDigest(
      staging,
      marker.artifactsAfter,
      marker.deliveryAfterFileDigest,
    ) !== marker.stagingDigestAfter
  )
    throw new Error(
      "delivery state transaction以外のstaging成果物またはdigestが変更されています",
    );

  const recordBefore =
    currentRecordFileDigest === marker.stagingRecordBeforeFileDigest &&
    currentRecord.digest === marker.stagingDigestBefore &&
    stableJson(currentRecord.artifacts) === stableJson(marker.artifactsBefore);
  const recordAfter =
    currentRecordFileDigest === marker.stagingRecordAfterFileDigest &&
  ...
  if (!recordBefore && !recordAfter)
    throw new Error(
      "staging recordがdelivery transactionのbefore/afterいずれとも一致しません",
    );
  ...
  const deliveryBefore =
    currentDeliveryDigest === marker.deliveryBeforeDigest &&
    currentDeliveryFileDigest === marker.deliveryBeforeFileDigest;
  const deliveryAfter =
    currentDeliveryDigest === marker.deliveryAfterDigest &&
  ...
  if (deliveryAfter) {
    if (
      stableJson(inventory) !== stableJson(marker.artifactsAfter) ||
  ...
  throw new Error(
    "delivery stateがtransactionのbefore/afterいずれとも一致しません",
  );
}

function recoverPendingDeliveryStateTransactionLocked(
  staging: string,
): "none" | "rolled-back" | "completed" {
  ...
  if (pending.state === "delivery-published") {
    ...
    if (!refreshed || refreshed.state !== "record-refreshed")
      throw new Error(
        "delivery state transactionのrecord復旧確認に失敗しました",
      );
  }
  clearDeliveryStateTransactionLocked(staging);
  return "completed";
...
export function recoverPendingDeliveryStateTransaction(
  stagingInput: string,
): "none" | "rolled-back" | "completed" {
  const staging = assertWorkflowStaging(stagingInput);
...
function beginDeliveryStateTransactionLocked(
  staging: string,
  state: DeliveryState,
): { marker: DeliveryStateTransaction; source: string } | null {
  ...
  const storedBeforeSource = fs.readFileSync(
    path.join(staging, STAGING_RECORD_FILE),
    "utf8",
  );
  if (
    stableJson(artifactsBefore) !== stableJson(storedBefore.artifacts) ||
  ...
  const beforeState = beforeSource
    ...
    : undefined;
  if (beforeState && stableJson(beforeState) === stableJson(state)) return null;
  if (
  ...
  if (
    artifactsAfter.length > DELIVERY_STATE_TRANSACTION_LIMITS.artifacts ||
    artifactsAfter.some((artifact) => !validArtifactPath(artifact))
  )
    throw new Error(
      "delivery transactionのartifact pathまたは件数が上限外です",
    );
  const deliveryAfterFileDigest = sha256(source);
  const stagingDigestAfter = projectedStagingDigest(
    staging,
    artifactsAfter,
    deliveryAfterFileDigest,
  );
  const storedAfter: StoredStagingRecord = {
    ...storedBefore,
    artifacts: artifactsAfter,
    digest: stagingDigestAfter,
  };
  const marker: DeliveryStateTransaction = {
    schemaVersion: DELIVERY_STATE_TRANSACTION_SCHEMA,
    ...
    otherArtifactsDigest: calculateStagingDigest(
      staging,
      artifactsBefore.filter((artifact) => artifact !== DELIVERY_STATE_FILE),
    ),
  };
  const markerSource = `${stableJson(marker)}\n`;
  if (
    Buffer.byteLength(markerSource, "utf8") >
    DELIVERY_STATE_TRANSACTION_LIMITS.bytes
  )
    throw new Error(
      "delivery state transaction markerがbyte上限を超えています",
    );
  ...
  if (!inspected || inspected.state !== "before-publish")
    throw new Error(
      "delivery state transaction markerの開始確認に失敗しました",
    );
 ...
 * 薄いobserverである。
 */
export function observeStoredDeliveryState(
  directory: string,
...
function persistLocked(staging: string, state: DeliveryState): DeliveryState {
  ...
  if (!transaction) {
    ...
    if (!current)
      throw new Error(
        "delivery stateのno-op transactionにcurrent stateがありません",
      );
    return current;
  }
...
function recoverAndReadLocked(staging: string): DeliveryState | undefined {
  recoverPendingDeliveryStateTransactionLocked(staging);
  return readLocked(staging);
}
...
export function resumeStoredPullRequestCreationAfterConfirmedAbsence(
  directory: string,
): DeliveryState {
  const staging = assertWorkflowStaging(directory);
  return withStagingMutationLock(staging, () => {
    const current = recoverAndReadLocked(staging);
    if (!current) throw new Error("PR create再開より前のintentがありません");
    return persistLocked(
      staging,
      resumePullRequestCreationAfterConfirmedAbsence(current),
    );
  });
}
