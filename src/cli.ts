...
} from "./adapters/supplemental-review-launch.js";
...
import {
  ...
  loadEffectiveTrustedPolicySetAtCommit,
...
export function assertWorkflowReadyForDelivery(
  ...
  if (
    ...
    throw new Error(
      `delivery直前のstaging成果物またはcontent digestが同期済み記録から変化しています${describeStagingDigestDrift(staging)}`,
    );
...
function assertWorkflowReadyForTerminalRedelivery(
  ...
  if (
    ...
    throw new Error(
      `再配送直前のstaging成果物またはcontent digestが記録から変化しています${describeStagingDigestDrift(staging)}`,
    );
...
function assertCreatableReviewHead(
  ...
  if (
    ...
    throw new Error(
      "H_implとH_finalが同一のためPRを作成できません。H_implで`review export`を実行し、生成したreview証跡（docs/reviews/<Issue番号>_review.json）1 fileだけを実装commitの後にcommitしてから再実行してください",
    );
}
...
function currentIndependentApprovals(input: {
  ...
  /** project policyの`merge.reviewIndependence`。未指定は`context-isolated`。 */
...
export function resolveImplementationCommitForMerge(
  ...
  let declaredImplementation: string;
  try {
    declaredImplementation =
      parseReviewEvidence(
        artifactContent,
      ).observed.implementationHeadSha.toLowerCase();
  } catch (error) {
    throw new Error(
      `review証跡の構造とH_implが不正です: ${error instanceof Error ? error.message : String(error)}`,
      { cause: error },
    );
  }
 ...
 */
function reconcileFixedMergeCiRun(
  input: { root: string; repository: string; pr: number; state: DeliveryState },
  fixedCiRunId: string,
): string {
  const fixedRun = github(
    "pr.ci-run",
    { repository: input.repository, runId: fixedCiRunId },
    input.root,
  );
...
function observeMergeReviewEvidence(input: {
  ...
  ciEventAt: string;
  /**
   * merge成功後にだけ渡す固定run ID（Issue #1280）。
   *
   * `pull_requests`は「現在openで同一headを持つsame-repo PR」の一覧であり、
   * **PRが閉じた瞬間に空になる。** head_shaでの一覧再検索はmerge成功後に必ず
   * 失敗するため、**このときだけ固定identityの直読みへ切り替える。**
   *
   * **merge前は渡さない。** 渡さない経路の選別は1文字も変えておらず、
   * 空`pull_requests`の許容はmerge前から到達できない。
   * **切り替えるのはCI runの取得だけであり、実装commitの再観測、独立approvalの
   * 再確認、review Evidence identityの照合はどちらの経路でも同じに走る。**
   */
  fixedCiRunId?: string;
  /**
   * **trusted policy由来のreview独立性の要求水準**（Issue #1317）。
   *
   * ここは`pr merge`の実経路であり`authorizeMerge`より前段にある。**この引数を
   * 必須にしないと、`authorizeMerge`側だけを2モード化しても既定modeが実経路では
   * 到達不能なまま残る。** candidate filesystemではなく既定branchのtrusted policy
   * から解決した値だけを渡す。
   */
  independenceMode: "context-isolated" | "actor-independent";
  /**
 ...
 */
function resolveTrustedReviewIndependence(
  root: string,
): "context-isolated" | "actor-independent" {
  const symbolic = git(
    ["symbolic-ref", "--short", "refs/remotes/origin/HEAD"],
    root,
    { allowFailure: true },
  ...
  if (symbolic.status === 0) {
    ...
    const trustedRef = git(
      ["rev-parse", "--verify", `origin/${base}^{commit}`],
      root,
      { allowFailure: true },
    );
    if (trustedRef.status === 0)
      return resolveReviewIndependence(
        loadEffectiveTrustedPolicySet(root, base).policy,
      );
  }
  /**
  ...
  if (!fs.existsSync(manifestPath)) return "context-isolated";
...
function collectWorktreeSurvey(root: string): WorktreeSurvey {
  ...
  const { heads: registered, errors: headErrors } =
    registeredWorktreeHeads(repositoryRoot);
  ...
  const errors: string[] = [...headErrors];
  ...
  for (const [index, worktree] of registered.entries()) {
    try {
       ...
       */
      let mergedIntoDefault: boolean;
      if (worktree.branch === null) {
        const ancestor = git(
          ["merge-base", "--is-ancestor", worktree.headSha, remoteDefaultRef],
          repositoryRoot,
          { allowFailure: true },
        );
        if (ancestor.status !== 0 && ancestor.status !== 1)
          throw new Error(
            `既定branchへのmerge状態を確認できません: ${ancestor.stderr.trim()}`,
          );
...
function printWorktreeSurveyText(survey: WorktreeSurvey): void {
  const lines = [
    ...
    ...survey.entries.map(
      (entry) =>
        `${entry.disposition}\t${entry.branch ?? "(detached)"}\t${entry.path}\t${entry.reasons.join("、")}`,
    ),
...
export async function main(
  ...
  if (usage !== undefined) {
    ...
    const separator =
      usage.acceptsCommandArgv === true ? commandArgs.indexOf("--") : -1;
    const usageArgs =
      separator < 0 ? commandArgs : commandArgs.slice(0, separator);
  ...
  if (command === "decision" && subcommand === "configure") {
    ...
    if (flags["shell-rc-append"] === true) {
      ...
      const confirm =
        typeof flags.confirm === "string" ? flags.confirm : undefined;
      ...
      const shellRcSucceeded =
        !apply || result.applied || result.alreadyPresent;
  ...
  if (command === "decision" && subcommand === "label") {
    ...
    if (root !== activeRoot)
      throw new Error(
        "decision labelの--rootは--stagingが属するrepository rootと一致する必要があります",
      );
    const primaryRoot = resolveGitWorkspace(activeRoot).primaryRoot;
    const labelInput = readJsonInput(
      resolveContained(root, required(flags, "input")),
    );
    const label = parseEvaluationLabelInput(labelInput);
    const apply = applyMode(flags);
    if (apply) appendEvaluationLabel(primaryRoot, staging, label);
  ...
  if (command === "routing" && subcommand === "launch") {
    ...
    if (
      ...
      throw new Error(
        "routing launchは定義済みflagだけを受理します。model/providerや任意Codex引数を指定できません",
      );
    const sandbox =
      typeof flags.sandbox === "string" ? flags.sandbox : "read-only";
  ...
  if (command === "workflow" && subcommand === "assess-discovery") {
    ...
    const unknown = Object.keys(flags).filter(
      (flag) => !["input", "root", "staging"].includes(flag),
    );
    if (unknown.length > 0)
      throw new Error(
     ...
     */
    const sealJournal =
      flags.staging === undefined
        ? undefined
        : readWorkflowJournal(
            resolveContained(
              root,
              path.relative(root, path.resolve(root, flags.staging)),
            ),
          );
    if (sealJournal && sealJournal.errors.length > 0)
      throw new Error(
        `workflow assess-discoveryのjournalが不正です: ${sealJournal.errors.join("; ")}`,
      );
    const planSealed =
      sealJournal !== undefined &&
  ...
  if (command === "workflow" && subcommand === "record") {
     ...
     * journal追記経路も同じ検査を持つ。
     */
    if (isPlanFrozenCheckStep(step.step)) assertPlanFrozen(staging);
    const repositoryRoot = stagingRepositoryRoot(staging);
    const needsHeadSha =
      step.step === 9 ||
      step.step === 10 ||
    ...
    if (terminalIntake && postPrIntake)
      throw new Error(
        "--post-terminal-intakeと--post-pr-intakeは同時に指定できません",
      );
    ...
    if (postPrIntake && step.step !== 10)
      throw new Error(
        "--post-pr-intakeはworkflow record --step=10だけに指定できます",
      );
    if (step.step === 10) {
      const session = assertConvergedReviewSession({
        staging,
      ...
      if (postPrIntake && (hasTerminal || delivery?.state !== "pr-bound"))
        throw new Error(
          "--post-pr-intakeはStep 11記録前かつdelivery stateがpr-boundのときだけ指定できます",
        );
      if (!terminalIntake && hasTerminal)
        throw new Error(
      ...
      if (!postPrIntake && delivery?.state === "pr-bound" && !hasTerminal)
        throw new Error(
          "pr-bound中のStep 10再記録には--post-pr-intakeが必要です。外部reviewer指摘を同じPRで取り込んだroundであることを明示してください",
        );
      entry = {
        ...entry,
        reviewSession: {
        ...
        ...(terminalIntake ? { postTerminalIntake: true as const } : {}),
        ...(postPrIntake ? { postPrIntake: true as const } : {}),
      };
  ...
  if (command === "issue" && subcommand === "start") {
    ...
    const observeAuthority = () => {
      ...
      if (
        ...
        throw new Error(
          "provider default branch tipとlocal trusted policy commitが一致しません",
        );
      return { authority, localTrustedCommit };
    };
    const initialAuthority = observeAuthority();
    const trustedPolicy = loadEffectiveTrustedPolicySetAtCommit(
      root,
      initialAuthority.localTrustedCommit,
    ).policy;
    ...
    if (plan.state === "pending" && plan.operations[0] === "add-item") {
      ...
      if (
        currentAuthority.authority.defaultBranchTipOid !==
        initialAuthority.authority.defaultBranchTipOid
      )
        throw new Error(
          "Issue着手中にprovider default branch tipが変化しました",
        );
      ...
      if (plan.state !== "pending" || plan.operations[0] !== "add-item")
        throw new Error(
          "Issue着手の直前状態が変化しました。再previewしてください",
        );
      try {
        addIssueProjectItem(
          {
  ...
  if (command === "review" && subcommand === "validate") {
    const { flags, positionals } = parse(rest);
    const unknown = Object.keys(flags).filter(
      (flag) => !["file", "artifact", "root", "staging"].includes(flag),
    );
    if (unknown.length > 0)
      throw new Error(
    ...
    if (flags.staging !== undefined && artifact === undefined)
      throw new Error(
        "review validateの--stagingは--artifactと併用してください",
      );
    if (artifact !== undefined) {
      ...
      const errors: string[] = [];
      let evidence: ReturnType<typeof parseReviewEvidence> | undefined;
      try {
  ...
  if (command === "review" && subcommand === "export") {
    ...
    if (unknown.length > 0)
      throw new Error(
        `review exportの未知optionです: --${unknown.join(", --")}`,
      );
    const root = path.resolve(
      typeof flags.root === "string" ? flags.root : process.cwd(),
    );
  ...
  if (command === "verify" && subcommand === "run") {
    ...
    if (positionals.length > 0)
      throw new Error(
        `verify runの位置引数は\`--\`の後に置いてください: ${positionals[0]}`,
      );
    const unknown = Object.keys(flags).filter(
      (flag) => !["staging", "scope", "base", "root"].includes(flag),
    );
    if (unknown.length > 0)
    ...
    if (
      ...
      throw new Error(
        "verify runの--rootはstagingを置いたrepositoryのrootと一致する必要があります",
      );
  ...
  if (command === "pr" && subcommand === "create") {
    ...
    if (!existingBefore) {
      const mergeBase = git(
        ["merge-base", "--all", observedBaseSha, headSha],
        root,
        { allowFailure: true },
      );
      ...
      if (mergeBase.status !== 0 || mergeBases.length !== 1)
        throw new Error(
          `PR作成anchorのbase SHA ${observedBaseSha}とhead SHA ${headSha}のancestor関係をGitで比較できません（exit ${mergeBase.status}、merge-base ${mergeBases.length}件）。PRとdelivery stateを作成せず、Git objectとrepositoryを復旧してから再実行してください`,
        );
      if (mergeBases[0]!.toLowerCase() !== observedBaseSha.toLowerCase()) {
        ...
        const recovery =
          "既定branchをmergeで取り込んでHEAD依存のtest・review artifact・Step 10を再生成するか、内容等価なrebase後にreview reanchorで証拠を再固定してからpr createを再実行してください";
        const authority =
  ...
  if (command === "impact") {
    ...
    const unknown = Object.keys(flags).filter(
      (flag) => !["base", "head", "root", "format"].includes(flag),
    );
    if (unknown.length > 0)
    ...
    const resolveImpactCommit = (label: string, value: string): string => {
      const observed = git(
    ...
    if (impact.mode === "full") {
      process.stderr.write(
        `impact mode=full: 影響を証明できないため全体検証が必要です\n${impact.reasons.map((reason) => `- ${reason}`).join("\n")}\n`,
      );
      return 1;
    }
    process.stdout.write(
      impact.features.length > 0 ? `${impact.features.join("\n")}\n` : "",
    );
    return 0;
  }
  if (command === "doctor") {
