 ...
 */
function requireInlineJson(
  source: string,
  flag: string,
  opener: "[" | "{",
): void {
  if (source.trimStart().startsWith(opener)) return;
  throw new Error(
    `--${flag}にはinline JSONを渡します（file pathではありません）。${opener === "[" ? "JSON配列" : "JSON object"}の本文を--${flag}='…'の形式で指定してください`,
  );
}
