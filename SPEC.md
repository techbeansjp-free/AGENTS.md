...
- 要件7: 要件5の検知は `--dry-run` 指定時にも同様に行われ、実書込みを一切行わないまま検知結果を標準エラー出力へ表示する（要件6の確定内容に合わせ、`CliError` による中断・標準エラー出力への正規化で実現する）。
...
- Then: 展開先のファイルシステムには一切書込みが行われないまま、AC-4 と同じ検知結果が標準エラー出力へ表示される
- 検証方法見込み: `automated`
