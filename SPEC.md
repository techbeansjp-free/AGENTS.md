...
- **事実と常時規則**: 状態や作業段階に依存せず常に成立する規範。`AGENTS.md` 本体に残す対象であり、次の(a)(b)のみから成る網羅的な分類とする（「など」による非網羅的な例示は用いない）。
  - (a) 不変条件的事実: 不変条件I1〜I8・4セグメントと4ゲートの対応・Coordination Backendの選択肢・writer leaseの基本規則（lease種別・WIP上限等の恒久値）・用語の正本参照・「ブランチ・worktree」節が定めるブランチ名/worktreeパスの命名規約パターン（不変条件I4の機械的検証〔`verify-branch-name.sh`・`verify-worktree-path.sh`〕が直接参照する固定パターンであり、状態や作業段階に依存しない恒久的事実であるため）。
  - (b) 常時適用される原則・制約: 特定の実行手順を説明しない、常に適用されるルール。成果物の自己完結性の原則、参照・コメントの陳腐化防止の原則、ディレクトリ構成の制約（root直下許可リスト。要件10により`.claude/`を追加する）、設定値の優先順位、プロジェクト固有ポリシーの優先順位、`docs/system-spec/`の役割定義、GitHub配布における正本ディレクトリの所在（`.agent-skill-chain/templates/github/.github/`が正本である旨の1文程度の事実）、「ゲートの継承・無効化」節が説明する`gate-reconcile.sh`の恒常的な挙動（特定の手順を人間・AIが実行する「手続き」ではなく、常に成立する仕組みの記述であるため）。

これらは(a)(b)いずれにも属さない例外を残さないよう、現行`AGENTS.md`本体の全節を(a)(b)へ排他的に対応付けた結果であり、AC-1がこの対応付けの完全性を検証する。
...
1. `AGENTS.md` 本体に残す内容を、用語節が定める「事実と常時規則」——(a)不変条件的事実（不変条件I1〜I8・4セグメントと4ゲートの対応・Coordination Backendの選択・writer leaseの基本規則・用語の正本・「ブランチ・worktree」節が定めるブランチ名/worktreeパスの命名規約パターン）と(b)常時適用される原則・制約（成果物の自己完結性の原則、参照・コメントの陳腐化防止の原則、ディレクトリ構成の制約、設定値の優先順位、プロジェクト固有ポリシーの優先順位、`docs/system-spec/`の役割定義、GitHub配布における正本ディレクトリの所在、「ゲートの継承・無効化」節が説明する`gate-reconcile.sh`の恒常的な挙動）——のみに限定し、150行の文書量上限（`.agent-skill-chain/ci/verify-doc-length.sh`）内へ収める。この(a)(b)の2区分は現行`AGENTS.md`本体の全節を排他的に分類する網羅的な構成であり、いずれの節も未分類のまま残らない。
2. ゲート審査の進め方・worktree操作手順・ADRライフサイクルの操作手順・成果物テンプレートの記入手順等の「手続き」を、配布元の正本アセット配下（`.agent-skill-chain/templates/claude/skills/` 等）に置く複数の `SKILL.md`（セグメント・役割対応、単一の巨大スキルにしない）へ切り出す。各 `SKILL.md` は自己完結性の原則（AGENTS.md §成果物の自己完結性）に従い、目的・対象範囲・前提・用語・入力・出力・要求または判断内容・制約・完了条件・検証方法・未決事項・対象外を内部に記載し、参照だけで意味を委譲しない。ただし②設計・③実装・④独立検証の3セグメントは、writer lease取得→`worker-launch.sh`起動→ゲートレビューという同一の操作手続きを共有するため、これらを「セグメント作業」という単一スキルへ統合してよい。この統合は重複記述を避けるための粒度選択であり、要件2が禁じる「単一の巨大スキル化」とは、本来性質の異なる複数の独立した手続き群（起票・ゲート審査・PR操作・後片付け等）を1つのスキルへ無差別に詰め込むことを指す。「セグメント作業」はゲート審査・PR操作・後片付け等の異種手続きを含まない単一種類の手続きの繰り返しであるため、この禁止の対象外である。この粒度分けはADR-0023（`status: proposed`）が推奨案の細目として自らIssue起票とworktree開始・セグメント作業・ゲート審査・PR作成とマージ・後片付けの5分割を例示していることとも整合する。
...
11. `docs/GLOSSARY.md`（用語・定義・禁止同義語の3列、20行以内が制約）へ、要件7・AC-7が`AGENTS.md`本体I2セルへ導入する「軽量プロファイル」の用語行を新規追加する。あわせて対比対象として参照される「既定プロファイル」の用語行も追加する。追加後も `docs/GLOSSARY.md` 全体が20行以内という既存制約を満たすこと（是正着手時点で16行であり、1〜2行の追加は制約内に収まる）。
...
- Then: `AGENTS.md` が150行以内であり、かつ本文が(a)不変条件的事実（不変条件I1〜I8の表・4セグメントと4ゲートの対応表・Coordination Backendの選択・writer leaseの基本規則〔lease種別・WIP上限等の恒久値〕・用語の正本参照・「ブランチ・worktree」節が定めるブランチ名/worktreeパスの命名規約パターン〔不変条件I4の機械的検証が直接参照する固定パターンであるため(a)に区分〕）と(b)常時適用される原則・制約（成果物の自己完結性の原則・参照とコメントの陳腐化防止の原則・ディレクトリ構成の制約〔root直下許可リストに要件10により`.claude/`を追加した状態〕・設定値の優先順位・プロジェクト固有ポリシーの優先順位・`docs/system-spec/`の役割定義・GitHub配布における正本ディレクトリの所在を示す1文程度の事実・「ゲートの継承・無効化」節が説明する`gate-reconcile.sh`の恒常的な挙動〔常に成立する仕組みの記述であるため(b)に区分〕）のみで構成され、ゲート審査の進め方・worktree操作手順・ADRライフサイクル操作手順・成果物テンプレート記入手順といった手続き的記述を含まず、かつディレクトリ構成節のroot直下許可リストに`.claude/`が含まれる。この(a)(b)への対応付けは現行`AGENTS.md`本体の全節を排他的網羅的に分類したものであり、「ブランチ・worktree」節・「ゲートの継承・無効化」節を含むいずれの節も未分類のまま残らないことを本ACで確認する
- 検証方法見込み: `automated`
