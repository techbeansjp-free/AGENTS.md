...
- Given: 拡張後の `test/unit/worker-selection-reference.test.ts`（`src/lib/worker-selection.ts` の内容に対し、禁止参照パターン `DESIGN\.md §`・受入条件ID形式（`AC-` に数字を続けた形。既存テストが用いる正規表現 `/AC-[0-9]+/` が対象とする形式）・自己参照文言「本Issue」「本 Issue」（正規表現 `/本 ?Issue/` が対象とする形式）の3種の不在を検査する既存3テストケースを含む）
- When: 同テストファイルへ、以下の自己検証テストケースを追加する。実ファイル `src/lib/worker-selection.ts` の内容は読み取り専用で用い、実ファイルへは一切書き戻さない。実ファイルの内容に、受入条件ID形式の文字列1件と「本Issue」という文言1件を連結しただけの別のin-memory文字列を作り、既存3テストケースが使用しているものと同一の正規表現（`/AC-[0-9]+/`・`/本 ?Issue/`）がこのin-memory文字列に対して `assert.match` でマッチすることを検証する。その上で `npm test`（または `node --test test/unit/worker-selection-reference.test.ts`）を実行する
- Then: (a) 是正後の実ファイル `src/lib/worker-selection.ts` を対象とする既存3テストケースがすべて成功し（green：陳腐化パターンが実際に存在しないことの確認）、かつ (b) 新設した自己検証テストケースも成功する（green：もし正規表現の書き方に不備があり陳腐化パターンを見逃すようになっていれば、この自己検証テストケース自体が失敗し赤くなることで、回帰防止テストが「検知できない壊れた状態」のまま気付かれず放置されるリスクを防ぐ）。全テストケースの成功をもって、要件が定める「受入条件ID形式の参照および『本Issue』文言が残存しないことを恒久的に機械検証する」が実効性を持って満たされていることが確認される
- 検証方法見込み: `automated`（実ファイルを書き換えずin-memory文字列のみを対象とするため、CI上で安全に自動実行できる。実ファイルへ一時的に陳腐化パターンを混入させてred/greenを確認する手法は、作業ツリーの汚染・元に戻し忘れのリスクを伴うため採用しない）
