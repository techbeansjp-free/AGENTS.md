...
- **Check Run発行後のoutput.text改ざん耐性（check-run-output-patch-poisoning-gap）**: AC-5 Thenは、承認済み基準の照合先を「GitHub Check Run発行履歴（正本）等のwriterが改変不能な情報源」に限定することを要求する。しかしGitHub Check Run APIの`output.text`は、発行後もCheck Run自体のIDを特定できさえすれば、当該リポジトリ内で`checks: write`権限を持つ別の（新規追加されたものを含む）ワークフローファイルから、同一App identity（標準`GITHUB_TOKEN`が認証する単一App）のもとPATCH（Update a check run API）で書き換え可能である。この書き換えはcheck_suiteおよびその発行元workflow runとの関連付け（`workflow_runs[].path`・`.event`）を変更しないため、AC-5・AC-6のために追加したsource identity検証（発行元パス・イベント種別の照合）を経由しても検出できず、「writerが改変不能な情報源」というAC-5の前提は、Check Run『新規発行』による偽装に対しては成立するが『既存の承認済みCheck Runのoutput.text事後改ざん』という別クラスの攻撃に対しては成立しない。本Issueの実際の運用における、ゲート合否の実効的な信頼源は、GitHub Actions内のCheck Run発行そのものではなく、進行役がwriter権限とは分離された別のcredential（trusted recorder）を用いてGitHub PR Review APIへ投稿するreview evidence（本Issue自身のゲート審査もこの経路で行われている）であり、reconcileのCheck Run照合はpushトリガーのYAML自己参照という当初の脆弱性（AC-1〜AC-4）を閉じる多層防御の一つとして機能する。GitHub Check Run APIそのものが「同一App identityを持つ任意のワークフローが既存Check Runを更新できる」という仕様である以上、この経路をGitHub Actions内で完結する追加検証（例: `output.text`自体への署名・タイムスタンプの埋め込み）だけで完全に閉じることには原理的な限界がある。この残存リスクを技術的に完全に閉じるには、Check Run発行・更新主体をrulesetの`integration_id`で専用GitHub Appへ限定する強制enforcement backend（`docs/adr/ADR-0013-trusted-gate-check-materialization.md`、既に本Issueのスコープ外）を要するため、本Issueでは既知の限界として記録するにとどめる。
- **mainを未取込の既存issueブランチに残存する旧`on: push`版ワークフローによるマージ可否偽装（migration-gap）**: DESIGN.mdが新設する`resolveApprovedBaseline`のsource identity検証（`workflow_runs[].event`の許可集合照合）は、本修正のマージ前から存在しmainを未取込のissueブランチに残存する旧`on: push`版`agent-skill-chain-reconcile.yml`（当該ブランチ内にファイル名として同一のまま残存、自己参照構成）が発行する偽Check Runを、reconcile自身が承認済み基準として採用することは防ぐ。しかしこの防御はreconcile内部の判断にのみ及ぶ。現在適用済みのGitHub native ruleset（`gh api repos/{owner}/{repo}/rulesets/19276510`で実測確認済み）の`required_status_checks`は`context`（Check Run名）のみを指定し発行元Appを限定する`integration_id`を一切含まないため、reconcile内部の判定がどれだけ正しくても、当該旧`on: push`版ワークフローが正規に発行する偽Check Run自体（`conclusion=success`）はGitHub側のマージ可否判定（required status check）を満たしてしまう。DESIGN.mdが定める運用手順（進行役による既存openブランチのmain取込み確認、未移行検知ステップによる`::warning::`アノテーション）はいずれも人手中心の確認・可視化の運用手順に留まり、マージ阻止・自動移行のいずれも技術的に実装しない。この経路は`check-run-output-patch-poisoning-gap`・`new-arbitrary-workflow-file-forgery-gap`と同一の構造的原因（適用済みrulesetがintegration_id不使用）による限界であり、技術的な完全解消には同じくrulesetの`integration_id`による専用GitHub App強制（ADR-0013、本Issueのスコープ外）を要するため、本Issueでは既知の限界として記録するにとどめる。
- **新規untrustedワークフローファイルによる直接Check Run偽造（new-arbitrary-workflow-file-forgery-gap）**: 攻撃者が自分のissueブランチへ、`agent-skill-chain-reconcile.yml`・`agent-skill-chain-gate.yml`のいずれとも無関係な全く新規のワークフローファイル（例: `.github/workflows/evil.yml`、`on: push`かつ`permissions: checks: write`を自身のYAML内で明示宣言）を追加してpushした場合、そのワークフローは攻撃者が完全に制御する内容のままそのpushをトリガーに実行され、GitHub Checks APIを直接呼び出して`agent-skill-chain/{spec,design,implementation,validation}-gate`と同名のCheck Run（`conclusion=success`）を任意のコミットSHAへ発行できる。この経路は`agent-skill-chain-reconcile.yml`の`pull_request_target`化にも`resolveApprovedBaseline`のsource identity検証にも一切触れないため、本Issueのいずれの対応によっても阻止されない。現在適用済みのGitHub native ruleset（`gh api repos/{owner}/{repo}/rulesets/19276510`で実測確認済み）の`required_status_checks`は`context`（Check Run名）のみを指定し発行元Appを限定する`integration_id`を一切含まないため、標準`GITHUB_TOKEN`（全workflowが共有する単一App ID）で発行されたCheck Runであれば`agent-skill-chain-gate.yml`由来か攻撃者の新規ワークフロー由来かをGitHub側のマージ可否判定は区別しない。この経路の技術的な完全解消は、Check Run発行主体をrulesetの`integration_id`で専用GitHub Appへ限定する強制enforcement backend（`docs/adr/ADR-0013-trusted-gate-check-materialization.md`、既に本Issueのスコープ外節が明示的に除外）を要し、本Issueの対応範囲には含まれない。この制約は本Issueが新たに生む欠陥ではない。同リポジトリで既に安全な信頼モデルの参照実装として扱う`agent-skill-chain-gate.yml`（`pull_request_target`で起動しYAML自体はbaseブランチから読まれる既存ファイル）自身も、新規untrustedワークフローファイルによる直接Check Run偽造という同一の経路に対しては本Issue以前から同じ限界を持っており、本Issueのスコープではこの限界を変更しない。
