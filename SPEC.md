...
- 絞り込みにより`path_prefixes`から外れた範囲（`src/commands/`・`src/lib/`の大部分、`.agent-skill-chain/project/`・`.agent-skill-chain/config/`・`.agent-skill-chain/schemas/`・`.agent-skill-chain/ci/`のうち信頼境界に無関係な部分等）への変更は、`risk: normal`かつ`autonomy != full`のIssueであれば`review_profile: standard`（1レビュアによるconformance→falsification）で完了できる。
- 絞り込み後も、真に信頼境界に関わる範囲（ゲート/reconcileワークフロー本体、対応する配布テンプレート、adapters、ゲート判定・lease操作の中核スクリプト）への変更は、Issue自身の`risk`分類に関わらず引き続き`required_profile: strict`として検出される。
...
#### AC-3: writer lease・ゲート判定の中核スクリプトとadaptersはStrict対象のまま

- Given: 絞り込み後の`path_prefixes`。
- When: ゲート判定・lease操作の中核スクリプト、および`.agent-skill-chain/adapters/`配下のファイルへの変更差分を`matchesCorePath`で判定する。
- Then: 該当ファイルへの変更は引き続きcore_review対象（`required: true`）として検出される。
- 検証方法見込み: `automated`
...
- Given: 絞り込み後の`path_prefixes`。
- When: `src/commands/`・`src/lib/`のうち信頼境界（ゲート判定・lease操作・adapters・ワークフロー本体）に無関係な大部分のファイルへの変更差分を`matchesCorePath`で判定する。
