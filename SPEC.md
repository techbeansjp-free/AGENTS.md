...
- 回帰防止のため、既存の `test/unit/worker-selection-reference.test.ts`（現状は `DESIGN\.md §` パターンのみを検査）を拡張し、`src/lib/worker-selection.ts` 全体から受入条件ID形式の参照および「本Issue」文言が残存しないことを恒久的に機械検証するテストへ強化する（実装セグメントで実施。詳細は本節後段の受入条件のうち、陳腐化参照の除去を検証する項の実装への申し送り段落、およびそれに続くテスト自己検証を扱う項に記載する）。
...
**実装への申し送り**：本ACの機械検証コマンド（`! grep -nE 'AC-[0-9]+|本 ?Issue' src/lib/worker-selection.ts`）は実装時の一時確認用のコマンドであり、それ自体はリポジトリへ組み込まれる恒久的な回帰防止策ではない。恒久的な回帰防止は、既存の `test/unit/worker-selection-reference.test.ts`（現状 `assert.doesNotMatch(contents, /DESIGN\.md §/, ...)` のみを検査）を拡張し、`src/lib/worker-selection.ts` の内容に対して `assert.doesNotMatch(contents, /AC-[0-9]+/, ...)` および `assert.doesNotMatch(contents, /本 ?Issue/, ...)` 相当の検査を追加することで実装する。この拡張の存在と、拡張された検査が実際に陳腐化パターンの再発を検知できることの機械的な自己検証は、本節に続く受入条件（テスト自体の自己検証を扱う項）が担う。

#### AC-5: 拡張後の回帰防止テストが陳腐化パターン混入を実際に検知できることを、テスト自体が機械的に自己検証する

strict spec-gateレビューにて「要件が『回帰防止テストの拡張』を必須としているにもかかわらず、これまでの受入条件のいずれもそのテスト拡張の存在・有効性を検証しない」という指摘を受け新設する。直前の陳腐化参照除去を検証する受入条件だけでは、テスト拡張自体を実装セグメントが省略しても他の受入条件はすべて充足してしまい、恒久的な回帰防止という目的が達成されないままIssueが完了しうる。

strict design-gateレビューにて「本ACの当初案は、実ファイル `src/lib/worker-selection.ts` の内容に受入条件ID形式の文字列と『本Issue』という文言を連結した文字列を汚染データとして用いる構成だったが、AC-4は是正後の実ファイルからこれら2パターンが1件も残らないことを恒久的に要求するため、是正後は実ファイル由来部分が常に空文字列となり『実ファイルの内容を使う』という構成自体が意味を成さない」という自己矛盾の指摘を受け、汚染データの構成方法を全面的に書き換える。是正後の合理的な構成は、実ファイルの内容には一切依存しない、テスト内で明示的に組み立てた合成文字列リテラルを汚染データとして用いるものである。

- Given: 拡張後の `test/unit/worker-selection-reference.test.ts`（`src/lib/worker-selection.ts` の内容に対し、禁止参照パターン `DESIGN\.md §`・受入条件ID形式（`AC-` に数字を続けた形。既存テストが用いる正規表現 `/AC-[0-9]+/` が対象とする形式）・自己参照文言「本Issue」「本 Issue」（正規表現 `/本 ?Issue/` が対象とする形式）の3種の不在を検査する既存3テストケースを含む）が、AC-4の恒久検査で使用する上記2つの正規表現（`/AC-[0-9]+/`・`/本 ?Issue/`）を、既存3テストケースと新設する自己検証テストケースの両方から共有できる形（同一ファイル内の定数として1箇所に定義し、テストケース間でリテラルを複製しない形）で定義していること
- When: 同テストファイルへ、以下の自己検証テストケースを追加する。実ファイル `src/lib/worker-selection.ts` の内容は一切参照・使用しない。受入条件ID形式の文字列例（例：`AC-9`）と「本Issue」という文言例（例：`本 Issue のスコープ外`）の両方を含む、テスト内で明示的に構成した合成の文字列リテラル（実ファイルに依存しない、意図的に汚染させたin-memory文字列）を用意し、上記の共有された2つの正規表現（`/AC-[0-9]+/`・`/本 ?Issue/`）がこの合成汚染文字列に対して `assert.match` でマッチすることを検証する。その上で `npm test`（または `node --test test/unit/worker-selection-reference.test.ts`）を実行する
- Then: (a) 各正規表現（`/AC-[0-9]+/`・`/本 ?Issue/`）が、上記の合成汚染文字列に対して実際にマッチすること（`assert.match` で検証）。もし正規表現の書き方に不備があり陳腐化パターンを見逃すようになっていれば、この自己検証テストケース自体が失敗し赤くなることで、回帰防止テストが「検知できない壊れた状態」のまま気付かれず放置されるリスクを防ぐ。(b) AC-4の恒久検査（既存3テストケースのうち受入条件ID形式・自己参照文言を検査する2件）と、本ACの自己検証テストケースが、同一の正規表現定数を共有していること（リテラルの二重管理を避けるため。AC-4側の正規表現が将来書き換えられて検知力を失った場合に、AC-5側が独立した複製の正規表現を保持していると、その複製だけが誤ってマッチし続け自己検証テストケースがgreenのまま維持されてしまい、AC-4側の劣化に気付けなくなる事態を防ぐ）。加えて是正後の実ファイル `src/lib/worker-selection.ts` を対象とする既存3テストケースもすべて成功する（green：陳腐化パターンが実際に存在しないことの確認）。全テストケースの成功をもって、要件が定める「受入条件ID形式の参照および『本Issue』文言が残存しないことを恒久的に機械検証する」が実効性を持って満たされていることが確認される
