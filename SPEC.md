...
- 敵対的入力集合: 本 SPEC の受入条件が contract 本文へ必ず含めることを要求する文字・文字列の集合。次のすべてを含む。(1) 単一引用符、(2) バックスラッシュ、(3) 改行、(4) `<`、(5) `$(`、(6) バッククォート、(7) リテラル文字列 `CODEX_CMD=`（行の途中に現れる形と、行頭に現れる形の**両方**を必ず含める）、(8) 非 ASCII 文字（UTF-8 マルチバイト文字を最低1文字）。(7) は仮想の入力ではない——本 Issue の議論・差し戻し指示・レビュア evidence 原文が当該トークンを含み、それらの Issue/PR コメントは contract 本文へ実際に取り込まれるため、正本行の一意性を壊しうる現実の入力である。(8) は出力エンコード不一致を本文の同一性比較で捕捉するために含める。
...
- 生成側が、提示しようとしている起動コマンドの構文妥当性を機械的に検査する。検査は組み立てた起動コマンド文字列に対し次の3条件を要求し、いずれか1つでも満たさない場合を検査失敗とする。(i) 生の改行文字（LF）を含まない。(ii) シェルの構文解析（`bash -n` 相当）でエラーが発生しない。(iii) 文字列自身が行頭プレフィックス `CODEX_CMD=` を新たに生じさせず、正本行がちょうど1本である性質を壊さない。この判定は文字列の由来を問わない——起動コマンドへ埋め込まれる素材由来か、contract 本文由来（本文が当該トークンを行頭に含む場合）かにかかわらず、組み立て後の文字列を対象に行う。
...
- Given: 本 SPEC が定める敵対的入力集合の要素をすべて含む本文（行頭に現れるリテラル `CODEX_CMD=` を含む）が role contract へ取り込まれ、adapter が `codex`、`worker.agent_tool_dispatch.enabled: true` である。
...
- Then: 行頭 `CODEX_CMD=` の行は、contract 本文自身が当該トークンを行頭に含む場合でも、出力全体を通じてちょうど1本だけ存在する。取り出した文字列は実行対象の起動コマンドと完全一致し、指示文の散文（`<issue_id>` 等のリダイレクト記号に見えるメタ文字を含む文言）を一切含まない。`prompt:` 行は起動コマンド文字列を再掲していない。取り出した文字列は生の改行文字を含まない。
- 検証方法見込み: `automated`

#### AC-3: 起動時に渡る contract が監査値と一致する（両経路）

- Given: AC-1 と同じ敵対的入力集合の要素をすべて含む contract 本文（行頭に現れるリテラル `CODEX_CMD=`・非 ASCII 文字を含む）を用い、次の2条件それぞれで dispatch 出力と dispatch 一時ディレクトリの `contract.md`・`contract.sha256` を得ている。(a) 本文サイズが標準入力安全閾値以下＝標準入力経由。(b) 本文サイズが同閾値を**超過**＝位置引数埋め込み経路（本 Issue で実際に破壊が起きた経路であり、必ず含める）。
- When: (a)(b) それぞれについて、AC-2 の抽出規則で取り出した起動コマンドをそのまま実行し、Codex CLI が受け取った本文（(a) は標準入力、(b) は位置引数）を捕捉する。
- Then: (a)(b) いずれにおいても、捕捉した本文のバイト列の SHA256 が `contract.sha256` の `CONTRACT_SHA256` と一致し、行数が同ファイルの `CONTRACT_LINES` と一致する。バイト列の比較であるため、非 ASCII 文字のエンコード不一致もこの比較で検出される。どちらか一方の経路のみの確認では本 AC を充足しない。
- 検証方法見込み: `automated`

#### AC-4: 妥当性検査失敗時は安全側へ倒す

- Given: adapter が `codex`、`worker.agent_tool_dispatch.enabled: true` の状態で、次の2経路それぞれを用意している。(A) 素材経路——本 SPEC の要件が定める注入点を用い、起動コマンドへ埋め込まれる素材（Codex 実行ファイルのパス等）を、組み立て結果が検査条件 (i) 生の改行を含まない・(ii) シェル構文解析でエラーにならない・(iii) 行頭 `CODEX_CMD=` を新たに生じさせない、のいずれかに違反する値へ差し替えている。(B) contract 本文経路——注入点を用いず、行頭に現れるリテラル `CODEX_CMD=` を含む contract 本文（敵対的入力集合を満たす本文）をそのまま与え、検査条件 (iii) 違反が本文由来で成立しうる状況を作る。
- When: (A)(B) それぞれについて `worker-launch.sh <issue_id> <segment>` を実行する。
- Then: 経路 (A) では次の (a)〜(g) がすべて成立する。経路 (B) では、生成側の検査が (iii) 違反を検出した場合は同じく (a)〜(g) がすべて成立し、検出せず起動コマンドを提示した場合は AC-2 の一意性（出力全体で行頭 `CODEX_CMD=` の行がちょうど1本）と AC-3 の本文一致が成立する。どちらでもない状態——正本行が2本以上出力される、正本行が0本になる、本文が欠落・切り詰められる、またはワーカーが起動されたまま検査失敗が報告される——を本 AC は許容しない。(a)〜(g) は次のとおり。(a) ワーカーを起動しない。(b) 日本語の診断メッセージを標準エラーへ出力する。(c) dispatch のために起動した lease 更新プロセス（`renew.pid` が指すプロセス）が実行終了時点で生存していない。(d) dispatch 一時ディレクトリが `contract.md`・`contract.sha256`・`renew.pid` ごと削除されている。(e) writer lease が解放されており、同一 Issue に対して直後に新規 writer lease を取得できる。(f) 終了コードが `5` である。`5` は起動コマンド妥当性検査失敗を一意に表す専用値であり、本 SPEC の「終了コードの既存体系」が列挙する `0`（完了）・`1`（起動前の前提不成立）・`2`（lease 取得前エラー）・`3`（human deferred）・`4`（dispatch 要求）のいずれとも重複しない。(g) 標準出力に `CODEX_CMD=` 行・`AGENT_TOOL_DISPATCH_REQUIRED` 行を出力しない。(c)(d) を欠き lease 解放のみを行う実装は本 AC を充足しない。
- 検証方法見込み: `automated`
