...
- Then: 一時作業領域は後始末され、CLI公開動作・package収録契約・repository状態に副作用が残らない。
  ただし、検証のために起動した子プロセスが生存したまま回収できない場合（reap不能）に限り例外を
  認める。生存中の子プロセスが使用中の一時作業領域を削除処理と競合させると、子プロセス側の
  ファイルI/Oが破損する、または子プロセスがcrashしうるため、この場合はOS一時ディレクトリ配下の
  検証専用workspace自体の削除を意図的に抑止し、fatal失敗として扱ってよい。この抑止は後始末要件
  そのものの例外的除外であり、削除を抑止した場合でもrepository内には検証用生成物を一切残さず、
  CLI公開動作・package収録契約・repository状態への副作用は生じさせない。
- 検証方法見込み: `automated`

## 制約・完了条件・未決事項

- 制約: npmの実際のpack計算を模倣実装へ置換せず、test runner全体の並列実行を無効化しない。
- 制約: AC-5のreap不能例外によりOS一時ディレクトリ配下の検証専用workspace削除を抑止した場合、
  reap不能が繰り返し発生する運用ではそのworkspaceがOS一時ディレクトリ配下に蓄積し得る。これは
  OS一時領域自体の運用（定期的な外部clean等）に属するtrade-offとして許容し、repository内へ
  検証用生成物を残さないというAC-5の意図を損なわない範囲に限定する。
