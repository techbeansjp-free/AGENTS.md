...
- Then: `AGENTS.md` が150行以内であり、かつ本文に含まれる各段落のいずれもゲート審査の進め方・worktree操作手順・ADRライフサイクル操作手順・成果物テンプレート記入手順といった「手順の段階的な説明」を含まず判定基準を満たすことを目視確認でき、かつディレクトリ構成節のroot直下許可リストに`.claude/`が含まれる。個々の節・段落を`AGENTS.md`本体に残すか`SKILL.md`へ移すか（または両方に分割するか）の具体的な対応付けは設計セグメント（`DESIGN.md`）が確定する設計判断であり、本ACはその結果が判定基準へ適合していることのみを検証する。「これで全節を尽くした」という網羅性の主張は本ACの検証対象ではない
- 検証方法見込み: `hybrid`（目視確認による判定基準適合確認と `verify-doc-length.sh` による自動行数検査の組み合わせ）
