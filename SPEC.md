...
`core_review`の要否・プロファイル強制ロジック自体（`src/lib/model-selection.ts`の`matchesCorePath`・`classifyCoreReview`、`src/commands/gate.ts`のプロファイル強制処理）、およびI2（セグメントゲート）・I4（分離）・I5（進行役の純粋性）・I7（仕様⇔検証の追跡）の検査手段を実装する`.agent-skill-chain/schemas/`・`.agent-skill-chain/ci/`内の一部ファイルは、絞り込みの対象そのものが自己言及的にstrict強制を弱体化しうる特別な範囲である。本Issueはこれらを絞り込み対象から明確に除外し、strict維持を要求レベルで明記する。
...
- `.agent-skill-chain/schemas/`・`.agent-skill-chain/ci/`は絞り込み対象の候補とするが、AGENTS.mdのI2（セグメントゲート）・I4（分離）・I5（進行役の純粋性）・I7（仕様⇔検証の追跡）の検査手段を実装するファイル（`.agent-skill-chain/schemas/`内の`gate-report.schema.yaml`・`lease.schema.yaml`・`worker-report.schema.yaml`・`state.schema.yaml`・`segments.schema.yaml`、`.agent-skill-chain/ci/`内の`verify-gate-report.sh`・`verify-artifacts.sh`・`verify-branch-name.sh`・`verify-worktree-path.sh`）は絞り込み対象から除外しstrict維持とする。これら以外の低リスクな`schemas/`・`ci/`内容（上記I2/I4/I5/I7検査手段に該当しないファイル）のみを絞り込み候補とする。
- 絞り込みにより`path_prefixes`から外れた範囲（`src/commands/`・`src/lib/`の大部分（ただし上記の`core_review`判定・プロファイル強制ロジックを除く）、`.agent-skill-chain/project/`・`.agent-skill-chain/config/`・`.agent-skill-chain/schemas/`・`.agent-skill-chain/ci/`のうち信頼境界に無関係な部分（ただし上記のI2/I4/I5/I7検査手段ファイルを除く）等）への変更は、`risk: normal`かつ`autonomy != full`のIssueであれば`review_profile: standard`（1レビュアによるconformance→falsification）で完了できる。
- 絞り込み後も、真に信頼境界に関わる範囲（ゲート/reconcileワークフロー本体、対応する配布テンプレート、adapters、ゲート判定・lease操作の中核スクリプト、`core_review`判定・プロファイル強制ロジック自体、I2/I4/I5/I7検査手段を実装する`schemas/`・`ci/`ファイル）への変更は、Issue自身の`risk`分類に関わらず引き続き`required_profile: strict`として検出される。
...
#### AC-3: writer lease・ゲート判定の中核スクリプト、adapters、`core_review`判定/プロファイル強制ロジック自体、およびI2/I4/I5/I7検査手段を実装する`schemas/`・`ci/`ファイルはStrict対象のまま
...
- When: 以下のいずれかへの変更差分を`matchesCorePath`（`src/lib/model-selection.ts`）で判定する。
  ...
  - (f) `.agent-skill-chain/ci/`内の`verify-gate-report.sh`・`verify-artifacts.sh`・`verify-branch-name.sh`・`verify-worktree-path.sh`
- Then: 該当ファイルへの変更は引き続きcore_review対象（`required: true`）として検出される。
- 検証方法見込み: `automated`
...
- Given: 絞り込み後の`path_prefixes`。
- When: `src/commands/`・`src/lib/`のうち信頼境界（ゲート判定・lease操作・adapters・ワークフロー本体・AC-3(c)/(d)の`core_review`判定/プロファイル強制ロジック）に無関係な大部分のファイル、および`.agent-skill-chain/schemas/`・`.agent-skill-chain/ci/`のうちAC-3(e)/(f)に列挙したI2/I4/I5/I7検査手段ファイルを除く部分への変更差分を`matchesCorePath`で判定する。
