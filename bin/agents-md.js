...
const ENFORCE_TEMPLATE_PATH = join(PACKAGE_ROOT, ".agents", "platforms", "claude", "settings.enforce.json");
...
function runSetup(projectRoot) {
    if (!existsSync(SETUP_PATH)) {
        console.error(`エラー: setup.sh が見つかりません: ${SETUP_PATH}\n` +
    ...
    if (result.error) {
        ...
        if (err.code === "ENOENT") {
            console.error(`エラー: bash が見つかりません。setup.sh の実行には bash が必要です。\n` +
...
function runDoctor() {
    ...
    check(".claude/hooks/PreToolUse.sh（enforcement フック）", existsSync(preHookPath), "Claude enforcement が未配備の場合は init を実行してください。");
    ...
    if (settings === null) {
        ...
        console.log(`[INFO] enforcement 配線 = ${on ? "on" : "off（既定）"}` +
    ...
    const installed = existsSync(join(projectRoot, ".agents")) || existsSync(join(projectRoot, "AGENTS.md"));
...
function runUninstall(projectRoot, opts) {
    ...
    const looksInstalled = existsSync(join(projectRoot, ".agents")) || existsSync(join(projectRoot, "AGENTS.md"));
    if (!looksInstalled) {
        console.error(`エラー: ${projectRoot} に配備の痕跡（.agents/ または AGENTS.md）が見つかりません。\n` +
    ...
    if (!purge)
        console.log("  .workflow/workflow.db*  （証跡 DB。--purge 指定時のみ削除）");
    if (!yes) {
...
function readEnforceTemplate() {
    try {
        const parsed = JSON.parse(readFileSync(ENFORCE_TEMPLATE_PATH, "utf8"));
...
function enforceIsOn(settings) {
    ...
    for (const event of HOOK_EVENTS) {
        ...
        if (Array.isArray(arr) && arr.some(isManagedHookEntry))
            return true;
    }
    return false;
}
...
function templateHookEntries(template, event) {
...
function enforceOff(_projectRoot, settingsPath, settings, template) {
    if (!existsSync(settingsPath)) {
    ...
    if (next.env && typeof next.env === "object") {
        next.env = { ...next.env };
        for (const k of templateEnvKeys(template))
            delete next.env[k];
...
function warnIfHookScriptsMissing(projectRoot) {
    for (const name of ["PreToolUse.sh", "PostToolUse.sh"]) {
        ...
        if (!existsSync(p)) {
            console.log(`警告: ${join(".claude", "hooks", name)} が見つかりません。init を実行して hook を配備してください。`);
        }
    }
}
// enforce status: 現在 on/off と hook スクリプト実在性を表示する。
...
function runEnforce(projectRoot, action) {
    ...
    const settings = readSettings(settingsPath);
    if (settings === null) {
        console.error(`エラー: ${settingsPath} が妥当な JSON ではありません。\n` +
    ...
    if (template === null) {
        console.error(`エラー: enforcement テンプレートが読めません: ${ENFORCE_TEMPLATE_PATH}\n` +
            `        パッケージが壊れている可能性があります。再インストールしてください。`);
        return 1;
    }
    if (action === "on")
        return enforceOn(projectRoot, settingsPath, settings, template);
    if (action === "off")
        return enforceOff(projectRoot, settingsPath, settings, template);
