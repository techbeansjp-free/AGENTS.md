...
# 進行役の指示による V1・V2 の自動テスト追加後に全テストを再実行した結果:
#     tests 1366 / pass 1365 / fail 0 / cancelled 0 / skipped 1 / todo 0
#     skip は上記と同じ opt-in テストであり、既存テストの退行は無い。
...
#
# 【V1】独立検証時点で、必須テストケース「免除解除かつ充足」（対応 AC-10）に
#       対応する自動テストが無かった
...
#   その後の是正: 進行役の指示により implementation セグメントで
#   test/integration/verify-artifact-targets.test.ts へ「quick免除解除後も対象集合が全充足なら
#   解除理由を示して成功する」を追加した。終了コード0、解除理由、R の4セグメント、欠落報告が
#   無いことを固定し、追加後の全テストは1365成功・0失敗・1skipだった。
#
# 【V2】独立検証時点で、必須テストケース「閉包追加あり・開始済み分のみ欠落」
#       （対応 AC-9）に対応する独立した自動テストが無かった
...
#   その後の是正: 進行役の指示により同じ結合テストファイルへ「閉包追加分が充足して
#   開始済み分だけ欠落する場合は開始済み分を報告する」を追加した。閉包追加分を対象集合へ
#   含めつつ欠落として報告せず、開始済み implementation の code・unit_test_results だけを
#   欠落として報告することを固定した。追加後の全テスト結果は V1 記載のとおりである。
#
...
# 制約
# ============================================================================
# - 独立検証時点では SPEC.md・DESIGN.md・PLAN.md・ADR・実装コード・CI ワークフロー・
#   テストコードのいずれも変更せず、V1・V2 を事実として記録した。その後、進行役の指示により
#   implementation セグメントで V1・V2 の自動テストだけを追加した。実装コードは変更していない。
# - AC-10・AC-9 の当該ケースは、独立検証時の手動実行に加えて、是正後は自動テストでも
#   再現される。独立検証時に欠落していた事実は上記 V1・V2 に保持している。
...
# - 独立検証時の検出事項と、implementation セグメントでの是正後の結果が区別して記録されている。
#
# ============================================================================
# 未決事項
# ============================================================================
  ...
  - ac_id: AC-9
    ...
    evidence:
      - |-
        ...
        開始済み分だけ欠落する場合は開始済み分を報告する（独立検証時に欠落を検出した後、
        進行役の指示により implementation セグメントで追加。閉包追加された spec・design と
        開始済み implementation が対象集合に現れ、欠落としては implementation の code・
        unit_test_results だけを報告することを検証。成功）
      - |-
      ...
      - |-
        独立検証セグメントでの直接実行（自動テスト追加前の履歴証跡。開始済み分にのみ欠落する形態）:
        一時リポジトリで閉包追加分を充足し、開始済み実装成果物を作らずに一括検査を実行した結果、
        終了コード1。標準エラー出力の対象集合の要約は
  ...
  - ac_id: AC-10
    ...
    evidence:
      - |-
        ...
        対象集合が全充足なら解除理由を示して成功する（独立検証時に欠落を検出した後、進行役の
        指示により implementation セグメントで追加。quick 要求・risk normal・ガードレール抵触、
        R の全成果物充足という状態で終了コード0、解除理由、4セグメントの対象集合、欠落報告なしを
        検証。成功）
      - |-
        独立検証セグメントでの直接実行（自動テスト追加前の履歴証跡）: 一時リポジトリで同じ状態を
        作って実行した結果、終了コード0。
        標準エラー出力は、まず解除通知
...
regression:
  ...
  evidence:
    - |-
      進行役の指示による V1・V2 の自動テスト追加後に npm test を再実行: 終了コード0。
      tests 1366 / pass 1365 / fail 0 / cancelled 0 / skipped 1 / todo 0。
      skipped 1件は従来と同じ実ネットワーク opt-in テストであり、追加した2件を含む全自動テストが
      成功し、既存テストの退行が無いことを再確認した。
    - |-
