...
- **同一 UID 読み取りの原理的限界（正直化・C-3）**: `0600` パーミッションは「別ユーザーからの読み取り」を防ぐものであり、**同一 OS ユーザー（同一 UID）で動作する AI プロセス自身が `${AGENTS_ROOT}/.scribe-nonce` を読むこと自体は、env 空間の「掌握」を要さない通常のファイル読み取り操作である**（`cat`/`head` 等で即座に読める。追加の権限昇格や特別な攻撃手順を要しない）。これはバグではなく、同一 OS ユーザーで実行される限りファイル権限層では原理的に防げない限界である。したがって nonce（ファイル出所制御）が実際に遮断できるのは、上記のとおり「期待値の**出所を実 nonce（env）と分離**したことで、env だけを書き換える手動 export 偽装の一致条件を崩す」という限定効果のみであり、ファイル読み取り自体を隠す・防ぐ効果は持たない。**DB 系監査の信頼の主線は本 nonce ではなく、外部 append-only 証跡（署名付き NDJSON export・`gen_entry_hash`/`prev_hash` によるハッシュチェーン）に置くことを設計主線とする**（当該証跡の実装は D-1/E-2/E-3 パッケージが所有し、本パッケージ〈enforcement〉は当該外部証跡へ依存・参照する形に留める）。nonce の暗号強度自体は現状維持とする（同一 UID 限界は強度を上げても解消しないため強化の優先度は低い）。
...
### enforcement 正本ファイル保護（C-1・R1B・ドッグフーディング両立）

強制正本（`.agent-skill-chain/source/enforcement/**`・`.agent-skill-chain/project/orchestrator-allowlist.txt`・`.agent-skill-chain/project/settings.enforce.json`・`.scribe-nonce`）を worker が自由に書き換えられると、PreToolUse 自体・allowlist・scribe nonce 検証など強制の実効性が丸ごと無効化される（消費者環境における high 経路）。一方、本リポジトリ自身のドッグフーディング開発では、worker が正規の作業として enforcement を編集する必要がある。両立のため以下の設計を採る。

- **runtime（`PreToolUse.sh` R1B）**: 対象パス（上記4種）への `Edit`/`Write` を、R1（`.agent-skill-chain/runtime/` 配下）とは独立した path 軸のガードとして、**全 ROLE（worker 含む）で block** する。R2（`ROLE=orchestrator` の許可ツールのみ）とは判定軸が異なる独立ガードである。
- **ドッグフーディング opt-out（`ASC_ENFORCEMENT_SELF_DEV`）**: 本リポジトリ自身の開発時のみ、env `ASC_ENFORCEMENT_SELF_DEV`（非空かつ `"0"` 以外）を設定すると block を解除する。解除時は既存の `ASC_WORKTREE_CLOSE_BYPASS`（`lib/worktree_record_guard.sh` の `_wt_record_bypass_warn`）と同型の `[enforcement:warn]` を stderr へ必ず出力し、監査痕跡を残す。
- **CI 補完（`audit.sh` #42）**: 対象差分（`GIT_RANGE`）が上記いずれかのパスに触れる場合、`workflow_log` に対応する command（`implement-feature`/`design-feature`/`verify-and-close`/`review-docs`/`create-pr-review-issue`）の記録が 1 件以上あることを要求する（無ければ FAIL）。`ASC_ENFORCEMENT_SELF_DEV` で runtime block を解除して編集した場合でも、本チェックにより委譲・レビュー証跡が突合され追跡可能になる。
- **参照用リスト（`PROTECTED_PATHS.txt`）**: 上記4種の正本パスを「参照用リスト」として追記済み（`docs/`・`src/` 等の orchestrator 拒否対象とは性質が異なる「全 ROLE 拒否対象」である旨を明記）。`PreToolUse.sh` はこのファイルを読まない（判定ロジックの実体は R1B に自己完結）。
- **残余リスク（受容済み・正直化）**: `ASC_ENFORCEMENT_SELF_DEV` は env であり、env 空間を掌握した相手には偽装可能である。これは既存 `ASC_WORKTREE_CLOSE_BYPASS`・scribe nonce と**同一の脅威モデル**（env 掌握相手は対象外、最終保証は CI audit ＋ 外部証跡）であり、新たなリスククラスを増やさない。現状比の改善は「消費者環境で無保護 → hard block」「本リポ開発で無痕跡 → env 痕跡＋audit 証跡必須」の2点である。
...
**Issue Forms 雛形の project 拡張点（opt-in・github_native 向け）**: `ISSUE_TRACKING_MODE=github_native`（[skills/agent/run_command.md](../skills/agent/run_command.md) §Constraints 参照）採用時、GitHub Web UI からの手動起票にも目的・成功基準・受け入れ基準の入力を構造強制できる。消費先プロジェクトが `.github/ISSUE_TEMPLATE/issue-request.yml`（ユーザー資産）へコピーして有効化する GitHub Issue Forms 形式の inert 雛形を `enforcement/github/issue-request.example.yml` に置く。本雛形は enforcement・`audit.sh`・hook のいずれからも読まれない（AI 起票 `gh issue create --body` は Forms を経由せず body テンプレートを直接渡すため、雛形の有無に関わらず不変）。使い方は [SETUP.md §Issue Forms 雛形の利用（opt-in）](../SETUP.md) を参照。
...
- **scribe 未実行の次 Task 拒否**: 検証・クローズ command で write-workflow-log を経ずに次に進むことを防ぐ。**強制レベル＝CI（audit.sh #3/#9）のみによる事後検知であり、hook による物理強制（次 Task 呼び出し自体のブロック）は実装されていない**（C-4・DESIGN.md §強制の4層 参照）。CI では workflow.db が存在しない環境（クリーンな checkout 等）では構造的に SKIP されうる。
...
**`AUDIT_STRICT=1`（C-2・唯一の実効検知経路の骨抜き防止）**: 各ゲートは `*_GATE_ENABLED=false`・許容窓・`WORKFLOW_DIRS` 等の env で個別に無効化できるため、上記の pre-push フック（唯一の実効的な検知経路）は、被監査 AI 自身が同一シェルで無効化系 env を設定することで骨抜きにされうる（例: `GITHUB_ISSUE_GATE_ENABLED=false git push`）。`audit.sh` は env `AUDIT_STRICT=1` を受け取ると、次の無効化系 env を無視して既定値を強制し、無効化トグルが指定されていた事実を stderr へ `WARN` として出力する（監査痕跡）。強制対象は `*_GATE_ENABLED` 系（`GITHUB_ISSUE_GATE_ENABLED`/`BRANCH_LINK_GATE_ENABLED`/`PR_LINK_GATE_ENABLED`/`MODEL_TIER_GATE_ENABLED`）・`MAIN_WORK_GATE_TOLERANCE_SECONDS`・`WORKFLOW_DIRS`（scan-nothing 化＝走査対象ゼロ化の防止）に限定する。**`AUDIT_GIT_RANGE` は CI が正当に渡す差分範囲の入力であり無効化トグルではないため、strict の強制対象に含めない**（含めると CI の差分範囲指定を壊すため）。同梱の `ci/pre-push.example` は `AUDIT_STRICT=1` を既定化している。個別に無効化したい場合は、コピー先で `AUDIT_STRICT=0` を明示的に export する。既定（`AUDIT_STRICT` 未設定または `0`）は非 strict であり、既存の env 上書き挙動は従来どおり後方互換を維持する。
...
| #25 メイン直接作業 | メインがサブ委譲せず直接 Write/Edit/Shell 実行［絶対強制］ | audit.sh check_25 ＋ #12/#13 補完 | 実装済み | CI FAIL（間接・変更者同一性は未識別。時系列突合・許容窓 48h 既定・`MAIN_WORK_GATE_TOLERANCE_SECONDS` で上書き可。**検出限界（C-8）**: 対象差分と無関係な別 issue の証跡でも許容窓内なら PASS しうる〈変更ファイル・issue の突合は行わない・低深刻度により見送り〉） | 03 or CORE/AGENTS 再確認し委譲し直す |
...
| #39 find prune 規約 | ルート起点 unbounded find（`find "$PROJECT_ROOT"`）が `.worktree` prune（`-path '*/.worktree' -prune -o …` の構造）を欠く追跡対象シェルを検知（best-effort lint）。`.worktree` 文字列を `-name`/print 対象に含むだけで prune 構造（`-prune`＋`-o`）を欠く迂回形も FAIL。 | audit.sh check_find_worktree_prune | 実装済み | CI FAIL（`.worktree`＋`-prune`＋`-o` の共起で許容・コメント行/scoped find は無検出・該当シェル不在/非 git は SKIP 相当） | 該当 find に prune 節を追加し audit 再実行 |
| #40 非準拠ブランチ名事後検知 | grandfather baseline 未登録の新規ブランチ名が命名規則 `<type>/<YYYYMMDD_HHMMSS>-<name>`（type=feature/bugfix/hotfix/release/chore・固有名 200 バイト上限）に非準拠なら FAIL（Tier1 hook R7 の網羅バックストップ）。 | audit.sh check_worktree_branch_naming | 実装済み | CI FAIL（`WORKTREE_NAMING_AUDIT_ENABLED=false`・非 git・grandfather baseline 不在は SKIP。CI では `GITHUB_HEAD_REF`＝PR の source branch も列挙対象に含む） | 命名是正 or `worktree-naming-grandfather.txt` へ追記して救済 |
| #41 worktree 削除前の記録 commit・push 漏れ検知（R9） | 削除形コマンド（`worktree remove`〈--force 含む〉／`clean -x`\|`-X`／`clean` が `.worktree` 対象）検知時に、対象 worktree の issue 記録（`00`〜`04`・`90_issues.md`）に未 commit 差分（未追跡 `??` を `-uall` で個別展開）または未 push ユニークコミット（記録格納ルートを pathspec で絞り未 commit とスコープ対称・`git fetch` を伴わない）があれば block。既存 R8（物理退避）評価の直後・`allow` 直前に併置し R8 を非破壊（SC-4）。 | PreToolUse.sh R9 ＋ `enforcement/lib/worktree_record_guard.sh`（A は source／B〈verify-and-close〉は `bash` 実行で共有） | 実装済み | runtime reject（ロール非依存・削除形コマンドに発火し exit 2。非 git／R7 命名規則非準拠パス〈ADR-3 グランドファザリング〉／`ASC_WORKTREE_CLOSE_BYPASS` バイパス／判定不能・内部エラーは SKIP＝fail-open） | 記録を `git add && git commit`／`git push` するか、単発 `ASC_WORKTREE_CLOSE_BYPASS=1 <cmd>` で明示バイパス（`export` は避ける・監査痕跡が stderr に出る） |
| #42 enforcement 正本差分の委譲・レビュー証跡必須検知（C-1） | 対象差分（GIT_RANGE）が enforcement 正本（`.agent-skill-chain/source/enforcement/**`・`.agent-skill-chain/project/orchestrator-allowlist.txt`・`.agent-skill-chain/project/settings.enforce.json`・`.scribe-nonce`）に触れるのに、workflow_log に対応する委譲・レビュー command（implement-feature/design-feature/verify-and-close/review-docs/create-pr-review-issue）の記録が 0 件。R1B の `ASC_ENFORCEMENT_SELF_DEV` バイパスで編集した場合の委譲・レビュー証跡追跡を担う。 | audit.sh check_enforcement_diff_has_evidence | 実装済み | CI FAIL（非 git／sqlite3・workflow.db 不在／対象差分に該当パスなしは SKIP） | 該当変更を implement-feature/design-feature/verify-and-close 等として委譲し直し、write-workflow-log で記録する |
...
| 39 | **ルート起点 unbounded find の `.worktree` prune 欠落** | 追跡対象シェル（`enforcement/*.sh`・`scripts/*.sh` 等）のルート起点 `find "$PROJECT_ROOT"` が `.worktree/` を prune（`-path '*/.worktree' -prune -o …`）していない。`.worktree` 文字列を `-name`/print 対象に含むだけで prune 構造（`-prune`＋`-o`）を欠く迂回形も FAIL。コメント行・scoped find（`find "$PROJECT_ROOT/<subdir>"`）は誤検知しない。best-effort lint（該当シェル不在は無検出）。 | 該当 find に prune 節を追加し audit を再実行 |
| 40 | **非準拠ブランチ名の事後検知** | grandfather baseline（`.agent-skill-chain/project/worktree-naming-grandfather.txt`）に無い新規ブランチ名が命名規則 `<type>/<YYYYMMDD_HHMMSS>-<name>`（type=feature/bugfix/hotfix/release/chore・固有名 200 バイト上限）に非準拠。Tier1 hook（R7）の網羅バックストップ。CI では `GITHUB_HEAD_REF`（PR の source branch）も列挙対象に含む。`WORKTREE_NAMING_AUDIT_ENABLED=false`・非 git・baseline 不在は SKIP。 | 命名是正 or baseline へ追記して救済 |
...
| #41 worktree 削除前の記録 commit・push 漏れ検知（R9・Tier1 hook） | 対象 worktree の issue 記録（`00`〜`04`・`90_issues.md`）を `git add && git commit`／`git push` で正式に記録してから削除を再実行する。意図的にバイパスする場合は単発 `ASC_WORKTREE_CLOSE_BYPASS=1 <cmd>` を用いる（`export` は避ける。バイパス通過は stderr に監査痕跡が出る）。終了時契約（B・verify-and-close）は同一 lib を `bash <lib> <target>` で共有する。 | 記録を commit・push 後に削除を再実行して block 解消を確認。非 git／R7 命名規則非準拠パス／バイパス／判定不能は SKIP（fail-open）。 |
| #42 enforcement 正本差分の委譲・レビュー証跡必須検知（C-1・`check_enforcement_diff_has_evidence`） | enforcement 正本の変更を implement-feature/design-feature/verify-and-close/review-docs/create-pr-review-issue のいずれかとして委譲し、write-workflow-log で証跡を記録する（本リポのドッグフーディング開発で R1B を `ASC_ENFORCEMENT_SELF_DEV` で解除して編集した場合も同様）。 | 記録後 audit を再実行して FAIL 解消を確認。非 git／sqlite3・workflow.db 不在／対象差分に該当パスなしは SKIP。 |
