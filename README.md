...
`setup ruleset`（`setup github` からの実行を含む）は既定の配布テンプレートに対しては環境変数 `ASC_GATE_APP_ID` を要求しない（`required_status_checks` は `verify` のみで、gate Checkを含まないため）。ローカルゲートレビュー後の `repository_dispatch` は配布済み trusted gate recorder workflow が受信し、専用GitHub Appで Check Runを発行する。この経路を使う場合、または手元のテンプレート複製へrequired gate Check（`agent-skill-chain/{spec,design,implementation,validation}-gate`）を加える場合のApp作成・installation・Actions設定手順は [docs/ASC_GATE_APP_ID_RUNBOOK.md](docs/ASC_GATE_APP_ID_RUNBOOK.md) を参照。
