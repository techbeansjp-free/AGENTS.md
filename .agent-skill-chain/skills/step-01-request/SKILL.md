...
入力はステップ0のステージングと利用者の意図。成果物は`00_要求定義.md`。`full`は目的、範囲、ドメイン影響、制約、成功・受入判断の観点、P-01〜P-07計画を記録し、FR/NFR/ACはステップ2へ分離する。`quick`は要件工程を物理的に集約するため、境界づけられたコンテキスト・不変条件、完全な受け入れ条件、最小Gherkin、確認方法、全Q根拠、再開地点も記録する。ステップ4前に構造を検証する。
