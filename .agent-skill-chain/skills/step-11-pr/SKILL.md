...
`pr-bound`後にreview artifactの命名だけを是正してHEADが動いた場合は、`pr reanchor`で同一SHAのreview済み`H_impl`、review判断本文の等価性、保存済み収束sessionと最新Step 10 binding、context-isolated approval、正規命名・構造・個別監査表の合格を再検証してから再固定する。Step chain・candidate・個別監査・配布物影響の機械導出領域だけを正規化し、finding、最終判定、独立性、test証拠は完全一致させる。一般のrebase、review判断本文の変更、`H_impl`差替え、artifact外差分を`pr-bound`で許可せず、既存のPR binding、delivery state、Step journalを書き換えない。
