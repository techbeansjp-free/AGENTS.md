...
`pr-bound`後にreview artifactの命名だけを是正してHEADが動いた場合は、`pr reanchor`で同じreview済み`H_impl`に対する正規命名・構造・個別監査表の合格を再検証してから再固定する。一般のrebaseや実装差分変更を`pr-bound`で許可せず、既存のPR binding、delivery state、Step journalを書き換えない。
