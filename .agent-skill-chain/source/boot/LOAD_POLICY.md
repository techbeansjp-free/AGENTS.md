...
本トリガー表が「オンデマンド読了（いつ何を読むか）」の正本である。起動時に必須で読了するコアは CORE / LOAD_POLICY / PHASES のみ（正本は `CORE.md §禁止事項`）で、本表の各行はトリガー発生時に読む対象を定める。**META_LAYER.md §メタ層の監視指標「参照文書 ≤ 8」の計測対象は、この起動時一括必読コア（CORE/LOAD_POLICY/PHASES の 3 ファイル）のみを指す。** 本表の各行に従いトリガー発生時にオンデマンドで読むファイルは、当該指標の加算対象に含めない（読了義務とコンテキスト経済の自己矛盾を避けるための定義。詳細は META_LAYER.md §メタ層の監視指標を参照）。
...
| **command を実行するとき** | プロジェクトルートの **.agent-skill-chain/project/** を読む（存在する場合）。**skills/agent/run_command.md** → **commands/{command}.md**（例: requirement-discovery, implement-feature） |
| 要求発見 command | **commands/requirement-discovery.md** に加え、**workflow/TEMPLATES.md** および **成果物 00/01 に対応するテンプレートファイル**（.agent-skill-chain/runtime/templates/00_要求定義.md, .agent-skill-chain/runtime/templates/01_要件定義.md。プロジェクトに .agent-skill-chain/runtime/templates が無い場合はパッケージの **`.agent-skill-chain/runtime/templates/`** の同ファイル）を**読む**。続けて記載された skill chain を順に読む。 |
