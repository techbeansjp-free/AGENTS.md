...
| **quick** | 小修正・軽微変更・短い回答 | 必要最小限 | **設計メモと変更理由のみ**。00/01/02/03/04 のうち「変更理由が追える形」の最小セット（設計メモ 1 件以上と変更理由の記載）。証跡は最小限。**実装着手前ゲートのうち #32（review-docs）・#34（GitHub Issue 起票）を免除する**（軽量化：review-docs 反復・GitHub Issue 起票は行わず、設計メモ＋変更理由と workflow.db 証跡で代替記録する。記録の省略ではない）。**#35（branch 紐づけ）は規模・モードに関係なく維持する**（quick でも必須）。full/standard は #32/#34/#35 を全て適用する。 |
