...
**有効（opt-in 済み）時の挙動**: ユーザーの作業依頼文に自律実行キーワードが含まれる場合、その依頼自体を commit・push・PR 作成に対するユーザー明示の事前承認とみなす。進行役は実装・レビュー完了後、commit・push・PR 作成の可否を都度追加確認せず、一連の完了（PR 作成まで）を自律的に遂行してよい。
