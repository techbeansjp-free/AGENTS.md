...
#
# 使い方:
#   bash .agent-skill-chain/source/scripts/verify-npm-pack.sh          # リポジトリルートで実行
#
# 前提: npm（>=7、`npm pack --dry-run --json` が使えること）。
#   npm が無い環境では検査をスキップせず明示的に失敗する（CI は npm 前提）。
#
# 配布対象の正本は package.json の files フィールド（このスクリプトの判定はそれと一致させる）。
#   files には統合ネストの正本サブツリー（.agent-skill-chain/source/・
#   .agent-skill-chain/runtime/templates/）とルート契約（AGENTS.md・CLAUDE.md・README.md・
#   package.json・bin/）のみを含める。ユーザー資産（project/・runtime/ の issue 履歴）・保守者
#   向け記録（docs/maintainer/）・自己テスト（test/）・生成物（.adapters/・workflow.db）は配布しない。
...
// 禁止パターン（リポ固有物・ユーザー資産・生成物。配布物に 1 件も含まれてはならない）。
// - .agent-skill-chain/project/            : プロジェクト固有オーバーライド（ユーザー資産・不可侵）
// - docs/maintainer/                       : 保守者向け開発記録
// - workflow.db（*-shm/-wal 含む）          : 証跡 DB（消費者ランタイム生成物）
// - .adapters/                             : 各ツール向け生成物（100% 生成物）
// - .agent-skill-chain/runtime/ 配下の issue: templates 以外の消費者ランタイム生成物（issue 履歴等）
// - test/                                  : 保守者自己テスト（非配布）。package.json の files allowlist
//   外だが、誤って files に加わった場合の保険として多重防御で明示検知する。
...
const forbidden = files.filter((p) => {
  ...
  if (/(^|\/)\.agent-skill-chain\/project(\/|$)/.test(p)) return true;
  if (/(^|\/)docs\/maintainer(\/|$)/.test(p)) return true;
  if (/(^|\/)workflow\.db($|[-.])/.test(p)) return true;
  if (/(^|\/)\.adapters(\/|$)/.test(p)) return true;
  if (/^\.agent-skill-chain\/runtime\//.test(p) && !/^\.agent-skill-chain\/runtime\/templates\//.test(p)) return true;
...
const required = [
  { label: ".agent-skill-chain/source/", test: (p) => p === ".agent-skill-chain/source" || p.startsWith(".agent-skill-chain/source/") },
...
if (forbidden.length > 0) {
  ...
  console.log("[OK] 禁止パターン（.agent-skill-chain/project / docs/maintainer / workflow.db / .adapters / .agent-skill-chain/runtime issue / test/）は含まれていません。");
}
...
if (missing.length > 0) {
  ...
  console.log("[OK] 必須の正本配布物（.agent-skill-chain/source/ / AGENTS.md / CLAUDE.md / bin / README / package.json / .agent-skill-chain/runtime/templates/）はすべて含まれています。");
}
