...
#   .agent-skill-chain/source/platforms/claude/plugin.json の version と一致させる/検証する。
#
# モード:
...
# 不変条件の要旨（自己完結）: version は package.json を単一正本とし、build-adapters.sh が
#   plugin.json をそのままコピーして配布する。ゆえに plugin.json の version を package.json と
#   一致させておくことで「同一入力→同一出力」の決定性（差分ゼロ検証）を保つ。build 側に version
#   注入ロジックを持ち込まない。
set -euo pipefail
