...
**目的**: `.agent-skill-chain/source/skills/{domain}/{capability}/` をプラットフォームの `skills/` にコピーするとき、**異なる domain で同名 capability** が存在すると配備先で上書きされる問題の対策。**案B（プレフィックス `{domain}__{capability}`）を採用済み。** setup.sh の sync_skills で配備先を `skills/{domain}__{capability}/` にしている。
...
- **正本**: `.agent-skill-chain/source/skills/{domain}/{capability}/`（例: `skills/requirements/write-bdd/`, `skills/testing/write-bdd/`）
...
| 種類 | 参照先 | 例 |
|------|--------|-----|
| **正本参照**（command・run_command・LOAD_POLICY） | `.agent-skill-chain/source/skills/{domain}/{capability}/` | `skills/requirements/write-bdd/` |
...
- **方式**: プラットフォームの `skills/` にはコピーせず、実行時に常に `.agent-skill-chain/source/skills/{domain}/{capability}/` を読む。
- **長所**: 衝突が原理的に発生しない。
