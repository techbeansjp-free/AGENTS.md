...
4. 単体テスト観点・BDD シナリオとの対応を確認する。**テストコードに `ユースケース:`・`シナリオ:`（doc 等）および Given / When / Then（必要時 And）のインラインコメントが付いているか**（.agent-skill-chain/source/TEST_BDD_FORMAT.md）。欠落は指摘する。
