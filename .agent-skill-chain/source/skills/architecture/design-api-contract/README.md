# capability: design-api-contract
...
- 実装の詳細（変数名・言語固有）は 02 に長く書かない。03_実装計画に委譲する。
- テスト観点は review-dependencies と連携する（ここでは「何をテストするか」の種だけ書く）。

---

## 成果物の形式

- **OUT**: API 一覧。各 API の入出力・契約の短文。02_設計に追記する形。
- review-dependencies では依存関係とテスト観点を確認する。
