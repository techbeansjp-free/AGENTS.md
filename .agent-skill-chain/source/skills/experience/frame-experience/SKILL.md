...
UI/UX を伴う機能について、ビジネス目的・成功基準（KPI）とユーザー・課題（誰の何の課題か・JTBD・仮説）を一人称ナラティブ＋却下案で枠づけ、02_設計.md §7.1 に記録する（体験設計フェーズ1。幻覚ペルソナ注意は 3 フェーズ共通の §7.4 に記録する）。
...
6. 幻覚ペルソナ注意（LLM が捏造したユーザー像は実ユーザー調査の代替にならない旨）を §7.4（3 フェーズ共通・未記載であれば新設、既存記載があれば流用）に記録する。
...
- **OUT**: 02_設計.md §7.0（体験サーフェス判定結果）・§7.1（前提ナラティブ・ペルソナ・JTBD・却下案）・§7.4（幻覚ペルソナ注意・3 フェーズ共通）。
- 「あり」判定時の §7.1 出力は次フェーズ map-experience の Inputs となる。

## Done

- 体験面=あり の場合: §7.1 に前提ナラティブ・ペルソナ・JTBD・却下案 1 件以上が揃っており、§7.4 に幻覚ペルソナ注意が記録されている。
