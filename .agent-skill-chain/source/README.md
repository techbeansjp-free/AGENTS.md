...
**優先順位**: プロジェクト固有のルールは **.agent-skill-chain/project/** に置く。**.agent-skill-chain/project が .agents より最優先**される。同名・同目的のルールは .agent-skill-chain/project を採用し、無い場合は本 .agents に従う（CORE §ルールの優先順位）。

**テンプレート正本を開発しているリポジトリでは**: 汎用版に載せる新規規約・正本ドキュメントは **本 `.agent-skill-chain/source/`（および配下）に追加する**。`.agent-skill-chain/project/` は「採用先がコピー後に上書きする層」の説明用であり、**配布テンプレートの仕様を `.agent-skill-chain/project/` だけに置いて完了としない**（ルート `AGENTS.md` / `CLAUDE.md` と同旨）。
...
| **プロジェクト固有・最優先** | **プロジェクトルートの .agent-skill-chain/project/**（本 .agents より優先。詳細は CORE §ルールの優先順位） |
