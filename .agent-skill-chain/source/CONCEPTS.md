...
**正本はここ 1 か所のみ。** テンプレート・他ドキュメントでは「用語は .agent-skill-chain/source/CONCEPTS.md §用語規約 を参照」と 1 行で参照すること。
...
- **使い分け**: **issue とタスクは必要に応じて置き換え可能**。プロジェクトの規模や管理方法に応じて、適切な粒度で使い分ける。小さな単位では「タスク」、大きな単位では「issue」として扱うことが多い。**ただし、enforcement ゲート判定単位としての「issue」は、`.agent-skill-chain/runtime/{issue}/`（00〜04 等の成果物を持つ issue フォルダ単位）を厳密に指す。** 用語の互換許容は説明上の言い換えに限り、「これはタスクだから issue ゲート対象外」という解釈でゲート適用（起票・review-docs・close 移動等）を回避することはできない。
...
- **スキル形式**: 正本は .agent-skill-chain/source/skills/{domain}/{capability}/ に 1 か所。各 capability に SKILL.md を置き、setup で各ツールの skills ディレクトリにコピーする。詳細は platforms/SKILLS.md。
