...
- **必須キー不足時は失敗**: 書記が記録するエントリは、必須カラムの正本である [ledger/schema.md §command ごとの必須カラム規約](../ledger/schema.md)（command 別の必須表）と [ledger/schema.sql](../ledger/schema.sql)（CHECK 制約）を満たす。独自の必須キーリストは本ファイルに持たず正本を参照する（正本一元化）。必須キーが欠けている場合は記録を失敗とみなし、親にエラーを返す。完了とみなさない。
- **どこに書くか**: **本則は SQLite（workflow.db）**。ledger/README.md の配置・スキーマに従う。**証跡は workflow.db のみ。memo は証跡の代替経路ではなく、移行期・例外時の思考メモ用である。** **workflow.db に書く場合は .agent-skill-chain/source/scripts/write-workflow-log.sh を必ず使用すること。** **workflow.db が無ければ作成する**（初回実行時に ledger/schema.sql を流して作成）。**workflow.db を採用することが本則**であり、**memo のみの運用は workflow.db を採用しない場合の過渡的・例外**とする（CONTRACT.md「どこに保存するか」を参照）。**memo のみの運用は非推奨（移行モード）であり、将来廃止予定とする。** 新規プロジェクトでは memo のみ運用の採用を禁止し、将来のメジャーバージョンで memo 出力経路を削除する予定である。
