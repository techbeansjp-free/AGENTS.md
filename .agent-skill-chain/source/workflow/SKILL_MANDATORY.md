...
| **設計** | design-feature | define-boundaries, design-api-contract, review-dependencies ＋**条件付き**: frame-experience, map-experience, detail-experience は `experience_surface`=あり のときのみ必須（規模比例で統合可） | （通常は 3 つとも実行） | 02 に責務・参照関係・テスト観点が記載されていること。spec 参照の証跡（どの spec に基づいたか）を 02 に 1 行残す。**体験面=なし／統合時に残すもの**＝02 §7.0 の判定記録／統合記録。 |
