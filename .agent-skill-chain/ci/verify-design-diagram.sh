#!/usr/bin/env bash
# 正本: AGENTS.md §ADR・テンプレート・テスト適用性 / .agent-skill-chain/templates/issue/DESIGN.md
#
# DESIGN.md に「### 図示要否の判断」セクションが存在し、判断（要|不要）と根拠が
# プレースホルダのまま残らず記載されているかを検査する。判断が「要」の場合は
# mermaid コードフェンスの存在も検査する（判断自体の妥当性は人間判断に委ねる）。
#
# 本スクリプトは agent-skill-chain CLI（src/agents-md.ts、ビルド後 bin/agents-md.js）の
# `verify design-diagram` サブコマンドへの薄いラッパーである（使い方は `verify design-diagram -h` 参照）。
...
exec "${ASC_CLI[@]}" verify design-diagram "$@"
