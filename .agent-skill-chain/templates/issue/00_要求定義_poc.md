...
> `poc`は仮説を最速で検証する。隔離した疑似project、事前定義したuse case・BDD scenario・機械observable、成功・中止条件、責任者、昇格または廃止の判断条件を記録し、PRを終端にする。high riskが1つでもあれば`full`へ昇格する。判断のある項目だけ具体的に書く。用語と責務の境界は`成果物用語と責務境界`（`.agent-skill-chain/docs/01_開発ワークフロー.md`）を正本とする。

| 読者 | 読む節 |
|---|---|
| 発注・評価する人 | §1 目的 → §2 対象範囲 → §4 PoC宣言 → §6 受け入れ条件 → §11 昇格・廃止判断 |
| 実装・レビューする人 | 全節 |
| 運用する人 | §2 権限 → §5 high risk確認 |
