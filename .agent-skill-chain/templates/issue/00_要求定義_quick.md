...
- 対象内: （所要時間ではなく、独立に完成・review・merge・rollbackできるかという結合度で成果物1単位を決める。独立した切り戻し、異なるreview担当・観点、先行merge、失敗影響の分離が必要なら分割候補とする。45分は再確認の補助指標であり、長いだけでは分割しない。quickは集約00でfullより固定費が小さいが、Step 0〜11、PR review、既定branch追随と有限review予算は分割数だけ必要になる。一体として扱うべきなら分割しない。複数成果物markerの非停止warningはfullだけに適用し、quick/poc集約形式では適用しない）
- 対象外:
- 維持する既存動作:
- 許可済みの書き込み:
- 別承認が必要な操作:
