...
- ノードIDはASCIIの`[A-Za-z][A-Za-z0-9_]*`で安定させる。表示labelはIDへ埋め込まず、project policyが選択した表示言語のplaceholderを明示して注入する。
...
```mermaid
flowchart LR
  input_node["{project-selected label: input}"] --> domain_node["{project-selected label: domain}"]
  domain_node --> output_node["{project-selected label: output}"]
```
