...
## ロールごとの権限境界（正本: `.agent-skill-chain/config/roles.yaml`）

各ロールの具体的な `capabilities` / `forbidden` の一覧は `.agent-skill-chain/config/roles.yaml` を正本とする。本ファイルでは境界の要旨のみを示す。
