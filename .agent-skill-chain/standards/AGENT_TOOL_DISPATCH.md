...
3. 進行役自身が同コマンドをBashツールで直接実行する（Agent tool経由のサブエージェント起動は行わない）。role_contractは`contract.md`（`DISPATCH_TEMP_DIR=`配下）からCodex CLIへ渡す形にすでに解決済みであり、成果物のcommit・push後にcompleted reportを投稿してから最終応答する指示を含む。
...
- 完了条件は、今回のworker起動またはdispatch開始時刻以降に作成されたworker reportが`completed`で、target SHAが対象worktreeのHEADと一致し、verifyが`0`を返すことである。adapter（claude/codex）に関わらず同一の完了条件を用いる。
