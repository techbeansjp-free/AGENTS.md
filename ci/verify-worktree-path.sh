#!/usr/bin/env bash
# 正本: AGENTS.md §ブランチ・worktree / standards/GIT_CONVENTIONS.md §worktreeパス規則・§worktreeの正本
#       / config/agent-skill-chain.yaml の worktree.path_pattern・worktree.timestamp
#
# worktree パスが config/agent-skill-chain.yaml の worktree.path_pattern
...
config/agent-skill-chain.yaml の worktree.path_pattern に適合するかを検査する。
