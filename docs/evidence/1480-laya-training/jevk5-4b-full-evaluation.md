...
JevK5はseverity判定では高精度だが、finding validityのfalse-negative biasとrequired actionの順序依存が大きい。現行dataset上では、Jevのローカル代替として不採用とする。採用を再検討する場合は、判断種別をseverityだけに限定した別契約、または選択肢順序を固定したうえでのabstention境界を先に設計する。
