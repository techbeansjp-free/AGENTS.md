...
### ADR-137-5: audit #33 の「猶予超過」重大度を実効モードで分岐し、github_native では WARN（非 FAIL）へ格下げする（ADR-137-4 を補完・上書きせず）（2026-07-18・issue: GitHub Issue #137・PR #138 CodeRabbit 指摘是正）

- **コンテキスト**: ADR-137-4 で #33（`check_close_move_pending`）を両モードで走らせるようにしたが、猶予超過（既定 `CLOSE_MOVE_GRACE_DAYS`=3 日）時は両モードとも一律 FAIL していた。`check_close_move_pending` は workflow.db の verify-and-close 証跡の時刻のみを見て経過日数を判定する。local_tracked の確定点は PR マージでありエージェント側で完結できるが、github_native の確定点は**ユーザーによる GitHub Issue の close**である。github_native では verify-and-close 完了後もユーザーが正当な理由で GitHub Issue をまだ close していない（close 待ちの）状態が正常にありうるため、3 日超過を一律 FAIL すると監査が正常状態を誤検知する（PR #138 に対する CodeRabbit の Major 指摘）。ADR-137-4 は監査の決定論的性質を守るため `gh` CLI 依存（ネットワーク・認証依存）の CLOSED 判定を明示的に却下している。
- **検討した選択肢**: (a) CLOSED 相当のローカル証跡を判定条件へ追加する（例: `gh issue view` で CLOSED 状態を確認）。(b) github_native では「猶予超過」を FAIL ではなく WARN（非 FAIL・EXIT_CODE 不変）に留める。(c) 現状維持（両モード一律 FAIL）。
- **決定**: (b)。`check_close_move_pending` の猶予超過分岐で、実効モード（`resolve_issue_tracking_mode`）が github_native の場合は `WARN`（EXIT_CODE を 1 にしない・放置防止の非ブロッキング督促）を出力し、local_tracked（既定・非 GitHub 含む）は従来どおり `FAIL`（EXIT_CODE=1）とする。検知走査・grandfather・猶予日数・`*/close/*` 除外・証跡取得ロジックは一切変更しない（ADR-137-4 の走査範囲は不変で、変更するのは猶予超過時の重大度のみ）。
- **根拠 [evidence_source: existing_code + human_decision]**: existing_code＝ADR-137-4 が `gh` 依存を却下している既存判断と整合させるため (a) のうち `gh issue view` 形は採らない（監査の決定論性・可搬性・オフライン動作を壊すため。ADR-137-4 の根拠を継承）。加えて `resolve_issue_tracking_mode` は #28/#33 と共用の既存シグナルであり、これを再利用して新規判定を作らない（単一責務）。human_decision＝github_native の確定点は「ユーザーの GitHub Issue close」であり verify-and-close 完了と時間的に乖離しうる（ADR-137-2 の実例＝#115 は PR 群マージ後にユーザーが手動 close）ため、close 待ちを FAIL 扱いすると正常状態を誤検知する。(c) は正常状態の誤検知を放置し、監査の信頼性（FAIL=真の逸脱）を損なう。
- **帰結**: local_tracked の #33 挙動は完全不変（回帰安全）。github_native では猶予超過が WARN の非ブロッキング督促になり、ユーザーが GitHub Issue を close して T5 の close 移動を確定させれば `*/close/*` 除外で自然収束する。本 ADR は ADR-137-4 を**補完**する（走査を両モードで有効化する ADR-137-4 の決定は維持し、その上で猶予超過時の重大度のみを github_native で緩和する）。ADR-137-4 の帰結中「github_native CI で FAIL 検知されうる」の記述は、本 ADR により github_native では WARN 検知へ改まる（走査・検知自体は不変・重大度のみ緩和）。

### ADR-491-1: `docs/README.md` と番号付きディレクトリ群（`docs/00_review/`〜`docs/99_ID命名規則と管理/`）を削除する（2026-08-08・issue: GitHub Issue #491）

- **コンテキスト**: `docs/README.md` と番号付きディレクトリ群（`docs/00_review/`〜`docs/99_ID命名規則と管理/`、計 27 ファイル）が正本として参照していた `.agent-skill-chain/source/` 配下は、2026-07-22 の全面再設計で既に削除済みであり、これらのファイルが持つリンクは網羅的に破損していた。放置すると保守者・AI エージェントが破損リンクへ迷い込む原因になる。これらは `docs/maintainer/workflow/close/20260713_035244_本リポジトリのシステム仕様書整備/`（2026-07-13 付、close 済み）が計画・作成したものであり、当該 close 記録の `02_設計.md` には「ADR-1: `02_画面設計` を該当なしとし削除しない」という、当該構造を明示的に保持する判断も含まれていた（このADR自体は close/ 配下の非追跡的アーカイブであり、本ADRで変更しない）。
- **検討した選択肢**: (a) 削除する。(b) `docs/adr/ADR-0001-docs-system-spec-construction.md`（`docs/system-spec/` 新設方針、`status: proposed`）の accepted 後に前身として統合する。(c) `.agent-skill-chain/source/` 参照を現行パス（`.agent-skill-chain/{adapters,ci,config,hooks,project,schemas,scripts,standards,templates}` 構成）へ修正した上で維持する。
- **決定**: (a)。削除する。ルート README.md から `docs/GLOSSARY.md`・`docs/AI_CI_CD_VISION.md` への明示リンクを追加することで、これらの文書への到達性は維持する。
- **根拠**: 正本参照が構造的・網羅的に破損しており (c) は部分修正では済まない規模であった [evidence_source: observed_runtime]。ADR-0001 はまだ `proposed` のままで (b) の統合先自体が未確定であった [evidence_source: existing_code]。ルート README.md・AGENTS.md・`.github/`・`.agent-skill-chain/{scripts,ci,config}` のいずれからもこれらのディレクトリへの参照がゼロであり、削除の実害は「迷い込むリンク」の解消に留まった [evidence_source: observed_runtime]。
- **帰結**: 2026-07-13 の計画記録が下した「`docs/00_review/` 等を新設し保持する」という決定を本 ADR が上書きする。特に `02_設計.md` の「ADR-1: `02_画面設計` を該当なしとし削除しない」を含む、当該構造の維持を前提とした過去の判断は本 ADR により無効化される。`docs/system-spec/` の実体構築（ADR-0001 accepted 後）に必要な場合は、close 済みの計画記録（`docs/maintainer/workflow/close/20260713_035244_本リポジトリのシステム仕様書整備/`）を参考情報として参照可能である（ただし正本としては扱わない）。
