...
- メイン文脈蓄積問題のスコープ線引き（本 issue はサブの fresh 分割義務を扱い、メインの feature 単位 /clear 粒度の再設計は除外要件である旨）を 1 節として追記する（対応: UC4-S1）。
...
- 境界値: メイン文脈蓄積のスコープ線引き節に「除外」「/clear 粒度」の語が含まれ、除外要件が判定可能。
