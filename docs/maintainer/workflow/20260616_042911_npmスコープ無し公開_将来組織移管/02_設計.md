...
- **主防御＝runtime hook（PreToolUse）での env 出所制御（C-4b の主たる対策・M-1 反映）**: AGENT_ROLE 偽装に対する**主防御は runtime hook 側に置く**。現状の `PreToolUse.sh` は `ROLE="${AGENT_ROLE:-${CLAUDE_AGENT_ROLE:-unknown}}"` で**呼び出し元 env の AGENT_ROLE をそのまま信頼**しており（実体確認済）、ユーザー/サブが任意に `export AGENT_ROLE=scribe` した値を区別できない。これを次の方式で**出所を制御する**:
  1. **信頼できる経路でのみ scribe を許可**: scribe 権限の付与は、setup/enforce が配線した `settings.json` の env（`settings.enforce.json` 由来）で渡る経路**のみ**を正規とする。具体的には、setup/enforce が配線時に**そのセッション固有の検証用シークレット（nonce）を hook の env と settings 双方に注入**し、PreToolUse 側で「`AGENT_ROLE=scribe` を主張する呼び出しは、同時に正規 nonce（環境変数）が一致する場合に限り scribe として扱う」判定を入れる（nonce 不一致の scribe 主張は `unknown` 扱いに降格＝write-workflow-log.sh 実行を block）。これにより、ユーザー/サブがシェルで `export AGENT_ROLE=scribe` しても nonce を知らないため scribe になれない。
  2. **過剰実装を避ける軽量代替**: nonce 配線が実装コストに見合わない場合の最小策として、少なくとも「`AGENT_ROLE` を呼び出し元のフリーな export ではなく、hook が settings 経由で受けた値のみで判定する（サブシェルの env 継承を断つ・hook 起動時に固定）」設計を必須とし、文書（enforcement/README）に「AGENT_ROLE をシェルで手動 export して scribe を騙ることは契約違反」と明記する。
  - **回帰テスト（SC-11 の主基準）**: 「正規経路（settings 配線 env / 正規 nonce）でのみ scribe として write-workflow-log.sh が allow され、**手動 export した AGENT_ROLE=scribe は block される**」ことを hook の回帰テストで検証する（**実在しない CI 検知に依存しない**）。
...
| C-4b env 偽装（SC-11） | ○ | ○ | 主防御=hook の env 出所制御（手動 export scribe を block） | 正規 nonce/settings 配線でのみ allow | CI は完全検知不可の既知残存リスク→export+append-only で補強（M-1） |
