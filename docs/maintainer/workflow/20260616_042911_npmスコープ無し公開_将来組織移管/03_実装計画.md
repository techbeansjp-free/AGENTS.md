...
- C-4b（M-1 反映・主防御は hook 側 env 出所制御）: `PreToolUse.sh` の `ROLE="${AGENT_ROLE:-...}"`（現状は呼び出し元 env を素通し）を、**正規経路（settings.enforce 配線 env／セッション固有 nonce 一致）でのみ scribe を許可**する判定へ強化する。手動 `export AGENT_ROLE=scribe` は nonce 不一致で `unknown` 降格＝write-workflow-log.sh 実行を block（02 §3.6.2）。enforcement/README に「AGENT_ROLE の手動 export 偽装は契約違反」を明記。
...
- **C-4b 主防御の回帰（M-1）**: 手動 `export AGENT_ROLE=scribe`（正規 nonce/settings 配線なし）での write-workflow-log.sh 実行が **block(exit 2)** される。正規経路（settings.enforce 配線 env / 正規 nonce）でのみ scribe として **allow(exit 0)**。※**実在しない CI 検知に依存しない**こと。
##### 結合/E2E
...
Feature: write-workflow-log バイパス耐性
  ...
  Scenario: 手動 export した AGENT_ROLE=scribe は hook が block する（主防御）
    Given 正規 nonce/settings 配線なしで AGENT_ROLE=scribe を手動 export した
    When write-workflow-log.sh の単独実行を PreToolUse hook 経由で試す
    Then exit 2 で block される（unknown 降格）
    And 正規経路（settings.enforce 配線 env / 正規 nonce）でのみ exit 0 で allow される
