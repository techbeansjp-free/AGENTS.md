...
- **検討した選択肢**:
  1. 1 行 1 ツール名の素テキスト `.agent-skill-chain/project/orchestrator-allowlist.txt`（`#` コメント可）。hook は**データとして** read し、厳密文字種（`^[A-Za-z][A-Za-z0-9_]*$`）に一致する行のみ採用する。
...
- **Done（DoD）**: 既存の「既知の指摘が 0 件」に加えて、「敵対的観点リスト」「must-preserve リスト」の**両方**が成果物に記載されていること（いずれか欠落は未完了、`REVIEW_DUAL_LENS.md` §3）を DoD に追加する。
...
| project 拡張ファイル | パス `.agent-skill-chain/project/orchestrator-allowlist.txt`。1 行 1 ツール名（`^[A-Za-z][A-Za-z0-9_]*$`）、`#` コメント可、空行可。不在時は無効（従来どおり）。 |
