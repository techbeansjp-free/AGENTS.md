...
`.agent-skill-chain` フレームワークの PreToolUse フック（`.agent-skill-chain/source/enforcement/claude/PreToolUse.sh`）は、orchestrator ロールに対して「読む・検索・委譲」に限定した許可ツールリスト（allowlist）方式でツール使用を制御している。本要件は、次の 7 点を対象とする。
...
- review-docs のレビュー＋修正ループの成果物に「敵対的観点リスト」と「must-preserve リスト」の**両方**を記載することが DoD に含まれ、いずれか欠落は未完了とする旨（`REVIEW_DUAL_LENS.md` §3 と整合）が記載されている。二観点の両リストの**記載先**は memo または 00-03 ドキュメント自体のいずれでもよい。
- 上記の記載先の選択とは独立に、**①レビューサイクルごとの `runtime/{issue}/memo/` への日時プレフィックス付きレビュー memo 作成 → ②指摘ゼロまでの反復 → ③完了後の `write-workflow-log` 委譲**の順序が DoD に必須手順として明記されており、いずれの段も省略・選択制にできないこと（既存 `review-docs.md` の DoD・Forbidden と整合）が確認できる。
