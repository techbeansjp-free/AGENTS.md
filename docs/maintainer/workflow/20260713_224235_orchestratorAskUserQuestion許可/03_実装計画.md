...
- `.agent-skill-chain/source/commands/review-docs.md`:
  - **Process（§レビュー＋修正ループ）**: 手順 1 のレビュー観点に「`RULES.md`・`PHASES.md` の監査観点」に加え「[REVIEW_DUAL_LENS.md](../REVIEW_DUAL_LENS.md) の二観点（敵対的観点＝反証・破壊を試み不確実なら要修正に倒す／肯定的観点＝must-preserve=不変条件の同定）」を明記。各ラウンドで must-preserve リストを次ラウンドへ継承する（`REVIEW_DUAL_LENS.md` §6）旨を追記。
