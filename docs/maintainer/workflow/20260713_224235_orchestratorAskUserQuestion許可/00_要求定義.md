...
- **FR-5（review-docs への二観点レビュー強制化）**: `.agent-skill-chain/source/commands/review-docs.md`（実装前の 00-03 ドキュメントレビュー）を改修し、`skills/review/review-code`・`skills/review/review-architecture` と同様に `REVIEW_DUAL_LENS.md` への参照を明記し、レビュー＋修正ループの中で「敵対的観点リスト」（反証・破壊を試みた観点と結論）と「肯定的（must-preserve=不変条件）リスト」の**両方**を記載することを DoD に追加する。
  - 現状の「既知の指摘が 0 件」という DoD だけでは、敵対的に反証を試みたか・壊してはならない不変条件を同定したかが担保されないため、二観点の両リスト記載を追加要件とする。
  - **必須 chain（省略不可・順序固定）**: 二観点の両リストの**記載先**は review-docs の memo または 00-03 ドキュメント自体のいずれでもよいが、それとは独立に、①レビューサイクルごとに `runtime/{issue}/memo/` へ日時プレフィックス付きレビュー memo を作成し、②指摘がゼロになるまでレビュー＋修正を反復し、③完了後に `write-workflow-log` へ委譲して証跡を残す、という 3 段は**この順序でいずれも必須**であり省略できない（既存 `review-docs.md` の DoD・Forbidden と整合。memo 作成・反復・ログ委譲を選択制にはしない）。
