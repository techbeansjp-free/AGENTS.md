...
> **前提（この競合が生じる条件）**: この状況は**真の並走**（別プロセスが確認と ALTER の間に同一カラムを追加）でのみ発生する。本スクリプトは同一 DB に flock 排他で移行ブロックを直列化するため、flock 有効な同一ホスト内では競合窓が開かず、この競合は **flock 不在環境・別ホスト（NFS 等）** で顕在化する（設計 ADR-1・02 §6.2 参照）。実行時再現テストは「並列＋flock 無効化」で行う（03 T-3）。

```gherkin
Feature: スキーマ移行 ADD COLUMN の冪等性
  Scenario: 対象カラムが確認直後に別プロセスにより追加され ADD COLUMN が競合する
    Given 旧スキーマの workflow.db に対し、あるプロセスが review_id 不足を確認済みである
    And 確認から ALTER までの間に別プロセスが review_id を ALTER TABLE ADD COLUMN で先に追加した
    When 当該プロセスが ALTER TABLE ADD COLUMN review_id を実行し duplicate column name で失敗する
    Then スクリプトは PRAGMA table_info を再確認して review_id の存在を検知し、exit 1 せず後続の INSERT 処理まで到達する
...
**シナリオ 2: 並列実行（flock 無効化）での競合再現確認**

```gherkin
Feature: スキーマ移行 ADD COLUMN の冪等性
  Scenario: セットアップINSERTを並列実行し競合窓を開いて再現する
    Given 新規作成された旧スキーマの workflow.db が隔離環境に用意されている
    And flock が無効化され移行ブロックが直列化されない（競合窓が開く）
    When 同一のセットアップ INSERT（write-workflow-log.sh 呼び出し）を N 並列 × M ラウンドで実行する
    Then すべての呼び出しが exit 0 となり、duplicate column name によるエラーが発生しない
...
Feature: スキーマ移行の真のエラー検知（fail-fast）
  Scenario: 書き込み不可な DB でスクリプトが fail-fast する
    Given 対象カラムが存在しない旧スキーマの workflow.db が、書き込み不可な状態（疑似異常系）になっている
    When write-workflow-log.sh を実行する
    Then スクリプトは exit≠0 で停止し、INSERT 処理は実行されない（証跡が記録されない）
    # ※ 実測では移行手前の PRAGMA journal_mode=WAL が readonly で先に停止する（設計 02 §6.2）。
    #    ensure_column の recheck でも不在時の "マイグレーションに失敗しました。" 分岐は
    #    コードレビューで担保する（duplicate column name 以外を握り込まない・受け入れ基準参照）。
```
