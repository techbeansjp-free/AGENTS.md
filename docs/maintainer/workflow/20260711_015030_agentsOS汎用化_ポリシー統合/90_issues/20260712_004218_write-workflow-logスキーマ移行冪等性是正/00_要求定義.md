...
- **効果 1**: `write-workflow-log.sh` のスキーマ移行検知ブロックを並走（並列）反復実行しても、`duplicate column name` エラーによる INSERT 失敗が発生しなくなる（前掲レビューの再現手順と同等の並列＋flock 無効化での反復実行で、失敗 0 件になることを実行確認できれば○。§6 成功基準 1 の再現条件を参照）。
...
1. 前掲レビュー（[`../20260711_055602_write-workflow-log_ts_utc検証/04_review.md`](../20260711_055602_write-workflow-log_ts_utc検証/04_review.md) §3.3）と同等の再現手順（新規作成した DB に対し、スキーマ移行を伴うセットアップ INSERT を反復実行する）で、是正後は `duplicate column name` エラーによる INSERT 失敗が 0 件であることを確認できる（○/×: 隔離環境での実行結果で判定）。**再現条件の補足**: この競合は Check-Then-Act の非アトミック性による**真の並走**でのみ生じ、本スクリプトは同一 DB を flock で直列化するため、実行時再現には「**並列実行＋flock 無効化**」が必要（設計 ADR-1・02 §6.2・03 T-3）。冪等化ロジック自体の妥当性はコードレビューでも判定する（成功基準 5 と同様の許容）。
...
- **リスク**: flaky な競合状態の再現・検証には、複数プロセスの並走（並列実行）＋flock 無効化が必要で、しかもタイミング依存で毎回は発火しないため、テスト実装の手間が増える。
  - **対策**: 前掲レビュー（04_review §3.3）が確立した再現の型（版比較）を踏襲しつつ、実行時再現は「並列＋flock 無効化」の反復（N 並列 × M ラウンド）で発火確率を上げる（03 T-3）。加えて recovery 分岐の妥当性はコードレビューで二重担保する（§6 成功基準 5 と同様の許容）。
