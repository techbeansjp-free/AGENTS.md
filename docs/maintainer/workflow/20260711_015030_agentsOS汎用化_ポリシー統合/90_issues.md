...
| 8 | write-workflow-logスキーマ移行冪等性是正 | 5dd2b887-3515-480e-afb8-5cd9d0989070 | サブ issue 3「write-workflow-log_ts_utc検証」の `04_review.md`（§3.3・§4.2 指摘 2・§10.1 課題 1）で監査者が独立発見・HEAD 版比較で確定した既存不具合への対応。`write-workflow-log.sh` のスキーマ移行検知ブロック（現行 298〜319 行目付近、`PRAGMA table_info` の grep 冪等判定 と `ALTER TABLE … ADD COLUMN`）が Check-Then-Act の非アトミック構成であるため、反復・近接実行下で散発的に `duplicate column name: review_id` / `duplicate column name: document_path` を起こし INSERT が失敗する（証跡欠落）。対象カラムが既に存在する場合の ADD COLUMN 失敗を冪等に吸収しつつ、それ以外の真のエラーは従来どおり fail-fast させる是正を行う。サブ issue 3 のスコープ外として起票を提案されたもの（run_command.md §Forbidden によりサブは自ら起票せず、本セッションでメインが Go 出しして起票） | 🟡 中 | **要求・要件定義完了（本サブエージェントが requirement-discovery を実施）。設計待ち** | [詳細](./90_issues/20260712_004218_write-workflow-logスキーマ移行冪等性是正/00_要求定義.md) |
| 9 | audit監査31番tmp隔離検証恒久テスト化 | 465a44fc-f3db-469f-b3be-1464c555429c | サブ issue 4「システム仕様書完備強制テンプレート刷新」の `04_review.md` §3.2・§10.2（改善提案）で検出: `audit.sh` の監査項目 #31（`check_docs_review_evidence`。DOCS_RULES.md §継続追随ゲート関連）の FAIL/PASS/SKIP 判定は当該 verify-and-close で `mktemp -d` 隔離環境を用い手動で 7 ケース（A〜G）を独立再現・確認したが、`test/` 配下に恒久的な回帰テストとして固定化されていない（現状 `test-audit.sh` に #31 専用ケース無し）。本 issue は既存 `test-audit.sh` の隔離パターン（`make_min_tree`・`TMP_DIRS`・sqlite3 ガード）に倣い、7 ケースおよび既存 #5 との非交差性を `test/` 配下の自動テストへ翻訳することを要求・要件として整理したもの（requirement-discovery のみ完了。design-feature 以降は未着手） | 🟢 低 | **要求・要件定義完了（本セッションで新規起票）。設計・実装計画・実装・レビューは未着手** | [詳細](./90_issues/20260712_004252_audit監査31番tmp隔離検証恒久テスト化/00_要求定義.md) |
| 10 | テンプレート相対リンク深度是正 | 1c89813b-42d0-4604-9621-9378f2bc90a8 | サブ issue 2「フィジビリティADR必須化」の `04_review.md` §3.2・§10.1（`.agent-skill-chain/runtime/templates/02_設計.md` の相対深度不整合 15 件検出）、およびサブ issue 4「システム仕様書完備強制テンプレート刷新」の `04_review.md` §4.2 指摘 1・§10.1（`templates/docs/` 配下複数 README の `.agent-skill-chain/source/` 参照深度不揃い・in-repo 未解決検出）で独立に検出され、いずれも「本 issue 範囲外・別 issue での対応を推奨」とされていた既知課題（Story8 の `.agents/` → `.agent-skill-chain/source/` 改名でパス文字列は機械置換されたが `../` 深度プレフィックスが旧構造のまま残存）。本 issue で `.agent-skill-chain/runtime/templates/` 配下全体を grep + realpath で機械棚卸しした結果、相対リンク総数 233 件中 in-repo unresolved 72 件、うち genuine な深度不整合 54 件（対象 15 ファイル。`02_設計.md` の 15 件は既存レビューの検出数と一致・独立再検証で整合）、プレースホルダ・例示リンク（是正対象外）18 件と判明。`docs/` 配下テンプレートは in-repo 参照用配置と配備後消費者 `docs/` 直下とで正しい深さが異なる構造的曖昧さがあることも確認し、方針候補を要件として整理した（requirement-discovery のみ完了。design-feature 以降は未着手） | 🟢 低 | **要求・要件定義完了（本セッションで新規起票）。設計・実装計画・実装・レビューは未着手** | [詳細](./90_issues/20260712_004401_テンプレート相対リンク深度是正/00_要求定義.md) |
| 11 | test-audit_AGENTS_ROOT未追随是正 | a876f925-3cd9-4f88-ac6f-eb8e6af2ca6b | サブ issue 5・7 が独立に検出した `test/test-audit.sh` シナリオ3（必須ファイル欠落検知）の偽陰性 FAIL（`exit 0`・`Missing required file` メッセージ無し）について、要求定義段階で根本原因を実測特定。両 issue の「Story8 ネスト移行未追随の可能性」という推測は誤りで、実際は (1) 呼び出し元シェルに `AGENTS_ROOT=${CLAUDE_PROJECT_DIR}/.agent-skill-chain/source`（`CLAUDE_PROJECT_DIR` 未展開の壊れた値）が事前設定される環境変数汚染と、(2) `audit.sh:216-225` の必須ファイルチェックが `AGENTS_ROOT` 解決先ディレクトリ不在時に丸ごとサイレントスキップする fail-open 設計、の複合が原因（`env -u AGENTS_ROOT` で同一ツリーが期待どおり FAIL することを実測確認済み）。`make_min_tree` はファイルツリーのみ隔離し環境変数を隔離していないための偽陰性 | 🟡 中 | **要求・要件定義完了（本サブ issue）。設計・実装計画は未着手** | [詳細](./90_issues/20260712_004515_test-audit_AGENTS_ROOT未追随是正/00_要求定義.md) |
...
**サブ issue 6 件（フィジビリティADR必須化・write-workflow-log_ts_utc検証・システム仕様書完備強制テンプレート刷新・workflowDB由来検知欠如是正・AGENT_ROLEスコープ是正・review-docs必須化）は本セッションで requirement-discovery→design-feature→review-docs→implement-feature→verify-and-close の全工程を完了した**。各 issue に `00`〜`04` が揃い、`04_review.md` はいずれも本 issue スコープ内の指摘 0 件で収束・書記記録済みで、リポジトリルートの `audit.sh .` は **FAIL 0 件（Audit passed）**。**`npm公開中止_APM転換` はユーザー指示により明示的に対象外**（`00`〜`03` のみ・`04` 未作成・未着手）のまま残る。**サブ issue 8 件目「write-workflow-logスキーマ移行冪等性是正」を新規起票**（サブ issue 3「write-workflow-log_ts_utc検証」の `04_review.md` が発見した既存不具合〔スキーマ移行 ADD COLUMN の非冪等性〕への対応。要求・要件定義（`00`・`01`）完了、設計待ち）。親 issue のうちストーリー 1〜7 は**実装＋verify-and-close 完了**（指摘 0 件収束、`04_review.md` 作成・書記記録済み）。ストーリー 8 は設計改訂・独立レビュー（指摘 12→3→0 件収束）まで完了し、**実装フェーズ（implement-feature）に進める状態だが実装は未着手**（詳細は上記「ストーリー8: 設計改訂の経緯と現状」参照）。したがって親トップレベル issue は未完了であり、close 移動は行わない（下記「親 issue の close 判定」参照）。

- **完了（実装＋verify-and-close 済）**: サブ issue は **6 / 8**（フィジビリティADR必須化・write-workflow-log_ts_utc検証・システム仕様書完備強制テンプレート刷新・workflowDB由来検知欠如是正・AGENT_ROLEスコープ是正・review-docs必須化）。親 issue はストーリー 1〜7 のみ完了（ストーリー 8 は設計・独立レビューは完了したが実装が未着手のため親 issue 全体としては未完了）。
- **設計・実装計画完了／fable レビュー指摘 0 件・実装待ち**: 2 件（親 issue ストーリー 8・サブ issue「npm公開中止_APM転換」）
- **要求・要件定義完了・設計待ち**: 1 件（サブ issue「write-workflow-logスキーマ移行冪等性是正」）
- **対象外（ユーザー指示・未着手）**: サブ issue 1 / 8（npm公開中止_APM転換。`04` 未作成）
- **未着手（未起票）**: 0

```mermaid
pie title サブ issue 進捗（8 件）
    "実装+verify-and-close完了(指摘0件収束)" : 6
    "対象外・実装待ち(npm公開中止_APM転換)" : 1
    "要求・要件完了・設計待ち(スキーマ移行冪等性是正)" : 1
```

※ 上記グラフはサブ issue 8 件の内訳。親 issue（ストーリー 1〜7 完了／8 は設計完了・実装待ち）は別枠であり、この円グラフには含めていない。
...
- **中優先度（🟡）**: 1 / 2 完了
  ...
  - サブ issue 8「write-workflow-logスキーマ移行冪等性是正」: **要求・要件定義完了・設計待ち**。サブ issue 3 の `04_review.md` §3.3・§4.2 指摘 2・§10.1 課題 1 で発見された既存不具合（スキーマ移行 ADD COLUMN の非冪等性による散発的 `duplicate column name` 失敗）への対応
- **低優先度（🟢）**: 1 / 1 完了
