...
- **対応済み**: frame-experience の README.md・SKILL.md の記録先記述を、(b) 案どおり「§7.1 は前提ナラティブ・ペルソナ・JTBD・却下案を記録し、幻覚ペルソナ注意は 3 フェーズ共通の §7.4 を参照・記録する」形へ修正した（commit `2f81938`）。map-experience／detail-experience の README/SKILL には同種の記録先記述が無いことを grep で確認済みのため変更不要。02_設計.md テンプレート自体（§7.4 は T2-T5 で新設済み）は無変更。
