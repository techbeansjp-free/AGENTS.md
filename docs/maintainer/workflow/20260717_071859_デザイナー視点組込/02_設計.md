...
| `skills/experience/detail-experience/`（新規 capability・フェーズ3） | フェーズ2 の流れの上で、UI/ビジュアル・デザインシステム適用・アクセシビリティ・実装可能性を具体化し（「格好いいが操作しづらい」は却下）、02_設計 §7.3 に記録して責務・API 候補を define-boundaries へ逆算・引き渡す。 |
...
体験設計の各フェーズが判断に迷ったときの優先順位を、ドメイン共通ルールとして固定する（プロジェクトオーナー提示）:

1. ビジネス目標 → 2. ユーザー体験 → 3. アクセシビリティ → 4. 保守性 → 5. 一貫性 → 6. デザイン性

- 「格好いいが操作しづらい」（デザイン性が UX を上回る案）は却下する。detail-experience の却下案（§7.3）にこの観点を必須で 1 件以上含める。
...
UI/UX を伴う機能について、体験の流れと 6 観点を「一人称ナラティブ＋却下案」で確認し、責務・API を逆算する土台を 02_設計 §7 に作る。責務は **3 フェーズ capability** に分担し（frame → map → detail）、`design-feature` chain の step 0a/0b/0c（トリガー該当時）として発動する。**各フェーズは別々の fresh サブが担当する**（ADR-7）。判断基準の優先順位は §2.1.4（ビジネス目標 > UX > アクセシビリティ > 保守性 > 一貫性 > デザイン性）を全フェーズ共通で適用する。
...
- 担当: UI/ビジュアル／デザインシステム適用／アクセシビリティ／実装可能性のすり合わせ。
- 入力: §7.2（流れ）。観点シード:
  5. UI・アクセシビリティ: 「視線誘導・操作容易性・アクセシビリティ（コントラスト・キーボード操作・代替テキスト等）を満たすか。『格好いいが操作しづらい』案を却下したか」
  6. 実装可能性・検証: 「体験を成立させる実装制約と矛盾しないか／どう計測・改善するか」
- 出力: §7.3 に UI 具体化＋却下案（デザイン性優先で操作性を損なう案を 1 件以上明示的に却下）＋**責務・API 候補（逆算の種）を抽出し define-boundaries へ引き渡す**。
...
- **Purpose**: フェーズ2 の流れの上で、UI・デザインシステム・アクセシビリティ・実装可能性を具体化し、責務・API を逆算する土台を 02_設計 §7.3 に作る。
- **Inputs**: §7.2（map の出力）・01_要件定義.md。
- **Process**: (1) UI/ビジュアル・視線誘導を検討 → (2) デザインシステム適用・一貫性を確認 → (3) アクセシビリティ（コントラスト・キーボード操作・代替テキスト等）を確認 → (4) 実装可能性・検証改善観点を確認 → (5) 却下案（デザイン性優先で操作性を損なう案）を 1 件以上記述 → (6) 責務・API 候補を抽出し define-boundaries へ渡す。
- **Outputs**: 02_設計 §7.3（UI 具体化・アクセシビリティ・却下案）。define-boundaries への「責務・API 候補」。
- **Done**: 体験面=あり なら §7.3 に UI 具体化＋アクセシビリティ＋「操作性を損なう案の却下」1 件以上＋責務・API 候補が揃う。
- **Forbidden**: 判断基準優先順位（§2.1.4）を無視し「デザイン性」を上位に置くこと。幻覚ペルソナを実ユーザー調査の代替として扱うこと。UI モックの作画（除外要件）。architecture の責務境界を確定すること（define-boundaries の責務）。
...
- **7.3 フェーズ3: 体験の具体化（detail-experience）**: UI/ビジュアル・視線誘導／デザインシステム適用／アクセシビリティ／実装可能性・検証改善。却下案（デザイン性優先で操作性を損なう案）を 1 件以上。責務・API 候補（逆算の種）。
...
- ラウンド1（A1-A10 検出）→ 修正 → ラウンド2（退行検知）で、M1-M11 を継承して確認。3 フェーズ化が M1（既存 chain 不変）・M4（新規ファイル 0）・M7（後方互換）を壊していないことを再確認済み。ラウンド3 で残指摘 0 を確認。詳細は memo 参照。
