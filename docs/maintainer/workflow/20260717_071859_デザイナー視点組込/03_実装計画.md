...
02_設計の決定（新規 domain `experience` を 1 つ・体験設計を分担する 3 フェーズ capability `frame-experience`／`map-experience`／`detail-experience` を追加し、design-feature chain 冒頭にトリガー付き 3 工程（各 fresh サブ委譲）として差す。既存 review-docs ゲート＋DUAL_LENS に相乗り、新規 command・新規 audit・新規成果物ファイルを作らない。ADR-7）に従い、`.agent-skill-chain/source/` 配下の新規作成ファイル・改修 6 ファイルを最小 diff で実装する。フェーズ数は 3 に絞り込み（11 工程を丸写ししない）、規模比例でフェーズ統合可とする。
...
gantt
    ...
    section ゲート・索引
    T6 review-docs+README :a6, after a2, 1d
```
...
| 12 | `.agent-skill-chain/source/commands/review-docs.md` | 改修（確認観点に体験観点 1 項） |
| 13 | `.agent-skill-chain/source/README.md` | 改修（skill domain 索引に experience 追記） |

> 注: 8/9/12/13 は「1 ファイル 1 責務・最小 diff」を厳守（META_LAYER Rule 3）。11 の 00/02 テンプレートは `runtime/templates/` 配下（配布テンプレート正本）。新規 skill は 7 ファイル（domain README + 3 フェーズ×README/SKILL）に増えるが、これはフェーズ分担（fresh サブ per phase・ADR-7）を成立させるための不可欠な粒度であり、新規 command・audit・成果物ファイルは作らないことで肥大化を最小化する（02 ADR-6）。
...
- `skills/experience/README.md`: domain 索引（`skills/requirements/README.md` の表形式に倣う。capability = frame-experience / map-experience / detail-experience の 3 つ）。**判断基準の優先順位（ビジネス目標 > UX > アクセシビリティ > 保守性 > 一貫性 > デザイン性）をドメイン共通ルールとして明記**（02 §2.1.4）。**「各フェーズは別 fresh サブが担当し一気通貫にしない」旨（ADR-7・CLOSEOUT §fresh サブ分割）と「規模比例でフェーズ統合可」を明記**。
- `skills/experience/frame-experience/{README.md,SKILL.md}`: 体験の前提づけ（ビジネス目的・KPI・ユーザー/課題・JTBD）。02 §5.2 契約1・§3.1.2 フェーズ1 に準拠。Process に「体験サーフェス判定の §7.0 記録」「前提ナラティブ」「却下案 1 件以上」「幻覚ペルソナ注意」を含める。
...
- `skills/experience/detail-experience/{README.md,SKILL.md}`: 体験の具体化（UI・デザインシステム・アクセシビリティ・実装可能性）＋責務逆算。02 §5.2 契約3・§3.1.2 フェーズ3 に準拠。Inputs に §7.2、Outputs に §7.3 ＋ define-boundaries への責務・API 候補。「格好いいが操作しづらい」を却下する観点を必須。
...
- domain README に判断基準優先順位・fresh サブ分割・規模比例統合が明記されている。
...
- OUTPUT/DONE に「体験面=あり なら 02 §7 に体験設計（3 フェーズ分または統合記録）記載」「体験面=なし なら判定結果記録」を追記。
- 「実行時の注意」に、**各体験フェーズは別 fresh サブへ委譲し一気通貫にしない旨（ADR-7・CLOSEOUT §fresh サブ分割）**、規模比例でフェーズ統合可の旨、体験面=なし はトリガー非該当の正常系であり工程欠落として扱わない旨を追記。
...
- 既存 step1-3（define-boundaries / design-api-contract / review-dependencies）が順序・内容とも不変。
...
- 7.0 体験サーフェス判定（＋判断基準優先順位）/ 7.1 フェーズ1 体験の前提（frame）/ 7.2 フェーズ2 体験の流れ（map）/ 7.3 フェーズ3 体験の具体化（detail）/ 7.4 幻覚ペルソナ注意 を追加（02 §7 の構成に準拠）。各フェーズ節に却下案欄を設ける。
...
### 2.6 タスク 6: `commands/review-docs.md` ＋ `source/README.md` の相乗り・索引改修

#### 2.6.1 概要（必須）

体験観点を既存 review-docs ゲート（DUAL_LENS）に相乗りさせ、skill domain 索引に experience を追記する。

#### 2.6.2 実装内容（必須: 1 件以上）

- `commands/review-docs.md`: 実装前レビューの確認観点に「体験面=あり の issue は 02 §7 に体験設計観点（ナラティブ・却下案・6 観点・幻覚ペルソナ注意）が記載されているかを DUAL_LENS で確認し、欠落時は差し戻す。体験面=なし は差し戻さない」を 1 項追加。新規 audit.sh チェックは追加しない。
...
- review-docs.md に体験観点確認の 1 項が追加され、既存の review-docs 完了定義（memo＋指摘収束＋書記委譲）を再定義していない。
...
Feature: review-docs ゲートへの体験観点相乗り
  ...
  Scenario: 体験面=なし の issue は体験観点で差し戻さない
    Given experience_surface が "no: 理由" の issue
...
全タスクの検証観点は「ドキュメント成果物・ワークフロー定義の静的検証」であり、review-docs / 監査のチェックリストとして機能する。クリティカル観点: (1) 新規 skill（3 フェーズ）の各 6 見出し充足・フェーズ責務非重複、(2) chain 順序と条件分岐の明確さ・各体験フェーズの fresh サブ委譲明記（ADR-7）、(3) テンプレート後方互換、(4) 既存ゲートとの整合（新規 audit を作らない）、(5) 過剰適用回避（体験面=なし を差し戻さない・規模比例でフェーズ統合可）。
...
| UC2-S2: トリガー判定が明確 | T4（experience_surface 枠）・T1（frame step1 判定記録） | あり/なし が一意に判定・記録可 |
| UC3-S1: fable は成果物本文を執筆しない | 02 §2.5 アドバイザー入力の扱い（設計内で担保）・本 issue の運用 | 02/03 は opus 執筆・fable は助言のみ |
...
- **リスク**: 体験観点を全 issue に強制し過剰適用。
  - **対策**: トリガー（experience_surface）＋体験面=なし を差し戻さない明記（T6-S2）。

---

## 6. サブ issue 分割の判断

- **90_issues.md は作成しない**。本件は「新規 skill 1 つ＋関連 command/テンプレート/ゲートの一体改修」という単一の凝集した変更であり、独立に完結・並行 PR 化すべき分割単位を持たない（T1-T6 は相互依存し 1 つの実装フェーズで完結する）。分割はかえって chain 接続の整合検証を分断しリスクを高める。したがってサブ issue 分割は不要と判断し、`90_issues.md` は作成しない（02 §ADR-6 の最小変更方針とも整合）。
