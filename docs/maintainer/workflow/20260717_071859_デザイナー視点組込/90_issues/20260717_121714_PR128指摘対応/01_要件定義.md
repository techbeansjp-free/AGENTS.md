...
- 是正後、`02_設計.md` 契約1の Outputs/Process 記述が、「§7.1 は前提ナラティブ・ペルソナ・JTBD・却下案のみを記録し、幻覚ペルソナ注意は 3 フェーズ共通の §7.4 に記録する」という、実装（`frame-experience` の README.md・SKILL.md、commit `2f81938` で是正済み）・`04_review.md`「指摘1」の対応済み記述と同じ記録先に揃っている。
...
- 是正後、`review-docs.md` の体験観点の確認観点が、§7.1（前提ナラティブ・ユーザー・課題・却下案 1 件以上・幻覚ペルソナ注意）、§7.2（情報設計・ユーザーフロー・体験ジャーニー・却下案 1 件以上）、§7.3（UI/ビジュアル・既存資産探索一覧・再利用結果／新規作成の正当化根拠・アクセシビリティ・却下案 1 件以上・責務/API 候補）それぞれの必須項目が個別に充足しているかを確認する観点に強化されている。
...
Feature: review-docsゲートの体験観点強化
  Scenario: 各フェーズのDone条件充足を確認する
    ...
    Then §7.1（前提ナラティブ・却下案・幻覚ペルソナ注意）・§7.2（IA・ユーザーフロー・却下案）・§7.3（探索一覧・正当化根拠・アクセシビリティ・却下案・責務API候補）それぞれの必須項目の個別充足が確認観点として明記されている
```
