...
- 本体 `02_設計.md` 契約1 Outputs（L380）・Process (5)（L379）・**Done（L381）** を §5.2-D-1 の after へ（Done も §7.1 局在のため 3 箇所同時に §7.4 寄せへ）。
...
- 正常系: 契約1 Outputs・Process (5)・Done から「§7.1 の注意書き／§7.1 に幻覚ペルソナ注意が揃う」が §7.4 記録の記述へ置換され、実装（`2f81938`）・04_review 指摘1 の §7.4 寄せ記述と整合する。
- 正常系: 04_review 指摘1 に `bab69a2..0032ade` 外である旨の注記が存在する。
- 異常系（非破壊）: 契約2/3 の Outputs/Process/Done に差分が無い。
...
Feature: 記録先記述と 04_review 証跡の整合
  Scenario: 契約1 が §7.4 寄せに揃い 04_review に範囲外注記がある
    ...
    Then 契約1 Outputs・Done は §7.1（前提のみ）＋幻覚ペルソナ注意は §7.4 と記述されている
...
- 正常系: review-docs.md L27 に §7.1/§7.2/§7.3 各フェーズの必須項目に加え、共通要件である §7.4（幻覚ペルソナ注意）の確認も個別確認観点として列挙され、「§7 存在だけでは通過させない」旨がある。
...
Feature: review-docs ゲートの体験観点強化
  Scenario: 各フェーズの必須項目個別充足を確認観点に持つ
    ...
    Then §7.1・§7.2・§7.3 それぞれの必須項目の個別充足、および共通要件 §7.4（幻覚ペルソナ注意）の確認が確認観点として明記される
...
- 02 §3.6.2 の表に列挙した各箇所（本体 00 L91/L161/L191、01 L94/L196/L237、03 L25 実装方針文・03 L51 見出し）へ `runtime/templates/` を反映。
- **不変**: CORE の概念定義（事実記述・01 L22/L29/L89・03 L14/L17 等）・制約趣旨（project 限定でない）・03 L446（既に整合）。
...
- 正常系: 03 L25 実装方針文・03 L51 見出しがいずれも「`source/` および `runtime/templates/` 配下」となり、表項目 10-11（runtime/templates）と整合する。
...
Feature: 変更対象スコープ記述の是正
  Scenario: スコープ記述に runtime/templates が含まれる
    ...
    When 00 §4.1 ほか列挙箇所と 03 L25 実装方針文・03 L51 見出しを是正後に確認する
