...
本 issue は、git worktree の**運用規律**を、①命名（B）と②削除ライフサイクルの安全化（C）の両面から一体で定める。B は worktree ディレクトリ名・ブランチ名の命名規則を新設し、issue との対応追跡性を確保する。C は `git worktree remove` 実行前に untracked 成果物を検知・退避し、2026-07-15 に発生した「未commit成果物の永久ロスト事故」の再発を機械強制で防ぐ。C の機械強制は既存 enforcement 機構（PreToolUse hook・CI audit）の**拡張**として実現し、新設機構は原則作らない。
...
| 命名規則（B） | worktree ディレクトリ名・ブランチ名が満たすべき規則。命名から対応 issue（または目的）を追跡可能にする。 |
...
- 規則は worktree ディレクトリ名とブランチ名の双方を対象とし、命名から対応 issue（または目的）を追跡できる形式を定める。
- 既存の無秩序な名前の遡及一括改名は要求しない（新規作成分から適用・段階対応）。

#### ストーリー 2: 未commit成果物が削除で消えない（C・中核）
...
- `git worktree remove` 系操作の実行前に、対象 worktree の untracked ファイルの有無を検知する（00 SC-2）。
- untracked が存在する場合、削除前に退避先へ保存してから削除する（または退避が完了するまで削除を止める）。退避後は復元可能である。
- 退避対象は untracked 限定（追跡済みファイルは対象外・00 SC-3）。
- この機械強制は既存 enforcement hook 機構の拡張として実現する（新設機構を原則作らない）。

#### ストーリー 3: 追跡済みのみの worktree は従来どおり消せる（C）
...
- untracked ファイルが無い worktree の削除は退避を行わず通常どおり完了する（00 SC-3）。

#### ストーリー 4: 退避先が肥大化しない（C）
...
- 退避先の自動パージ方針（保持期限・容量等）が定義されている（00 SC-4）。
- 期限・容量を超過した退避エントリが除去される。

#### ストーリー 5: 対象外環境で誤作動しない（互換・非破壊）
...
- 非 git ツリー・対象外環境ではゲートが自然に SKIP される（00 SC-5）。
- 既存 enforcement チェック（#1〜#38 相当）を破壊しない（00 SC-6）。
- hook 内部エラー時は fail-safe（allow 側）に倒し、正常な削除を妨げない（evidence_source: existing_code＝`PreToolUse.sh` の `set +e`・「内部エラーは allow に倒す」原則）。

### 2.2 BDD ユースケース

#### ユースケース 1: worktree／ブランチの命名（B）

新規に worktree・ブランチを作成する際、命名規則に沿った名前を付け、命名から対応 issue を追跡可能にする。
...
Feature: worktree／ブランチ命名規則
  Scenario: 規則に沿った命名で worktree を作成する
    Given worktree／ブランチ命名規則が正本に定義されている
    And ある issue に対する新しい作業ツリーを作ろうとしている
    When 命名規則に従って worktree ディレクトリ名とブランチ名を決める
...
sequenceDiagram
    ...
    D->>N: When 規則に従い worktree/ブランチ名を決める
    N-->>D: 追跡可能な名前
    D->>W: worktree add -b <規則準拠名>
    W-->>D: Then 命名から issue を追跡できる
```
...
- 新規外部 API は設けない。検知・退避は既存 PreToolUse hook（stdin JSON の `tool_input.command` 検査）と CI audit（`audit.sh`）の拡張として実現する（evidence_source: existing_code＝既存 enforcement 機構）。
...
- **残余リスク（機械強制の限界）**: PreToolUse hook は Bash の `command` を前置検査するため、hook を経由しない削除経路（別ツール・別プロセスからの `git worktree remove` 等）は捕捉できない場合がある。この限界は 02 設計で明示し隠さない（evidence_source: existing_code＝`PreToolUse.sh` は Bash `tool_input.command` を検査対象とする）。
