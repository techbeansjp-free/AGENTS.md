...
| 命名規則（B） | worktree ディレクトリ名・ブランチ名が満たすべき規則。パターン `<type>/<YYYYMMDD_HHMMSS>/<固有名>`（`type`=feature/bugfix/hotfix）。命名から対応 issue（または目的）を追跡可能にする。 |
...
- 規則は**パターン `<type>/<YYYYMMDD_HHMMSS>/<固有名>`** を採用する。`<type>` は `feature`/`bugfix`/`hotfix` の 3 種を基準とする（evidence_source: human_decision）。`YYYYMMDD_HHMMSS` は既存のプレフィックス取得規約（`TZ=Asia/Tokyo date +%Y%m%d_%H%M%S` 等）を再利用する。例: `feature/20260716_143000/worktree運用規律`。
- **worktree ディレクトリ名についても、ブランチ名と別パターンにする合理的理由がない限り同一パターンを適用する**（統一する方向を確定要件とし、細部の最終確定のみ 02_設計 に委ねてよい）。
- **注意**: ブランチ名（git ref）における `/` は既存の `feature/xxx` 同様、階層区切りとして自然に機能し 3 階層でも技術的問題は無い。一方 worktree ディレクトリ名に同一パターンを適用する場合、`/` はディレクトリ名の文字ではなく**実際のディレクトリ階層**を作る点がブランチ名とは意味が異なる（例: `worktree-agent-root/feature/20260716_143000/worktree運用規律/` という 3 階層ディレクトリになる）。これが意図どおりか、ディレクトリ名は依然としてフラットな 1 文字列にすべきかは 02_設計 への残課題とする（§5 残余論点を参照）。
...
- 規則（`<type>/<YYYYMMDD_HHMMSS>/<固有名>`、`type`=feature/bugfix/hotfix）に反する命名での worktree／ブランチ作成が検知される（00 SC-2）。
...
新規に worktree・ブランチを作成する際、命名規則（パターン `<type>/<YYYYMMDD_HHMMSS>/<固有名>`、`type`=feature/bugfix/hotfix）に沿った名前を付け、命名から対応 issue を追跡可能にする。規則に反する命名は機械的に検知される。
...
Feature: worktree／ブランチ命名規則
  Scenario: 規則に沿った命名で worktree を作成する
    Given worktree／ブランチ命名規則（<type>/<YYYYMMDD_HHMMSS>/<固有名>）が正本に定義されている
...
**シナリオ 3.5: ブランチ名とディレクトリ名で「/」の意味の違いを踏まえる（残課題・02_設計へ送付）**

```gherkin
Feature: worktree／ブランチ命名規則
  Scenario: 「/」がref階層とディレクトリ階層で異なる意味を持つことを認識する
    ...
    When worktree ディレクトリ名の具体的な表現方法（実ディレクトリ階層のままにするか、区切り文字を変換してフラットな1文字列にするか）を確定する
    Then その決定は 02_設計 で行う（本 01 ではパターンの3要素構成のみ確定要件とする）
```
...
Feature: worktree／ブランチ命名規則の機械強制
  Scenario: 規則に反する命名での作成が検知される
    Given 命名規則（<type>/<YYYYMMDD_HHMMSS>/<固有名>、type=feature/bugfix/hotfix）が定義されている
...
- **BR-6（新規）**: B の命名規則は `<type>/<YYYYMMDD_HHMMSS>/<固有名>` を採用し、`type` は `feature`/`bugfix`/`hotfix` の 3 種を基準とする（evidence_source: human_decision）。タイムスタンプ部は本プロジェクト既存のプレフィックス取得規約（`TZ=Asia/Tokyo date +%Y%m%d_%H%M%S` 等）と同一の取得手段を用いる。
...
- **残余論点（02_設計へ送付・B・新規）**: ブランチ命名パターン `<type>/<YYYYMMDD_HHMMSS>/<固有名>` の `/` は git ref namespace の階層区切りとして自然に機能するが、worktree**ディレクトリ名**に同一パターンを適用した場合、`/` は実際のディレクトリ階層を作る点でブランチ名とは意味が異なる（例: `worktree-agent-root/feature/20260716_143000/worktree運用規律/` という3階層ディレクトリになる）。これが意図どおりか、ディレクトリ名は区切り文字を変換してフラットな1文字列にすべきかは未確定。本 01 ではパターンの3要素構成（type・タイムスタンプ・固有名）のみを確定要件とし、ディレクトリ名での具体表現は 02_設計 に委ねる。
...
- ユーザーからの追加フィードバック（2026-07-16・2回目、evidence_source: human_decision）: 命名パターンの区切り文字をアンダースコアからスラッシュ・3階層（`<type>/<YYYYMMDD_HHMMSS>/<固有名>`）へ変更する指定。ストーリー 1 の受け入れ基準・シナリオ 3.5・§5 残余論点（ブランチ名とディレクトリ階層の意味差異）の直接の根拠。
