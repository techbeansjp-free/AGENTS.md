...
> **設計時に発見した既知の事実（隠さず明記）**: 現在この自己適用リポの配備物 `.claude/hooks/PreToolUse.sh` は正本 `enforcement/claude/PreToolUse.sh` より**古い**（正本には R1 carve-out・symlink 実体すり替え耐性が入っているが配備物には未反映）。本 issue の実装後、`agents-md upgrade`／`setup.sh` 再実行で配備物を同期する必要がある（03 のタスクに含める。evidence_source: existing_code＝両ファイル diff 実測 2026-07-16）。
...
- evidence_source: inference_only（fable アドバイザー助言を統合）＋ external_spec（UTF-8 自己同期性・`git check-ref-format` の ref 規則）＋ existing_code（`is_sqlite3_invocation` の `LC_ALL=C`／`case` 作法）。
