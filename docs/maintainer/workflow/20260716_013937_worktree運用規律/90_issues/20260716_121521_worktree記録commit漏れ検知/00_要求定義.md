...
> - **本ファイルの現在の完成度（重要）**: 本ファイルは requirement-discovery の **フェーズ 2（前提整理 / identify-assumptions）完了時点のたたき台**である。§1.1 目的・§6 成功基準（候補）に加え、§1.2 背景（前提・必要性）・§2 機能要件（スコープ候補）・§3 非機能要件・§4 制約条件・§5 除外要件・§7 リスクと対策を、前提・制約・リスクの洗い出しとして埋めた。ただし §2 の機能要件・§6 の成功基準はいずれも「候補」段階であり、検証可能な受け入れ基準への確定（受入基準定義 define-constraints）・BDD シナリオの執筆（write-bdd）は未着手であり次フェーズに引き継ぐ。次フェーズの担当者は §9 に記載の未決事項を参照のうえ作業すること。
>
...
（次フェーズ〔受入基準定義 define-constraints〕で検証可能な受け入れ基準へ精緻化する前提で、目的抽出フェーズ時点の暫定案をそのまま維持している。前提整理フェーズでは §1.2 の前提・§7 のリスクを新たに追加したのみで、本節自体は変更していない）
...
- 親issue「worktree 運用規律」は、B（命名規則）・C（untracked 退避）を「本issue完了時点以降に新規作成される worktree／ブランチにのみ適用」するグランドファザリングを採用している（親 00 §4.3）。本issueの検知機構を、**既存の（本issue完了前から存在する）worktree にも遡及適用するか、新規作成分のみに限定するかは未確定**であり、次フェーズ（受入基準定義）で判断する。
