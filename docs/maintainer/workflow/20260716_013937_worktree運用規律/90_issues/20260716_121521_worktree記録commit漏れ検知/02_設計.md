...
| SC-6（GitHub 非採用・fail-open） | シナリオ 9 | 環境判定§3.5・共通エラー方針（§3 冒頭） | 結合＋境界値 | 既存ゲート群と同一シグナル（`git remote -v` の `github.com`）で対象外／判定不能・内部エラー時は allow に倒れる（過剰 block 回避）。 |

**単体テストの成立根拠**: §6.1 の疎結合確認どおり、純関数群（`worktree_record_scan`・`_wt_record_*`）は既存テストの `# >>> worktree-discipline lib (BEGIN/END)` マーカ抽出→ source 方式（`test-worktree-discipline.sh` L?? の LIB 抽出と同型）で直接呼べる。結合は正本 hook を `{"tool_name":"Bash",...,"tool_input":{"command":"git worktree remove ..."}}` の stdin JSON で駆動する既存パターンを踏襲する。
