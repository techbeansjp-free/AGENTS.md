...
(cd "$tmp" && git check-ignore -q docs/maintainer/workflow/_probe_/00_要求定義.md); [ $? -eq 0 ]
...
# Then: 追跡され・ADR 最小 5 要素の見出しをすべて持ち・ignore されない
git ls-files docs/maintainer/decisions/DECISIONS.md | grep -q DECISIONS.md
for h in 'コンテキスト' '検討した選択肢' '決定' '根拠' '帰結'; do
  grep -qE "$h" docs/maintainer/decisions/DECISIONS.md || { echo "missing heading: $h" >&2; exit 1; }
done
...
local_tracked=$(cd "$tmp" && unset ISSUE_TRACKING_MODE; bash -c 'source .agent-skill-chain/source/enforcement/ci/audit.sh; resolve_issue_tracking_mode')
