...
> - **本ステップ（extract-goals）の範囲**: 本コミット時点では「1. 目的・背景」「6. 成功基準」のたたき台までを確定する。前提・制約の洗い出し（identify-assumptions）、制約・受け入れ基準の確定（define-constraints）、BDD シナリオ（write-bdd）は後続ステップで追記する。§2〜5・§7 は後続ステップで精緻化する暫定記述である。
>
...
> 本節は後続ステップ（identify-assumptions／define-constraints）で精緻化する暫定記述。現時点では S-2 スコープ（T2・T3・T5・T7＋スイッチ投入）の骨子のみ示す。

### 2.1 既存機能の維持（該当する場合）

- 既定 `local_tracked`・非 GitHub 環境フォールバックを壊さない（消費者環境で本リポ固有のスイッチ投入が波及しないこと）。
- S-1 の audit.sh ロジック・S-3 の source 抽象契約は変更しない（本 issue は本リポ固有の project／`.gitignore`／`.github/`／決定ログのみを対象）。
...
5. **`ISSUE_TRACKING_MODE=github_native` スイッチ投入**: 本リポの実効モードを github_native に切り替える（最後に投入）。
...
> 本節は後続ステップ（define-constraints）で確定する暫定記述。
...
- audit.sh のコード改修（`resolve_issue_tracking_mode`・#33 モードガード）は **S-1 スコープ**（実装済み・PR #116）。
...
- **SC7**（親 S6 再確認）: 非追跡化後も #3/#9/#29/#31/#32 および #34/#35/#36 が想定どおり機能する（またはローカル pre-push 経路のみで機能する前提が妥当である）ことを再確認・記録する（○/×）。
- **SC8**（親 S8）: 本リポの github_native 採用が、非 GitHub 環境・既定 local_tracked の消費者へ波及せずロックアウトしないことを確認する（○/×）。

> **親成功基準との対応**: SC1＝親 S1、SC2＝親 S2/S3、SC3＝親 S7、SC4＝親 ADR-9（本リポ側 T7）、SC5/SC6＝親 S3/S4、SC7＝親 S5/S6、SC8＝親 S8。SC5〜SC8 の検証は tmp 隔離（`mktemp -d`＋`git archive`）で行い、本番自己インストールを伴わない方針を後続ステップで制約として確定する。
...
- **リスク**: スイッチ投入や非追跡化が、本リポの既存監査（#3/#9/#29/#31/#32・#34/#35/#36）を誤 FAIL させる／ロックアウトする。
  - **対策**: 変更を本リポ固有ファイルに限定し、tmp 隔離で監査挙動を検証したうえでスイッチを最後に投入する。
...
この要求定義書の目的・成功基準（extract-goals）確定後、以下を順に追記・作成します：

- **次（本 command 内）**: identify-assumptions（前提・制約・リスクの洗い出し）→ define-constraints（制約・受け入れ基準の確定）→ write-bdd（[`01_要件定義.md`](./01_要件定義.md) の BDD シナリオ）
