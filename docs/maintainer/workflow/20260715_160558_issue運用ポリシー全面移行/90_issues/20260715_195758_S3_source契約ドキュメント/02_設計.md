...
| **手動** | 「全文説明を重複させていないか」「既存の分岐と矛盾しないか」等、文意の質的判断を要し機械化しない事項。review-docs（人手レビュー）で確認する | review-docs 実施時の目視確認（チェックリスト化して 04_review.md に記録） |
...
| 5b | シナリオ5 後段: 雛形は inert で enforcement から読まれない（SC8） | C5 | 単体 | `grep -rL 'enforcement/github' enforcement/ci/audit.sh enforcement/claude/PreToolUse.sh enforcement/claude/PostToolUse.sh` 等、いずれのファイルも `enforcement/github/` を参照しないことを確認（ヒットしないことが FAIL 条件） | T6-TS2 |
