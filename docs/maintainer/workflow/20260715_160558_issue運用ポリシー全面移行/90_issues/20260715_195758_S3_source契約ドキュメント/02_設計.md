...
>
> **完了の注記（2026-07-16 更新）**: 本 02 は design-feature の 3 フェーズ（architecture\_\_define-boundaries §2.1／architecture\_\_design-api-contract §2.2〜§2.5・§3・§4・§5・§7〜§9／architecture\_\_review-dependencies §6）をすべて完了し、確定した。§6 テスト戦略は、01 の BDD シナリオ 1〜6 と C1〜C7・K1〜K7 の対応表・テスト種別割り当てを本フェーズで追記済みである。次工程は [`03_実装計画.md`](./03_実装計画.md)（たたき台）を経た review-docs／GitHub Issue 起票ゲートである（§12 参照）。
>
...
- C2・C3 → C1（run_command.md）（**論理的な参照**: close 分岐注記は `ISSUE_TRACKING_MODE` という同一 env を扱うため、値の意味・既定値の**全文説明はC1のみが担い**、C2/C3 は「local_tracked 専用」という結論のみを短く記載する。C1 を抽象原則の正本とすることで SC1 の「1 箇所記載」要件と整合させる。物理的なリンクは付す（ADR-S3-1・§2.5 で確定済み）ため、この論理参照は §2.5 の物理リンク方針の上位判断として扱う）
...
**方針**: 本 issue はドキュメント・inert 雛形のみを扱い（§1.2）、実行時コードを持たない。したがって「テスト」は通常のユニットテスト（アプリケーションコードの関数検証）ではなく、(a) **grep 等で機械的に真偽判定できるテキスト突合**、(b) **ファイル存在・命名・YAML スキーマ等の構造検証**、(c) **人間のレビューでしか判定できない質的判断**（配置境界の遵守・既存文脈との非矛盾等）の 3 種に区分する（review-dependencies §手順 3「単体・結合・境界値」の本 issue 向け読み替え）。

### 6.1 テスト種別の定義

| テスト種別 | 内容 | 実行手段 |
|---|---|---|
| **単体** | 1 コマンド（主に `grep`）で真偽が決まる機械判定。01 §2.2 に記載済みの bash スニペットをそのままテストスクリプト化できるもの | shell スクリプト（`grep -q`／`grep -E`／行範囲指定 `sed -n` 併用） |
| **構成** | ファイルの存在・配置・YAML としての構文妥当性など、内容の意味解釈を伴わない構造検証 | `ls`／YAML パーサ（`python3 -c "import yaml,sys; yaml.safe_load(open(sys.argv[1]))"` 等、既存 CI に依存を増やさない範囲で）／`grep -A` によるアンカー直後の見出し存在確認 |
| **手動** | 「全文説明を重複させていないか」「既存の分岐と矛盾しないか」等、文意の質的判断を要し機械化しない事項。review-docs（実装前）と verify-and-close（実装後）の人手レビューで確認する | 実装前は review-docs 実施時に目視確認し **memo に記録**（review-docs は 04_review.md を作らない＝PHASES §レビュー成果物の配置ルール・review-docs DoD）。実装後は verify-and-close で実 diff を目視確認し **04_review.md に記録** |

### 6.2 01 BDD シナリオ ↔ C1〜C7・K1〜K7 対応表

| # | 01 のシナリオ（§2.2） | 検証対象（C／K） | テスト種別 | テスト観点（概要） | 03 テスト観点 ID |
|---|---|---|---|---|---|
| 1 | シナリオ1: 抽象は source・具体は project（二重記載なし） | C1・C2・C3（K1・K2・K3） | 構成＋手動 | (構成) 既存 3 ゲート（run_command.md）の既存ブレット文言・順序が変更されていないこと（diff で「追加行のみ」であることを確認）。(手動) C2／C3 が既定値・二値・フォールバック・自律設定禁止等の全文説明を持たず「1 文＋C1 へのリンク」のみに留まっていること（§3.1.3 配置境界違反の防止）を review-docs で目視確認 | T4-TS1 |
| 2 | シナリオ2: env 名・既定値が audit.sh と一致（SC4） | C1（K1） | 単体 | 01 §2.2 の bash をそのままテスト化: `grep -q 'ISSUE_TRACKING_MODE' run_command.md && grep -q 'local_tracked' run_command.md && grep -q 'ISSUE_TRACKING_MODE' enforcement/ci/audit.sh` | T4-TS2 |
| 3 | シナリオ3: close 分離の二重モード注記 | C2・C3（K2・K3） | 単体＋手動 | (単体) `grep -q 'local_tracked' CORE.md`・`grep -q 'github_native' CORE.md`・同 PHASES.md の 2 ファイルいずれにも存在すること。(手動) 既存の「GitHub 連携時は PR 経由・非連携時はユーザー明示指示」という実行確定手段の分岐（PHASES.md 既存記述）と矛盾しないことをレビューで確認 | T4-TS3 |
| 4 | シナリオ4: モード切替 env の自己無効化禁止（SC3） | C4（K4） | 単体 | 本文セクション・凝縮版セクションそれぞれの行範囲を区切って `ISSUE_TRACKING_MODE` を grep し、**両方**にヒットすること（`sed -n '<本文開始>,<本文終了>p' AGENT_CONDUCT.md \| grep -q ISSUE_TRACKING_MODE` を本文・凝縮版それぞれに適用。行範囲は実装時点の実測行に合わせる）。片方のみのヒットは FAIL（§3.1.3） | T4-TS4 |
| 5a | シナリオ5 前段: 手動起票の構造強制（inert 雛形） | C5（K5） | 構成 | 01 §2.2 の bash をそのままテスト化: `ls enforcement/github/*.example.yml && grep -Eq '目的\|成功基準\|受け入れ基準' ... && grep -q 'required: true' ...`。追加で YAML パース可否（構文妥当性）を構成テストに含める（SC5） | T6-TS1 |
| 5b | シナリオ5 後段: 雛形は inert で enforcement から読まれない（SC8） | C5 | 単体 | `grep -rl 'enforcement/github' enforcement/ci/audit.sh enforcement/claude/PreToolUse.sh enforcement/claude/PostToolUse.sh` の**出力が空**であること（`enforcement/github/` を参照するファイルが 1 つも無い＝出力が非空なら FAIL）。`-L`（パターンを含まないファイル名を返す）は固定ファイル列に対して inert 判定を反転させ誤りやすいため、`-l`＋空判定を用いる | T6-TS2 |
| 6 | シナリオ6: SETUP／README への雛形記述追記 | C6・C7（K6・K7） | 構成＋手動 | (構成) `grep -A3 '<C6 見出しアンカー>' SETUP.md`／`grep -A3 '<C7 段落アンカー>' enforcement/README.md` で追記が指定アンカー直後に存在することを確認。(手動) フィールド定義（required/optional 区分等）が C6／C7 に再掲されていないこと（§3.2.3 二重記載防止）をレビューで確認 | T6-TS3 |

### 6.3 補助テスト観点（非機能・§8〜9 対応）

| 対応 | 検証対象 | テスト種別 | テスト観点 |
|---|---|---|---|
| §9.2 可用性（非 GitHub フォールバック明記） | C1 | 単体 | C1 の追記ブロックに「非 GitHub 環境では `local_tracked` へフォールバック」に相当する文言（フォールバック・非 GitHub 等のキーワード）が含まれることを grep で確認 |
| ADR-S3-1（C2/C3 → C1 の物理リンク） | C2・C3 | 単体 | C2・C3 の追記文に `run_command.md` への相対リンク（`](../` 形式）が含まれ、かつリンク先ファイルが実在すること（`realpath -m` 等でリンク解決を確認）。既存の close 移動時リンク補正手順（自己拡張ワークフロー.md）と同様の「実在解決の確認」を踏襲する |
| §8.2 トークン非残留 | C5・C6・C7 | 手動 | 雛形・記述例にトークン実値・認証情報が含まれないことをレビューで確認（新設ファイルのみが対象で分量が小さいため機械化しない） |

### 6.4 テスト対象外（理由明記・網羅性の限界を隠さない）

| 対象外の事項 | 理由 |
|---|---|
| GitHub Issue Forms の実際の Web UI レンダリング・必須欄未入力時の submit 不可（K5 の契約後半） | GitHub 側の既存機構が保証する契約であり、本リポジトリでの自動テスト対象外（01 §2.2 シナリオ5 に明記済み）。C5 が GitHub Forms スキーマとして構文妥当であること（6.2 5a）までを本 issue のテスト範囲とする |
| `.agent-skill-chain/project/自己拡張ワークフロー.md`（project 側）の具体手順自体の検証 | project 側の改訂は S-2 のスコープであり本 issue では変更しない（§2.1.2）。本 issue では C1／C3 からのリンクが実在解決することのみを確認する（6.3） |
| `enforcement/ci/audit.sh` の `resolve_issue_tracking_mode` 実装ロジック自体の単体テスト | S-1（PR #116）で実装済み・テスト済みであり、S-3 はコードを変更しない対象外ファイル（§2.1.1 対象外表）。本 issue のテストは「契約の記述が実装の値と grep 突合で一致するか」（6.2 #2）までを範囲とする |
| 新規 CI チェック（audit.sh への新規 `check_*` 追加） | 本 issue はドキュメント追記のみであり、実行時に検証すべき新しい機械的な不変条件を持たない。既存の内部参照禁止（enforcement #6）・DOCS_NOISE 等の既存監査に抵触しないこと（6.5）の確認で足り、新規チェック関数の追加は本 issue のスコープ外とする |

### 6.5 監査・レビューでの確認（証跡）

- **既存ドキュメント監査の非抵触**: 既存 `audit.sh`（内部参照禁止 #6・DOCS_NOISE 等）が本 issue の追記後もグリーンであることを実測する（01 §2.1 ストーリー5 受け入れ基準）。
- **既存記述の非破壊**: 01 のビジネスルール「非交差・非破壊」（§2.3）に基づき、C1〜C4・C6・C7 いずれも diff が「追加行のみ」（既存行の変更・削除を含まない）であることを、実装完了時に `git diff` で確認する。
- **依存関係の最終確認（循環なし）**: §2.1.3・§2.2 の参照関係図のとおり、T4 群（C1〜C4）と T6 群（C5〜C7）の間に参照関係はなく、C4 は独立、C5 は依存なし（inert）である。本フェーズで改めて確認し、循環参照は存在しないと結論づける（define-boundaries／design-api-contract からの変更なし）。
- **実装順序への示唆（依存の向き）**: ファイルレベルの依存はゼロだが、**文言確定の順序**には実務上の向きがある。C2・C3 は C1 への物理リンクを含む（ADR-S3-1）ため、C1 の見出し・追記位置を先に確定してから C2・C3 を書くと手戻りが少ない。同様に C6・C7 は C5（コピー先・ファイル名）を参照するため、C5 を先に確定するとよい。この順序は 03_実装計画のタスク分割・実装順序（§2）に反映する。
...
**本 02_設計.md は design-feature command（define-boundaries → design-api-contract → review-dependencies）の 3 フェーズをすべて完了し、確定した。**

- **次**: [`03_実装計画.md`](./03_実装計画.md)（本フェーズでたたき台を作成済み。タスク一覧・依存順・完了条件レベル）を経て、review-docs（実装前ドキュメントレビュー）→ GitHub Issue 起票ゲート（本 issue は `90_issues/` 配下サブ issue のため親 #115 に集約・個別起票なし）→ implement-feature（C1〜C4 の最終プロース文言確定・C5〜C7 の実ファイル作成を含む）と進む。
- **実装フェーズへの申し送り**: 03_実装計画.md のたたき台は、タスク分割・依存順・完了条件までを定めたものであり、C1〜C4 の最終日本語プロース文言そのものは 03_実装計画の詳細確定フェーズ（review-docs 前の文面確定タスク）または implement-feature の作業として扱う（本 02 のスコープ外・§3 冒頭方針のとおり）。
...
**design-api-contract フェーズからの申し送り（処理状況）**:

5. **C1〜C4 の最終プロース文言・03_実装計画への落とし込み**: **未解決のまま次工程（03_実装計画の詳細確定フェーズ・implement-feature）へ申し送り**。§3.1〜§3.2 で確定したのは追記の**構成要素**（何を含むか）であり、実際の日本語文言の確定は本 review-dependencies フェーズの対象外として意図的に扱わなかった（design-api-contract §制約「実装の詳細は 02 に長く書かない」、および本フェーズへの委譲指示に基づく）。03_実装計画.md のたたき台（本フェーズ作成）で、どのタスクが文言確定を担うかのみ位置づけた。
6. **§6 テスト戦略・依存関係の最終確認**: **解決済み（本フェーズ・§6）**。01 の BDD シナリオ 1〜6（5 は前段・後段の 2 観点に分割し計 7 行）と C1〜C7・K1〜K7 の対応表、テスト種別（単体／構成／手動）の割り当てを §6.2 に確定した。§2.1.3 の依存関係（循環なし・T4/T6 独立）は §6.5 で改めて確認し、変更なしと結論づけた。

**本フェーズ（review-dependencies）からの新規申し送り**:

7. **C1〜C4 の行範囲・アンカーの実測未確定**: §6.2 のテスト観点（#4 の本文／凝縮版の行範囲、#6 の C6／C7 アンカー直後の grep -A 行数等）は、実装時点（C1〜C7 の実追記後）の実測行に合わせて調整が必要な箇所を含む。実装フェーズで確定すること。
8. **YAML パーサ依存の要否**: §6.2 5a の「YAML パース可否」検証は `python3`／`yamllint` 等の既存 CI に依存を増やさない範囲での実現方法を実装フェーズで選定すること（本フェーズでは構文妥当性を確認すべきという観点のみを確定し、具体的なコマンドは実装時の環境に委ねる）。
