...
# github_issue: この issue に対応する GitHub Issue 番号／URL を記録する frontmatter キー。
#   値は (a) 実 Issue 参照（"#<番号>" or URL）、または (b) 意図的に起票しない決定の理由付き記録（"declined: <理由>"）のいずれか。null/空/理由なし declined は未通過（02 ADR-2・ADR-7）。
#   本 issue の GitHub Issue は #13（https://github.com/techbeansjp-free/AGENTS.md/issues/13）。判定手順・キーの具体形式は docs/maintainer/workflow/20260712_175901_GitHubIssue起票ゲート追加 の 02 設計・.agent-skill-chain/project/ を参照。
github_issue: "#13"
---
...
- **close 移動手順そのものの「移動前検証」化**（別ブランチ `chore/close-completed-issues`・コミット `af90d4e`「close 移動手順を移動前検証方式へ修正し完了済み 4issue を close 移動」で対応済み・**main にマージ済み（PR #11＝`c9873a7`）**。本作業ブランチ `docs/close-move-enforcement`（当初 `e3b9fc4` 基点）にも取り込み済み（HEAD `67709a2`））は対象外。本 issue では重複させず参照で接続する（§背景・申し送り参照）。
...
- **リスク（解消済み）**: `chore/close-completed-issues`（移動前検証化）は main（PR #11＝`c9873a7`）へマージ済みで、取り込み時に同じ PHASES.md/project override の同一節でコンフリクトしうる。
  - **対策**: 本 issue は close 移動手順本文を編集せず、参照リンクで接続する設計とする。**本作業ブランチ `docs/close-move-enforcement`（当初 `e3b9fc4` 基点）は main を取り込み済み（HEAD `67709a2`）**でコンフリクトなく解消。取り込み後の PHASES.md §完了 issue の close 移動・project override §close 移動時の相対リンク補正 は既に「移動前検証」化されており、02_設計・03_実装計画が引用する行番号は取り込み後の実態と一致することを確認済み。
...
- 別ブランチ（除外・参照のみ）: `chore/close-completed-issues`（コミット `af90d4e`・close 移動の移動前検証化・**main にマージ済み（PR #11＝`c9873a7`）・本作業ブランチ取り込み済み（HEAD `67709a2`）**）
