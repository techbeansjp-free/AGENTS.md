...
本リポジトリは「パッケージ正本」かつ「自己拡張する消費者」の二役を兼ねるため、git の扱いを名前空間で分離している。ところがルート `.gitignore` の `**/memo/*`（43 行、=**正**）が**すべての** memo を無視する一方、`.agents-project/自己拡張ワークフロー.md` は保守者名前空間（`docs/maintainer/workflow/<issue>/memo/`）の memo を「git 追跡対象」と宣言しており矛盾している（=**この宣言が誤りで是正対象**）。
...
- ルート [`.gitignore`](../../../../.gitignore) - 43 行 `**/memo/*`
