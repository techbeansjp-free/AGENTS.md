...
- **現状の課題**:
  - 親 issue のストーリー 7（fable-like 行動規範統合）実装中、書記記録（write-workflow-log）の 3 件の呼び出しで、`ts_utc` カラムに `20260711_052131`（JST の memo プレフィックス形式 `YYYYMMDD_HHMMSS`）が誤って渡され、[.agent-skill-chain/source/scribe/CONTRACT.md](../../../../../../../.agent-skill-chain/source/scribe/CONTRACT.md) が要求する「ISO8601 時刻」形式に違反した状態で INSERT された（verify-and-close 監査で検出）。
  - [.agent-skill-chain/source/scripts/write-workflow-log.sh](../../../../../../../.agent-skill-chain/source/scripts/write-workflow-log.sh) の `TS_UTC`（第 4 引数）は**非空チェックのみ**（同スクリプト 200〜203 行目）であり、ISO8601 形式であることのバリデーションが一切ない。`AGENT_ROLE=scribe .agent-skill-chain/source/scripts/write-workflow-log.sh implement-feature "テスト" 1 "20260101_000000"` のように非 ISO8601 文字列を渡しても、現行スクリプトはエラーなく成功し INSERT してしまう（再現確認済み）。
  - [.agent-skill-chain/source/enforcement/ci/audit.sh](../../../../../../../.agent-skill-chain/source/enforcement/ci/audit.sh) の `bad_ts` チェック（364 行目）は「ts_utc が数字を 1 文字も含まない場合のみ FAIL」（`ts_utc NOT GLOB '*[0-9]*'`）という緩い判定のため、`YYYYMMDD_HHMMSS` のような誤形式も CI をすり抜ける（サイレントな品質劣化）。
...
- `.agent-skill-chain/source/scribe/CONTRACT.md` の「ts_utc = ISO8601 時刻」の契約定義を正とし、契約自体は変更しない。
...
1. `AGENT_ROLE=scribe .agent-skill-chain/source/scripts/write-workflow-log.sh implement-feature "テスト記録..." 1 "20260101_000000" ...`（非 ISO8601 の JST プレフィックス形式）が exit 1 で失敗し、workflow.db に行が INSERT されない（○/×: 隔離環境で実行して終了コードと行数を確認）。
...
- **リスク**: 本サブ issue が親 issue の実装ロードマップ（ストーリー 1〜7 → 8 → APM 転換）と並走し、ストーリー 8 のディレクトリ統合ネスト改名（対象スクリプトの現行パスは `.agent-skill-chain/source/scripts/write-workflow-log.sh`）とパス変更が競合する。
  - **対策**: 変更は `write-workflow-log.sh` 内の局所的な追加のみでパス構造に依存しない。ストーリー 8 の改名（commit 44dac77 で完了済み）はディレクトリ名の一括変更であり、本改修のコード内容と衝突しない。着手順は orchestrator がロードマップと整合させる。
...
- [.agent-skill-chain/source/scripts/write-workflow-log.sh](../../../../../../../.agent-skill-chain/source/scripts/write-workflow-log.sh) - 改修対象（TS_UTC は 200〜203 行目で非空チェックのみ）
- [.agent-skill-chain/source/scribe/CONTRACT.md](../../../../../../../.agent-skill-chain/source/scribe/CONTRACT.md) - ts_utc「ISO8601 時刻」の契約定義（正本）
- [.agent-skill-chain/source/enforcement/ci/audit.sh](../../../../../../../.agent-skill-chain/source/enforcement/ci/audit.sh) - `bad_ts` 事後検知（364 行目、`NOT GLOB '*[0-9]*'` の緩判定）
- [.agent-skill-chain/source/enforcement/README.md](../../../../../../../.agent-skill-chain/source/enforcement/README.md) - 失敗条件 #8「workflow.db 品質違反（ts_utc 形式異常）」
