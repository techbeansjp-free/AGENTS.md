...
- **UNIX 哲学・単一責務**（spec/01）: hook の新規判定は既存 `is_sqlite3_invocation` と同型の小さな純関数（トークナイザ）として追加し、1 関数 1 責務に保つ。B（作成検知）と C（削除検知）は別関数として分離する。ただし `validate_worktree_path` は無関係な `.worktree` 迂回（PR#120 finding-5）を塞ぐため、呼び出し側 R7 が解決したリポジトリルート（git 文脈）を任意引数で受け取り照合する（純関数の例外・I4b に明記）。git 文脈の解決は R7 側に閉じ込め、判定関数へは値として渡す（副作用を判定関数内に持ち込まない）。
...
5. 作成形（下表）と確定できたら、ブランチ名（`-b/-B <name>` または暗黙名＝path basename。`--detach` 時はブランチを作らないため空＝検証しない・finding-4）と worktree path を抽出し、`validate_branch_ref`／`validate_worktree_path`（後者は R7 が `git rev-parse --show-toplevel` で解決したリポジトリルートを供給・finding-5）で照合。違反なら `worktree_name_reject`（exit 2）。作成形と確定できない（listing 系・曖昧）なら allow（fail-open）。
...
git_subcommand_of() {
  ...
  while (( i < n )); do
    case "${tok[i]}" in
      ...
      --exec-path) return 1 ;;                   # bare `--exec-path`: git 自体がここで終了しサブコマンド無し（下記脚注・return 1＝allow）
      -p|--paginate|-P|--no-pager|--bare|--no-replace-objects|--literal-pathspecs|--no-optional-locks|--no-advice)
        ((i++)) ;;                               # 引数なし既知フラグ
...
```
- **実装との対応（構造）**: 正本実装（`PreToolUse.sh`）は上記のスキャンを `_wt_effective`（グローバルオプションをスキップし `WT_ARGV` にサブコマンド以降の argv を格納・0/1 を返す）へ分離し、`git_subcommand_of` はその薄いラッパ（`WT_ARGV[0]` を印字）とする。分離は R7（命名強制）・R8（削除前退避）双方が同じトークナイザ結果（`WT_ARGV`）を再利用するためであり、状態機械のロジック（グローバルオプションのスキップ規則・未知オプション fail-open・`--exec-path` の扱い）は本節の記述と同一である。
- **未知オプション時 fail-open**: 未知 `--foo` を「引数なし」と誤推定しても、次トークンが対象サブコマンドでなければ allow に落ちる（誤 block でなく見逃しに倒れる非対称・fable 助言）。
- **`--exec-path` は 2 トークン消費グループに含めない（round2 レビューで発見・observed_runtime で実機検証済み）**: `-C`/`-c`/`--git-dir`/`--work-tree`/`--namespace`/`--config-env`/`--attr-source` は space 区切り形（例 `git --git-dir /path status`）でも次トークンを自身の引数として正しく消費することを実機（git 2.43.0）で確認した。一方 `--exec-path` は **bare 形（`=` 無し）では次トークンを一切消費せず、常に exec-path を表示して即終了する**（`git --exec-path status` は `status` を実行せず path を印字して exit 0）。これを 2 トークン消費グループに含めると次トークン（例えば `worktree`）を誤って `--exec-path` の引数として読み飛ばし、後続の真のサブコマンド判定を狂わせる。逆に「引数なし既知フラグ（1 進み）」として扱うと、bare `--exec-path` の次トークン（`status` 等）を実 git が実行しないにもかかわらずサブコマンドとして拾ってしまう。実 git の挙動（git 自体がここで終了しサブコマンドを実行しない）に忠実に合わせ、**bare `--exec-path` は `return 1`＝サブコマンド無し扱い（＝allow）**とする（上記コードブロックおよび `PreToolUse.sh` の `_wt_effective` 実装・`test/test-worktree-discipline.sh` の `git_subcommand_of "git --exec-path status" == ""` 回帰テストと一致）。`--exec-path=<path>` の `=` 形は既存の `--*=*` パターンで別途正しく処理され後続を実行する（別扱い）。実際には bare `--exec-path` の直後で本物の git 自体が終了するため到達可能な bypass ではないが、状態機械の前提誤りとして是正した（evidence_source: observed_runtime＝`git --git-dir /path status`・`git --exec-path status`・`git --namespace foo status`・`git --config-env core.editor=X status` を実機実行し space 区切り消費・bare `--exec-path` の終了挙動を個別確認、2026-07-16）。
...
| I4a | `validate_name <name>` | 名前文字列 | return 0（準拠）/ 1（違反）（純関数・Query） |
| I4b | `validate_worktree_path <path> [repo_root]` | path 文字列 ＋ **任意の repo_root**（呼び出し側が `git rev-parse --show-toplevel` で解決して供給） | return 0（準拠）/ 1（違反）。**repo_root 供給時は絶対/ネスト表記の `.worktree` を `<repo_root>/.worktree/` 直下に限定**（`_wt_abspath`＝realpath で正規化照合）。先頭 `.worktree/` の相対表記は従来どおり許容。**repo_root 未供給（非 git・解決不能）時は構造チェックのみ＝従来動作へ fail-safe フォールバック**。git 文脈（リポジトリルート）を呼び出し側から受け取るため、単一文字列の純関数ではない（呼び出し側 R7 が git 文脈を解決して供給する・PR#120 finding-5 で無関係な `/tmp/other/.worktree/…` 迂回を塞ぐ改修） |
| I5 | `worktree_untracked_rescue <target>` | worktree path | 退避先へ copy（Command）＋ stderr 通知＋ lazy purge。**退避先は削除対象（target）の外側に解決する**（既定は main worktree ルート直下・target 配下に入る場合はシステム temp へフォールバック・不能時は退避見送りで fail-safe。PR#120 finding-6/7 で clean path 省略時の退避物ごと非可逆消失を防止） |
...
| UC1シナリオ3（worktree ディレクトリ名が `.worktree/` 配下でブランチ名パターンをミラー） | I4b（`validate_worktree_path`） | 単体 | T-B6b |
