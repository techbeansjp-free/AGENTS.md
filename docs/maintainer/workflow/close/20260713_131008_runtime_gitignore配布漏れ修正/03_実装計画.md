...
[02_設計.md](./02_設計.md) の ADR-1/2/3 に基づき、`setup.sh`（タスク 1）・`SETUP.md`（タスク 2）・`PreToolUse.sh` 正本（タスク 3）へ局所追記を行う。3 タスクは互いに独立ファイルへの変更であり並行実施可能だが、タスク 1 の変数命名（`WF_GITIGNORE`/`WF_GITIGNORE_SOURCE`）確定後にタスク 2 の文言を最終化するため、実施順はタスク 1 → タスク 2 → タスク 3 とする。**本 03_実装計画.md（および 02_設計.md）確定後・実装（タスク 1〜3 のコード変更）着手前に、review-docs（02/03 対象・design-feature 完了後の必須ゲート）を必ず経ること。** review-docs 完了後に implement-feature へ進み、その中でタスク 1→2→3 の順にコード変更を実施する（**2026-07-13 修正・CodeRabbit指摘4**: 旧記述「実装後、review-docsを経てからimplement-featureに進む」は、review-docsが実装前必須ゲートであることと矛盾する誤読を招く表現だったため、上記のとおり順序を明確化した）。

**追記（ADR-4・実装完了後の verify-and-close フェーズで発覚した齟齬への対応）**: タスク 1〜3 実装・全テスト PASS 確認後、実際に `npm pack` して tarball を検証したところ、`package.json` の `files` 配列に `.agent-skill-chain/runtime/.gitignore` を追加しても npm-packlist の仕様上配布されないことが判明した（詳細根拠は 02_設計.md ADR-4 参照）。これにより **タスク 4** を追加し、コピー元をパッケージ内の別名テンプレートファイルに変更する。タスク 1 の実装（未存在時のみコピーというロジック自体）はそのまま維持し、コピー元パスの変数値のみをタスク 4 で修正する。タスク 4 は review-docs 完了後の implement-feature フェーズ中（verify-and-close での発覚を受けた追加対応）に実施したものであり、review-docs 前の計画段階には含まれていなかった。

### 1.2 実装順序（必須: 1 件以上）

**フェーズ順（誤読防止のため明示。CodeRabbit指摘4対応）**:

1. **review-docs（02/03 対象・design-feature 完了後・実装着手前の必須ゲート）** — 指摘 0 件になるまで反復（詳細は §3.2 参照）。
2. **implement-feature 着手**（review-docs 完了後）。以下の順でコード変更を実施する:
...
3. **タスク 4（ADR-4・追加）**: npm 配布制約への対応（テンプレートファイル新設・`setup.sh` コピー元変更・`package.json`/`verify-npm-pack.sh` 修正）。implement-feature フェーズ中に verify-and-close での発覚を受けて追加実施。
...
- 境界値（**2026-07-13 更新・CodeRabbit指摘2対応**）: `$WF_GITIGNORE_SOURCE`（パッケージ側）が存在しない場合（パッケージが壊れている・配布物からの欠落等の異常系）は、`cp` を実行せず**かつ stderr へ明示的な警告を出力したうえで**処理を継続する（無言スキップにしない。当初計画では既存の `runtime/templates` ブロックの `[[ -d "$WF_SOURCE" ]]` ガードと同じ無言スキップ方針を想定していたが、本 issue 自体が配布漏れを扱うため同種の回帰が無言で再発しうるとの指摘を受け、02_設計.md §3.1.3/§3.1.5 のとおり是正した。詳細は §2.4.6 参照）。
...
#### 2.4.6 追記（2026-07-13・CodeRabbit指摘2対応・PR #33）

- **概要**: `$WF_GITIGNORE_SOURCE`（`runtime-gitignore.template`）が万一パッケージから欠落していた場合、当初の実装は無言でスキップしていた。本 issue 自体が「配布物からファイルが漏れる」問題を扱っているため、同じ回帰が無言で再発しうるという CodeRabbit（PR #33）の指摘を受け対応した。
- **実装内容**: `setup.sh` の既存エラーハンドリング方針を調査した結果、(a) `AGENTS_SOURCE` 欠落は即 `exit 1`（致命的）、(b) `runtime/templates` 欠落は無言スキップ、(c) `enforcement/claude`/`enforcement/cursor` 欠落は非致命の note（stdout）、(d) `workflow.db` の由来不明は `workflow-db-guard.sh` による 3 要素警告（stderr・非致命）、の 4 パターンが既存することを確認した。`runtime-gitignore.template` は「無くても setup 全体は継続可能」という点で (c)/(d) に近いため、**非致命・stderr 明示警告**（(d) の 3 要素書式を踏襲）を採用し、`exit` による中止化はしない（既存の「軽量警告・非中止」方針との整合を優先）。具体コードは 02_設計.md §3.1.3/§3.1.5 参照。
- **テスト**: `test/e2e-install-uninstall.sh` に新規シナリオ `test_gitignore_template_missing_warns`（N2c）を追加し、(1) テンプレート欠落時も `setup.sh` が exit 0 で継続すること、(2) stderr に警告が出ること、(3) `runtime/.gitignore` は配布されないこと、(4) 他の配備処理（`source/` 等）には影響しないこと、を検証する。
...
- `test/e2e-install-uninstall.sh`: 既存 N1 シナリオへのアサーション追加＋新規シナリオ `test_gitignore_not_overwritten_on_reinstall`（§2.1.4）。既存 N1〜N7・R1〜R3 の回帰確認を implement-feature 実施直後に実行する。ADR-4 適用後は N1 にコピー元テンプレート配備・配布内容の確認を追加（§2.4.3）。CodeRabbit指摘2対応で新規シナリオ `test_gitignore_template_missing_warns`（§2.4.6）を追加。
...
### 3.1 マイルストーン（2026-07-13 更新: タスク4・CodeRabbit指摘6対応）
...
| **タスク 4（ADR-4）実装完了** | verify-and-close での指摘 1 発覚後 | `.agent-skill-chain/source/runtime-gitignore.template` 新設・`setup.sh` コピー元変更・`package.json`/`verify-npm-pack.sh` 修正、実 `npm pack` tarball 検証（`tar -tzf`）・packed tarball 展開による end-to-end 検証が PASS |

### 3.2 実装フェーズ

#### フェーズ 1: review-docs（02/03 対象）（2026-07-13 更新: CodeRabbit指摘7対応・詳細化）

- **期間**: 実装着手前（design-feature 完了直後・必須ゲート）
- **タスク**: 02_設計.md・03_実装計画.md のレビューと修正の反復。具体手順は以下のとおり。
  1. レビュワー（review-docs 委譲サブ）が 02_設計.md・03_実装計画.md を通読し、指摘事項を洗い出す。
  2. 指摘内容を `memo/` 配下に **日時プレフィックス付き**（`YYYYMMDD_HHMMSS_` 形式。取得手順は `.agent-skill-chain/source/scripts/new-workflow-memo.sh` 等の専用経路。手動でシステム時刻以外を用いない）のレビュー memo として記録する。
  3. 指摘があれば 02_設計.md・03_実装計画.md を修正し、再度レビューする。**指摘 0 件になるまで 1〜3 を反復**する。
  4. 指摘 0 件確認後、書記（write-workflow-log）に委譲し、review-docs 完了を workflow.db に記録させる（DOCUMENT_ID/DOCUMENT_PATH は 02・03 それぞれについて 1 回ずつ記録。実際の記録は本 issue の workflow.db 参照）。
  5. 完了後、implement-feature へ進む（本ゲートを経ずに実装着手すると enforcement #32 で FAIL する。[review-docs command](../../../../../.agent-skill-chain/source/commands/review-docs.md) 参照）。
...
#### フェーズ 3: npm 配布制約への対応（タスク 4・ADR-4。2026-07-13 追加・CodeRabbit指摘6対応）

- **期間**: verify-and-close フェーズで実配布経路の検証により指摘1が発覚した後
- **タスク**: `.agent-skill-chain/source/runtime-gitignore.template` の新設、`setup.sh` のコピー元変更、`package.json`・`verify-npm-pack.sh` の修正（§2.4 参照）。完了条件は実 `npm pack` tarball 検証（`tar -tzf` での内容物確認）・packed tarball 展開による `setup.sh` end-to-end 実行検証の両方が PASS すること。
