...
**（2026-07-13 更新: ADR-4 反映後の最終スコープ）** 本 issue は当初、パッケージ配布ロジック（`setup.sh`）・ドキュメント（`SETUP.md`）・enforcement 正本（`PreToolUse.sh`）の 3 ファイルへの**局所的な追記**として設計された。しかし実装完了後の verify-and-close フェーズで、`package.json` の `files` allowlist に単純追加するだけでは npm-packlist の仕様上 `.gitignore` という名前のファイルが配布されないことが実機検証で判明し（§2.5 ADR-4）、対応として新規ファイル `.agent-skill-chain/source/runtime-gitignore.template`（コピー元テンプレート）を追加し、`package.json`（`files` 配列から実効性の無いエントリを削除）・`.agent-skill-chain/source/scripts/verify-npm-pack.sh`（必須物検査への追加）の 2 ファイルを追加変更した。最終的な変更対象は **setup.sh・SETUP.md・PreToolUse.sh・package.json・verify-npm-pack.sh の 5 ファイル変更＋ runtime-gitignore.template の新規追加 1 件**である。新規コンポーネントの導入や既存アーキテクチャの変更は伴わない（`package.json`・`verify-npm-pack.sh` への変更も、既存の「配布物 allowlist」「配布物検査」という既存責務への整合化であり新規責務の追加ではない）。既存の「未存在時のみコピー」パターン（`init_workflow_db` 等）と「毎回上書き」パターン（`runtime/templates`）が併存する `setup.sh` の設計に、`runtime/.gitignore` 用の**未存在時のみコピー**ブロックを 1 つ追加する。`PreToolUse.sh` R1 には、既存の「全 ROLE 一律禁止」の構造を壊さない**狭い例外**を 1 条件だけ追加する。

### 1.2 設計原則（本プロジェクトで適用するもの）

- **単一責務**: 適用する。`setup.sh`＝配布ロジック、`SETUP.md`＝配布契約の文書化、`PreToolUse.sh`＝実行時強制、`package.json`＝配布物 allowlist の単一情報源、`verify-npm-pack.sh`＝配布物検査、という既存の責務分担を維持したまま、各ファイルに 1 責務分のみ追記する（§2.5 ADR-2・ADR-4）。
...
- **明確な境界（npm 配布）**: 適用する。`runtime-gitignore.template` は「`.agent-skill-chain/source/` 配下＝パッケージ配布物」という既存境界に乗せ、`.gitignore` という名前そのもので配布しようとした結果 npm-packlist の制約に阻まれた反省を踏まえ、配布境界を跨ぐファイル追加時は必ず `package.json`/`verify-npm-pack.sh` との整合を確認するという教訓を明記する（§2.5 ADR-4）。
- 他の原則（CQRS・イベント駆動・AI フレンドリー設計の詳細観点・UNIX 哲学の分割志向）は、本 issue が既存ファイルへの小規模追記でありコンポーネント分割・イベント発行・状態変更/取得の分離を伴わないため**該当なし**。
...
#### 3.1.3 追加ブロック（L125 の `fi` の直後・L127 コメントの前に挿入。ADR-4・CodeRabbit指摘2対応 反映済み・最終実装）
...
if [[ -f "$WF_GITIGNORE_SOURCE" ]]; then
  ...
  # コピー元テンプレートが見つからない（本 issue が修正した配布漏れと同種の回帰の可能性）。
  # 処理は中止せず（enforcement/claude・enforcement/cursor 欠落時と同じ非致命方針）、
  # workflow-db-guard.sh の 3 要素警告書式を踏襲して stderr へ明示的に警告する
  # （無言スキップにしない。CodeRabbit指摘2・PR#33）。
  {
...
- **出力**: 未存在時は配布された `.gitignore`（内容 `workflow.db*`。コピー元テンプレートからリネームされる）、既存時は何も変更しない。コピー元テンプレート自体が無い場合は stderr へ警告を出力し、`runtime/.gitignore` は配布されない（CodeRabbit指摘2対応）。

#### 3.1.5 エラーハンドリング

- `cp` 失敗時は `setup.sh` 全体が `set -e`（既存スクリプトの前提。実装計画で `set` オプションを確認する）等の既存エラーハンドリング方針に従う。本ブロック単体で新規のエラーハンドリング機構は導入しない。
- `$WF_GITIGNORE_SOURCE` が存在しない異常系（パッケージ自体が壊れている・配布物からの欠落等）は、**無言スキップではなく stderr への明示的な警告**を出したうえで処理を継続する（CodeRabbit指摘2・2026-07-13 追記）。当初は既存の `runtime/templates` ブロックの `if [[ -d "$WF_SOURCE" ]]; then`（無言スキップ）と同じガード方針を踏襲していたが、本 issue自体が「配布物からファイルが漏れる」問題を扱っており同じ回帰が無言で再発しうるため、`enforcement/claude`/`enforcement/cursor` 欠落時の非致命ノート方針（既存 L187-207）と `workflow-db-guard.sh` の 3 要素警告書式（既存の唯一の stderr 明示警告の先例）を組み合わせ、**非致命・stderr 明示警告**に是正した。中止（`exit`）にしなかった理由は、他の配備処理（`source/`・スキル・DB 等）は独立して継続可能であり、1 ファイルの配布失敗のために setup 全体を失敗させることは既存の「軽量警告・非中止」方針（`workflow-db-guard.sh` 冒頭コメント）と整合しないため。
