...
- 導入配下の数字接頭辞を除去（ADR-3）: `### apm 経由（バンドル参照・複数ツール横断配布向け）`／`### Claude marketplace 経由`／`### ローカル配備`（見出し文言は【as-built・2026-07-13】02 ADR-2 補足のとおり一般利用者向け推奨導線を npx へ変更したことに合わせて更新。ADR-3 自体の決定＝数字接頭辞除去には影響なし）。
- **`## 導入（プロジェクトへ配備するとき）` 見出しは一字も変更しない**（アンカー保全）。
- §導入 lede に「npx 経由の GitHub 直接参照配備の具体的な手順は [CONTRIBUTING.md] に記載する」を追記。§入口と参照 表に CONTRIBUTING.md 行を追加（【as-built・2026-07-13】文言は上記推奨導線変更に合わせて更新。「開発者・自己拡張向け」という読者限定枠は npx が一般利用者向け推奨導線になったため外した）。
