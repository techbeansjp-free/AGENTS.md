...
### 1.2.5 追加スコープ（レビュー指摘・2026 年 07 月 13 日追記）

- **問題**: `README.md` は導入先プロジェクトへ**配布されない**（コピー対象は `AGENTS.md`・`CLAUDE.md`・`.agent-skill-chain/source/` のみ。[SETUP.md §コピー対象](../../../../.agent-skill-chain/source/SETUP.md#コピー対象パッケージルートからプロジェクトルートへ)）。そのため**既に導入済みのプロジェクト**からは README の「配備後の管理（CLI）」節を読む手段が実質無く、アップデート方法（`upgrade` の実行コマンド自体）を知る手段が無い。
- **現状分析**: 配布される [`.agent-skill-chain/source/SETUP.md`](../../../../.agent-skill-chain/source/SETUP.md) には `init`/`upgrade`/`uninstall`/`enforce` が「何を保持し何を上書きするか」という**契約**は詳しく記載されているが、実際に打つ**コマンド自体**（`npx github:techbeansjp-free/AGENTS.md upgrade` 等）は `uninstall` 以外は記載が無い。同じく配布される `AGENTS.md`・`CLAUDE.md` からも SETUP.md のコマンド節へ辿る導線が弱い。
- **追加対象**: (a) SETUP.md に `init`/`upgrade`/`enforce`/`uninstall` の実行コマンド自体を明記する節を追加する。(b) `AGENTS.md`・`CLAUDE.md` から当該節への導線を確認し、無ければ追加する。README.md への追記（①②③）とは別の追加対象（④）として扱う。
...
- **効果 4**: **既に導入済み**のプロジェクトの利用者が、配布物（SETUP.md・AGENTS.md・CLAUDE.md）だけを頼りに `init`/`upgrade`/`enforce`/`uninstall` の**実行コマンド自体**を把握できる（SETUP.md に実行コマンドの節が存在し、AGENTS.md・CLAUDE.md からその節へ導線があることを○/×で判定できる）。
...
| **④ 配布物からのアップデート方法可視化** | SETUP.md は保持・上書き**契約**は詳しいが実行**コマンド自体**（`init`/`upgrade`/`enforce`）が uninstall 以外記載なし。AGENTS.md/CLAUDE.md から導線が弱い | 本 README §配備後の管理（CLI）の実コマンド例 | SETUP.md に実行コマンド節を追加し、AGENTS.md/CLAUDE.md から導線を張る |
...
- README 以外の利用者向け文書（CONTRIBUTING.md 等）の**内容拡充**は対象外。ただし §1.2.5 の追加スコープ（④）に基づく **SETUP.md への実行コマンド節追加・AGENTS.md/CLAUDE.md からの導線追加**は、既存内容の拡充ではなく「配布物からアップデート手段を辿れるようにする」ための最小限の追加として本 issue の対象に含める（SETUP.md/AGENTS.md/CLAUDE.md の内容を全面的に書き直すことは対象外）。
...
- **配布される** SETUP.md に `init`/`upgrade`/`enforce`/`uninstall` の**実行コマンド自体**が記載されている（○/×）。
- **配布される** AGENTS.md・CLAUDE.md から SETUP.md の当該コマンド節へ辿れる導線がある（○/×）。
...
- 追加スコープ（④）の対象・材料正本: [`.agent-skill-chain/source/SETUP.md`](../../../../.agent-skill-chain/source/SETUP.md)、[`AGENTS.md`](../../../../AGENTS.md)、[`CLAUDE.md`](../../../../CLAUDE.md)（いずれも導入先へ配布される）
