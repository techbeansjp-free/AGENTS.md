...
### 実機再現検証（追加調査・evidence_source: reproduction）

静的読解のみでは机上の仮説にとどまるため、`.agent-skill-chain/project/自己拡張ワークフロー.md` §テストの tmp 隔離 に従い、`mktemp -d` で作成した隔離環境内で実機再現を行った（本リポの `.claude/` `.cursor/` `.agent-skill-chain/runtime/` 等は一切変更していない）。

**手順**:

1. `mktemp -d` で隔離ディレクトリを作成し、`git archive HEAD | tar -x` で本ブランチのコミット済み内容をクリーンコピーした（`package/` として展開）。
2. 別途 `consumer/` を隔離ディレクトリ内に新規 git リポジトリとして作成し、消費者リポジトリを模擬した。
3. `bash package/.agent-skill-chain/source/scripts/setup.sh consumer`（初回 init 相当）を実行し、`.claude/skills/agent/SKILL.md`（パッケージ本体の run-command スキル）が配備されることを確認した。
4. `.claude/skills/agent/` に、本パッケージと無関係な自作スキルを模した `unrelated-marker.txt` と、内容を差し替えた独自の `SKILL.md`（`name: my-totally-unrelated-skill`）を追加し、「ユーザーが同名 `agent` で無関係スキルを既に置いていた」状態を再現した。
5. 同じ `setup.sh consumer` を再実行（upgrade 相当。setup.sh は init/upgrade で分岐しない単一脚本のため同一コマンドで代替可能）した。

**結果（setup.sh / bash 経路）**:

- 実行後、`.claude/skills/agent/` の中身は `unrelated-marker.txt` を含め完全に消え、パッケージ本体の `SKILL.md`（`name: run-command`）のみに置き換わっていた。`test -f .../unrelated-marker.txt` は失敗（DELETED）。
- 事前警告・確認プロンプトは一切表示されなかった。

**結果（TS 版 `uninstall` 経路）**:

- 隔離環境の `package/` で `npm install && npm run build` を実行し、`bin/agents-md.js` をビルドした。
- `consumer/.claude/skills/agent/` に再度 `unrelated-marker2.txt` を追加した状態で `node bin/agents-md.js uninstall`（dry-run）を実行したところ、削除対象一覧に `[dir] .claude/skills/agent` と `[dir] .cursor/skills/agent` が（他のパッケージ所有エントリと区別なく）列挙された。
- `node bin/agents-md.js uninstall --yes` を実行すると、`.claude/skills/agent` ディレクトリごと削除され、`unrelated-marker2.txt` は失われた（DELETED）。

**結論**: 机上の仮説どおり、`setup.sh`（init/upgrade）・TS 版 `uninstall` の両経路で、`.claude/skills/agent/`（および `.cursor/skills/agent/`）に同居する無関係な自作スキルの中身が、警告なく完全に削除されることを実機で確認した。**evidence_source: reproduction**（本項目全体。隔離環境での実行ログにより裏付け）。検証後、隔離ディレクトリ（`mktemp -d` で作成したもの）は破棄し、本リポジトリの実ファイルには一切影響していない。
