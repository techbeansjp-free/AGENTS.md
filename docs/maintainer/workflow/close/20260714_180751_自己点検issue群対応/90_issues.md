...
| 20260714_163235_scribeロール検証unenforced問題 | AGENT_ROLE自己申告によるscribeなりすましを防止するか判断する（再評価の結果、却下・対応不要。既存nonce機構で実装済み） | 中 | 却下（対応不要） | [詳細](./90_issues/20260714_163235_scribeロール検証unenforced問題/00_要求定義.md) |
...
| 20260714_163333_ゲート無効化envによる自己バイパス問題 | ゲート無効化envの自己バイパスリスクを低減するか判断する（PR #105 でマージ済み） | 中 | 完了 | [詳細](./90_issues/20260714_163333_ゲート無効化envによる自己バイパス問題/00_要求定義.md) |
...
| 20260714_163408_オンボーディング読了義務過大 | オンボーディング読了義務の分量と検証手段の不在を見直す（PR #109 でマージ済み） | 低 | 完了 | [詳細](./90_issues/20260714_163408_オンボーディング読了義務過大/00_要求定義.md) |
...
| 20260714_163639_templates配置がruntime名前空間同居 | 配布物であるテンプレートの配置場所と runtime/ 名前空間規約の整合を見直す（PR #106 でマージ済み） | 低 | 完了 | [詳細](./90_issues/20260714_163639_templates配置がruntime名前空間同居/00_要求定義.md) |
| 20260714_163640_リンク切れ大量発生の是正 | 正本ドキュメント間の相対リンク切れを解消し、参照網の整合を保つ（PR #86 でマージ済み） | 中 | 完了 | [詳細](./90_issues/20260714_163640_リンク切れ大量発生の是正/00_要求定義.md) |
...
| 20260714_163956_issue分類優先度過剰複雑 | 依頼分類・優先度規則の曖昧ケースにおけるダブルバインドを解消する（PR #104 でマージ済み） | 低 | 完了 | [詳細](./90_issues/20260714_163956_issue分類優先度過剰複雑/00_要求定義.md) |
...
| 20260714_164050_行番号相互参照の脆弱性是正 | 行番号（`:18` `:33` `:57` 等）による正本間の相互参照をアンカー参照へ統一し脆弱性を解消する（PR #86 でマージ済み） | 中 | 完了 | [詳細](./90_issues/20260714_164050_行番号相互参照の脆弱性是正/00_要求定義.md) |
...
**全件対応完了（2026-07-14〜07-15実施。様子見 5 件は再評価トリガー条件成立につき 2026-07-15 に再評価済み）**: 「対応する」32 件すべてが main へマージ済み、残り 3 件は却下（対応不要）。

全 35 件のうち、内訳は次のとおり。

- **対応する**: 32 件（うち実装作業は統合により実働 21 worktree＋再評価分 4 件に集約）
- **様子見**: 0 件（再評価トリガー条件成立により全件再評価済み。163333・163408・163639・163956 は対応完了、163235 は却下へ移行）
- **却下（対応不要）**: 3 件（163525・163601 は前提解消済みのため close 移動のみで完結、163235 は再評価の結果として対応不要と判断）
- **完了**: 32 / 35（バッチ1・7並列＋バッチ2・4並列＋バッチ3・4並列＋バッチ4・3並列＋バッチ5・1並列＋バッチ6・1並列＋バッチ7・1 worktree、および様子見5件の再評価（2026-07-15）が全て mainへマージ済み。統合により実働 21 worktree、issue 単位では 32 件: 163129, 163203, 163206, 163252, 163305, 163330, 163333, 163403, 163408, 163432, 163447, 163502, 163531, 163600, 163629, 163639, 163640, 163717, 163724, 163756, 163810, 163833, 163852, 163913, 163931, 163956, 164009, 164034, 164050, 164128, 164206, 164245 / PR #50, #51, #52, #53, #54, #55, #57, #60, #61, #63, #64, #66, #67, #69, #70, #72, #74, #76, #81, #84, #86, #104, #105, #106, #109）
- **進行中**: 0 / 35
- **未着手**: 0 / 35（全件が完了または却下として解消済み）

**進捗状況の可視化**:

```mermaid
pie title 進捗状況（トリアージ区分）
    "対応する（実働21 worktree＋再評価4件）" : 32
    "却下（対応不要）" : 3
```

### 優先度別進捗

- **高優先度**: 7 / 7（163129, 163203, 163305, 163502, 163531, 163833, 164009。全件完了）
- **中優先度**: 16 件中 完了 15・却下 1（完了: 163206, 163252, 163330, 163333, 163403, 163432, 163600, 163629, 163640, 163724, 163852, 163913, 164034, 164050, 164128／却下: 163235）
- **低優先度**: 10 / 10（完了: 163408, 163447, 163639, 163717, 163756, 163810, 163931, 163956, 164206, 164245）
- **却下（優先度なし）**: 2 / 2（163525, 163601）
...
| 4 | 163235_scribeロール検証unenforced問題 | 中 | 大 | 却下（判断確定・163235個別要求定義参照） | 再評価の結果、対応不要と判断。懸念の核心（自己申告に依存しないrole検証）は既存のnonce機構で既に実装済みであり、#163833（enforce配線既定on化・PR #76）により新規配備では既定onとなり実運用でも機能するため、本issue固有の追加対応は不要 | `platforms/claude/settings.enforce.json`, `source/enforcement/claude/PreToolUse.sh`, `scripts/write-workflow-log.sh` |
...
| 8 | 163333_ゲート無効化envによる自己バイパス問題 | 中 | 中 | 対応する（完了・PR #105） | #163833・#163203のenforcement再設計の帰結に依存 | `source/enforcement/ci/audit.sh:64-92`, `project/自己拡張ワークフロー.md` |
...
| 10 | 163408_オンボーディング読了義務過大 | 低 | 大 | 対応する（完了・PR #109） | 情報アーキテクチャ全体の再設計。README集約・#163206の帰結を見てから着手 | `AGENTS.md`, `source/boot/CORE.md`, `source/boot/LOAD_POLICY.md`, `enforcement/README.md` |
...
| 19 | 163639_templates配置がruntime名前空間同居 | 低 | 大 | 対応する（完了・PR #106） | 実害限定的。ディレクトリ移動は参照網全体に波及する破壊的変更 | `.agent-skill-chain/runtime/templates/`, `自己拡張ワークフロー.md`, `PreToolUse.sh`, `run_command.md` |
| 20 | 163640_リンク切れ大量発生の是正 | 中 | 大 | 対応する（#164050を統合・完了・PR #86） | 約97箇所のリンク切れは実害。全バッチの最後に単独実施 | `CORE.md`, `RULES.md`, `PHASES.md`, `GETTING_STARTED.md`, `CONTEXT_EFFICIENCY.md`, `project/*`, `skills/*` ほぼ全域 |
...
| 29 | 163956_issue分類優先度過剰複雑 | 低 | 中 | 対応する（完了・PR #104） | #22/#23未実装で機械強制されず実害未顕在 | `AGENTS.md:39,55-62` |
...
| 32 | 164050_行番号相互参照の脆弱性是正 | 中 | 中 | 対応する（#163640へ統合・完了・PR #86） | 行番号参照はサイレントにずれて誤誘導する | `CLAUDE.md`, `PHASES.md`, `project/自己拡張ワークフロー.md`, `run_command.md` |
...
- **バッチ7（最終・単独）**: 163640+164050統合 — 参照網全域の整合是正・CIリンク検査導入 — **完了**（PR #86 が main へマージ済み）

バッチNはバッチN-1の全PRマージ後に開始。着手前にユーザー判断が必要な3件: 163810（API障害時運用ドラフト確定）、163833（enforce配線方針）、163206（実装ゲート簡素化方針）。

### 却下・様子見の内訳

却下（対応不要・close移動のみ）: 163525（ルートworkflow.db追跡、前提解消済み）、163601（memo非追跡ルール、前提解消済み）、163235（scribeロール検証unenforced問題。2026-07-15に再評価トリガー条件成立につき再評価し、対応不要と判断。理由: 懸念の核心（自己申告に依存しないrole検証）は既存のnonce機構（commit d79545c）で既に実装済み。#163833（enforce配線既定on化、PR #76）により新規配備では既定onとなり実運用でも機能する。本リポジトリ自身がenforce offなのは別の意図的判断であり、本issue固有の追加対応は不要）。

様子見: 0 件（2026-07-15、再評価トリガー条件成立につき残り5件をすべて再評価済み。163333・163408・163639・163956 は対応完了（それぞれ PR #105・#109・#106・#104）、163235 は却下へ移行。詳細は上記却下欄および Issue 一覧表を参照）。
