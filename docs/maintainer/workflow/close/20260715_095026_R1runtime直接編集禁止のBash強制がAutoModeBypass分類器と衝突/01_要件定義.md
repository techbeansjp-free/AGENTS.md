...
- **symlink/hardlink 実体すり替え耐性（CodeRabbit PR#92 指摘対応）**: carve-out の allowlist 判定は文字列 basename だけでなく `realpath` 実体解決を経ること。doc 名（例 `00_要求定義.md`）の symlink/hardlink が実体として `memo/` 配下・`workflow.db*` を指す場合は、たとえ善意の通常 Edit/Write であっても保護対象を破壊しないよう block されること（権限昇格ではなく事故防止として要求する）。**受け入れ基準**: (1) doc 名の symlink が `workflow.db`/`memo/` を指す Write/Edit は exit 2、(2) doc 名のハードリンクが `workflow.db` と inode 共有する場合は exit 2、(3) 実在する通常 doc ファイル・未作成の新規 doc・非保護 doc を指す symlink は従来どおり allow（exit 0・過剰 block しない）。`test/test-pretooluse-hook.sh` の UC14 で jq/nojq 両系統を検証する。
