...
#### ADR-4: carve-out は basename 一致後に realpath 実体解決で symlink/hardlink すり替えを block する（CodeRabbit PR#92 指摘対応）

- **コンテキスト**: ADR-2 で採用した「basename 厳密一致 + `/memo/` 非包含」判定は、`file_path` の**文字列**のみを見て `realpath` 解決を行わない。CodeRabbit（PR#92 自動レビュー）が、doc 名（例 `00_要求定義.md`）の **symlink/hardlink** が実体として `memo/` 配下や `workflow.db*` を指す場合、basename だけでは実体を見抜けず carve-out を通過してしまう、と指摘した。
- **前回判断の誤り（正直化）**: 初版レビュー（04_review §12.1 の旧記述）は「symlink 作成には Bash が必要で、Bash を持つ非 scribe subagent worker は既に Bash 経由で memo・workflow.db へ直接書込可能だから新規の権限昇格は生じない」として**コード変更せず残存リスク受容**と結論づけた。しかしこれは評価軸が的外れだった。R1 の本来の目的は「攻撃者による権限昇格の防止」だけでなく「memo タイムスタンプ整合性・workflow.db 書込整合性の**事故防止**」であり、**事前に（過去のコミット・操作等で）仕込まれた symlink** が存在する場合、Bash 権限の有無に関わらず、**善意の通常の Edit/Write が気づかず実体（memo/・workflow.db）を破壊しうる**。この事故シナリオは前回評価が見落としていたため、コードで対応する方針へ是正する。
- **検討した選択肢**: (1) realpath 実体解決を basename 一致後に追加し、解決先が `/memo/` を含む・`workflow.db*` に一致する場合 block（採用）。(2) 残存リスク受容のまま文書化のみ（前回方針・却下）。(3) symlink を一律禁止（正当な symlink 運用まで壊すため却下）。
- **決定**: 選択肢 1。R5（scribe 経路の `write-workflow-log.sh` 単独実行判定）が既に用いている `norm_path()`（`realpath`→`readlink -f`→`cd+pwd` フォールバック）と同型の実体解決を、R1 carve-out にも適用する（defense-in-depth の一貫性）。
- **実装方針（03_実装計画 タスク6 に対応）**:
  - basename allowlist 一致かつ `/memo/` 非包含（文字列）を満たした**後**に、対象パスを `r1_norm_path()` で実体解決する。Edit/Write の対象は「未作成の新規ファイル（Write）」と「既存 symlink」の双方があるため、**欠損許容フラグを優先**する（`realpath -m` → `readlink -f` → `realpath`）。純 bash の `cd+pwd` は末尾 symlink を解決できないため採らず、その限界は下記 `-L` チェックで補償する。
  - 解決後の実体パスが (a) `/memo/` を含む、または (b) basename が `workflow.db*` に一致するなら block。
  - symlink が**実在するのに**解決できない場合（`-L` かつ解決結果が空）は、検証不能として**安全側で block**（fail-closed）。
  - hardlink は実体（リンク先）を持たず realpath でたどれないため、`stat` によるリンク数検査（`nlink>1`）で best-effort に検知して block する。
- **限界（is_sqlite3_invocation 等と同じく正直に明記）**: hardlink の相手側リンク名までは列挙できず、真に `workflow.db` 等を指すかまでは断定しない（通常の doc は `nlink==1` のため実運用の誤 block はほぼ起きないが、意図的にハードリンクされた正当な doc はごく稀に carve-out から外れて block される）。`realpath`/`readlink`/`stat` が全て無い環境では該当検査を省略する（真の防御は R2/R3 の role 軸・CI audit が多層で担う）。
- **ADR-1(c) 保護対象不変性の強化**: 本 ADR により、ADR-1 が掲げた「保護対象（memo・workflow.db\*）は変更しない」は、**basename 詐称（symlink/hardlink 経由）に対しても実体解決で維持される**ようになり、保護目的の中核がより堅牢になる。
- **帰結**: `PreToolUse.sh` に `r1_norm_path()`・`r1_has_extra_hardlink()` の 2 ヘルパを追加し、R1 carve-out 判定にフォールスルー方式を保ったまま実体解決の block 分岐を挿入する（03_実装計画 タスク6）。`test/test-pretooluse-hook.sh` に UC14（symlink/hardlink 実体すり替え耐性・jq/nojq 両系統）を追加する（タスク6）。`enforcement/DESIGN.md` の R1 carve-out 記述に symlink/hardlink 耐性を追記する。
