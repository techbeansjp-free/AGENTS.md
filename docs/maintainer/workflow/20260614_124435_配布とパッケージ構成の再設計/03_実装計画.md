...
| # | 未決事項 | 影響フェーズ | 確定方法 |
| --- | --- | --- | --- |
| 1 | **npm スコープ名／レジストリ**（`@techbeansjp-free/agents-md` 仮置き、public/npmjs 仮定） | 1,4 | publish 前にユーザー確定。確定まで dry-run で受け入れ |
| 2 | **marketplace ブランチ戦略**（案A: リリース時に CI が専用ブランチ/タグへ生成物 commit） | 4 | ブランチ名・タグ規約・トリガを確定 |
...
| 6 | **LICENSE/MIT 確定**（package.json は MIT 仮置き） | 1 | LICENSE ファイル追加で確定 |
