...
- **npm スコープ／レジストリ**: 未決。`@techbeansjp-free/agents-md` を仮置き、public（npmjs）を仮定（§未決事項）。
- **LICENSE**: MIT で確定。リポルートに `LICENSE`（MIT）を追加済み。著作権者は `techbeansjp-free`（暫定。法人正式名は要確認＝§未決事項#6）【判断】。
- **marketplace リリース（案A）の暫定既定**（フェーズ4 実装で確定。§9 未決#2 への回答）【判断】:
  ...
  - **CI**: [.github/workflows/release.yml](../../../../.github/workflows/release.yml)。タグ・`package.json`・`plugin.json` の version 一致検証 → build → 再生成 diff ゼロ → リリースブランチへ push。実 push は CI 上のみ。
...
| 1 npm 土台 | `node bin/agents-md.js version`=0.1.0 ／ `npm pack --dry-run` にリポ固有物が無い ／ README に npx 導線 | SC-2,3 ／ シナリオ5-1 | スコープ未確定で publish 不可。対策: dry-run までで受け入れ、publish は別タスク化 |
...
| 4 marketplace | version 一致検証 ／ リリース成果物の差分ゼロ ／ main に `.adapters` 未コミット | SC-1,2 ／ シナリオ3-2,4-1,7-4 | リリースブランチ運用が複雑化。対策: 案A 単一方式に固定、手順を README/CI に明文化 |
...
- **リスク**: npm スコープ/レジストリ・LICENSE 未確定で publish と marketplace 公開が滞留。
  - **対策**: フェーズ1 は dry-run まで、フェーズ4 は手順整備までを受け入れ範囲とし、確定後に publish を別タスク化。
...
| # | 未決事項 | 影響フェーズ | 確定方法 |
| --- | --- | --- | --- |
| 1 | **npm スコープ名／レジストリ**（`@techbeansjp-free/agents-md` 仮置き、public/npmjs 仮定） | 1,4 | publish 前にユーザー確定。確定まで dry-run で受け入れ。**未確定**（release.yml に publish step は含めない） |
| 2 | ~~**marketplace ブランチ戦略**~~ → **確定（暫定既定）**: トリガ＝タグ `v*`、リリースブランチ＝`release/marketplace`、version 同期＝`sync-version.sh`（正本 package.json）。[release.yml](../../../../.github/workflows/release.yml) で実装。運用上の最終承認はユーザー確認に回す | 4 | （確定済み・運用承認待ち） |
...
| 6 | ~~**LICENSE/MIT 確定**~~ → **確定**: リポルートに `LICENSE`（MIT）追加済み。著作権者 `techbeansjp-free`（暫定）。**法人正式名（例: TechBeans Inc. 等）はユーザー確認に回す** | 1 | （MIT 確定済み・著作権者の正式名のみ要確認） |
