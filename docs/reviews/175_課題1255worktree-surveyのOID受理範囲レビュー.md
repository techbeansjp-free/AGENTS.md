...
| 対象 | 実装 |
| ラウンド | 3 |
| H_impl | `ff785959c8dc68744d0279a890cdbbb812822785` |
| 比較基点 | `f54d3f739483e847c75bff673da120705ed5fc5d` |
| 対象SHA・文書ダイジェスト | `ff785959c8dc68744d0279a890cdbbb812822785` |
| 対象差分 | `f54d3f739483e847c75bff673da120705ed5fc5d..86a808c6f5fde7274ea672971036ea44ea5eabf7`、11 path。うち`dist/`配下2件は生成物として個別監査の対象外とし、配布影響は§8へ残す。**ラウンド3でdistの是正を前進commitしたためH_implが動いた** |
...
| 差分 | `f54d3f73..ff785959` | 11 path。うち生成dist 2件 | 既存コード |
...
| commit前candidate | `git diff --name-only f54d3f73 ff785959` | 11 path。うち生成dist 2件 | Git index |
...
- 既定branch追随を行った場合: **行った。** PR #1282（Issue #1211）とPR #1286（Issue #1285）のmerge後に既定branchが2度動き、PRが`CONFLICTING`になった。**rebaseではなくmergeで追随した。** rebaseはreview sessionのanchorを殺し、reanchorも`implementation-diff-changed`で正しく拒否するためである。衝突は`docs/specs/15_要件追跡/01_変更履歴.md`の1件で、双方がheader区切り直後へ1行を足していた。**両側の行を保存した。** **追随mergeによって比較基点とH_implが動いた。** 比較基点は追随先の既定branch tip`f54d3f739483e847c75bff673da120705ed5fc5d`へ、H_implは追随merge上の是正commit`ff785959c8dc68744d0279a890cdbbb812822785`へ移した。**追随merge自体をH_implにできない。** 既定branch側が更新した変更履歴の行を私の衝突解消が古い版と二重に残しており、その是正が実装側の差分になるためである。実装差分の内容は変わっていない。
