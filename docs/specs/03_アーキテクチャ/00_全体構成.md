...
**コンポーネントIDは上図のnode識別子である。** 連番を新設しない。IDが所有境界を名前で表すことは、`FR-001`のような連番を「どの境界づけられたコンテキストに属するかを表さない」として退けたIssue #930の判断に従う。

| コンポーネントID | 責務 | 入力 | 出力 | 許可依存 | 禁止依存 |
|---|---|---|---|---|---|
| `cli` | ドメイン規則を合成し、外部境界の観測結果を値として渡す | CLI引数、project choice、trusted policy | 終了値とJSON診断 | `domain`、`adapter`、`routing`、`provider_adapter`、`graph_domain` | ドメイン不変条件の再定義 |
| `domain` | 不変条件を所有する。`src/domain` | 値として渡された観測結果と設定 | 判定結果 | 同一層と、`src/lib`のうち`security`・`version`・`package-root`。**`process`と`atomic`は副作用を持ち、依存する module は限定する** | `adapter`層への依存 |
| `adapter` | GitHub外部プロセスとの境界。`src/adapters/github.ts` | CLIからの要求 | 観測結果 | 外部プロセス | ドメイン不変条件の判定 |
| `routing` | scopeごとのrole、provider、model、実行設定、identity独立性を決定的に解決する | project choice、trusted mapping、provider観測 | routing decision | `role_policy`、`routing_inputs` | `provider_adapter`への依存 |
| `role_policy` | 6 roleの許可path・操作・必要証拠と、implementerとreviewerの独立性を純関数で検証する | role設定と解決結果 | 独立性判定 | なし | 外部処理、authorityの追加 |
| `provider_adapter` | provider実行入口とのread-only境界。`src/adapters/provider.ts` | provider名 | 可用性観測 | 外部プロセス | routing判定そのもの |
| `graph_domain` | Semantic Graphの型、決定論的bounded BFS、Kahn法、iterative Tarjan法、Dijkstra | canonical snapshot | 探索結果 | `graph_port` | GraphQLite固有import、SQL、Cypher |
| `graph_port` | Graph storageの抽象境界 | Graph操作要求 | Graph操作の結果 | なし | 任意Cypher APIの公開、storage固有の問い合わせ形式の露出 |
| `graph_adapter` | GraphQLite v0.6.1によるstorage実装。**固定parameterized queryはこの層が所有する** | `graph_port`経由の要求 | 永続化結果と問い合わせ結果 | `graph_runtime` | Domainへの逆依存 |
| `host_adapter` | `asc-step`正本をhostの探索pathへ写し、mapping・hash・所有権を管理する | package正本のskill | 展開結果と診断 | `domain` | 外部CLIの直接呼び出し |
...
- Decision Contract: 有限選択判断（deterministic codeでもGenerative Modelの深い推論でもない判断）の入出力型契約（`src/domain/decision-contract.ts`、既存`domain`コンポーネントに属し新規コンポーネントIDは持たない）。値・confidence・呼び出し先（軽量tier／Jev等）を表現し、fail-open方向のauthorityを型として持たない。v0.4.1（Issue #1483）時点では型定義と候補一覧（file:line、採否、除外理由）だけを持ち、呼び出し先を実際に起動する実行ロジックは無い（依存辺0本、callerも無い）。routing/authorityへの実際の組込みはv0.4.2以降が行う。
