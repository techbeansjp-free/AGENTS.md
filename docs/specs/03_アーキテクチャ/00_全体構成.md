...
| `domain` | 不変条件を所有する純関数群。`src/domain` | 値として渡された観測結果と設定 | 判定結果 | 同一層の純関数と`src/lib` | `adapter`層への依存 |
...
| `graph_port` | Graph storageの抽象境界 | Graph操作要求 | 固定parameterized queryの結果 | なし | 任意Cypher APIの公開 |
| `graph_adapter` | GraphQLite v0.6.1によるstorage実装 | `graph_port`経由の要求 | 永続化結果 | `graph_runtime` | Domainへの逆依存 |
