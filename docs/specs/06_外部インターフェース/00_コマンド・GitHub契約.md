...
| merge | 既定branch上policy、branch保護、成功check、同じHEAD SHAの独立approval | 許可されたmethodで実行 | 直前に同じtrusted観測で再認可し、merged SHAと状態を再読取 |
...
| `trace validate` | project adapterが作成した`--evidence` JSONとproject choices | runner・file形式・表示言語を所有せず、stable ID、canonical step role、選択層、禁止file証拠を検証 |
...
`pr merge`はCLI flagをhuman approvalとして扱わない。GitHubからreviewのcommit SHAとactor、PR author、HEAD commit authorを取得し、現在のHEAD SHAに一致して両authorから独立したapprovalだけを数える。repository、SHA、branch protectionの観測欠落はfail-closedとし、適用直前にPR、check、review、protectionを再取得して同じ条件で再認可する。
