...
- `pull_request_target`はワークフローYAML自体・チェックアウトされるCLIソースを常にPRのbaseブランチから読むため、本修正のマージ後は、マージ前から存在する進行中のPR（既存issueブランチ）であっても、次のsynchronizeイベントから即座に新しい安全な構成が適用される。`workflow_run`分離案が抱えていた「マージ前から存在するブランチは新しいmain内容を取り込むまで旧構成が残存する」という移行ギャップは、本設計では構造的に発生しない。
