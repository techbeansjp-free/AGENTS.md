...
id: ADR-0019
status: proposed
title: gate-reportのhuman_requiredは確定値としてgate publish経由で詳細Check Runを発行し、救済分岐はリテラルpendingのみを対象とし、Publish Check Runのskip条件・reconcile()のconclusion導出是正を決定に含める
...
Issue #349のstrictレビュアによるblocking findingで、ADR-0018のDecision、およびそれを引き継いだ当初実装のアプローチに、以下の問題が指摘された。

1. **`human_required`をpending救済分岐へ倒すと詳細Check Runが失われること**: `.agent-skill-chain/schemas/gate-report.schema.yaml`の`final`定義が記すとおり、`human_required`は「レビューは完了したが判定不能（inconclusive・origin衝突等）で人間判断を要する」確定値であり、`pending`（レビュー未了）とは意味が全く異なる。`gate verify-evidence`（`src/commands/gate.ts`の`verifyGithubReviewEvidence()`、実運用でこのコマンドへ渡るgate-reportを生成する唯一の経路）は、`final`として`approved`/`rejected`/`human_required`の3値のみを返し、リテラル`pending`を返す経路を持たない（`pending`は`gate review`が生成する白紙スキャフォールド——`gate.ts`の`review()`関数——にのみ残る値である）。つまり実運用で到達する唯一の「未確定」値は`human_required`である。一方`gate publish`（`gate.ts`の`publish()`）はリテラル`final=pending`のみを拒否し、`human_required`は拒否せずexit 0で、title `<gate> gate: human_required`・summary（blockersの実値）・output.text（gate-report全体のcanonical JSON）を含む詳細なaction_required Check Runを発行できる。当初実装は`final === 'pending' || final === 'human_required'`の両方をexit 2（救済分岐）へ倒していたため、実運用の唯一の経路である`human_required`で、この詳細な発行が`Verify gate report schema`ステップの汎用Check Run（title `gate review pending`・固定summary）に置き換わって失われ、Check Runのみを読んで次を判断する進行役（AGENTS.md I6正準モデル）が「レビュー未了でまだ待つべき状態」と「レビュー完了・人間判断が必要な状態」（I8の人間判断への昇格対象）を区別できなくなっていた。
2. **Publish Check Runステップのskip条件**: ADR-0018のDecisionは「`Verify gate report schema`ステップがexit code 2を検出した場合のみaction_required Check Runを発行したうえでジョブ自体はexit 0で終了する」とのみ記述しており、同一ジョブ内の後続`Publish Check Run`ステップが二重発行や`gate publish`のリテラルpending拒否ガードによる失敗を避けるためにskipされる、という具体的な制御機構に触れていない。
...
`docs/adr/ADR-0018-gate-report-pending-as-action-required.md`を`supersedes: [ADR-0018]`として置き換え、以下を決定する。ADR-0018が確定させた基本構造（`otherErrors`/`pendingErrors`分離、`otherErrors`が1件でも存在すれば無条件exit 1、`otherErrors`が0件で`pendingErrors`が1件以上ならexit 2、exit 2時の`Verify gate report schema`ステップでのaction_required Check Run発行）は維持するが、`pendingErrors`の計上基準はADR-0018の「`conformance`/`falsification`/`final`のpending検出」からリテラル`final === 'pending'`のみへ狭め、`human_required`を救済分岐の対象から外す。

1. **`pendingErrors`計上基準はリテラル`pending`のみとし、`human_required`は確定値として通常のpublish経路に乗せる**: `verify gate-report`（`src/commands/verify.ts`の`gateReport()`）は、リテラル`report.gate.final === 'pending'`のみをpendingErrors計上のトリガー条件とする。`human_required`はpendingErrorsに計上せず、スキーマ違反・digest不一致・target_sha不正等の非pending違反が無ければexit 0で通過させる。これにより`final=human_required`のgate-reportは`verify-and-publish`ジョブの後続`Publish Check Run`ステップ（`gate publish`）へ通常どおり進み、`publish()`が拒否せずexit 0で、title `<gate> gate: human_required`・summary（blockersの実値）・output.text（gate-report全体のcanonical JSON）を含む詳細なaction_required Check Runを発行する。進行役はこの詳細Check Runにより「レビュー完了・人間判断が必要な状態」を「レビュー未了」と区別して読める（I6・I8）。リテラル`pending`分岐（exit 2）はコード上維持するが、`gate verify-evidence`が`pending`を返す経路を持たない以上、これは`gate review`が生成する白紙スキャフォールドに対する理論上の安全側フォールバックであり、`verify-and-publish`ジョブが検証する現実の入力では到達しない。`final === 'pending'`の場合のみ`conformance`/`falsification`個別のpendingチェックを併せて行う（白紙スキャフォールドは3フィールドとも同時にpending）。`final`が確定値（`approved`/`rejected`/`human_required`）の場合は、`conformance`/`falsification`が個別に`pending`のまま提出されていても`gateReport()`はその2フィールドを一切チェックしない（`gate.final`という単一の権威あるフィールドのみを判定基準とする）。
2. **Publish Check Runステップのskip条件**: `.github/workflows/agent-skill-chain-gate.yml`の`verify-and-publish`ジョブにおいて、`Verify gate report schema`ステップ（`id: schema`）はexit code 2（リテラル`final=pending`）を検出した場合、action_required Check Runを発行したうえで`echo "pending=true" >> "$GITHUB_OUTPUT"`してからexit 0する。exit code 0（`human_required`を含む確定値）の場合は`echo "pending=false" >> "$GITHUB_OUTPUT"`してから素通りする。後続の`Publish Check Run`ステップは`if: steps.schema.outputs.pending != 'true'`を条件に持ち、リテラルpending時のみこのステップ自体をskipする。これは、`Verify gate report schema`ステップが既にaction_required Check Runを発行済みであることに加え、`gate publish`（`src/commands/gate.ts`の`publish()`関数）が`report.gate.final === 'pending'`（真に未レビュー）を明示的に拒否し非0で終了する実装であるため、skipしないと二重発行・後続ジョブ失敗を招くことによる。`final=human_required`ではskipせず`gate publish`を実行する——救済分岐はこの場合Check Runを発行していないため二重発行は起きず、`publish()`はexit 0で終了するためジョブ失敗も起きない。
...
- ADR-0018が解決した効果（レビュー未確定であるだけの正常な状態のPRで`verify-and-publish`ジョブが恒常的にFAILUREになる問題の解消、`agent-skill-chain/<gate>-gate`のCheck Runがaction_requiredとして常に発行される）は引き続き有効である。加えて本ADRにより、実運用の未確定経路（`final=human_required`）ではそのaction_required Check Runが`gate publish`発の詳細情報付き（title `<gate> gate: human_required`・blockers・gate-report本文）となり、進行役が確定・未確定の別と人間判断の要否をCheck Runのみから読み取れる。
...
- `verify gate-report`の呼び出し元が増えた場合、終了コードの意味（0=final確定（approved/rejected/human_required）、1=違反、2=リテラル`final=pending`のみ）を踏襲する必要がある。特に「非0=失敗」と単純に扱うとexit 2（違反ではない）を誤動作させ、`human_required`をexit 2側と誤解して`gate publish`を経由させないと本ADRが是正した詳細Check Runの欠落を再発させる。
