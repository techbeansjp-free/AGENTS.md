...
- 欠点: `remediation_round`はゲート全体で共有される単一カウンタであり、`review:light`が要求される前から既に
  複数ラウンド進行していたIssue/PRでは、軽量プロファイル適用後に許容される「猶予1ラウンド」が実質的に縮小・
  消失しうる（`review:light`要求時点で`remediation_round`が既に1以上の場合、次のラウンドで即座に打ち切り
  条件が成立し得る）。ただしこれは打ち切り（`human_required`化）を早める安全側にのみ作用し、AC-8が要求する
  安全性（未解消のblockingを自動承認しない）を損なわない。カウンタを軽量レビュー開始時点に再アンカーする
  対応は本ADRのスコープでは行わない。
