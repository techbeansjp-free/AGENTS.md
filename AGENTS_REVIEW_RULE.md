...
レビュー時は、**関連するテストやコードだけでなく、プロジェクト全体のすべてのテストやコードを徹底的に調査する**ことが原則です。ただし、大規模プロジェクトや monorepo では、**スコープ定義のガードレール**に従って効率的に実施します。
...
1. **影響範囲の特定**:
   - `git diff` 起点で変更されたファイルとその依存関係を特定
   - 依存グラフを構築し、影響を受ける可能性のあるファイルを洗い出す
   - 重要ディレクトリ（認証/課金/権限/共通基盤）は**必ず全探索対象**とする

2. **テストの確認**:
   - **全テストは CI で担保**することを前提とする
   - AI は「失敗時の調査・原因特定」に重心を置く
   - CI で失敗したテストの原因を特定し、修正案を提示する

3. **コード品質の確認**:
   - 変更ファイルとその依存関係に対してリント・型チェックを実行
   - 重要ディレクトリ（認証/課金/権限/共通基盤）は**必ず全探索対象**とする
   - プロジェクト全体のリント・型チェック結果を確認し、新規エラーがないことを確認

4. **ドキュメントの整合性確認**:
   - 変更内容に関連するドキュメントの整合性を確認
   - 設計書と実装の不整合がないことを確認

**運用方針**:

- **原則**: プロジェクト全体を徹底的に調査する
- **現実的な運用**: `git diff` 起点の影響範囲 + 依存グラフ + 重要ディレクトリ（認証/課金/権限/共通基盤）を「全探索対象」とし、それ以外は影響範囲に限定
- **CI との役割分担**: 全テストは CI で担保、AI は失敗時の調査・原因特定に集中
...
1. **システム仕様書（`docs/`）の確認**
   - 実装内容がシステム仕様書に反映されているか確認する
   - 実装内容とシステム仕様書に矛盾がないか確認する
   - システム仕様書が最新の状態になっているか確認する
...
**詳細**: システム仕様書の更新チェックの詳細なルールは [`AGENTS_DOCS_RULES.md`](./AGENTS_DOCS_RULES.md) の「Issue 完了時のシステム仕様書更新チェック」セクションを参照。
...
- [ ] **システム仕様書（`docs/`）を確認した（必須）**
- [ ] 実装内容がシステム仕様書に反映されているか確認した
- [ ] 実装内容とシステム仕様書に矛盾がないか確認した
- [ ] システム仕様書の更新が必要か判断した
- [ ] 更新が必要な場合、該当するセクションを更新した
- [ ] 更新内容を `04_review.md` に記録した
- [ ] 更新履歴を `docs/README.md` に記録した
...
### 5.4 ドキュメント徹底レビュー（4.5）における最低回数

**重要**: 実装前の**ドキュメント作成後の徹底レビュー**（[`AGENTS.md`](./AGENTS.md) の「4.5 ドキュメント作成後の徹底レビューワークフロー」）を実施する場合は、**指摘事項がなくても最低 3 回**はレビューを実施すること。見落としがないか複数回確認し、3 回以上のレビューを実施した時点でドキュメント作成フェーズを完了とする。本ルール（`04_review.md` の実装レビュー）とは別のフェーズであるが、規約遵守のため忘れずに実施すること。
...
**最終更新**: 2026 年 2 月 9 日（レビュー規約にスコープ定義のガードレール追加、`git diff`起点の影響範囲+依存グラフ+重要ディレクトリを全探索対象とする運用方針を明確化、CIとの役割分担を明確化）
