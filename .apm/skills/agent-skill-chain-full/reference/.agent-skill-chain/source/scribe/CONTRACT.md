...
| review_path | TEXT | △（verify-and-close は必須） | 例: .agent-skill-chain/runtime/{issue}/04_review.md。 |
...
| 運用 | 保存先 | 備考 |
|------|--------|------|
| **workflow.db 採用（本則）** | プロジェクトで定めた 1 パス（推奨: .agent-skill-chain/runtime/workflow.db）。[ledger/README.md](../ledger/README.md) の配置に従う。 | 証跡の正本。採用可能なプロジェクトは必ずこちらを用いる。 |
| **memo のみ（過渡的・例外）** | .agent-skill-chain/runtime/{issue}/memo/ に YYYYMMDD_HHMMSS_*.md。**{issue} は YYYYMMDD_HHMMSS_ をプレフィックスとするフォルダ名（必須）。** 例: .agent-skill-chain/runtime/YYYYMMDD_HHMMSS_example-issue-name/memo/YYYYMMDD_HHMMSS_実施結果.md | workflow.db を採用しない場合の過渡的・例外運用。**memo はログの別経路ではなく、workflow.db を採用しない場合の一時的・移行用の思考メモである。** 第一の選択肢ではない。**非推奨。移行モード。将来廃止予定。** |
