...
#   両モードで有効（実効モードによる一括 SKIP はしない）。github_native は新規の非追跡ドラフトが実在する
#   ローカルで発火し、CI では新規非追跡ドラフト不在で no-op になるが、追跡済みの完了 issue（遡及分等）は
#   CI でも展開されるため発火しうる（下記関数本体コメント参照）。
...
check_close_move_pending() {
  ...
  # github_native の新規の非追跡ドラフトはローカル環境（pre-push・手動）で発火する。CI
  # （actions/checkout）では新規の非追跡ドラフトは展開されず走査対象不在＝no-op になるが、
  # 追跡済みの完了 issue（遡及分の #119/#127 等）は CI でも展開されるため発火しうる
  # （continue-on-error の非ブロッキング督促・T5 移動後に */close/* 除外で収束）。
  # resolve_issue_tracking_mode は #28 と共用のため参照しない・不変。
