...
1. **徹底的なレビューの実施**
   ...
   - **必須のレビュー観点**:
     ...
     - **規約・ルール準拠**: AGENTS 規約・[`実行ルール.md`](./.agents/実行ルール.md)・[`コーディングルール.md`](./.agents/コーディングルール.md)・[`ドキュメントルール.md`](./.agents/ドキュメントルール.md)・プロジェクト固有ルール（`.agents/unique-rule/`）に沿っているか
...
#### `99_PR.md` テンプレート（PR メッセージ作成）

**テンプレートファイル**: [`.workflow/templates/99_PR.md`](./.workflow/templates/99_PR.md)（実際のプロジェクトでは `.cursor_workflow/templates/99_PR.md` または `.workflow/templates/99_PR.md`）

issue/タスク完了後に PR メッセージを作成する際に使用。

**PR メッセージ作成の絶対ルール（参照パス）**:

- **内部への参照パスは設定しない。外部のみとする。**
- **理由**: 完了後に issue ディレクトリを別ディレクトリ（例: `.workflow/completed/`）に移動するため。内部パス（`.workflow/` 配下の相対パスやリポジトリ内ファイルへのリンク）を記載すると、移動後にリンクが壊れる。
- **許可する参照**: GitHub Issue/PR の URL、外部サイト（公式ドキュメント等）の URL のみ。リポジトリ内の `.workflow/` や `docs/` 等への相対パス・絶対パスは記載しない。

**必須セクション**: テンプレートの各セクションに沿って記載。参考資料は上記のとおり外部リンクのみとする。
...
- **参照パス確認は必須（重要）**: ドキュメント作成・更新時は、**必ずすべての参照パスが正しいか確認すること**
  - **確認タイミング**: ドキュメント作成時、更新時、レビュー時
...
- **日付の記載方法**:
  ...
  - 推測や記憶に基づく日付の記載は禁止
- **会話出力制限（会話ログ肥大化防止）**: 会話ログを増やさない設計に寄せ、Agent を短い"指令"＋短い"成果物"で回す
  ...
  - **長文の出力先**: 以下のファイルに出力する：
    ...
    - 実行結果サマリ: `docs/run/last_result.md`（200〜400字程度の要約のみ、オプショナル）
...
会話ログを増やさない設計に寄せるため、以下の成果物ファイルを運用することが推奨されます：

```
リポジトリルート/
├── .workflow/              # ワークフローファイル（必須）
└── docs/                   # 成果物ファイル（オプショナル）
    ├── tasks/              # タスク定義
    ├── run/                # 実行結果
    │   ├── last_result.md  # 直近の結果サマリ（200〜400字程度）
    │   └── logs/           # 詳細ログ（必要時のみ）
    │       └── YYYYMMDD_HHMMSS_*.md
    ├── decisions/          # 決定ログ（ADR形式など）
    ├── spec/               # 仕様
    └── review/             # レビュー詳細
        └── PR-xxx.md
```
...
- **会話はコマンドだけ**: 会話では「TASK-20260209-001 を実行して」のような短い指令のみ
- **成果物はファイルへ**: Agent は変更を Git 差分で、文章を `docs/run/last_result.md` に、詳細ログを `docs/run/logs/` に出力
- **会話への報告は最小限**: 完了/未完了、追加ファイル一覧、必要な判断（最大3つ）のみ
- **詳細はファイル参照**: 会話には要点だけを書き、詳細はファイルへのリンクを提供

**注意**: この運用はオプショナルです。プロジェクトの規模や運用方針に応じて、必要に応じて導入してください。
