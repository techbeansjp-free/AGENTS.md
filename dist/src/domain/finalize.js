...
export function planRootUpdate(input) {
    ...
    const reasons = [];
    const recovery = [];
    const reject = (reason, next) => {
        reasons.push(reason);
        if (!recovery.includes(next))
            recovery.push(next);
    };
    if (rootPath.trim() === "")
        reject("root worktreeのパスが空です", "対象repositoryのroot worktreeパスを指定して再実行する");
    if (currentBranch.trim() === "" ||
        defaultBranch.trim() === "" ||
        currentBranch !== defaultBranch)
        reject("root worktreeが検証済みの既定branchではありません", "root worktreeを既定branchへ戻し、branch名を確認してから再実行する");
    const untracked = value.untracked;
    if (value.dirty !== false ||
        !Array.isArray(untracked) ||
        untracked.some((item) => typeof item !== "string") ||
        untracked.length > 0)
        reject("root worktreeに変更または未追跡ファイルがあります", "変更をcommitまたはstashしてから再実行する");
    if (upstreamRef.trim() === "")
        reject("root worktreeのupstreamが不明です", "既定branchのupstreamを確認して設定してから再実行する");
    if (upstreamSha.trim() === "" ||
        remoteSha.trim() === "" ||
        upstreamSha !== remoteSha)
        reject("upstream SHAとremote SHAが一致しません", "originの既定branchをfetchし、remote同一性を再確認してから再実行する");
    if (!/^[a-f0-9]{40}$/iu.test(mergeSha) || remoteSha !== mergeSha)
        reject("検証済みmerge SHAが40桁Git SHAでないかremote SHAと一致しません", "PRのmerge結果をread-after-writeで再確認し、完全なmerge SHAで再実行する");
    const alreadyUpdated = /^[a-f0-9]{40}$/iu.test(mergeSha) && localSha === mergeSha;
    if (!alreadyUpdated && value.fastForwardable !== true)
        reject("root worktreeをmerge SHAへfast-forwardできません", "local mainの分岐を調査し、利用者の判断で復旧してから再実行する");
    return {
...
export function planWorktreeCleanup(input) {
    ...
    const targetPathValid = typeof targetPath === "string" && targetPath.trim() !== "";
    const targetValid = targetPathValid &&
        typeof targetBranch === "string" &&
        targetBranch.trim() !== "";
    if (!targetValid && input.targetAbsent !== true)
        reasons.push("対象PR専用worktreeのpathまたはbranchが不明です");
    if (!registeredValid)
        reasons.push("登録済みworktree一覧が不明です");
    if (input.targetAbsent !== undefined &&
    ...
    if (typeof input.trackedChanges === "boolean" &&
        typeof input.clean === "boolean" &&
        input.trackedChanges !== !input.clean)
        reasons.push(`追跡対象fileの観測が矛盾しています: trackedChanges=${String(input.trackedChanges)}、clean=${String(input.clean)}`);
    if (input.consumerAssets !== undefined &&
        !Array.isArray(input.consumerAssets))
        reasons.push("未追跡fileの種別が不明です");
    if (Array.isArray(input.untracked) &&
        Array.isArray(input.consumerAssets) &&
        (input.untracked.length !== input.consumerAssets.length ||
            input.untracked.some((entry, index) => entry !== input.consumerAssets?.[index])))
        reasons.push(`未追跡fileの観測が矛盾しています: untracked=${JSON.stringify(input.untracked)}、consumerAssets=${JSON.stringify(input.consumerAssets)}`);
    ...
    const exact = registered.filter((worktree) => worktree.path === targetPath && worktree.branch === targetBranch);
    if (typeof input.repositoryRoot === "string" && targetPathValid) {
        const relative = path.relative(path.resolve(input.repositoryRoot), path.resolve(targetPath));
        if (relative === "" ||
            relative === ".." ||
    ...
    if (typeof input.targetCanonicalPath === "string" &&
        targetPathValid &&
    ...
    if (input.targetAbsent === true) {
        ...
        if (registered.some((worktree) => worktree.path === targetPath))
            reasons.push("対象pathは別branchを含む登録済みworktreeとして残っています");
    ...
    if (exact.length !== 1)
        reasons.push("対象PR専用worktreeのpathとbranchに完全一致する登録が1件ではありません");
    const ignoredArtifacts = input.ignoredArtifacts;
    const untracked = input.untracked === undefined
        ? Array.isArray(input.consumerAssets)
            ? input.consumerAssets
            : undefined
        : input.untracked;
    const derivedUntracked = Array.isArray(untracked) && Array.isArray(ignoredArtifacts)
        ? untracked.concat(temporaryArtifacts.filter((artifact) => !ignoredArtifacts.includes(artifact)))
        : untracked;
    const safety = assessWorktreeRemovalSafety({
        repositoryRoot: input.repositoryRoot,
        worktreePath: input.repositoryRoot === undefined || !targetPathValid
            ? undefined
            : targetPath,
        trackedChanges: input.trackedChanges === undefined
            ? typeof input.clean === "boolean"
                ? !input.clean
                : undefined
        ...
        ignoredArtifacts,
        ignoredPathAllowlist: input.ignoredPathAllowlist === undefined
...
function completionPhase(phase, state, reasons = [], recovery = []) {
    return { phase, state, reasons, recovery };
}
function skippedCompletionPhases(phases, reason) {
    return phases.map((phase) => completionPhase(phase, "skipped", [reason]));
}
function validRootUpdatePlan(value) {
...
function validCleanupPlan(value) {
    return (isRecord(value) &&
        ["ready", "rejected", "already-absent"].includes(typeof value.state === "string" ? value.state : "") &&
...
export function planCompletion(input) {
    ...
    if (value.mergeConfirmed !== true || !/^[a-f0-9]{40}$/iu.test(mergeSha)) {
        const reason = value.mergeConfirmed !== true
            ? "対象PRのmerge確認が成立していません"
            : "検証済みmerge SHAが40桁Git SHAではありません";
        return {
            state: "rejected",
            phases: [
                completionPhase("merge-confirm", "rejected", [reason], ["GitHubで対象PRのmerge状態と完全なmerge SHAを再確認する"]),
                ...skippedCompletionPhases(COMPLETION_PHASES.slice(1), "merge確認が成立しないため実行しません"),
            ],
            requiredAuthority: [],
        };
    }
    if (!rootUpdate) {
        ...
        return {
            ...
            phases: [
                ...
                completionPhase("root-update", "rejected", [reason], ["root更新状態を再読取して完了previewを再実行する"]),
                ...skippedCompletionPhases(COMPLETION_PHASES.slice(2), "root更新計画が成立しないためcleanupへ進みません"),
            ],
            requiredAuthority: [],
        };
    }
    if (rootUpdate.state === "rejected") {
        return {
            ...
            phases: [
                completionPhase("merge-confirm", "succeeded"),
                completionPhase("root-update", "rejected", [...rootUpdate.reasons], [...rootUpdate.recovery]),
                ...skippedCompletionPhases(COMPLETION_PHASES.slice(2), "root更新が拒否されたためcleanupへ進みません"),
            ],
            requiredAuthority: [],
        };
    }
    if (rootUpdate.to.toLowerCase() !== mergeSha.toLowerCase()) {
        ...
        return {
            ...
            phases: [
                completionPhase("merge-confirm", "succeeded"),
                completionPhase("root-update", "rejected", [reason], ["root更新状態とmerge SHAを再読取して完了previewを再実行する"]),
                ...skippedCompletionPhases(COMPLETION_PHASES.slice(2), "root更新の到達SHAが一致しないためcleanupへ進みません"),
            ],
            requiredAuthority: [],
        };
    }
    if (!cleanup || cleanup.state === "rejected") {
        ...
        return {
            ...
            phases: [
                ...
                completionPhase("cleanup-preview", "rejected", [...reasons], ["対象worktreeの安全条件を再読取してcleanup previewを再実行する"]),
                ...skippedCompletionPhases(COMPLETION_PHASES.slice(3), "cleanup previewが成立しないため適用しません"),
            ],
            requiredAuthority: [],
        };
    }
    ...
    if (value.cleanupAuthorityGranted !== true) {
        return {
            ...
            phases: [
                ...
                completionPhase("root-update", "succeeded"),
                completionPhase("cleanup-preview", "succeeded"),
                completionPhase("cleanup-apply", "pending", ["worktree cleanup operationの明示authorityがありません"], [
    ...
    const previewDigest = typeof value.previewDigest === "string" ? value.previewDigest : "";
    const approvedDigest = typeof value.approvedDigest === "string" ? value.approvedDigest : "";
    if (!/^[a-f0-9]{64}$/u.test(previewDigest) ||
        ...
        return {
            ...
            phases: [
                ...
                completionPhase("root-update", "succeeded"),
                completionPhase("cleanup-preview", "succeeded"),
                completionPhase("cleanup-apply", "rejected", ["承認済みdigestが最新cleanup previewの64桁digestと一致しません"], [
                    "worktree finalize --completeをpreviewで再実行し、新しいpreview digestを確認する",
                    "確認したpreview digestを--approved-digest=<preview digest>として渡す",
                ]),
                completionPhase("post-verify", "skipped"),
            ],
            requiredAuthority: [],
...
function isCompletionPhaseState(value) {
    return (value === "succeeded" ||
        value === "rejected" ||
        value === "pending" ||
        value === "skipped");
}
function validCompletionPhase(value) {
    return (isRecord(value) &&
...
export function summarizeCompletion(input) {
    const value = isRecord(input) ? input : {};
    ...
    if (phases.length !== COMPLETION_PHASES.length ||
        ...
        !["removed", "retained", "absent"].includes(typeof postVerify.containerState === "string"
    ...
    if (!postVerify.targetPathAbsent)
        recovery.push("対象worktreeを保持したままcleanup previewとauthorityを確認して再実行する");
    if (!postVerify.otherWorktreesUnchanged)
        recovery.push("他worktree一覧を直前snapshotと照合し、差分を復旧してから再実行する");
    const completed = phases
        .filter((phase) => phase.state === "succeeded" && phase.phase !== "post-verify")
        .map((phase) => phase.phase);
    const verificationFailed = postVerify.rootSha !== postVerify.expectedRootSha ||
        !postVerify.targetPathAbsent ||
        !postVerify.otherWorktreesUnchanged;
    const postVerifySucceeded = phases.some((phase) => phase.phase === "post-verify" && phase.state === "succeeded");
    if (!verificationFailed && postVerifySucceeded)
        completed.push("post-verify");
    const pending = COMPLETION_PHASES.filter((phase) => !completed.includes(phase));
    const rejectedPhase = phases.some((phase) => phase.state === "rejected");
    const pendingPhase = phases.some((phase) => phase.state === "pending");
    const rootSucceeded = phases.some((phase) => phase.phase === "root-update" && phase.state === "succeeded");
    const requiredPlanningSucceeded = [
        "merge-confirm",
        "root-update",
        "cleanup-preview",
    ].every((required) => phases.some((phase) => phase.phase === required && phase.state === "succeeded"));
    if (!requiredPlanningSucceeded)
        recovery.push("merge確認、root更新、cleanup previewの順序を再確認して完了フローを再実行する");
    const cleanupAppliedOrAbsent = phases.some((phase) => phase.phase === "cleanup-apply" &&
        (phase.state === "succeeded" ||
            (phase.state === "skipped" && postVerify.targetPathAbsent === true)));
    const incompleteOutcome = !requiredPlanningSucceeded ||
        !cleanupAppliedOrAbsent ||
        !postVerifySucceeded;
    const state = verificationFailed ||
        pendingPhase ||
...
export function buildFinalizeReport(state) {
    ...
    const derivedUntracked = Array.isArray(state.untracked) &&
        Array.isArray(state.temporaryArtifacts) &&
        Array.isArray(ignoredArtifacts)
        ? [
            ...state.untracked,
            ...state.temporaryArtifacts.filter((artifact) => !ignoredArtifacts.includes(artifact)),
        ]
        : state.untracked;
    ...
    const safety = assessWorktreeRemovalSafety({
        ...
        worktreePath: state.repositoryRoot === undefined ? undefined : state.worktree,
        trackedChanges: state.dirty === undefined ? undefined : state.dirty === true,
        untracked: derivedUntracked,
        ignoredArtifacts,
        ignoredPathAllowlist: state.ignoredPathAllowlist === undefined
            ? resolveFinalizeIgnoredPathAllowlist()
    ...
    const snapshot = structuredClone({
        ...state,
        ignoredPathAllowlist: state.ignoredPathAllowlist ?? resolveFinalizeIgnoredPathAllowlist(),
    });
    const hash = crypto
        .createHash("sha256")
...
export function applyFinalize(input, destructive) {
    ...
    if (!boundary.allowed)
        throw new Error(`${boundary.diagnostic?.ruleId ?? "ASC-FINALIZE"}: ${boundary.diagnostic?.reasons.join("; ") ?? "boundary違反"}`);
    if (!input.report.safe)
        throw new Error(`安全でないため完了処理を拒否しました: ${input.report.reasons.join("; ")}`);
    if (!/^[a-f0-9]{64}$/.test(input.approvedHash) ||
