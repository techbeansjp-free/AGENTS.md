...
const CURRENT_TRACKER = /^https:\/\/github\.com\/[^/\s]+\/[^/\s]+\/issues\/[1-9]\d*$/u;
 ...
 */
export function withStagingMutationLock(directory, mutate) {
    ...
    const lockPath = path.join(path.dirname(staging), `.${path.basename(staging)}.mutation.lock`);
    if (heldMutationLocks.has(lockPath))
        return mutate();
    ...
    for (let attempt = 0; attempt < 2 && !acquired; attempt += 1) {
        const temporary = path.join(path.dirname(staging), `.${path.basename(staging)}.mutation-${process.pid}-${nonce}.tmp`);
        ...
        const value = parseJsonStrict(fs.readFileSync(lockPath, "utf8"), "staging writer lock");
        if (!isRecord(value) ||
        ...
        if (processIsAlive(value.pid))
            throw new Error(`別process（pid=${String(value.pid)}）がstagingを更新中です`);
        try {
    ...
    if (!acquired)
        throw new Error("staging writer lockを取得できませんでした");
        ...
        if (current !== source)
            throw new Error("staging writer lockの所有権が実行中に変化しました", {
                ...(!mutationFailed
                    ? {}
                    : {
                        cause: mutationError instanceof Error
                            ? mutationError
...
function canonicalRepositoryRoot(inputRoot) {
    if (typeof inputRoot !== "string" ||
        inputRoot.trim() === "" ||
        hasUnsafeMeta(inputRoot) ||
        hasParentReference(inputRoot))
        throw new Error("rootにはglob、制御文字、tilde、親参照を含まない明示pathが必要です");
    ...
    const topLevel = git(["rev-parse", "--show-toplevel"], realRoot).stdout.trim();
...
function stagingRootBoundary(root) {
    let current = root;
...
function inventory(directory) {
    ...
    const artifacts = [];
    const unsafe = [];
    const visit = (current, parentRelative) => {
        let children;
        try {
            children = fs
            ...
            unsafe.push(`${parentRelative || "."}を読み取れません: ${error instanceof Error ? error.message : String(error)}`);
            return;
        }
        for (const child of children) {
            const relative = slash(parentRelative ? path.join(parentRelative, child.name) : child.name);
            const absolute = path.join(current, child.name);
            if (child.name.normalize("NFC") !== child.name ||
            ...
            try {
                ...
                unsafe.push(`${relative}を検査できません: ${error instanceof Error ? error.message : String(error)}`);
                continue;
            }
            if (stat.isSymbolicLink()) {
            ...
            try {
                ...
                unsafe.push(`${relative}の実体pathを解決できません: ${error instanceof Error ? error.message : String(error)}`);
                continue;
            }
            ...
            if (stat.isDirectory()) {
                ...
                unsafe.push(`${relative}は通常fileまたはdirectoryではないため保持します`);
            }
        }
    };
    visit(directory, "");
...
export function calculateStagingDigest(directory, artifacts) {
...
export function listStagingArtifacts(directory) {
    const contents = inventory(directory);
    if (contents.unsafe.length > 0)
        throw new Error(`安全にinventoryできないstaging内容があります: ${contents.unsafe.join("; ")}`);
    return contents.artifacts;
}
function requiredArtifacts(mode) {
    return mode === "full"
...
function parseStoredRecordWithTrackerContract(source, trackerContract) {
    const parsed = parseJsonStrict(source, STAGING_RECORD_FILE);
    if (!isRecord(parsed))
        throw new Error("記録はobjectでなければなりません");
    const unknown = Object.keys(parsed).filter((field) => !STORED_FIELDS.has(field));
    ...
    if (!isNullableString(parsed.syncDigest) ||
    ...
    if (parsed.state === "sync-verified" &&
        (syncValues.some((value) => value === null) ||
            parsed.checkpoint !== (parsed.mode === "full" ? 8 : 4) ||
            parsed.syncDigest !== parsed.readBackDigest))
        throw new Error("sync-verifiedにはmode別最終checkpointと一致する完全な再読取証拠が必要です");
    if (parsed.state === "promotion-active" &&
        (parsed.mode !== "full" ||
            syncValues.some((value) => value === null) ||
            parsed.checkpoint !== 4 ||
            parsed.syncDigest !== parsed.readBackDigest))
        throw new Error("promotion-activeには昇格前Step 4の完全な再読取証拠とmode=fullが必要です");
    return {
...
function parseStoredRecord(source) {
    return parseStoredRecordWithTrackerContract(source, "current");
}
function parseLegacyStoredRecordForMigration(source) {
    return parseStoredRecordWithTrackerContract(source, "legacy-migration");
}
export function readStoredStagingRecord(directory) {
    return parseStoredRecord(fs.readFileSync(path.join(directory, STAGING_RECORD_FILE), "utf8"));
}
/**
 ...
 */
export function migrateLegacyStagingTracker(directory, input) {
    const resolved = path.resolve(directory);
    return withStagingMutationLock(resolved, () => migrateLegacyStagingTrackerLocked(resolved, input));
}
/**
 ...
 */
export function migrateLegacyStagingTrackerLocked(directory, input) {
    ...
    const resolved = path.resolve(directory);
    const current = parseLegacyStoredRecordForMigration(fs.readFileSync(path.join(resolved, STAGING_RECORD_FILE), "utf8"));
    const expected = `https://github.com/${input.repository}/issues/${input.issue}`;
    if (current.tracker?.toLowerCase() === expected.toLowerCase())
        return current;
    const legacy = typeof current.tracker === "string"
        ? LEGACY_TRACKER.exec(current.tracker)
        : null;
    if (!legacy || Number(legacy[1]) !== input.issue)
        throw new Error("legacy trackerが移行対象のrepository・Issueと一致しません");
    writeFileAtomic(path.join(resolved, STAGING_RECORD_FILE), `${stableJson({ ...current, tracker: expected })}\n`, { temporaryDirectory: path.dirname(resolved) });
...
export function refreshStoredStagingDigest(directory) {
    return withStagingMutationLock(directory, () => {
        ...
        writeFileAtomic(path.join(directory, STAGING_RECORD_FILE), `${JSON.stringify(updated, null, 2)}\n`, { temporaryDirectory: path.dirname(path.resolve(directory)) });
 ...
 */
export function promoteStoredStagingModeToFull(directory) {
    return withStagingMutationLock(directory, () => {
        if (!fs.existsSync(path.join(directory, STAGING_PROMOTION_TRANSACTION_FILE)))
        ...
        const updated = {
            ...
            artifacts,
            digest: calculateStagingDigest(directory, artifacts),
            state: current.state === "sync-verified" ? "promotion-active" : "local-active",
        };
        writeFileAtomic(path.join(directory, STAGING_RECORD_FILE), `${JSON.stringify(updated, null, 2)}\n`, { temporaryDirectory: path.dirname(path.resolve(directory)) });
        const reread = readStoredStagingRecord(directory);
        if (JSON.stringify(reread) !== JSON.stringify(updated))
 ...
 */
export function finalizeStoredStagingPromotion(directory) {
    return withStagingMutationLock(directory, () => {
        ...
        const artifacts = listStagingArtifacts(directory).filter((artifact) => artifact !== STAGING_PROMOTION_TRANSACTION_FILE);
        const updated = {
            ...current,
            artifacts,
            digest: calculateStagingDigest(directory, artifacts),
...
function baseRecord(absolute, relative, input) {
    return {
        path: absolute,
        relative,
...
function inspectDirectory(context, absolute, relative) {
    ...
    const missingRequired = required.filter((artifact) => !contents.artifacts.includes(artifact));
    if (missingRequired.length > 0)
        reasons.push(`mode別の必要成果物が不足しています: ${missingRequired.join(", ")}`);
    ...
    try {
        ...
        reasons.push(`成果物digestを検証できません: ${error instanceof Error ? error.message : String(error)}`);
    }
    if (currentDigest !== stored.digest)
        reasons.push("記録済みcontent digestと現在の成果物が一致しません");
    const expectedCheckpoint = stored.mode === "full" ? 8 : 4;
    const evidenceComplete = stored.state === "sync-verified" &&
        stored.tracker !== null &&
    ...
    if (!evidenceComplete)
        reasons.push(`mode=${stored.mode}の最終checkpoint Step ${expectedCheckpoint}に対する書き込み後読み取り済み同期証拠がありません`);
    const createdAtMs = parseInstant(stored.createdAt, "createdAt");
    const elapsed = context.nowMs - createdAtMs;
    if (elapsed < 0)
        reasons.push("createdAtが現在時刻より後のため保持します");
    ...
    if (state === "sync-verified")
        reasons.push(`同期確認済みですが保持期限${context.retentionDays}日を経過していないため保持します`);
    if (state === "deletion-ready")
        reasons.push(`同期証拠、成果物digest、fingerprintを確認し保持期限${context.retentionDays}日を経過しています`);
    return baseRecord(absolute, relative, {
        mode: stored.mode,
        artifacts: contents.artifacts,
...
export function inspectStaging(input) {
    ...
    if (!fs.existsSync(boundary.path))
        return [];
    let entries;
    try {
        entries = fs
    ...
    return entries.map((entry) => {
        ...
        if (path.dirname(real) !== boundary.path ||
...
export function applyStagingCleanup(input, remove) {
    ...
    try {
        ...
        return rejected(`apply直前のstaging再検証に失敗しました: ${error instanceof Error ? error.message : String(error)}`);
    }
    ...
    if (input.plan.root !== canonicalRoot ||
        input.plan.hash !== suppliedPlanHash ||
        input.approvedHash !== input.plan.hash ||
        current.hash !== input.plan.hash)
        return rejected("root、承認済みreport hash、preview内容、またはapply直前の再planが一致しません");
    const removed = [];
 ...
 */
export const STAGING_LIFECYCLE_AREAS = Object.freeze([
