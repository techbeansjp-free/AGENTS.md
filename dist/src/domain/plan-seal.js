...
export const PLAN_SEAL_ARTIFACTS = Object.freeze({
    full: Object.freeze([
        "00_要求定義.md",
        "01_要件定義.md",
        "02_設計.md",
...
export function planSealStep(mode) {
    return mode === "full" ? 8 : 4;
}
/** 封印後に計画凍結を検査するStep。 */
export function isPlanFrozenCheckStep(step) {
...
export function parsePlanSeal(input) {
    ...
    const mode = input.mode === "full" || input.mode === "quick" || input.mode === "poc"
        ? input.mode
        : undefined;
    ...
    if (errors.length > 0)
        return { errors };
    return {
        value: Object.freeze(Object.fromEntries(expected.map((name) => [name, record[name]]))),
        errors,
    };
}
/** journal上で最後に記録された封印。**無ければ封印前（旧journalを含む）である。** */
export function latestPlanSeal(entries) {
...
export function planResealRejection(input) {
    if (input.humanOverride || input.step !== planSealStep(input.mode))
        return undefined;
    const sealed = input.entries.find((entry) => entry.planSeal !== undefined && entry.mode === input.mode);
    if (sealed)
    ...
    if (input.entries.some((entry) => entry.step >= 9 && entry.mode === input.mode))
...
export function planSealDigest(seal) {
    return sha256Text(stableJson(seal));
}
export function planGenerationDigest(previousDigest, amendments) {
    return sha256Text(stableJson({ previousDigest, amendments }));
}
export function baseGeneration(seal) {
    return {
        generation: 1,
        previousDigest: null,
        amendments: [],
 ...
 */
export function planAmendmentDigests(markdown) {
...
function describeAmendments(amendments) {
 ...
 */
function appendOnlyViolation(recorded, current) {
    const broken = recorded.amendments.find((amendment, index) => current[index]?.id !== amendment.id ||
 ...
 */
export function latestPlanGeneration(seal, recorded) {
    let latest = baseGeneration(seal);
    const errors = [];
    for (const generation of recorded) {
        if (stableJson(generation) === stableJson(latest))
            continue;
        const next = generation.generation === latest.generation + 1 &&
        ...
        if (!next) {
            errors.push(`journalの計画世代${generation.generation}が封印と直前の世代${latest.generation}から辿れません`);
            break;
        }
        latest = generation;
    }
 ...
 */
export function nextPlanGeneration(previous, current) {
    const violation = appendOnlyViolation(previous, current);
    if (violation)
        return { error: violation };
    if (current.length === previous.amendments.length)
 ...
 */
export function deliveredAmendmentViolation(reviewed, current, where) {
    const violation = appendOnlyViolation(reviewed, current);
    if (violation)
        return `${where}: ${violation}`;
    if (current.length === reviewed.amendments.length)
        return undefined;
...
export function parsePlanGeneration(input) {
    ...
    const hex = (candidate) => typeof candidate === "string" && /^[a-f0-9]{64}$/u.test(candidate);
    ...
    const parsed = [];
    for (const [index, item] of amendments.entries()) {
    ...
    return {
        value: {
            generation,
            previousDigest: generation === 1 ? null : previousDigest,
            amendments: parsed,
            digest,
        },
        errors: [],
 ...
 */
export function changedSealedArtifacts(seal, current) {
...
export function stagingDriftDiagnostic(input) {
    ...
    if (input.changedPlanning.length > 0)
        parts.push(`承認済み計画は封印済みです。${input.changedPlanning.join("、")}を封印時の内容へ戻し、計画の変更は${PLAN_AMENDMENT_FILE}へAMD-NNNとして追記してください`);
    if (input.terminalDelivery === true || steps.has(11))
        parts.push("terminal delivery stateではstagingのdigestを再固定できません。編集した成果物を編集前の内容へ戻してください");
    else if (steps.has(10))
        parts.push("Step 10記録後にstaging digestを再固定するのは、前進commitを伴う新しいreview roundだけです（再固定後にStep 10を再記録します）。stagingだけの変更は編集前の内容へ戻してください");
    else if (steps.has(9))
        parts.push("review roundを実行するとstaging digestが現在の成果物へ再固定されます");
    else
        parts.push("workflow record --step=<最新のStep> を再実行してdigestを更新してから再試行してください");
    return `。${parts.join("。")}`;
}
/**
 ...
 */
export function validatePlanAmendment(markdown) {
    const errors = [];
    const ids = [];
    const entries = [];
    let current;
    ...
    for (const line of markdown.split(/\r?\n/u)) {
        ...
        if (/^##\s/u.test(line)) {
            ...
            if (!/^AMD-\d{3}$/u.test(id)) {
                errors.push(`${PLAN_AMENDMENT_FILE}の見出し${id}はAMD-NNN形式が必要です`);
                continue;
            }
            current = { id, lines: [] };
            entries.push(current);
    ...
    entries.forEach((entry, index) => {
        ...
        if (entry.id !== expected)
            errors.push(`${PLAN_AMENDMENT_FILE}の${entry.id}は${expected}であるべきです。entryは001から欠番・重複なく昇順に追記し、既存entryを削除・並べ替えしません`);
        for (const field of PLAN_AMENDMENT_FIELDS) {
            ...
            if (value === "" || /^（[^）]*）$/u.test(value))
                errors.push(`${PLAN_AMENDMENT_FILE}の${entry.id}に${field}がありません`);
        }
    });
