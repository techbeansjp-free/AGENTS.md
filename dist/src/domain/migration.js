...
const OWNED_PATHS = {
    policy: /^\.agent-skill-chain\/(?:project-policy\.json|project\/(?:choices|rules|conformance|providers)\/[a-z0-9][a-z0-9.-]*\.json)$/u,
 ...
 */
function candidateChoicesSource(candidate) {
...
function rejected(reasons) {
    return {
        ...
        allowed: false,
        diagnostic: {
            ...diagnostic("ASC-MIGRATION-TOCTOU-001", "migration stateの改竄とTOCTOUを防止する", "authority", reasons, ["migration"], ["policy、hash、fingerprint、revision、実fileを再検証した"], [], "trusted/candidateとmanifestから新しいdry-run planを作成してください", "project policy owner", "snapshotの内容を保持して適用しない"),
...
function requireExternalAuthority(state, options, requiredRevision) {
    const reasons = [];
    if (typeof options.approvedPlanHash !== "string" ||
    ...
    if (!Number.isInteger(options.expectedRevision))
        ...
        if (options.expectedRevision !== state.revision)
            reasons.push(`expected revisionがstateと一致しません: expected=${options.expectedRevision} actual=${state.revision}`);
        if (requiredRevision !== undefined &&
            options.expectedRevision !== requiredRevision)
            reasons.push(`operationのexpected revisionが不正です: required=${requiredRevision} actual=${options.expectedRevision}`);
    }
...
function simulatedCrash(error) {
    return Boolean(error && typeof error === "object" && "simulatedCrash" in error);
}
    ...
    if (kind === "policy") {
        if (fragmentedSet(candidateInput)) {
            ...
            if (!(sourcePath in candidateInput.rawEntries))
                reasons.push(`policy artifactがcandidate set inventoryにありません: ${relative}`);
            else if (after !== candidateInput.rawEntries[sourcePath])
                reasons.push(`policy artifactがcandidate raw bytesと一致しません: ${relative}`);
        }
        else {
            const canonical = `${JSON.stringify(policyOf(candidateInput), null, 2)}\n`;
            if (relative !== ".agent-skill-chain/project-policy.json" ||
                after !== canonical)
        ...
        reasons.push("runtime artifactは不正制御文字なしの非空sourceでなければなりません");
    else if (kind === "CI" &&
        ...
        (after.trim() === "" || CONTENT_CONTROL.test(after)))
        reasons.push("template artifactは不正制御文字なしの非空文書でなければなりません");
    return reasons;
}
export function planFileMigration(root, trusted, candidate, entries) {
    ...
    if ((fragmentedSet(trusted) || fs.existsSync(projectDirectory)) &&
    ...
    for (const entry of [...entries].sort((a, b) => ORDER.indexOf(a.kind) - ORDER.indexOf(b.kind))) {
        ...
        if (artifactErrors.length)
            return rejected(artifactErrors);
    ...
    const manifest = artifacts.map(({ kind, path: relative, source, owner, retention, beforeHash, afterHash }, order) => ({
        order,
        kind,
        path: relative,
        source,
...
function verify(state, trusted, candidate, revision, expectedFileHash, approvedPlanHash) {
    ...
    if (state.candidateSetHash !== undefined &&
        ...
        reasons.push("candidate fragmented policy setのhashまたはinventoryが変化しました");
...
export function applyFileMigration(state, trusted, candidate, options = {}) {
    ...
    try {
        ...
        if (state.candidateSetHash !== undefined) {
            ...
            if (loaded.setHash !== state.candidateSetHash ||
                loaded.semanticPolicyHash !== state.candidateSemanticPolicyHash)
                throw new Error("apply後project policy setのset hashまたはsemantic hashがcandidateと一致しません");
        }
    }
    catch (error) {
...
function restore(root, artifact) {
    ...
    if (artifact.before === null)
        ...
        writeFileAtomic(file, artifact.before);
}
...
export function recoverFileMigration(state, trusted, candidate, options = {}) {
    ...
    const approved = options.approvedPlanHash;
    const structural = verify(state, trusted, candidate, options.expectedRevision ?? -1, "beforeHash", approved);
    const observed = [];
    for (const artifact of state.artifacts ?? []) {
        const file = resolveContained(state.root, artifact.path, {
            allowMissingLeaf: true,
        ...
        observed.push(currentHash === artifact.beforeHash
            ? "before"
    ...
    if (!approved ||
        ...
        structuralDiagnostic?.reasons.some((reason) => !reason.includes("計画後に変化")))
        return structural;
