...
const STATES = ["planned", "started", "completed", "blocked"];
 ...
 */
export function tryBuildReviewProgressInventory(targetPath, source, target) {
    /**
    ...
    const unbuildable = (reason) => Object.freeze({
        state: "unbuildable",
        targetPath,
        reason,
...
export function describeReviewProgressUnbuildable(input) {
    ...
    const base = {
        code: "ASC-REVIEW-PROGRESS-INVENTORY-UNBUILDABLE",
        reason: input.reason,
 ...
 */
export function tryBuildReviewProgressInventories(targets) {
    if (targets.length === 0 || targets.length > 16)
        throw new Error("parallel progress targetは1件以上16件以下が必要です");
    ...
    for (const item of targets) {
        const outcome = tryBuildReviewProgressInventory(item.targetPath, item.source, item.target);
...
export function parseReviewProgressInventory(value) {
    ...
    if (typeof value.targetPath !== "string" ||
        ...
        value.allowedTaskIds.some((task) => typeof task !== "string" || !TASK_ID.test(task)) ||
...
export function makeReviewProgressEntry(input) {
    ...
    if (!TASK_ID.test(input.taskId))
        throw new Error("progress taskIdが不正です");
...
export function latestReviewProgressDigest(records) {
...
export function parseReviewProgressRecords(source) {
    ...
    for (const [index, line] of lines.entries()) {
        ...
        if (!isRecord(value))
            throw new Error("progress recordはobjectが必要です");
        const isSeal = value.schemaVersion === "agent-skill-chain/review-progress-seal/v1";
        ...
        if (Object.keys(value).some((field) => !expectedFields.includes(field)) ||
        ...
        const withoutDigest = Object.fromEntries(Object.entries(value).filter(([field]) => field !== digestField));
        if (!SHA256.test(claimed) || digestRecord(withoutDigest) !== claimed)
            throw new Error("progress record digestが不正です");
        if (isSeal) {
            if (!SHA256.test(String(value.sessionId ?? "")) ||
...
export function renderReviewProgress(records, taskIds) {
    const latest = new Map();
    for (const taskId of taskIds ?? [])
        latest.set(taskId, "planned");
    for (const record of records)
        if ("taskId" in record)
            latest.set(record.taskId, record.state);
    const rows = [...latest]
...
export function parallelCriticalPath(reviewDuration, progressDuration) {
