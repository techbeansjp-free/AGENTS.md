...
import { isSafeFinalizeIgnoredPathPrefix, resolveFinalizeIgnoredPathAllowlist, } from "./worktree-removal-safety.js";
export const DEFAULT_WORKTREE_PLACEMENT = {
    root: ".worktrees",
...
const WORKTREE_TIMESTAMP_MAX_AGE_MS = WORKTREE_TIMESTAMP_MAX_AGE_MINUTES * 60 * 1000;
...
function placementPolicyErrors(policy) {
    const errors = [];
    const constants = [
    ...
    if (policy.allowedBranchTypes.length < 1 ||
        policy.allowedBranchTypes.length > 32 ||
        new Set(policy.allowedBranchTypes).size !==
            policy.allowedBranchTypes.length ||
        policy.allowedBranchTypes.some((item) => !/^[a-z][a-z0-9-]{0,31}$/u.test(item)))
        errors.push("worktree policyのallowedBranchTypesが不正です");
    const finalizeIgnoredPathAllowlist = policy.finalizeIgnoredPathAllowlist ?? [];
    if (!Array.isArray(finalizeIgnoredPathAllowlist) ||
        finalizeIgnoredPathAllowlist.length > 64 ||
        new Set(finalizeIgnoredPathAllowlist).size !==
            finalizeIgnoredPathAllowlist.length ||
        finalizeIgnoredPathAllowlist.some((prefix) => !isSafeFinalizeIgnoredPathPrefix(prefix)))
...
function issueFromRegistered(entry) {
    const branch = BRANCH_NAME.exec(entry.branch);
    if (branch?.[2])
        return Number(branch[2]);
...
export function validateWorktreePlacement(input) {
    ...
    if (!directory)
        add("worktree directory名が規定書式ではありません");
    else {
        if (!validTime(input.currentTime))
            ...
            if (!timestamp)
                ...
                if (age < 0)
                    add("worktree directory名のtimestampが未来です");
                else if (age > WORKTREE_TIMESTAMP_MAX_AGE_MS)
                    add("worktree directory名のtimestampは現在時刻以前かつ10分以内でなければなりません");
            }
        }
    ...
    if (!branch)
        ...
        if (branch[3] !== input.slug)
            add("branch名のslugが一致しません");
    }
...
export function matchesTargetWorktree(input) {
    return (input.candidatePath === input.targetPath &&
...
export function canonicalWorktreePath(target) {
...
export function enforceTrustedWorktreeBoundary(input) {
    ...
    const gitInternal = gitRelative === "" ||
        (!gitRelative.startsWith(`..${path.sep}`) &&
            gitRelative !== ".." &&
            !path.isAbsolute(gitRelative));
    const destinationConflict = canonical === repositoryRoot || pathEntryExists(destination);
    ...
    if (input.trustedPolicy) {
        const observations = input.trustedPolicy.rules
            ...
            : rule.riskClass === "identity"
                ? {
                    ruleId: rule.ruleId,
                    violated: rootMismatch || destinationConflict || repositoryMismatch,
                    reasons: [
        ...
        if (!enforcement.allowed)
            throw new Error(`${enforcement.diagnostic?.ruleId ?? "ASC-WORKTREE"}: ${enforcement.diagnostic?.reasons.join("; ") ?? "boundary違反"}`);
    }
    return {
        repositoryRoot,
...
function sourceStatusExcludingTarget(repoRoot, destination) {
    const relative = path
...
export function createWorktree(input) {
    ...
    const { repositoryRoot, canonicalDestination: canonical, rootMismatch, gitInternal, destinationConflict, repositoryMismatch, } = boundary;
    ...
    const rootEscaped = rootRelative !== policy.root ||
    ...
    if (baseCheck.status !== 0)
        throw new Error("基点コミットを検証できません");
    if (!/^[0-9a-f]{40}$/iu.test(input.remoteDefaultSha))
        throw new Error("remote default branch SHAは40桁hexで指定してください");
    const remoteHead = git(["symbolic-ref", "refs/remotes/origin/HEAD"], input.repoRoot, { allowFailure: true });
    ...
    const remoteDefault = git(["rev-parse", "--verify", `${expectedRemoteRef}^{commit}`], input.repoRoot, { allowFailure: true });
     ...
     */
    const selectedBaseSha = selectedBaseBranch === input.remoteDefaultBranch
    ...
    if (selectedBaseBranch === input.remoteDefaultBranch &&
        ...
        throw new Error("既定branchをbaseにするとき、base branch SHAはremote default branch SHAと一致しなければなりません");
    if (selectedBaseBranch !== input.remoteDefaultBranch) {
        ...
        const acceptance = inspectBaseBranchAcceptance({
            policy: input.trustedPolicy ??
                (() => {
                    throw new Error("既定branch以外をbaseにするにはtrusted policyの観測が必要です");
                })(),
        ...
        ], input.repoRoot, { allowFailure: true });
        if (observedBase.status !== 0 ||
            observedBase.stdout.trim().toLowerCase() !== selectedBaseSha.toLowerCase())
            throw new Error(`base branch ${selectedBaseBranch}のtipが取得済みSHAと一致しません`);
    }
    if (baseCheck.stdout.trim().toLowerCase() !== selectedBaseSha.toLowerCase())
        throw new Error("基点は取得済みbase branch commitと一致しなければなりません");
    /** 検査を通過した基点commit。以降はrefを再解決しない。 */
    const verifiedBaseCommit = baseCheck.stdout.trim();
     ...
     *
     * 検査は`baseCheck`が解決したcommitに対して行っている。ここで可変なrefを
     * 渡し直すと、検査から`worktree add`までの間に並行writerがrefを動かした
     * 場合に、検査したcommitと実際の基点がずれる。受理するbaseをN本へ広げると
     * この窓が広がるため、固定SHAを渡す（Issue #1139）。
     */
    git(["worktree", "add", "-b", input.branch, destination, verifiedBaseCommit], input.repoRoot);
    const dirtyAfter = sourceStatusExcludingTarget(input.repoRoot, destination);
    if (dirtyAfter !== dirtyBefore)
        throw new Error("作業元worktreeの状態が予期せず変化しました。復旧のため両方のworktreeを保持してください");
    return {
        path: destination,
        branch: input.branch,
        base: baseCheck.stdout.trim(),
...
export function inspectFinalizeState(repoRoot, worktreePath, evidence, ignoredPathAllowlist = resolveFinalizeIgnoredPathAllowlist()) {
    ...
    const ignoredArtifacts = git(["ls-files", "--others", "--ignored", "--exclude-standard", "-z"], worktreePath)
        .stdout.split("\0")
        .filter(Boolean);
...
export function inspectRecoveryState(worktreePath, knownHeadSha) {
    const headSha = knownHeadSha ?? git(["rev-parse", "HEAD"], worktreePath).stdout.trim();
    const upstream = git(["rev-parse", "--abbrev-ref", "--symbolic-full-name", "@{upstream}"], worktreePath, { allowFailure: true });
    const remoteSha = upstream.status === 0
    ...
    const reachableFromDefaultBranch = defaultRef !== undefined &&
    ...
    return {
        ...
        recoveryReachable: (Boolean(recoveryRef) && pushed) || reachableFromDefaultBranch,
    };
}
/**
 ...
 */
function resolveDefaultBranchRef(worktreePath) {
    const symbolic = git(["symbolic-ref", "--short", "refs/remotes/origin/HEAD"], worktreePath, { allowFailure: true });
