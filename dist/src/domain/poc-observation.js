...
import { POC_OBSERVABLE_KINDS, } from "./mode.js";
...
export function pocObservationArtifact(headSha) {
...
export function pocDeclarationDigest(declaration) {
    return sha256(stableJson(declaration));
}
export function pocObservationResultDigest(result) {
    return sha256(stableJson(result));
}
export function pocScenarioExecutionDigest(execution) {
    return sha256(stableJson(execution));
}
export function pocObservationEvidenceDigest(evidence) {
    return sha256(stableJson(evidence));
}
export function parsePocObservationEvidence(source) {
    ...
    else if (!nonEmpty(value.fixture.id) ||
    ...
    if (!Array.isArray(value.executions) || value.executions.length === 0)
        ...
        value.executions.forEach((item, index) => {
            if (!isRecord(item) || !exactFields(item, EXECUTION_FIELDS)) {
                errors.push(`poc-observation executions[${index}]のfield集合が不正です`);
                return;
            }
            if (!nonEmpty(item.scenarioId) ||
            ...
            if (pocScenarioExecutionDigest(identity) !== executionDigest)
                errors.push(`poc-observation executions[${index}]のdigestが一致しません`);
            executions.push(execution);
        });
    if (new Set(executions.map(({ scenarioId }) => scenarioId)).size !==
        executions.length)
    ...
    if (!Array.isArray(value.results) || value.results.length === 0)
        ...
        value.results.forEach((item, index) => {
            ...
            const typedValueValid = result.kind === "exit-code"
            ...
            if ((result.actual === result.expected) !== (result.status === "passed"))
                errors.push(`poc-observation results[${index}]のstatusが計測値と一致しません`);
    ...
    if (new Set(results.map(({ observableId }) => observableId)).size !==
        results.length)
...
export function validatePocObservationEvidence(input) {
    ...
    if (!GIT_SHA.test(input.headSha))
        errors.push("検証対象HEAD SHAが不正です");
    ...
    if (evidence.executions.length !== input.declaration.scenarios.length)
        errors.push("poc-observation execution集合が宣言済みscenario集合と一致しません");
    for (let index = 0; index < input.declaration.scenarios.length; index += 1) {
        ...
        if (execution.scenarioId !== scenario.id)
            errors.push(`poc-observation executions[${index}]が宣言済みscenario ${scenario.id} と一致しません`);
        if (execution.exitCode !== 0 || execution.signal !== null)
            errors.push(`poc-observation scenario ${scenario.id} のrunnerが正常終了していません`);
    }
    if (evidence.results.length !== input.declaration.observables.length)
        errors.push("poc-observation result集合が宣言済みobservable集合と一致しません");
    for (let index = 0; index < input.declaration.observables.length; index += 1) {
        const expected = input.declaration.observables[index];
        const observed = evidence.results[index];
        if (!expected || !observed)
            continue;
        if (observed.observableId !== expected.id ||
            observed.scenarioId !== expected.scenarioId ||
            observed.kind !== expected.kind ||
            observed.target !== (expected.target ?? null) ||
            observed.expected !== expected.expected)
            errors.push(`poc-observation results[${index}]が宣言済みobservable ${expected.id} と完全一致しません`);
        if (observed.actual !== observed.expected || observed.status !== "passed")
            errors.push(`poc-observation observable ${expected.id} が合格していません`);
    }
    return {
...
//# sourceMappingURL=poc-observation.js.map