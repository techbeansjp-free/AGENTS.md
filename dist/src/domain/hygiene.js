...
function repositoryIdentity(inputRoot) {
    if (typeof inputRoot !== "string" ||
        inputRoot.trim() === "" ||
        hasUnsafeMeta(inputRoot) ||
        hasParentReference(inputRoot)) {
        throw new Error("rootには未解決env、glob、制御文字、tilde、親参照を含まない明示pathが必要です");
    }
    ...
    const commonRaw = git(["rev-parse", "--git-common-dir"], realRoot).stdout.trim();
...
function validateKinds(kinds) {
    const selected = kinds ?? [...HYGIENE_KINDS];
    const result = new Set();
    for (const kind of selected) {
        if (!HYGIENE_KINDS.has(kind))
            throw new Error(`未対応のworkspace hygiene operationです: ${String(kind)}`);
        result.add(kind);
    }
    return result;
}
...
function trackedPaths(root) {
    return new Set(git(["ls-files", "-z"], root).stdout.split("\0").filter(Boolean).map(slash));
}
function isTemporaryArtifact(relative) {
...
export function previewWorkspaceHygiene(input) {
    ...
    const exclude = (relative, reason) => {
        excluded.push({ relative: slash(relative), reason });
    };
    const addCandidate = (absolute, relative, kind, reason, owner, stat) => {
    ...
    const scanDirectory = (absolute, relative) => {
        let entries;
        try {
            entries = fs.readdirSync(absolute, { withFileTypes: true });
        }
        catch (error) {
            exclude(relative, `directoryを検査できないため保持します: ${error instanceof Error ? error.message : String(error)}`);
            return false;
        }
        ...
        for (const entry of entries) {
            ...
            if (isStagingLifecyclePath(childRelative)) {
                exclude(childRelative, "一時・派生lifecycle領域は専用lifecycle証拠なしにworkspace hygieneで削除しません");
            ...
            try {
                ...
                exclude(childRelative, `pathを再検査できないため保持します: ${error instanceof Error ? error.message : String(error)}`);
                containsOnlyRemovableEmptyDirectories = false;
                continue;
            }
            ...
            try {
                ...
                exclude(childRelative, `実体pathを解決できないため保持します: ${error instanceof Error ? error.message : String(error)}`);
                containsOnlyRemovableEmptyDirectories = false;
                continue;
            }
            if (!isContained(identity.root, realChild)) {
                exclude(childRelative, "repository root外へ解決されるpathは削除対象外です");
                containsOnlyRemovableEmptyDirectories = false;
                continue;
            }
            if (entry.name === ".git" ||
                ...
                exclude(childRelative, "Git common directory、object、ref、reflog、worktree metadataは常に対象外です");
                containsOnlyRemovableEmptyDirectories = false;
                continue;
            }
            if (entry.isDirectory() && entry.name === "node_modules") {
                exclude(childRelative, "package manager所有directoryは部分的に再帰削除しません");
                containsOnlyRemovableEmptyDirectories = false;
                continue;
            }
            if (entry.isDirectory()) {
                const childIsRemovableTree = scanDirectory(childAbsolute, childRelative);
            ...
            if (isTemporaryArtifact(normalized)) {
                if (tracked.has(normalized)) {
                    exclude(childRelative, "tracked fileは一時生成物に一致しても保持します");
                }
                else if (entry.isFile()) {
                    addCandidate(childAbsolute, childRelative, "temporary-artifact", "repository内の未追跡一時生成物です", "workspace owner", stat);
                }
                else {
        ...
        if (containsOnlyRemovableEmptyDirectories) {
            ...
            addCandidate(absolute, relative, "empty-directory", relative === "memo" || slash(relative).startsWith("memo/")
                ? "内容を持たないmemo directoryです"
                : "実体fileを含まない空directoryです", "workspace owner", stat);
        }
    ...
    const scanWorktreeContainers = (directory) => {
        let entries;
        try {
            entries = fs.readdirSync(directory, { withFileTypes: true });
        }
        catch (error) {
            exclude(".worktrees", `worktree containerを検査できないため保持します: ${error instanceof Error ? error.message : String(error)}`);
            return;
        }
        if (entries.length === 0) {
            const stat = fs.lstatSync(directory);
            addCandidate(directory, ".worktrees", "completed-worktree-container", "登録済みworktreeを含まない空のworktree container rootです", "Git worktree owner", stat);
            return;
        }
        ...
        for (const entry of entries) {
            ...
            try {
                ...
                exclude(relative, `pathを再検査できないため保持します: ${error instanceof Error ? error.message : String(error)}`);
                continue;
            }
            if (stat.isSymbolicLink()) {
            ...
            if (!entry.isDirectory()) {
                exclude(relative, "worktree container直下のdirectoryではないため保持します");
                continue;
            }
            ...
            if (worktrees.has(real)) {
                exclude(relative, "登録済みworktreeです。worktree本体の削除は本commandの責務ではなくGit公式commandだけで行います");
                continue;
            }
            let contents;
            try {
                ...
                exclude(relative, `worktree containerを検査できないため保持します: ${error instanceof Error ? error.message : String(error)}`);
                continue;
            }
            if (contents.length !== 0) {
                exclude(relative, "未登録でも空ではないworktree containerは他project資産として保持します");
                continue;
            }
            addCandidate(absolute, relative, "completed-worktree-container", "未登録かつ空の完了済みworktree containerです。登録済みworktree本体の削除は本commandの責務ではありません", "Git worktree owner", stat);
        }
    };
    const rootEntries = fs.readdirSync(identity.root, { withFileTypes: true });
    const worktreeContainersOnly = selectedKinds.size === 1 &&
    ...
    for (const entry of rootEntries) {
        ...
        const stat = fs.lstatSync(absolute);
        if (entry.name === ".git") {
            exclude(entry.name, "Git common directory、object、ref、reflog、worktree metadataは常に対象外です");
            continue;
        }
        if (worktreeContainersOnly) {
            exclude(entry.name, "completed-worktree-container以外は今回の検査operationに含まれていません");
            continue;
        }
        if (stat.isSymbolicLink()) {
        ...
        const real = fs.realpathSync(absolute);
        if (real === identity.commonDirectory ||
            isContained(identity.commonDirectory, real)) {
            exclude(entry.name, "Git common directory、object、ref、reflog、worktree metadataは常に対象外です");
            continue;
        }
        if (entry.isDirectory() && entry.name === "node_modules") {
            exclude(entry.name, "package manager所有directoryは部分的に再帰削除しません");
            continue;
        }
        if (entry.isDirectory()) {
        ...
        if (!tracked.has(normalized) &&
            ...
            addCandidate(absolute, entry.name, "temporary-artifact", "repository内の未追跡一時生成物です", "workspace owner", stat);
        }
    ...
    candidates.sort((left, right) => {
        const depth = right.relative.split("/").length - left.relative.split("/").length;
        return (depth ||
            left.relative.localeCompare(right.relative) ||
            left.kind.localeCompare(right.kind));
    });
    excluded.sort((left, right) => left.relative.localeCompare(right.relative) ||
...
function snapshot(root) {
    return {
        ...
        refs: git(["for-each-ref", "--format=%(refname)%00%(objectname)"], root)
            .stdout,
        root: fs.realpathSync(git(["rev-parse", "--show-toplevel"], root).stdout.trim()),
        status: git([
            "-c",
...
function expectedStatus(before, removedCandidates) {
    const removedTemporaryFiles = new Set(removedCandidates
...
function recoveryMessage(removed, remaining) {
    return [
...
function selectedKindsFromReport(report) {
    const disabled = new Set();
    for (const kind of HYGIENE_KINDS) {
        if (report.excluded.some((entry) => entry.reason === `${kind}は今回の検査operationに含まれていません`)) {
            disabled.add(kind);
        }
    }
    return [...HYGIENE_KINDS].filter((kind) => !disabled.has(kind));
}
function pathStillExists(target) {
    try {
        fs.lstatSync(target);
        return true;
    }
...
export function applyWorkspaceHygiene(input, remove) {
    ...
    const selectedPaths = input.paths
        ? new Set(input.paths.map((selected) => {
            if (typeof selected !== "string" ||
                selected.trim() === "" ||
                path.isAbsolute(selected) ||
                hasParentReference(selected) ||
                hasUnsafeMeta(selected))
                throw new Error("apply対象pathは安全なrepository相対pathで指定してください");
    ...
    const targets = input.report.candidates.filter((candidate) => candidate.removable &&
        operations.has(candidate.kind) &&
        (!selectedPaths || selectedPaths.has(candidate.relative)));
    if (selectedPaths &&
        [...selectedPaths].some((selected) => !targets.some((candidate) => candidate.relative === selected)))
    ...
    for (let index = 0; index < targets.length; index += 1) {
        ...
        try {
            ...
            try {
                if (!pathStillExists(target.path)) {
                    if (!removed.includes(target.relative))
                        removed.push(target.relative);
            ...
            const remaining = targets
                ...
                .map((candidate) => candidate.relative);
            throw new Error(`workspace hygieneは部分失敗し、成功とは報告しません: ${error instanceof Error ? error.message : String(error)}。 ${recoveryMessage(removed, remaining)}`, { cause: error });
        }
    }
    ...
    if (changed.length > 0) {
        throw new Error(`削除後のGit状態が期待値と一致しません（${changed.join(", ")}）。 ${recoveryMessage(removed, [])}`);
    }
