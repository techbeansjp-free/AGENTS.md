...
function reviewInput(value) {
    ...
    if (record.rationales !== undefined) {
        if (!isPlainRecord(record.rationales))
            return false;
        if (Object.keys(record.rationales).some((key) => !["affirmative", "adversarial"].includes(key)))
            return false;
        for (const [field, expected] of [
            ["affirmative", AFFIRMATIVE],
            ["adversarial", ADVERSARIAL],
        ])
            if (record.rationales[field] !== undefined &&
                (!isStringRecord(record.rationales[field]) ||
                    Object.keys(record.rationales[field]).some((key) => !expected.includes(key))))
                return false;
    }
    if (record.focus !== undefined) {
        if (!isPlainRecord(record.focus))
            return false;
        if (Object.keys(record.focus).some((key) => ![
            "unresolvedBlocking",
            "fixedDiff",
            "adjacentScope",
            "fullRescan",
    ...
    if (record.findings !== undefined &&
        (!Array.isArray(record.findings) ||
            !record.findings.every((finding) => isPlainRecord(finding) &&
                ...
                (finding.riskAcceptance === undefined ||
                    (isPlainRecord(finding.riskAcceptance) &&
                        Object.keys(finding.riskAcceptance).every((key) => ["authority", "owner", "reason", "reviewCondition"].includes(key)))))))
        return false;
    if (record.candidateEvidence !== undefined &&
        !isPlainRecord(record.candidateEvidence))
        return false;
    if (record.externalEvidence !== undefined &&
...
function validateImmutableCandidateEvidence(review) {
    ...
    exactFields(external?.ci, [
        ...
        "conclusion",
        "pullRequestNumbers",
    ], "externalEvidence.ci", errors);
    exactFields(external?.review, [
        "repository",
        "prNumber",
        "reviewId",
        "commitSha",
        ...
        "submittedAt",
        "verdict",
    ], "externalEvidence.review", errors);
    if (!commitOid(candidate?.implementationCommitSha))
        errors.push("H_implが不正です");
    if (!commitOid(candidate?.finalCommitSha))
        errors.push("H_finalが不正です");
    ...
    if (!Array.isArray(candidate?.changedPaths) ||
        ...
        errors.push("H_impl..H_finalはartifact pathだけのevidence-only差分でなければなりません");
    ...
    if (external?.implementation?.repository !== repository ||
    ...
    if (external?.implementation?.commitSha !== candidate?.implementationCommitSha)
    ...
    if (typeof external?.pr?.number !== "number" ||
    ...
    if (typeof external?.ci?.runId !== "string" ||
    ...
    if (!Array.isArray(external?.ci?.pullRequestNumbers) ||
    ...
    if (typeof external?.review?.reviewId !== "string" ||
    ...
    if (review.headSha !== final)
        errors.push("headShaがH_finalと一致しません");
...
export function evaluateReview(reviewValue) {
    ...
    const errors = [];
    errors.push(...validateDevelopmentConsiderationRecords(review.developmentConsiderations, "review").errors);
    const perspectives = [
        ["affirmative", AFFIRMATIVE],
        ["adversarial", ADVERSARIAL],
    ];
    ...
    for (const finding of findings) {
        ...
        if (typeof finding.status === "string" &&
            ...
            errors.push(`${finding.id ?? "指摘"}は分類前に現コードでの再現手順と再現結果が必要です`);
