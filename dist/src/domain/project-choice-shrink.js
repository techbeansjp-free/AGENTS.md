...
import { MONOTONIC_SHRINK_REASON_PREFIX, } from "./project-choice-diff.js";
/**
 * 縮小提案で受理できる対象field。
 *
 * **owner決裁がこの3つに限ると定めている。** 追加には別のowner決裁が要る。
 */
const SHRINKABLE_FIELD_PATHS = Object.freeze([
...
function shrinkFieldPath(entry) {
    return SHRINKABLE_FIELD_PATHS.find((fieldPath) => entry.startsWith(`${fieldPath}: ${SHRINK_REASON_PREFIX}`));
}
/**
 ...
 */
function describeRejection(entry, observation) {
    if (observation.fragmentPath === undefined ||
        observation.observedSha256 === undefined)
        return entry;
    const proposed = observation.proposedSha256 === undefined
        ? "登録済み提案なし"
        : `提案のsha256は${observation.proposedSha256}`;
    const malformed = observation.malformed > 0
...
export function acceptApprovedShrinks(input) {
    ...
    const observedSha256 = input.candidateChoicesRaw === undefined
        ? undefined
        : crypto
            .createHash("sha256")
    ...
    for (const entry of input.diff.weakened) {
         ...
         */
        const candidates = fieldPath === undefined
            ? []
            : proposals.filter((item) => item.fieldPath === fieldPath);
        const proposal = candidates.find((item) => item.afterSha256.toLowerCase() === observedSha256) ?? candidates[0];
        if (fieldPath === undefined ||
            proposal === undefined ||
            observedSha256 === undefined ||
            input.choicesFragmentPath === undefined ||
            proposal.afterSha256.toLowerCase() !== observedSha256) {
            remaining.push(describeRejection(entry, {
...
//# sourceMappingURL=project-choice-shrink.js.map