...
function requiredLabelSource(value) {
    if (typeof value !== "string" ||
        !LABEL_SOURCES.includes(value))
        throw new Error(`labelSourceは${LABEL_SOURCES.join("、")}のいずれかが必要です`);
    return value;
}
function requiredEvidenceRefs(value) {
    if (!Array.isArray(value) || value.length === 0)
        throw new Error("evidenceRefsは1件以上の配列が必要です");
    return value.map((entry, index) => requiredString(entry, `evidenceRefs[${index}]`));
}
/**
 ...
 */
export function parseEvaluationLabelInput(value) {
    if (!isRecord(value))
        throw new Error("evaluation label入力はobjectが必要です");
    const labelSource = requiredLabelSource(value.labelSource);
    const evidenceRefs = labelSource === "owner-adjudicated" && !Array.isArray(value.evidenceRefs)
        ? []
...
//# sourceMappingURL=evaluation-label.js.map