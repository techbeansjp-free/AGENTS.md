...
import { assertIssueStagingLocation, DEFAULT_ISSUE_STAGING_ROOT, matchesStagingRoot, readStagingLayout, stagingRepositoryRoot, TRACKED_STAGING_GITIGNORE, } from "./staging-layout.js";
...
function withoutPlaceholderCodeAndComments(text) {
    ...
    while (cursor < text.length) {
        ...
        const opening = !unclosedComment && atLineStart
            ? /^\s*(`{3,}|~{3,})/u.exec(line)?.[1]
            : undefined;
        if (fence || opening) {
            if (fence) {
...
function prefillRoutingRows(content, choices, kind) {
    ...
    const modelSettingCell = (selected) => "mode" in selected
...
function assertStagingDirectoryName(name) {
    if (name === "" ||
        name !== name.normalize("NFC") ||
        name.startsWith(".") ||
        /[\\/\0\p{Cc}\p{Cf}]/u.test(name) ||
        name.split(".").every((part) => part === ""))
        throw new Error("--nameは区切り・制御文字・先頭.を含まないdirectory名が必要です");
    return name;
}
/**
 ...
 */
function resolveStagingParent(root, requested) {
    const layout = readStagingLayout(root);
    const pattern = layout.rootPattern;
    if (requested === undefined) {
        if (pattern.includes("*"))
            throw new Error(`project policyのstaging.root（${pattern}）は*を含むため、--staging-rootで実際のdirectoryを指定してください`);
        return {
    ...
    if (path.isAbsolute(normalized) || normalized.split("/").includes(".."))
        throw new Error("--staging-rootはrepository相対の親参照を含まないpathが必要です");
    if (!matchesStagingRoot(pattern, normalized))
        throw new Error(`--staging-root（${normalized}）がproject policyのstaging.root（${pattern}）に一致しません`);
    return {
...
export function createIssueStaging(root, options) {
    ...
    const directoryName = options.name !== undefined
    ...
    publishDirectoryAtomic(finalPath, (temporary) => {
        if (placement.tracked)
            fs.writeFileSync(path.join(temporary, ".gitignore"), TRACKED_STAGING_GITIGNORE, {
                flag: "wx",
            });
...
export function escapeFoldBoundary(text) {
    ...
    const replace = (value) => value.replace(structural, (tag) => `&lt;${tag.slice(1, -1)}&gt;`);
    let fence;
    return text
        .split("\n")
        .map((line) => {
 ...
 */
function extractSections(markdown, needles) {
 ...
 */
export function renderIssueSyncPointerBody(mode, checkpoint, artifacts, relativeStaging) {
    const lead = artifacts[0];
    const title = lead?.text.split("\n").find((line) => line.startsWith("# ")) ??
        "# 00 要求定義";
    const rows = artifacts
        .map((artifact) => `| \`${artifact.name}\` | \`${relativeStaging}/${artifact.name}\` | \`${crypto.createHash("sha256").update(artifact.text).digest("hex")}\` |`)
        .join("\n");
    const sections = lead
...
export function buildIssueSyncBody(stagingInput, checkpoint, gherkinDialect) {
    ...
    const location = assertIssueStagingLocation(staging, stagingRepositoryRoot(staging));
    const body = location.layout.issueBody === "pointer"
        ? renderIssueSyncPointerBody(record.mode, checkpoint, texts, location.relative)
...
export function assertStagingSyncTarget(stagingPath, checkpoint, target, options = {}) {
    ...
    try {
        ...
        throw new Error(`同期記録は${DEFAULT_ISSUE_STAGING_ROOT}/またはproject policyのstaging.root直下のstagingだけに書き込めます`);
    }
