 ...
 *    手書き・整形し直しを受理しない
 */
export const REVIEW_EVIDENCE_SCHEMA_VERSION = "agent-skill-chain/review-evidence/v1";
...
function parseEvidenceValue(value) {
    ...
    if (session.status !== "converged")
        throw new Error("review evidence.session.statusはconvergedだけを受理します");
    const counted = positiveInteger(session.countedRounds, "review evidence.session.countedRounds");
    if (counted > REVIEW_RECOVERY_ROUND)
        throw new Error(`review evidence.session.countedRoundsが上限${REVIEW_RECOVERY_ROUND}を超えています: ${counted}`);
    ...
    const independence = exactObject(evidence.independence, "review evidence.independence", ["mode", "reviewer", "implementer", "reviewerModifiedCandidate"]);
    const reviewer = actor(independence.reviewer, "review evidence.independence.reviewer");
    const implementer = actor(independence.implementer, "review evidence.independence.implementer");
    if (reviewer === implementer)
        throw new Error("review evidence.independence.reviewerとimplementerは異なるidentityが必要です");
    if (independence.reviewerModifiedCandidate !== false)
        throw new Error("review evidence.independence.reviewerModifiedCandidateはfalseだけを受理します");
    ...
    if (!Array.isArray(evidence.verification) ||
        ...
        throw new Error(`review evidence.verificationは1〜${MAX_VERIFICATIONS}件が必要です`);
    const verification = evidence.verification.map((candidate, index) => {
        const item = exactObject(candidate, `review evidence.verification[${index}]`, ["command", "result"]);
        if (item.result !== "pass")
            throw new Error(`review evidence.verification[${index}].resultはpassだけを受理します`);
        return Object.freeze({
            command: text(item.command, `review evidence.verification[${index}].command`),
            result: "pass",
        });
    });
    ...
    return Object.freeze({
        ...
        implementationHeadSha: oid(evidence.implementationHeadSha, "review evidence.implementationHeadSha"),
        session: Object.freeze({
            sessionId: sha256(session.sessionId, "review evidence.session.sessionId"),
            latestRoundDigest: sha256(session.latestRoundDigest, "review evidence.session.latestRoundDigest"),
            status: "converged",
        ...
        independence: Object.freeze({
            mode: oneOf(independence.mode, MODES, "review evidence.independence.mode"),
            reviewer,
            implementer,
...
export function renderReviewEvidence(evidence) {
    const ordered = {
        ...
        independence: {
            ...
            reviewerModifiedCandidate: evidence.independence.reviewerModifiedCandidate,
        },
...
export function validateReviewEvidenceAgainstSession(evidence, session, options = {}) {
    ...
    if (options.independenceMode !== undefined &&
        ...
        errors.push(`review証跡の独立性モード${evidence.independence.mode}がtrusted policyの${options.independenceMode}と一致しません`);
...
export function comparableReviewEvidence(evidence, kind) {
    const omitted = new Set(["evidenceDigest"]);
    if (kind === "rebase") {
        omitted.add("baseSha");
    ...
    return stableJson(Object.fromEntries(Object.entries(evidence).filter(([field]) => !omitted.has(field))));
}
