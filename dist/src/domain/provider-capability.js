...
const SCHEMA_VERSION = "agent-skill-chain/provider-capability-mapping/v2";
...
export function validateProviderCapabilityMapping(value) {
    const errors = [];
    if (!isRecord(value))
        return {
            valid: false,
    ...
    if (!exactKeys(value, ["schemaVersion", "mappingVersion", "providers"]))
        errors.push("provider capability mappingに未知fieldまたは不足fieldがあります");
    if (value.schemaVersion !== SCHEMA_VERSION)
        errors.push("provider capability mappingのschemaVersionが不正です");
    if (typeof value.mappingVersion !== "string" ||
    ...
    for (const providerValue of value.providers) {
        if (!isRecord(providerValue)) {
            errors.push("provider capability mappingのproviderが不正です");
            continue;
        }
        if (!exactKeys(providerValue, ["provider", "capabilities", "selectionSource"]))
        ...
        if (typeof provider !== "string" || !SLUG.test(provider))
            ...
            errors.push(`provider capability mappingのproviderが重複しています: ${provider}`);
        else
            providerNames.add(provider);
        const capabilities = providerValue.capabilities;
        if (!Array.isArray(capabilities) ||
            capabilities.length === 0 ||
            capabilities.some((capability) => typeof capability !== "string" || !CAPABILITY.test(capability)) ||
...
export function isProviderCapabilityMapping(value) {
    return validateProviderCapabilityMapping(value).valid;
}
export function readProviderCapabilityMapping(source) {
    const value = parseJsonStrict(source, "provider capability mapping");
    const validation = validateProviderCapabilityMapping(value);
    if (!validation.valid)
        throw new Error(`provider capability mappingが不正です: ${validation.errors.join("; ")}`);
    return value;
}
