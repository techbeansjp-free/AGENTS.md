...
export const PRIVATE_STRUCTURAL_CASE_VERSION = "asc/laya-private-structural-case/v1";
...
const GENERIC_PRIVATE_VALUE = /(?:\b\d{12}\b|arn:aws:|https?:\/\/|git@[\w.-]+:|\b[A-Z0-9._%+-]+@[A-Z0-9.-]+\.[A-Z]{2,}\b|\/(?:Users|home)\/|[A-Z]:\\|(?:^|\s)(?:\d{1,3}\.){3}\d{1,3}(?:\s|$)|\b[0-9a-f]{8}-[0-9a-f]{4}-[1-5][0-9a-f]{3}-[89ab][0-9a-f]{3}-[0-9a-f]{12}\b|\beyJ[A-Za-z0-9_-]+\.[A-Za-z0-9_-]+\.[A-Za-z0-9_-]+\b|\b(?:localhost|[\w-]+\.(?:local|internal|corp))\b)/iu;
function normalize(value, label, maximum) {
    if (typeof value !== "string")
        throw new Error(`${label}は文字列が必要です`);
    const normalized = value.normalize("NFC").trim();
    if (!normalized || normalized.length > maximum || CONTROL.test(normalized))
        throw new Error(`${label}が空、長過ぎる、または制御文字を含みます`);
    return normalized;
}
function assertNoPrivateValue(value, label, maximum, forbidden) {
    ...
    if (redactSecrets(normalized) !== normalized ||
        ...
        forbidden.some((item) => folded.includes(item.normalize("NFKC").toLocaleLowerCase("und"))))
        throw new Error(`${label}に秘密または固有情報があります`);
    return normalized;
}
function hmac(key, scope, salt, kind, value) {
    return `${kind}-${crypto
...
export function createPrivateStructuralCase(value, policy) {
    ...
    if (!Buffer.isBuffer(policy.pseudonymKey) ||
    ...
    const forbidden = policy.forbiddenLiterals.map((item) => normalize(item, "forbidden literal", 256));
    ...
    if (!["yes", "no", "insufficient-evidence"].includes(value.gold))
        throw new Error("private goldが不正です");
    return {
        schemaVersion: PRIVATE_STRUCTURAL_CASE_VERSION,
        caseId: hmac(policy.pseudonymKey, policy.authorizedRepository, policy.corpusSalt, "case", `${value.sourceCommit}\0${issueKey}\0${rootCauseKey}\0${stableJson({ claim: value.claim, evidence: value.evidence })}`),
        sourceClass: "authorized-private-structural",
        groupId: hmac(policy.pseudonymKey, policy.authorizedRepository, policy.corpusSalt, "group", `${issueKey}\0${rootCauseKey}`),
...
export function assessPrivateStructuralLeakage(cases, forbiddenValues) {
    ...
    const rawIdentifierMatches = forbiddenValues
        ...
        .filter((value) => folded.includes(value.normalize("NFKC").toLocaleLowerCase("und"))).length;
    const secretCanaryMatches = /ASC_PRIVATE_CANARY_|github_pat_|gh[pousr]_/u.test(serialized) ? 1 : 0;
    const rawPathMatches = /\/(?:Users|home)\/|[A-Za-z]:\\|(?:src|docs|test)\//u.test(serialized)
        ? 1
        : 0;
    const sourceIdentityMatches = /[0-9a-f]{40}/iu.test(serialized) ? 1 : 0;
    const rawTextFields = /"(?:claim|evidence|path|excerpt|sourceCommit|sourceRepository)"/u.test(serialized)
        ? 1
        : 0;
    return {
...
export function validatePrivateStructuralCase(value) {
    ...
    if (value.schemaVersion !== PRIVATE_STRUCTURAL_CASE_VERSION ||
        ...
        new Set(value.signals).size !== value.signals.length ||
        value.signals.some((signal) => !PRIVATE_CASE_SIGNALS.includes(signal)) ||
        !["yes", "no", "insufficient-evidence"].includes(value.gold))
...
export function createPrivateStructuralTrainingView(value) {
...
export function validatePrivateStructuralCorpus(input, forbiddenValues) {
    if (!input || typeof input !== "object" || Array.isArray(input))
        throw new Error("private structural corpusはobjectが必要です");
    const value = input;
    if (Object.keys(value).sort().join(",") !== "cases,report,schemaVersion" ||
    ...
    const cases = value.cases.map((item) => {
        ...
        return validatePrivateStructuralCase(item);
    });
    ...
    if (stableJson(value.report) !== stableJson(expectedReport))
        throw new Error("private structural leakage reportが再計算値と一致しません");
    return {
