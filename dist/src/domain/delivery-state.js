...
export const DELIVERY_STATE_SCHEMA_VERSION = "agent-skill-chain/delivery-state/v1";
...
const OBSERVATION_INPUT_FIELDS = new Set([...OBSERVATION_FIELDS].filter((field) => field !== "observationId"));
...
function redeliveryDecisionId(value) {
...
function expectedIssueUrl(repositoryValue, issue) {
    return `https://github.com/${repositoryValue}/issues/${issue}`;
}
function expectedPullRequestUrl(repositoryValue, prNumber) {
...
function notBefore(value, lowerBound, label) {
    if (value < lowerBound)
        throw new Error(`${label}は先行event ${lowerBound} より前にできません`);
}
function parseCreate(value) {
    if (!isRecord(value))
        throw new Error("createはobjectが必要です");
    ...
    const parsed = {
        ...
        issue,
        issueUrl: exactUrl(value.issueUrl, expectedIssueUrl(repositoryValue, issue), "create.issueUrl"),
        ...
        pullRequestDigest: digest(value.pullRequestDigest, "create.pullRequestDigest"),
        bodyClosingDigest: digest(value.bodyClosingDigest, "create.bodyClosingDigest"),
    ...
    if (parsed.dispatchClaimedAt)
        notBefore(parsed.dispatchClaimedAt, parsed.preparedAt, "create.dispatchClaimedAt");
    return parsed;
}
function parsePullRequest(value, create) {
    if (value === null)
        return null;
    if (!isRecord(value))
        throw new Error("prはobjectまたはnullが必要です");
    unknownFields(value, PR_FIELDS, "pr");
    const number = positiveInteger(value.number, "pr.number");
    const parsed = {
        number,
        url: exactUrl(value.url, expectedPullRequestUrl(create.repository, number), "pr.url"),
        boundAt: instant(value.boundAt, "pr.boundAt"),
    };
    notBefore(parsed.boundAt, create.preparedAt, "pr.boundAt");
    return parsed;
}
function observationIdentityValue(value) {
    return {
...
function parseProviderRequest(value) {
    if (value === null)
        return null;
    if (!isRecord(value))
        throw new Error("merge.observation.providerRequestはobjectまたはnullが必要です");
    if (value.kind === "auto-merge") {
        ...
        return {
            kind: "auto-merge",
            requestedAt: instant(value.requestedAt, "auto-merge providerRequest.requestedAt"),
            method: value.method,
    ...
    if (value.kind === "merge-queue") {
        ...
        return {
            kind: "merge-queue",
            requestId: nonEmpty(value.requestId, "merge-queue providerRequest.requestId"),
            requestedAt: instant(value.requestedAt, "merge-queue providerRequest.requestedAt"),
...
export function pullRequestTerminalEvidenceId(create, pr) {
    return canonicalDigest({
...
export function closingContractDigest(input) {
    ...
    const match = /^https:\/\/github\.com\/[^/\s]+\/[^/\s]+\/issues\/([1-9]\d*)$/iu.exec(input.canonicalIssueUrl);
    if (!match || Number(match[1]) !== issue)
        throw new Error("canonicalIssueUrlは完全なGitHub Issue URLが必要です");
    if (!Array.isArray(input.closingIssueNumbers) ||
...
function normalizeObservation(value, create, pr) {
    ...
    const mergeCommitSha = value.mergeCommitSha === null
        ? null
        : oid(value.mergeCommitSha, "merge.observation.mergeCommitSha");
    const providerMergedAt = value.providerMergedAt === null
        ? null
    ...
    if ((providerState === "merged" &&
        ...
        throw new Error("providerStateとproviderRequest・providerMergedAt・mergeCommitShaが一致しません");
    const withoutId = {
        repository: repository(value.repository),
        prNumber: positiveInteger(value.prNumber, "merge.observation.prNumber"),
        prUrl: exactUrl(value.prUrl, expectedPullRequestUrl(create.repository, pr.number), "merge.observation.prUrl"),
        headSha: oid(value.headSha, "merge.observation.headSha"),
        issue: positiveInteger(value.issue, "merge.observation.issue"),
        issueUrl: exactUrl(value.issueUrl, expectedIssueUrl(create.repository, create.issue), "merge.observation.issueUrl"),
        bodyClosingDigest: digest(value.bodyClosingDigest, "merge.observation.bodyClosingDigest"),
        providerState,
        providerRequest,
        providerMergedAt,
...
function parseMerge(value, create, pr) {
    if (value === null)
        return null;
    if (!pr)
    ...
    const authorizedBaseSha = oid(value.authorizedBaseSha, "merge.authorizedBaseSha");
    const trustedPolicyCommitSha = oid(value.trustedPolicyCommitSha, "merge.trustedPolicyCommitSha");
    ...
    const reviewArtifactPath = nonEmpty(value.reviewArtifactPath, "merge.reviewArtifactPath");
    if (!reviewArtifactPath.startsWith("docs/reviews/") &&
    ...
    const parsed = {
        ...
        authorizedBaseSha,
        trustedPolicyCommitSha,
        implementationCommitSha: oid(value.implementationCommitSha, "merge.implementationCommitSha"),
        reviewArtifactPath,
        reviewArtifactDigest: digest(value.reviewArtifactDigest, "merge.reviewArtifactDigest"),
        ciRunId,
        reviewId,
    ...
    if (parsed.reviewEvidenceId !== expectedReviewEvidenceId)
        throw new Error("merge.reviewEvidenceIdが固定review Evidenceと一致しません");
    notBefore(parsed.preparedAt, pr.boundAt, "merge.preparedAt");
    if (parsed.dispatchClaimedAt)
        notBefore(parsed.dispatchClaimedAt, parsed.preparedAt, "merge.dispatchClaimedAt");
    if (parsed.observation)
        notBefore(parsed.observation.observedAt, parsed.preparedAt, "merge.observation.observedAt");
    if (parsed.observation) {
        ...
        if (request &&
            ...
            throw new Error("merge providerRequestがprovider mergedAtより後になっています");
    }
    return parsed;
}
function parseStep11(value, create, pr, merge) {
    if (value === null)
        return null;
    if (!isRecord(value))
        throw new Error("step11はobjectまたはnullが必要です");
...
function parseTerminalRedelivery(value, step11, merge) {
    if (value === undefined)
        return undefined;
    if (!isRecord(value))
        throw new Error("redeliveryはobjectが必要です");
    ...
    if (!merge)
        throw new Error("redeliveryには固定済みmerge intentが必要です");
    ...
    if (parsed.priorStep11EvidenceId !== step11.evidenceId ||
    ...
    if (parsed.outcome === "pending") {
        ...
        if (merge.observation?.providerState !== "merged" ||
...
function validateShape(state) {
    ...
    if (state.state === "step11-recorded") {
        ...
        if (!step11)
            throw new Error("delivery state step11-recordedが不正です: Step 11記録が必要です");
    ...
    if (reconciliation === null)
        throw new Error(`delivery state ${state.state}が不正です: reconciliation記録が必要です`);
    if (state.step11 && !state.redelivery)
        fail("reconciliation中はstep11がnullでなければなりません");
    if (reconciliation.phase === "create") {
        if (state.pr || state.merge)
            fail("create照合中はprとmergeがnullです");
        notBefore(reconciliation.enteredAt, state.create.preparedAt, "reconciliation.enteredAt");
    }
    else {
        const merge = state.merge;
        if (!state.pr || !merge)
            throw new Error(`delivery state ${state.state}が不正です: merge照合中は固定済みmerge intentが必要です`);
        notBefore(reconciliation.enteredAt, merge.preparedAt, "reconciliation.enteredAt");
    }
}
export function parseDeliveryState(source) {
    const raw = parseJsonStrict(source, DELIVERY_STATE_FILE);
    if (!isRecord(raw))
        throw new Error("delivery stateはobjectが必要です");
    const value = { ...raw };
    ...
    if (value.state !== "create-prepared" &&
...
export function renderDeliveryState(state) {
    const parsed = parseDeliveryState(stableJson(state));
    return `${stableJson(parsed)}\n`;
}
export function deliveryStateDigest(state) {
    return canonicalDigest(parseDeliveryState(stableJson(state)));
}
export function preparePullRequestCreation(create) {
    const candidate = {
        schemaVersion: DELIVERY_STATE_SCHEMA_VERSION,
        revision: 1,
...
export function claimPullRequestCreationDispatch(current, claimedAt) {
...
export function resumePullRequestCreationAfterConfirmedAbsence(current) {
    if (current.state !== "reconciliation-required" ||
        current.reconciliation?.phase !== "create")
        throw new Error(`${current.state}はPR createのabsence確認から再開できません`);
    if (current.create.dispatchClaimedAt !== null)
        throw new Error("消費済みPR create dispatch claimはabsence確認で再開できません");
    if (current.pr || current.merge || current.step11)
        throw new Error("PR create再開前のdelivery state形状が不正です");
    return parseDeliveryState(stableJson({
        ...current,
        revision: current.revision + 1,
        state: "create-prepared",
        reconciliation: null,
...
export function assertImmutablePullRequestBinding(state, input) {
    if (!state.pr)
...
export function bindPullRequest(current, binding) {
    if (current.state !== "create-prepared" &&
...
export function prepareMergeIntent(current, merge) {
...
export function completeTerminalRedelivery(current, completedAt) {
...
export function claimMergeDispatch(current, claimedAt) {
...
export function observeMerge(current, observation) {
    ...
    if (current.state === "merge-observed") {
        ...
        if (!previous)
            throw new Error("merge-observedに観測記録がありません");
        if (stableJson(previous) === stableJson(complete))
            return current;
        if (previous.providerState !== "merge-requested" ||
            complete.providerState !== "merged")
            throw new Error("merge observationはmerge-requestedからmergedへだけ更新できます");
        notBefore(complete.observedAt, previous.observedAt, "merge.observation.observedAt");
    }
    const candidate = {
        ...current,
        revision: current.revision + 1,
...
export function recordStep11(current, input) {
    ...
    if (!current.pr)
        throw new Error("Step 11記録には固定済みPRが必要です");
    const candidate = {
        ...current,
        revision: current.revision + 1,
        state: "step11-recorded",
...
export function requireDeliveryReconciliation(current, input) {
    const expectedPhase = current.state === "create-prepared"
        ? "create"
        : current.state === "merge-prepared" || current.state === "merge-observed"
            ? "merge"
            : undefined;
    if (!expectedPhase || input.phase !== expectedPhase)
        throw new Error(`${current.state}から${input.phase} reconciliationへ遷移できません`);
    const candidate = {
        ...current,
        revision: current.revision + 1,
