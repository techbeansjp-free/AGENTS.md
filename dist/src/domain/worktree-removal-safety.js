...
function safeLiteralSegment(value) {
    return (value !== "" &&
        value !== "." &&
        value !== ".." &&
        value !== ".git" &&
...
function parseFinalizeIgnoredPathPattern(value) {
...
function matchesPrefix(artifact, prefix) {
    ...
    if (pattern.kind !== "directory-prefix" &&
    ...
    if (pattern.kind === "recursive-basename") {
        ...
        if (!pattern.wildcard)
            return basename === pattern.prefix;
        return (basename.length >= pattern.prefix.length + pattern.suffix.length &&
            basename.startsWith(pattern.prefix) &&
            basename.endsWith(pattern.suffix));
    }
    return (artifact === pattern.value.slice(0, -1) ||
