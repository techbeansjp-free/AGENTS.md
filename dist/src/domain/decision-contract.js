...
export const DECISION_CANDIDATES = Object.freeze([
    ...
    Object.freeze({
        ...
        exclusionReason: "BR-01（呼び出し元を名指しできない）。分類行為自体はSKILL.md手順内で進行役が行い、review-convergence.ts:347のoneOf(finding.severity, ...)は分類結果を検証するだけでcompiled codeへのseamが無い",
    }),
    Object.freeze({
        id: "DCAND-007",
    ...
    Object.freeze({
        id: "DCAND-012",
        label: "classifyPackageAssets/pathIsSensitive/contentIsSensitive（secret/credentialのfilename・content heuristic分類）",
        decisionSiteFile: "src/domain/enforcement.ts",
        decisionSiteLine: "1478-1530",
        direction: "fail-closed",
        disposition: "excluded",
        exclusionReason: "BR-01（production codeに呼び出し元が無い）。参照はtest/steps/risk-policy.steps.tsのtestだけであり、Semantic Graphと同型の「実装されているが誰も呼ばない」状態",
    }),
]);
