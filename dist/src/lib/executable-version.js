...
function versionTuple(value) {
    const match = /(?:^|\s)v?(\d+)\.(\d+)(?:\.(\d+))?(?!\.\d)(?=$|[^\p{L}\p{N}])/u.exec(value);
    if (!match)
        return null;
...
export function inspectExecutableVersion(command, args, cwd, minimumVersion) {
    const minimum = versionTuple(minimumVersion);
    if (!minimum)
        throw new Error(`最低version ${minimumVersion}が不正です`);
    ...
    return inspectExecutableVersionOutput(command, `${result.stdout}\n${result.stderr}`, minimumVersion);
}
export function inspectExecutableVersionOutput(command, output, minimumVersion) {
    const minimum = versionTuple(minimumVersion);
    if (!minimum)
        throw new Error(`最低version ${minimumVersion}が不正です`);
...
export function assertMinimumExecutableVersion(command, args, cwd, minimumVersion) {
...
//# sourceMappingURL=executable-version.js.map