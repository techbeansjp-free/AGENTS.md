...
import { parseReviewProgressRecords, projectReviewProgressTarget, reviewProgressTargets, } from "../domain/review-progress.js";
...
export function recordLayerSuffix(staging, root, fromSha, toSha, session) {
    const inventory = session.anchor.progressInventory;
    if (!inventory || fromSha === toSha)
        return undefined;
    ...
    for (let index = 0; index < fields.length; index += 2) {
        const match = /^:(?<srcMode>[0-7]{6}) (?<dstMode>[0-7]{6}) [0-9a-f]+ [0-9a-f]+ (?<status>[AM])$/u.exec(fields[index] ?? "");
        if (!match?.groups || match.groups.dstMode !== "100644")
            return undefined;
        if ((match.groups.status === "A" && match.groups.srcMode !== "000000") ||
            (match.groups.status === "M" && match.groups.srcMode !== "100644"))
            return undefined;
        changed.push(fields[index + 1] ?? "");
    }
    const targets = new Map(reviewProgressTargets(inventory).map((target) => [
        path
    ...
    if (changed.filter(isEvidenceOnlyPath).length !== 1 ||
        changed.some((changedPath) => !isEvidenceOnlyPath(changedPath) && !targets.has(changedPath)))
        return undefined;
...
//# sourceMappingURL=review-record-layer.js.map