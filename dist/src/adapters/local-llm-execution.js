...
import { DEFAULT_LOCAL_REVIEW_MAX_OUTPUT_TOKENS, isValidLocalReviewMaxOutputTokens, } from "../domain/local-review-limits.js";
function requestLocalLlm(url, body, signal) {
...
function parseOllamaResponse(text) {
    const parseRecord = (value) => JSON.parse(value);
    try {
...
export async function executeLocalLlm(input, limits = {}) {
    ...
    const maxOutputTokens = input.maxOutputTokens ?? DEFAULT_LOCAL_REVIEW_MAX_OUTPUT_TOKENS;
    ...
    try {
        let response;
        try {
            response = await requestLocalLlm(new URL("/api/generate", url), JSON.stringify({
                model: input.model,
                prompt: input.prompt,
        ...
        if (parsed === undefined)
            return {
                state: "unknown",
                reason: "ローカルLLM応答が既定の形状（改行区切りresponse/done）と一致しません",
            };
        if (parsed.doneReason === "length")
            return {
                state: "unknown",
                reason: "ローカルLLM出力が生成token上限に達しました。maxOutputTokensまたは対象範囲を調整してください",
            };
        return {
            state: "succeeded",
