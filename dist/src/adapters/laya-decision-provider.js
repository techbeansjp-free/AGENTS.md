...
function parseResponse(source, maximumBytes) {
    if (Buffer.byteLength(source) > maximumBytes)
        throw new Error("Laya responseが上限を超えました");
    const parsed = parseJsonStrict(source, "Laya response");
    if (!parsed || typeof parsed !== "object" || Array.isArray(parsed))
        throw new Error("Laya responseがobjectではありません");
    const value = parsed;
    if (Object.keys(value).some((key) => !["answer", "confidence"].includes(key)) ||
...
export class LayaDecisionProvider {
    id = "laya";
    #config;
    #transport;
    constructor(config, transport) {
        ...
        if (!Number.isSafeInteger(config.timeoutMs) ||
            config.timeoutMs < 1 ||
        ...
        if (!Number.isSafeInteger(config.maximumResponseBytes) ||
    ...
    async decide(request) {
        ...
        try {
            ...
            const parsed = parseResponse(response.body, this.#config.maximumResponseBytes);
...
//# sourceMappingURL=laya-decision-provider.js.map