...
export const EVALUATION_LABEL_DIRECTORY = ".agent-skill-chain/runtime/decisions";
function labelDirectory(primaryRoot, id) {
    return path.join(primaryRoot, EVALUATION_LABEL_DIRECTORY, path.basename(id));
}
function labelPath(primaryRoot, id) {
    return path.join(labelDirectory(primaryRoot, id), "evaluation-labels.jsonl");
}
export function readEvaluationLabels(primaryRoot, id) {
    const file = labelPath(primaryRoot, id);
    if (!fs.existsSync(file))
        return { labels: [], errors: [] };
    const stat = fs.lstatSync(file);
    if (stat.isSymbolicLink() || !stat.isFile())
        throw new Error("evaluation label journalはsymlinkでない通常fileが必要です");
    const lines = fs
...
export function findEvaluationLabel(primaryRoot, id, decisionRecordId) {
    const { labels, errors } = readEvaluationLabels(primaryRoot, id);
    if (errors.length > 0)
        throw new Error(`evaluation label journal（${id}）に不正な行があるため参照できません: ${errors.join("; ")}`);
 ...
 */
export function appendEvaluationLabel(primaryRoot, id, label) {
    const existingDecision = findDecisionJournalRecord(primaryRoot, id, label.decisionRecordId);
    if (existingDecision === undefined)
        throw new Error(`decision journalに存在しないdecisionRecordIdへlabelを記録しようとしました: ${label.decisionRecordId}`);
    ...
    withStagingMutationLock(directory, () => {
        ...
        if (existing.errors.length > 0)
            throw new Error(`evaluation label journalの既存内容が不正です: ${existing.errors.join("; ")}`);
        if (existing.labels.some((entry) => entry.decisionRecordId === label.decisionRecordId))
            throw new Error(`decisionRecordIdが既存evaluation label journalと重複しています: ${label.decisionRecordId}`);
        ...
        if (reread.errors.length > 0 ||
            !reread.labels.some((entry) => entry.decisionRecordId === label.decisionRecordId))
            throw new Error("evaluation label journalの書き込み後read-backが一致しません");
    });
}
//# sourceMappingURL=evaluation-label-store.js.map