...
import { evaluateLocalLlmReview, } from "../domain/review-verdict.js";
 ...
 * fileへは依存しない（INV-05）。
 */
function readPrompt(root, promptFile) {
    if (/[\p{Cc}\p{Cf}]/u.test(promptFile) ||
...
export async function launchReview(input, dependencies = {}) {
    const identifiers = [
        input.scope,
        input.coordinator,
        input.implementer,
    ...
    const executor = dependencies.execute ?? REVIEWER_EXECUTORS[decision.provider];
    if (!executor)
...
//# sourceMappingURL=review-launch.js.map