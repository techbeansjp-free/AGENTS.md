...
export function buildReviewRoundDraft(input) {
    ...
    if (previous === null) {
        ...
        for (const targetPath of requestedTargets) {
            ...
            if (!stat) {
                if (explicitTargets)
                    throw new Error(`parallel progress targetが存在しません: ${targetPath}`);
                continue;
            }
            /**
            ...
            if (!source.includes(PROGRESS_START) && !source.includes(PROGRESS_END)) {
                if (explicitTargets)
                    throw new Error(`parallel progress markerがありません: ${targetPath}`);
                continue;
            }
...
function verifyReviewRoundDecisionRefs(root, staging, candidateHeadSha, findings) {
    const decisionRefFindings = findings.filter((finding) => finding.decisionRef !== null && finding.decisionRef !== undefined);
    ...
    for (const finding of decisionRefFindings) {
        ...
        if (record.effectiveValue !== finding.severity)
            throw new Error(`review round finding ${finding.id}のseverity ${finding.severity} がdecisionRef ${decisionRecordId} のeffectiveValue ${record.effectiveValue} と一致しません`);
    }
}
...
export function assertConvergedReviewSession(input) {
    ...
    if (effectiveHeadSha !== input.currentHeadSha &&
        ...
        recordLayerSuffix(staging, stagingRepositoryRoot(staging), effectiveHeadSha, input.currentHeadSha, session) === undefined)
