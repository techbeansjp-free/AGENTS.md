...
export const GIT_WORKSPACE_REJECTION = "Git workspace境界を検証できません。rootとGit worktree登録を修復して新しい起動要求を行ってください";
...
function hasGitOverrides() {
    return Object.keys(process.env).some((key) => /^GIT_(?:DIR|WORK_TREE|COMMON_DIR|INDEX_FILE|OBJECT_DIRECTORY|ALTERNATE_OBJECT_DIRECTORIES|NAMESPACE|CONFIG.*)$/u.test(key) && process.env[key] !== undefined);
}
/** Resolve only primary repositories and mutually registered linked worktrees. */
export function resolveGitMetadataWriteRoots(root) {
    try {
        if (hasGitOverrides())
            return { allowed: false };
        const identities = new Map();
        const contents = new Map();
        const checkPath = (value, directory) => {
            ...
            const stat = fs.lstatSync(current);
            if (fs.realpathSync(value) !== current ||
        ...
        if (lines.length !== 4 ||
            ...
            bare !== "false")
            return { allowed: false };
        if (checkPath(gitDir, true) !== gitDir ||
        ...
        if (linked) {
            ...
            if (!forward.startsWith("gitdir: "))
                return { allowed: false };
            const resolveLink = (base, value, directory) => checkPath(path.isAbsolute(value) ? value : `${base}${path.sep}${value}`, directory);
            if (resolveLink(root, forward.slice(8), true) !== gitDir ||
