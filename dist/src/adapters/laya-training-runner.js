...
import { createLayaTrainingArtifactBindings, validateLayaTrainingDatasets, } from "../domain/laya-training-contract.js";
...
export function launchLayaTrainingRunner(request, execute = run) {
    ...
    const readArtifact = (reference, label) => {
    ...
    validateLayaTrainingDatasets(fs.readFileSync(train, "utf8"), fs.readFileSync(validation, "utf8"), validated, bindings);
    ...
    if (path.dirname(output) !== runRoot)
        throw new Error(`Laya runner outputは${LOCAL_RUN_ROOT}直下でなければなりません`);
