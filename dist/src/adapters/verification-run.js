 ...
 */
function verificationDeliveryPhase(staging) {
    ...
    if (delivery !== undefined &&
        ...
        throw new Error(`delivery stateが${delivery.state}（merge段階以降）のためverify runを記録できません。merge済み・merge準備済みの証跡へ後から検証記録を足してstagingを動かすことはできません`);
    const journal = readWorkflowJournal(staging);
    if (journal.errors.length > 0)
        throw new Error(`workflow journalを読めないためverify runを記録できません: ${journal.errors.join("; ")}`);
...
export async function runVerification(input) {
    ...
    const violation = verificationCommandViolation(argv, input.scope, policy, impact.features);
