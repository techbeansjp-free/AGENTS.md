...
const CANONICAL_MAIN_API = "https://api.github.com/repos/techbeansjp-free/AGENTS.md/git/ref/heads/main";
const CANONICAL_PIN_RAW_PREFIX = "https://raw.githubusercontent.com/techbeansjp-free/AGENTS.md/";
const CANONICAL_PIN_PATH = ".agent-skill-chain/policy/laya-private-training-authorization-pins.json";
...
export function readOwnerOnlyFile(file, maximumBytes) {
    const resolved = path.resolve(file);
    const direct = fs.lstatSync(resolved);
    if (direct.isSymbolicLink() || fs.realpathSync(resolved) !== resolved)
        throw new Error("private fileのsymbolic linkまたはsymlink祖先を拒否しました");
    ...
    try {
        ...
        if (!before.isFile() ||
            ...
            (before.mode & 0o077) !== 0)
            throw new Error("private fileはownerだけが読める上限以下の通常fileが必要です");
...
export function readPrivateAuthorizationPins(file) {
    return parsePrivateAuthorizationPins(readOwnerOnlyFile(file, 65_536).toString("utf8"));
}
function parsePrivateAuthorizationPins(source) {
    ...
    const value = parsed;
    if (Object.keys(value).some((key) => !["schemaVersion", "sha256"].includes(key)) ||
        value.schemaVersion !== "asc/laya-private-authorization-pins/v1" ||
        !Array.isArray(value.sha256) ||
        value.sha256.some((digest) => typeof digest !== "string" || !/^[0-9a-f]{64}$/u.test(digest)) ||
...
async function readBoundedResponse(response, maximumBytes) {
    if (!response.ok)
        throw new Error("ASC公式GitHubの読取に失敗しました");
    const source = await response.text();
    if (Buffer.byteLength(source) > maximumBytes)
        throw new Error("ASC公式GitHubの応答が上限を超えました");
    return source;
}
export async function readTrustedPrivateAuthorizationPins() {
    const commitSource = await readBoundedResponse(await fetch(CANONICAL_MAIN_API, githubRequest()), 65_536);
    ...
    const object = commit.object;
    const trustedSha = object && typeof object === "object" && !Array.isArray(object)
        ? object.sha
        : undefined;
    ...
    const source = await readBoundedResponse(await fetch(pinUrl, githubRequest()), 65_536);
    return parsePrivateAuthorizationPins(source);
}
export function readPrivateCorpusAuthorization(file, pins) {
    ...
    const parsed = parseJsonStrict(bytes.toString("utf8"), "private authorization");
    if (!parsed || typeof parsed !== "object" || Array.isArray(parsed))
    ...
    if (value.schemaVersion !== "asc/laya-private-authorization/v1" ||
        ...
        value.forbiddenLiterals.some((item) => typeof item !== "string" || item.length < 4))
        throw new Error("private authorization contractが不正です");
    return value;
}
//# sourceMappingURL=laya-private-authorization.js.map