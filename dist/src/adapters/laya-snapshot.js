...
import { digestLayaArtifact, LAYA_DATASET_SCHEMA_VERSION, validateDecisionCase, } from "../domain/laya-decision-training.js";
...
export function assertPublicAscRepository(rootInput) {
...
export function extractPublicAscReviewCases(rootInput, revision) {
    ...
    for (const artifactPath of paths) {
        ...
        const observedAt = new Date(git(["log", "-1", "--format=%cI", commit, "--", artifactPath], root).stdout.trim()).toISOString();
        ...
        for (let index = 0; index < sourceLines.length; index++) {
            ...
            if (inFindings && NEXT_SECTION.test(line))
                break;
    ...
    for (const fixCommit of fixCommits) {
        const metadata = git(["show", "-s", "--format=%cI%n%s", fixCommit], root).stdout.split(/\r?\n/u);
        const observedAt = new Date(metadata[0] ?? "").toISOString();
        const claim = (metadata[1] ?? "").trim();
        const artifactPath = lines(git([
            "-c",
            "core.quotePath=false",
            "show",
            "--format=",
            ...
            "--diff-filter=AM",
            fixCommit,
        ], root).stdout).find((entry) => entry && !entry.startsWith(".agent-skill-chain/tmp/"));
        if (!claim || !artifactPath)
            continue;
...
export function extractDistributionDecisionCases(rootInput, revision) {
    const root = assertPublicAscRepository(rootInput);
    const commit = exactCommit(root, revision);
    ...
    for (const changeCommit of commits) {
        const metadata = git(["show", "-s", "--format=%cI", changeCommit], root).stdout.trim();
        const observedAt = new Date(metadata).toISOString();
        const changed = new Map();
        for (const row of lines(git([
            "-c",
            "core.quotePath=false",
            "diff-tree",
            "--root",
            ...
            if (changedPath)
                changed.set(changedPath, status[0] ?? "?");
        }
        for (const sourcePath of [...changed.keys()]
            .filter((entry) => (entry.startsWith("src/") || entry.startsWith("bin/")) &&
            entry.endsWith(".ts") &&
            !entry.endsWith(".d.ts"))
            .sort()) {
            ...
            cases.push(validateDecisionCase(decisionCase));
            if (cases.length >= 2048)
                return cases;
        }
    }
    return cases.sort((left, right) => left.caseId.localeCompare(right.caseId));
}
export function snapshotManifest(rootInput, revision) {
    const root = assertPublicAscRepository(rootInput);
    ...
    return {
        schemaVersion: "asc/laya-source-snapshot/v1",
        sourceRepository: "techbeansjp-free/AGENTS.md",
        sourceCommit,
...
//# sourceMappingURL=laya-snapshot.js.map