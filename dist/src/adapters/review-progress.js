...
import { latestReviewProgressDigest, makeReviewProgressEntry, makeReviewProgressSeal, parseReviewProgressRecords, projectReviewProgressTarget, reviewProgressTargets, } from "../domain/review-progress.js";
...
function context(stagingInput) {
    ...
    if (!session?.anchor.progressInventory)
        throw new Error("review sessionにparallel progress inventoryがありません。従来の直列経路を使用してください");
    const root = stagingRepositoryRoot(staging);
    const head = git(["rev-parse", "--verify", "HEAD^{commit}"], root).stdout.trim();
    ...
    for (const record of records)
        if (record.sessionId !== session.sessionId ||
            ...
            throw new Error("progress journalのreview sessionまたはH_impl bindingが不正です");
    const targets = reviewProgressTargets(session.anchor.progressInventory).map((inventory) => {
        ...
        if (targetStat.isSymbolicLink() ||
            ...
            throw new Error(`parallel progress targetのidentityまたはmodeが不正です: ${inventory.targetPath}`);
        const targetDigest = crypto
            .createHash("sha256")
            .update(fs.readFileSync(target))
            .digest("hex");
        if (targetDigest !== inventory.baselineDigest)
            throw new Error(`parallel progress targetがreview開始時点から変化しました: ${inventory.targetPath}`);
...
function readJournal(file) {
    ...
    if (stat.isSymbolicLink() ||
        ...
        throw new Error("progress journalはmode 100600の単一link通常fileが必要です");
    return parseReviewProgressRecords(fs.readFileSync(file, "utf8"));
}
function writeJournal(file, records) {
    fs.mkdirSync(path.dirname(file), { recursive: true, mode: 0o700 });
    writeFileAtomic(file, records.map((record) => stableJson(record)).join("\n") + "\n", { temporaryDirectory: path.dirname(path.dirname(file)) });
...
export function appendReviewProgress(input) {
    ...
    const duplicate = observed.records.find((record) => "taskId" in record &&
