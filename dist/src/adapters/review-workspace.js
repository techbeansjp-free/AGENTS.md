...
export function peekPrimaryReviewRoot(root) {
    try {
...
export function resolveReviewRoot(root) {
    ...
    const [primaryTop, primaryGitDir, primaryCommonDir, primaryBare] = gitIdentity(primaryRoot);
    if (!isCanonicalDirectory(primaryRoot) ||
...
export function resolveReviewWorkspace(root, stagingPath) {
    ...
    const stagingRoot = git(["rev-parse", "--show-toplevel"], staging).stdout.trim();
    if (stagingRoot !== root)
        throw new Error("stagingとrootは同じGit worktreeである必要があります");
    return workspace;
}
//# sourceMappingURL=review-workspace.js.map