...
export const COMMAND_USAGE = Object.freeze([
    ...
    },
    {
        ...
        inputContract: {
            description: "共通envelope: {candidateHeadSha（40桁16進数、実HEADと一致必須）, subjectRef（判断対象を指す文字列）, payload?（deterministic resolverへ渡す型別入力）, proposedValue?（provider実行時の提案。lightweight-tierは呼び出し側の提案をそのまま受ける自己申告provider）, confirmedBy?（advisory/one-way-escalationの緩和方向を確認した進行役識別子）}。constrained-choice（DCAND-009）は payload.candidateSet（文字列配列）が必須。出力の state/resolution系field（jevProviderConfig、workspace、authorityMode、requiresConfirmation、rejected）を読めば、なぜProviderが使われた/使われなかったかを別途スクリプトなしで確認できる",
            example: {
