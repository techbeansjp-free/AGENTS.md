...
export const CLI_USAGE = "npx agent-skill-chain <issue|project|spec|review|trace|graph|conformance|policy|decision|routing|workflow|worktree|pr|install|update|delete|doctor> ...";
    ...
    switch (ruleId) {
        case "FR-836-02":
            return {
                ...
                resume: "provider実行入口（Codex）のread-only観測がavailableになった時点",
            };
        case "FR-836-10":
            return {
...
export function routingDiagnostic(ruleId, reason, options = {}) {
    return {
        ruleId,
        ...
        rollback: options.rollback ?? "実装を開始せず、既存のscopeとworktreeを保持する",
    };
}
