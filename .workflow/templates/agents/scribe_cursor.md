...
1. You receive a structured log entry from the parent. **Required** per [scribe/CONTRACT](../../../.agents/scribe/CONTRACT.md): command, summary, dod_met; **you MUST set** ts_utc and created_at at record time (ISO8601). Optional: issue_path, changed_files. See CONTRACT §入力・必須キー一覧.
2. Record exactly one log entry to **workflow.db** (SQLite **workflow_log** table) by calling **.agents/scripts/write-workflow-log.sh** only. Do **not** run sqlite3 directly; sqlite3 直接実行は禁止。.agents/scripts/write-workflow-log.sh のみ使用すること。Do not write to `.workflow/**/logs/` (deprecated). Schema: [ledger/schema.md](../../../.agents/ledger/schema.md).
...
- 「書記だけがログを書く」は、**メインがログ記録タスクを書記サブにだけ委譲する**運用で担保する。他サブはログを書かず結果だけ返す。詳細は [.agents/scribe/README.md](../../../.agents/scribe/README.md) および [.agents/agents/scribe.md](../../../.agents/agents/scribe.md) を参照。
