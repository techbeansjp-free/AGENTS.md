...
| AC-4（誤検出しない） | 同モジュールの判定ロジック自体（`state !== 'CHANGES_REQUESTED'` を除外・`since` 基準時刻以前のコメントを除外・`trusted_reviewer_actors`該当authorまたは`<!-- agent-skill-chain:`定型marker始まりのコメントを除外）＋ `src/commands/segment.ts` 側で空配列時はブロック自体を省略 | 「対応が必要な既存レビューが存在する」という虚偽の通知を作らないためには、該当が無い場合にセクション自体を出力しないのが最も確実（`buildIssueBlock` が採る既存パターンと同じ方針）。author除外の根拠は「未対応の判定基準」節参照。 |
...
- **author除外（AC-4関連の設計判断）**: 上記の時刻基準だけでは、workerやgate-review自体がIssue/PRへ投稿する定型コメント（完了報告・review evidence等）も「未対応」として誤検出しうる。`detectGithubReviewStatus()` は、コメント投稿者（`author.login`）が `.agent-skill-chain/config/agent-skill-chain.yaml` の `review.trusted_reviewer_actors`（`.agent-skill-chain/project/manifest.yaml` の `core_review.execution.trusted_reviewer_actors` と同一の識別子集合）に含まれる、またはコメント本文が既知の定型marker（`<!-- agent-skill-chain:` で始まる行、worker完了報告・gate-review-evidence双方が用いる既存prefix）で始まる場合、そのコメントを「未対応」の判定対象から除外する。これによりAC-4（誤検出の禁止）が、人間以外が投稿した自動化由来コメントの再検出という未処理エッジケースも含めて満たされる。
...
- 想定される失敗モード:
  ...
  - (f) worker自身・gate-review自身がIssue/PRへ投稿した定型コメント（完了報告・review evidence等）が、次回resume時に「未対応の既存コメント」として誤って再検出される: 「未対応の判定基準」節のauthor除外により対処済み（`trusted_reviewer_actors` 該当author、または `<!-- agent-skill-chain:` 定型marker始まりの本文を除外）。
