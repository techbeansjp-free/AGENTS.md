...
| AC-4（dependabot許可判定の意図維持） | `agent-skill-chain-reconcile.yml`が参照するpush由来コンテキストフィールドの移行対象は次の2箇所に限定され、これ以外の箇所は移行不要である。(1)`Derive issue_id`ステップ（3分岐ロジック）の`env.BRANCH`：`github.ref_name`→`github.event.workflow_run.head_branch`。(2)`Reconcile gates against pushed SHA`ステップの対象SHA引数（`gate-reconcile.sh`第2引数）：`github.sha`→`github.event.workflow_run.head_sha`。両ステップとも3分岐dependabot判定ロジック・step名・step id・呼び出し引数の並びを含め字句はそのまま維持し、参照元イベントフィールドのみ置き換える。同じ`Derive issue_id`ステップの`env.GH_TOKEN`（`github.token`）・`env.REPO`（`github.repository`）・`env.OWNER`（`github.repository_owner`）の3フィールドは移行対象外とする（理由は備考欄） | 移行対象2箇所（BRANCH・対象SHA）はいずれもpushされた内容ではなくGitHubが提供するイベントメタデータへの参照元変更であり、3分岐判定ロジック自体は不変。移行対象外の3フィールド（GH_TOKEN・REPO・OWNER）は、`workflow_run`イベントでも`push`イベントと同じ値（実行トークン・対象リポジトリ・リポジトリオーナー）を返す、トリガー種別に依存しない安定値であるため、見落としではなく意図的に対象外と判断する |
| AC-5（承認済み基準データ自体の改ざん耐性） | `resolveApprovedBaseline`（新規・`src/commands/gate.ts`の`reconcile()`内部） | GitHubモードのみ対象。pushされたブランチ上でwriterが改変可能な`issues/<n>/.agent-skill-chain/reviews/<gate>.yaml`（`reviewFilePath`）を承認済み基準として信頼せず、GitHub Check Run発行履歴（`commits/{target_sha}/pulls` → `pulls/{number}/commits` → `commits/{sha}/check-runs`）から`approved_artifacts`を再構築する。さらに候補Check Runごとに`actions/runs?check_suite_id=`でsource identity（発行元workflowファイルパス）を検証し、trusted path（`agent-skill-chain-gate.yml`・`agent-skill-chain-reconcile.yml`）以外から発行された候補（攻撃者が新規追加したuntrustedワークフローが同名で偽装発行したCheck Runを含む）は採用しない。source identity検証を通過した候補の`output.text`（GitHub Check Run APIが持つ65,535文字上限により切り詰められうる）を`canonicalJson(GateReport)`としてパースする際にJSON構文エラー（切り詰めによるものを含む）が発生した場合も、当該候補を承認済み基準として使用不可として棄却し次候補（より古いCheck Run）を試す（候補が尽きれば承認済み基準なしとして扱う、I8の安全側）。ローカルモードは対象外（既存通り`reviewFilePath`を正本のまま維持、AGENTS.md I6のローカル正本モデルと整合） |
...
- `agent-skill-chain-reconcile.yml`（`workflow_run`化）: `on: workflow_run: workflows: ["agent-skill-chain / reconcile-trigger"]`で起動するtrusted workflow。既存の`permissions`（`contents: read`, `checks: write`, `pull-requests: read`）・job名`reconcile`・step名・step id `ctx`・3分岐dependabot判定ロジックはすべて維持する。push由来コンテキストフィールドを参照している箇所は`Derive issue_id`ステップの`env.BRANCH`（`github.ref_name`）と`Reconcile gates against pushed SHA`ステップの対象SHA引数（`github.sha`）の2箇所のみであり、それぞれ`github.event.workflow_run.head_branch`・`github.event.workflow_run.head_sha`へ置き換える。同ステップの`env.GH_TOKEN`（`github.token`）・`env.REPO`（`github.repository`）・`env.OWNER`（`github.repository_owner`）はpush/workflow_runいずれのトリガーでも同一の値を返す安定したコンテキスト値であり、移行対象に含めない。defaultブランチをtrust rootとしてcheckoutし、pushされたSHAは`git fetch`でread-onlyなgit objectとして取得するのみで、checkoutやビルド対象には含めない。
...
- `resolveApprovedBaseline`（新規・`src/commands/gate.ts`）: `reconcile()`がGitHubモード（`config.coordination.backend === 'github'`）で承認済み基準（`approved_artifacts`）を決定する際に用いる。pushされたブランチ上でwriter権限を持つ者が改変できるコミット済みファイル（`reviewFilePath`が指す`issues/<n>/.agent-skill-chain/reviews/<gate>.yaml`）を参照せず、次の手順でGitHub Check Run発行履歴から復元する。
  ...
  4. （source identity検証）手順(3)の各候補Check Runについて、`GET /repos/{owner}/{repo}/actions/runs?check_suite_id=<候補Check Runの`check_suite.id`>`を照会する（`head_sha`も候補コミットのSHAと併せて指定し二重に絞り込む）。応答`workflow_runs[]`のうち`check_suite_id`が一致するエントリの`path`フィールド（そのCheck Runを発行したworkflow定義ファイルのリポジトリ内パス。GitHubがワークフロー実行時に記録する権威あるメタデータであり、`checks: write`権限を持つ`GITHUB_TOKEN`を用いて任意のワークフローファイルから発行されたCheck Runであっても、発行元ファイル自身の実際のパスとしてしか報告されないため、pushされた内容から偽装できない）を検証する。`path`が既知のtrusted workflowファイルパス（`.github/workflows/agent-skill-chain-gate.yml`・`.github/workflows/agent-skill-chain-reconcile.yml`のいずれか）と一致しない場合、その候補は承認済み基準として採用せず、手順(3)へ戻り次に新しい候補を試す（例：攻撃者が新規追加した`.github/workflows/evil.yml`から同名の偽Check Runが発行されていても、`path`が`evil.yml`となり両trusted pathのいずれとも一致しないため棄却される）。source identity検証を通過した候補については、続けてその候補Check Runの`output.text`フィールドを`canonicalJson(GateReport)`としてパースし`approved_artifacts`を復元する。GitHub Check Run APIの`output.text`には65,535文字という上限があり、承認済み成果物が多いゲート（実装ゲート・検証ゲート、`docs/system-spec/`のような多数ファイルパッケージを含むゲート等）では`canonicalJson(GateReport)`がこの上限を超え切り詰められた不正なJSON文字列になりうる。このパースがJSON構文エラー（切り詰めによるものを含む）で失敗した場合も、その候補は承認済み基準として採用せず手順(3)へ戻り次に新しい候補を試す（source identity不一致の場合と同型のfail-safe、I8の安全側）。手順(3)の候補が尽きるまで有効な候補（trusted pathのsource identity検証を通過し、かつ`output.text`のJSONパースにも成功した候補）が1件も無い場合は、手順(1)でPRが0件の場合と同型に「承認済み基準なし」として当該ゲートの照合をスキップする（I8の安全側）。
  - (1)〜(4)（source identity検証のための`actions/runs`照会を含む）いずれかのGitHub API呼び出し自体がエラー応答した場合（0件応答ではなくAPI呼び出し失敗）は、reconcileコマンド全体を失敗させる（`CliError`）。判定不能を「変化なし」や「スキップ」に倒さない（I8）。0件応答（対応PR無し・過去のsuccess Check Run無し・trusted pathと一致する候補が0件）とAPI呼び出し失敗は区別する。候補Check Runの`output.text`のJSONパース失敗（65,535文字上限による切り詰めを含む）はAPI呼び出し失敗とは区別し、当該Check Run自体の`output.text`取得（HTTP応答）は成功しているため、より古い正当な候補が存在しうるとみなして手順(3)への次候補フォールバックとして扱う（reconcileコマンド全体は失敗させない）。
...
- 想定される失敗モード（5）: GitHub Check Run APIの`output.text`フィールドには65,535文字という上限があり、これを超える内容は切り詰められる。承認済み成果物点数が多いゲート（実装ゲート・検証ゲート、あるいは`docs/system-spec/`のような多数ファイルパッケージを含むゲート）では、Check Run発行時に書き込んだ`canonicalJson(GateReport)`がこの上限を超え、`resolveApprovedBaseline`が候補Check Runの`output.text`を読み出す際に切り詰められた不正なJSON文字列となりうる。この場合`resolveApprovedBaseline`は当該候補のJSONパース失敗時点で「承認済み基準として使用不可」と判定し、手順(3)へ戻り次に新しい候補（より古いCheck Run）を試す。候補が尽きれば手順(1)・(4)と同型に「承認済み基準なし」として当該ゲートの照合をスキップする（I8の安全側）。これは想定される失敗モード（3）が定めるGitHub API呼び出し自体のエラー応答（reconcileコマンド全体を失敗させる）とは区別する——`output.text`の取得自体（HTTP応答）は成功しており、取得できた文字列の中身がパース不能なだけであり、より古い正当なCheck Runが存在する可能性があるため、判定不能としてコマンド全体を失敗させるのではなく次候補へのフォールバックとして扱う。通常のgate-report（SPEC/DESIGN/PLANの数ファイル程度）ではこの上限に到達しないことが見込まれるが、実装・検証ゲートや`docs/system-spec/`のような多数ファイルパッケージでは到達しうるため、本設計に明記する。
- ロールバック手順（想定される失敗モード（5）分）: 本対処は`resolveApprovedBaseline`のJSONパース失敗時の分岐処理のみであり独立にロールバック可能だが、ロールバックするとJSON切り詰め発生時に不正なJSONパース結果が未定義動作（例外送出によるreconcileコマンド全体の意図しない失敗、またはパース失敗を検知できず誤った`approved_artifacts`を採用する動作）につながりうる点に留意する。
...
- 移行ギャップ（本修正マージ前から存在する既存issueブランチへの適用範囲）: 本設計が導入する`workflow_run`分離（変更単位#1・#2）は、その内容が保護されたdefaultブランチ（main）へマージされて初めて効果を持つ。GitHub Actionsの`push`トリガーは、実行されるワークフローYAML自体を常にpushされたコミットが属するブランチの内容から読み込む（本設計冒頭「背景の要約」で述べた性質そのもの）ため、本Issueのマージ前から存在し、かつマージ後もそのブランチが新しいmainの内容を取り込んでいない既存issueブランチは、自身のコミット履歴上に残る旧`agent-skill-chain-reconcile.yml`（`on: push`、判定ロジック自体をpushされた内容から読み込む自己参照構成）をそのまま保持し続ける。したがって、そうした既存ブランチへの新規pushに対しては、本Issueのマージ後であっても、当該ブランチが新しいmainを取り込むまでの間、AC-1・AC-5が解消しようとする脆弱性（判定ロジック改変・承認済み基準データ改ざんによるCheck Run偽造）がそのブランチに限り残存する。新規に`issue-start`等で作成されるブランチは、作成時点でdefaultブランチの内容（新しい2ファイル構成）を継承するため、この移行ギャップの対象にならない。
- 移行ギャップへの対応方針（実装単位としてはスコープ外とする判断）: 本設計・PLAN.mdの変更単位は、進行中の既存issueブランチを強制的に新しいmain内容へ追従させる機構（強制rebase・強制merge・CI側でのブランチ内YAMLバージョン検査等）を一切含まない。これは、`push`トリガーの読み込むワークフロー定義がブランチごとに独立して保持されるというGitHub Actionsの一般的な性質に起因し、`push`トリガーを持つ他の多くのワークフロー修正一般に共通する不可避な性質であって、本Issueが新規に生み出す問題ではないため、本Issueの実装単位として個別の強制追従機構を追加することはSPEC.mdスコープ外節が定める「本Issueはpushトリガーのワークフロー自己参照という個別の脆弱性の解消に限定する」方針と整合する形でスコープ外とする。したがってPLAN.mdへ本ギャップ対応のための変更単位を追加する必要はないと判断する。
- 移行ギャップに対する運用上の対応（推奨手順であり本Issueが新規導入する特別な手順ではない）: 本修正のマージ後、進行中の既存issueブランチについては、次回のpush前に`git merge origin/main`（または同等のrebase操作）で新しいmainの内容（`agent-skill-chain-reconcile-trigger.yml`および`workflow_run`化された`agent-skill-chain-reconcile.yml`を含む）を取り込むことを運用上推奨する。これは他のセキュリティ修正がmainへマージされた際に進行中のブランチが新しいmain内容を取り込むこととの整合を図る一般的な運用手順であり、取り込みが完了するまでの間は当該ブランチが前項の移行ギャップの影響を受け続ける点に留意する。
