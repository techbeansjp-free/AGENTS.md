...
- golden snapshot生成は、コード変更を一切含まない独立した変更単位として、`buildReviewerPrompt()`（`src/commands/gate.ts`）への`--full-index`追加より**前**に着手する（PLAN.mdの実装順序はこの前後関係を維持する）。`buildReviewerPrompt()`（`src/commands/gate.ts`）がまだ`--full-index`引数を含まない現状のコード状態のまま、固定入力（`issue_number`・`gate_id`・`target_sha`・`base_sha`を固定した最小fixtureリポジトリ、`test/fixtures/`配下に既存の類似fixture方式があればそれに合わせて配置）に対して`buildReviewerPrompt()`を1回実行し、その出力を`test/fixtures/gate-reviewer-prompt-golden.txt`としてリポジトリに固定コミットする。この変更単位を`--full-index`追加より先に完了・コミットしておくことで、「修正前」コードを事後的に復元する手段を用意する必要なく、その出力を確実に捕捉できる。このgoldenファイルは以降再生成しない固定値として扱う。
- fixtureリポジトリの`target_sha`・`base_sha`はテスト実行のたびに変化してはならない。共有ヘルパー`test/helpers/tmp-repo.ts`の`createTmpRepo()`はコミット日時を固定しないため本用途には使わず、本テスト専用に`git init`直後の一時ディレクトリへ`GIT_AUTHOR_DATE`・`GIT_COMMITTER_DATE`を固定値（例: `2000-01-01T00:00:00Z`/`...:01Z`）で明示指定して2コミットを作成する。commit content・author・committer・timestampのすべてを固定することでSHA自体が実行環境に依存しない決定的な値になり、goldenファイル生成時に記録した`target_sha`・`base_sha`の定数値をテストコード内でアサーションし、fixture構築手順が変化した場合はSHA不一致としてテストが即座に失敗する構成にする。
- 新規テスト（`test/integration/gate-judgment.test.ts`へ追加。golden snapshot生成・`--full-index`追加の完了後に実装する）では、`--full-index`追加後の`buildReviewerPrompt()`出力に対し、「判定対象の差分」セクション内のhash桁数表記行（`index <hash>..<hash>`等）のみを正規表現で除去・正規化したうえで、goldenファイルの同一箇所を同じ正規化にかけた文字列と完全一致することを検証する。これにより「diff区間のhash桁数表記を除き修正前と同一」というAC-4の条件を、実際に修正前のコードを再実行することなく機械的に証明する。
