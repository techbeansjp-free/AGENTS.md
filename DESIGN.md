...
|---|---|---|
| 要件1（準備段を信頼実行環境の用意に限定） | E1, E2, E3 | consumer project では調達モードが `package_copy` に確定し npm を起動しない |
| 要件2（package.json・lockfile・build script を持たない consumer） | E1, E3 | 判定入力に consumer の Node 構成を用いない |
| 要件3(a)（実行コードと asset が審査対象由来でない） | E3, E6, E8, E10 | 候補の除外規則・解決の閉鎖・記録時の再検査・asset 解決基点の維持 |
...
| 要件6（launcher digest の算出対象の上限と下限） | E7 | 配布集合内で下限を満たす列挙、全件取得の前提維持 |
| 要件7（調達実行コードの由来・完全性・記録） | E3, E4, E5, E8 | 期待値供給元の限定と二重検証 |
| AC-1 | E1, E3, E11 | 非 Node consumer で npm 呼び出し記録が空 |
| AC-2 | E1, E3, E11 | build script の痕跡ファイルが隔離 clone 内に生じない |
| AC-3 | E3, E6, E11 | 起動スクリプトと adapter の解決元が隔離 clone |
| AC-4 | E8, E9, E11 | 既存 3 メッセージの保持 |
| AC-5 | E3, E6, E11 | 供給元不在時の非0終了と日本語メッセージ |
...
| AC-15 | E10, E11 | prompt 生成が読む asset の解決基点 |
...
- `E3 package_copy 経路（調達段）`（`.agent-skill-chain/scripts/cli-resolve.sh` の新規公開関数として実装し、`gate-local-review.sh` が隔離 clone 内の同ファイルを読み込んで呼ぶ）: 隔離 clone の外にある配布パッケージの実体を検証し、隔離 clone 配下へ複製して実行可能にすることだけを責務とする。npm による依存導入・build script の実行を一切行わない。
  ...
  - 失敗時の分岐: 候補が 1 つも存在しない場合は「信頼実行コードの供給元が実行環境に存在しない」ことを、候補は存在したが期待値と一致するものが無い場合は「調達候補の完全性検証に失敗した」ことを、それぞれ前提として明示し、是正手段（前者は配布パッケージの導入、後者は導入版と導入マーカーの整合回復のための `upgrade` 実行とその結果の default branch への反映）を含む日本語メッセージを標準エラーへ出して非0終了する。いずれの場合もレビュアを起動せず、審査対象の実行コードへフォールバックしない（要件4）。
...
- `E5 正準ツリー digest`: 配置場所・導入経路・時刻に依存しない単一値を与えることだけを責務とする。
  - 対象: パッケージ root 配下の通常ファイル。`node_modules/` 配下と `.git/` 配下は対象から除く。symbolic link は対象に含めず、対象範囲内に symbolic link を見つけた場合は算出を中止して非0終了する（リンク先の差し替えで実効内容が変わりうるため、安全側へ倒す）。
...
- `E7 launcher digest の算出対象`（`src/commands/gate.ts` の固定パス集合）: 要件6 の上限と下限の内側で算出対象を確定する。
  - 除外: `.agent-skill-chain/project/manifest.yaml` と `.agent-skill-chain/project/MODEL_TIER_TABLE.md`。いずれも配布集合の外にあり、要件6 の上限に反するため。
  - 追加: `.agent-skill-chain/scripts/cli-resolve.sh`。レビュア起動スクリプトが読み込んで実行する実行コードであり、本設計では調達段の実装も含むため、要件6 の下限に該当する。
  - 維持: 現行の 9 要素（`gate-local-review.sh`・`gate-launch-reviewer.sh`・`gate-review.sh`・3 adapter・`roles.yaml`・2 schema）と、「算出対象のいずれかを trusted base SHA から取得できない場合は部分集合で算出せず、取得できなかった要素を示す日本語メッセージを出して非0終了する」現行挙動。
...
- `E10 prompt 生成が読み込む asset の解決基点の維持`（`src/commands/gate.ts` の既存の解決基点）: レビュア起動段が読む asset（レビュー観点の template・schema・role contract・project policy 分類）の解決基点を、隔離 clone 内から起動された CLI プロセスの作業ディレクトリが指す protected base worktree の root、または隔離 clone のいずれかに保つ。Issue worktree を基点にしない。本設計はこの基点を変更せず、証跡投稿時の既存検査（作業ツリー root と repository root の一致、HEAD と trusted base SHA の一致）が Issue worktree を基点とした実行を拒否する構造も変更しない。
- `E11 検証`: 上記の振る舞いを固定する自動テスト。既存のローカルレビュー統合テストの stub 構成を、consumer 形状（配布集合と導入マーカーのみを持ち、期待値に一致する stub パッケージを隔離 clone の外に置く）と自リポジトリ形状（`agent-skill-chain` を名乗る `package.json` と build 定義を持つ）の両方へ拡張する。npm は `PATH` 先頭に置く記録用の代替コマンドで模し、実ネットワークへはアクセスしない。詳細な構成と網羅範囲は PLAN.md に置く。
...
- 既存の gate-local-review 統合テストが、変更後の期待値（`package_copy` では npm 呼び出し記録が空、`clone_build` では従来の 2 コマンド）へ更新されたうえで成功する。
...
- ローカルの package キャッシュだけが存在し、`node_modules` 配下にも `PATH` 上にも実体が無い実行環境については、キャッシュから実体を展開する導入操作を経て候補列挙の対象となる形へ帰着させる。準備段がキャッシュから直接展開することはしない（レジストリ到達性・ネットワークへゲート実行を依存させないため）。
