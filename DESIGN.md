...
既存 ADR-0021 が定めた Git から GitHub 本文への一方向転記、3設定、固定対象成果物、非致命性を変更しない。本 Issue は gate publish に限定されていた呼出し契機と、その実装で不足していた可逆無害化・厳密境界・排他的上限・SHA付きゲート表示を具体化するため、新しい長期判断や設定は追加せず proposed ADR は作成しない。
