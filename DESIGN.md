...
- `.agent-skill-chain/schemas/gate-report.schema.yaml`（既存、拡張）: `gate.light_review`（任意プロパティ：`requested`・`applied`・`disabled_reasons`・`remediation_round`・`strict_locked`）を追加。`strict_locked`は一度`true`になったら当該Issue/PRのゲートライフサイクル中`false`へ戻らない一方向ラチェットである。`disabled_reasons`は`applied=false`のときに、その理由を示す文字列配列。非空になるのは次の3ケースのみ。(1) `requested=true`かつ3層ガードレール該当により`strict_locked=true`が当該ラウンドで確定：`quick-mode.ts`の`GUARDRAIL_PATHS`と同型の日本語理由文字列。(2) `requested=true`かつ`grantorConfirmed=false`：「軽量シグナルの付与主体を人間と確認できませんでした」。(3) `requested=false`かつ`persistedStrictLocked=true`（過去のラウンドで`strict_locked=true`が確定済みのため、`review:light`ラベル除去後もStrict固定が解除されない）：「過去のラウンドで軽量プロファイルがStrictへ確定済みのため」。`requested=false`かつ`persistedStrictLocked=false`（一度も軽量プロファイルが要求されておらず、Strict固定も確定していない通常の未要求状態）のときは説明を要しないため常に空配列。`applied=true`のときも常に空配列。
