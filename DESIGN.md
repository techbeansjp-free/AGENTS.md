...
| 要件7 / `AC-8` | `src/lib/stale-assets.ts`（`Absent` 分岐） | AC-8 の Given は物理的不存在（ENOENT）であり要件7に対応する |
| 要件8 / `AC-10` | `src/lib/stale-assets.ts`（`Unreadable` 分岐） | AC-10 の Given は ENOENT 以外の理由での読み取り失敗であり要件8に対応する |
| 要件2〜3 / `AC-9` | `src/lib/stale-assets.ts`（差分計算：現行配布元に存在するパスは候補集合に入らない） | 「(b) を満たさないファイルは対象外」の具体的挙動は要件3が定める |
...
- `src/commands/upgrade.ts`（既存を拡張）: 既存のミラーコピー処理（`copyTreeMirror`）が返す実際の配布ファイルパス一覧を「現行配布元パス集合」として `stale-assets.ts` に渡し、削除候補の分類・（非dry-runなら）削除実行を行う。削除結果・警告を既存の `summary` へ既存のフォーマット（`${prefix}${action}: ${path}`）で追記する。削除失敗が1件以上あった場合は、成功した全結果を含む `summary` を先に標準出力へ書き出してから、失敗ファイルの一覧を含むメッセージで非ゼロ終了する（既存の `ok()`/`fail()` の単純な二者択一ではなく、両方を順に使う）。dry-run 時は所有権記録ファイルへの書き込みを一切行わない。非dry-run時のみ、処理完了後（削除失敗の有無に関わらず）、`ownership-record.ts` を通じて所有権記録を更新する。この更新では、削除に成功したファイル（`Deleted` 分岐）のエントリのみを次回記録から除去し、削除syscall自体が失敗したファイル（`DeleteFailed` 分岐）のエントリは除去せず保持する（`Unreadable` 分岐と同じ、次回実行での再試行を可能にする設計思想に揃える）。
...
stateDiagram-v2
  ...
  ContentMatch --> Deleted: dryRun=false かつ削除syscallが成功。次回記録からエントリを除去
  ContentMatch --> DeleteFailed: dryRun=false かつ削除syscallが失敗(権限不足・I/Oエラー等)。要件6・要件11のエラー集約に含め、upgrade全体を異常終了させる。次回記録にエントリを保持(再試行のため、Unreadableと同じ再試行可能性の設計思想)
...
- 想定される失敗モード（削除実行中のI/Oエラー）: 複数の削除候補のうち一部が失敗しても、失敗した候補の処理で全体を中断しない（best-effort継続）。全候補の処理が終わった後に、成功した更新・削除結果を含む `summary` を先に標準出力へ書き出し、その後で失敗ファイル一覧を含む非ゼロ終了を返す（要件6・要件11）。削除に失敗した個々のファイルのエントリは次回所有権記録から除去せず保持し、次回実行時に再び削除候補として検討できるようにする（`DeleteFailed` 分岐、`Unreadable` 分岐と同じ再試行可能性の設計思想）。
