...
- **slot**: 1 つの attempt の中で個々のレビュアへ割り当てられる識別子。attempt の開始時に要求体数の分だけ確定する。**起動済み slot の集合**は、そのうちレビュアの起動に成功した slot の集合をいい、起動に失敗した slot を含まない。その要素数が当該 attempt の実際の起動体数である。
...
`SPEC.md` は、代替判定基準の展開可否を「Issue 本文から要求・期待する挙動・受入基準に相当する記述を 1 件以上取り出して判定軸区間へ書き出せるか」で定め、取り出せる記述が 0 件であれば展開不能として `inconclusive 指示` へ倒すことを確定している。したがって抽出規則は「記述が空でないこと」ではなく「要求・期待する挙動・受入基準に相当する記述であること」を判定できなければならない。他方、判定機構もレビュアも自然文の意味的な十分性は判定できないため、判定は本文の構造だけで機械的に決まらなければならない。本設計はこの 2 つを、**本文の見出し構造による節の選別**として同時に満たす。
...
6. 残ったブロックを、すべての要求記述節にわたって原文の順序のまま連結したものを抽出結果とし、その空白文字を除いた総文字数を数える。総文字数が**実質性下限**（40 文字。コード上の固定定数とし、設定ファイル・環境変数・成果物のいずれからも与えない）以上である場合にのみ「展開できる」とし、抽出結果を判定軸区間へ書き出す。総文字数が下限未満である場合（残ったブロックが 1 件も無い場合を含む）は「展開できない」とする。要求記述節が 1 件も無い場合と、Issue 本文そのものを取得できない場合も「展開できない」とする。
7. 要求記述節以外の節と前文は、要求記述節の有無にかかわらず判定軸区間へ書き出さない。

**本規則の既定は「展開できない」である（安全側の既定）。** 上記の各段は、**展開できると判定してよい条件を網羅的に列挙したもの**であり、その条件をすべて満たす入力だけが「展開できる」となる。抽出のいずれかの段で、規則の前提が満たされない・規則が一意に適用できない・入力の構造から要求記述節を確定できないなど、要求・期待する挙動・受入基準に相当する記述を含むと確信できない場合は、例外なく「展開できない」とし `inconclusive 指示` へ倒す。これは `SPEC.md` が「取り出せる記述が 0 件なら展開不能」と定める要求と整合する——判定できない入力を 0 件側へ寄せることは、記述が 1 件以上あることを確信できない場合に展開しないという意味であり、当該要求に反しない。判定不能を展開可能側へ寄せると、実質的な判定軸が無い状態から `approved` へ到達しうるため、不変条件 I8（安全側ラチェット・silent pass 禁止）にも反する。

**なぜ「空でないこと」を採用条件にしないか。** 空でないブロックを内容によらず採用すると、`担当者: 未定`・`詳細は後日追記` のような管理上の記述だけが書かれた本文でも「展開できる」となる。このとき判定軸区間には、要求でも期待する挙動でも受入基準でもない文字列が唯一の判定軸として置かれ、レビュアは実質的な判定軸を持たないまま conformance を判定させられる。`SPEC.md` はこの入力を展開不能として `inconclusive 指示` へ倒すことを要求しており、空でないことだけを条件とする規則はその要求を満たさない。

**なぜ管理上の記述を列挙で排除せず、実質性下限で排除するか。** 記入省略表現の列挙（`未定`・`後日追記` 等）だけを排除条件にすると、列挙に無い書き方（`担当者: 未定`・`詳細は後日追記します` のように語を含むが完全一致しない 1 行）が排除されずに残り、実質的な判定軸が無いまま「展開できる」となる。この漏れは列挙を増やしても閉じない——自然文の言い換えは有限個の列挙に収まらず、列挙を足すたびに列挙外の表現が残るためである。そこで採否の決め手を列挙ではなく、抽出結果全体の分量という単一の下限に置く。列挙に載る記入省略表現はいずれも下限を大きく下回るため、下限は既存の列挙を包含する。既存の列挙は下限判定の前段で明らかな空記述を落とす目的で残すが、これ以上要素を増やさない。

**実質性下限の値と、それが生む誤りの向き。** 下限を 40 文字（空白文字を除く）とするのは、記入省略表現および `担当者: 未定` のような 1 行の管理記述がいずれも 20 文字未満に収まる一方、要求・期待する挙動・受入基準を 1 件でも述べた記述はこれを超えるためである。この値に意味論的な必然性は無く、下限未満の実質的な要求（極端に短い 1 行の要求）は「展開できない」と判定されうる。この誤りは判定軸を `inconclusive 指示` へ倒し最終判定を `human_required` にするだけであり、`approved` を導出しない。逆向きの誤り（管理上の記述しかない本文を展開可能と判定すること）は `approved` へ到達しうるため、2 つの誤りは回復可能性が対称でない。下限はコード上の固定定数とし、設定ファイル・環境変数・成果物のいずれからも与えない（判定を受ける側が下限を下げて採否を変えられないようにするため）。
...
**なぜ見出し一致で機械的に決まるか。** 上記 1〜7 はいずれも行の形（`#` の個数・囲み記号の並び）と文字列の完全一致、および文字数の計数だけで決まる。記述が要求として十分かどうかを意味から判定する段は無く、判定機構にもレビュアにも意味の解釈を要求しない。同じ本文に対して同じ結果が決定的に定まる。
...
**見出しを持たない本文の扱い。** 要求記述節が 1 件も無ければ展開不能となり、判定軸は `inconclusive 指示` へ倒れて最終判定は `human_required` となる。実質的な要求が書かれていても見出しが無ければこの経路へ倒れる。ただし結果は `approved` を導出しない側であり、不変条件 I8 に反しない。是正は当該 Issue 本文へ要求記述節の見出しを 1 行加えるだけで足り、その操作は Issue 本文を編集できる主体（人間・進行役）の権限で完結し、変更は本文上で可視である。逆向きの誤り（要求が書かれていない本文を展開可能と判定すること）は `approved` へ到達しうるため、2 つの誤りは回復可能性が対称でない。したがって本設計は見出しの欠落を安全側の展開不能として扱う。
...
| 起動済み slot の集合 | attempt へ割り当てられた slot 識別子のうち、レビュアの起動に成功したものの集合（空集合を含む） | 同上。起動に失敗した slot は含めない。要素数が実際の起動体数である |
...
**attempt の識別子と要求体数の供給元。** attempt の識別子・レビュープロファイル・要求体数・各レビュアの slot は、attempt を開始した起動側が attempt の開始時点で 1 度だけ確定させ、起動する各レビュアへ同一の値として渡す。起動済み slot の集合は、起動側が各 slot のレビュア起動の成否から確定させ、判定値を記録する経路へ同じく渡す。記録経路はこれらの値を受け取って集約に用い、受け取った判定値の件数から要求体数も起動体数も推定しない。判定値を受け取れなかった起動済み slot は入力から落とさず、判定結果へ `未確定` を与える。起動側が確定させた値を用いることで、起動に失敗したレビュアの分も母数に残る。
...
- **Issue 本文からの抽出規則**: 転記区間を除去したうえで、囲み区間（fenced code block・HTML コメント）の内部を見出し検出から除き、見出しが要求記述節ラベルへ一致する節を選別し、当該節の内容ブロックのみを採用する。採用したブロックの総分量が実質性下限に達する場合にだけ「展開できる」とし、それ以外の入力と判定できない入力はすべて「展開できない」へ倒す（判断 D6）。抽出結果が 0 件のときに展開不能として扱うことは `SPEC.md` が確定済みであり、本設計はその判定を変えない。
