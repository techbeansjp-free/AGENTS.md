...
| `AC-5`（既存動作への非破壊） | `test/unit/project-policy.test.ts`（新規）・`test/integration/issue-lifecycle.test.ts`・`test/integration/worker-adapters.test.ts`・`test/integration/lease-resume.test.ts`・`test/integration/github-backend.test.ts`（いずれも既存） | 新規回帰テストを`test/unit/project-policy.test.ts`へ追加する。既存の4件のintegrationテストは`segment start`の出力を正規表現部分一致（`assert.match`）で検証しており、`documents.common`の内容が出力へ追記されても既存アサーションは壊れないことを確認済みのため、変更は不要である |
...
- 影響を受ける既存機能: `agent-skill-chain segment start` の出力を消費する箇所（`.agent-skill-chain/scripts/worker-launch.sh` 経由でワーカーへ渡すプロンプト）。manifest.yaml未導入のconsumer projectでは出力が変わらないため影響なし。本リポジトリ（manifest.yaml導入済み）では出力にproject policy文書が追加されるため、`segment start` の出力を検証している既存の4件のintegrationテスト（`test/integration/issue-lifecycle.test.ts`・`test/integration/worker-adapters.test.ts`・`test/integration/lease-resume.test.ts`・`test/integration/github-backend.test.ts`）を確認した。いずれも正規表現部分一致（`assert.match`）で `role: <role>` 等の必要箇所のみを検証しており、出力末尾への追記に対して非破壊であることを確認済みのため、更新は不要（AC-5と同一の結論であり、本節と対応表AC-5行の間に矛盾は無い）。
