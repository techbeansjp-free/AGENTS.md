...
**Agent tool戻り値のコンテキスト混入への対応（design-gate round9指摘agent-tool-return-value-context-leakへの対応）**: 上記までの議論はcontract本文の往路（進行役→worker）にのみ着目していたが、復路（Agent tool呼び出しの戻り値＝workerサブエージェントの最終応答テキストが進行役の生成コンテキストへ読み込まれること）は別の論点であり、これまで検討していなかった。現行のheadless subprocess方式では、`bash -c "$worker_cmd" <"$prompt_file" &`のサブプロセス標準出力を呼び出し元シェルが単に破棄し（`wait "$worker_pid"`は終了コードのみを見る）、workerの自由文出力が進行役LLMの生成コンテキストへ入ることは構造的に無い。一方Agent tool呼び出しでは、呼び出したサブエージェントの最終応答テキストが呼び出し元LLM（進行役）のコンテキストへ必ず現れるため、この性質が失われる。

対応として、`_dispatch_via_agent_tool()`が組み立てる定型文（カスタムsubagent種別`agent-skill-chain-worker`への指示、下記コンポーネント構成参照）へ、次の制約を追加する: 「作業完了後の最終応答は、既存の`report_status`が使う固定スキーマ（完了状態・target_sha、`.agent-skill-chain/schemas/worker-report.schema.yaml`）相当の最小限のステータス要約——完了状態・target_sha・簡潔な1文要約——のみとし、成果物本文・diff・引用等の実質的内容を一切含めてはならない」。これは既存workerロールが既に持つ`report_status`という構造化された完了報告の概念と対称的な制約であり、新規の概念発明ではない。

ただし、この制約は`cat`方式（往路、AC-4の機械的完全性担保）とは異なり指示ベースであり、ツール制限やスキーマ強制のような構造的強制ではない。workerが指示に反し最終応答へ成果物の実質的内容を含めてしまう可能性は、design-gate round8で受容した「Readツール残存」（コンポーネント構成カスタムsubagent種別説明参照）と同種の、構造的強制ではなく指示ベースの残存リスクとして受容し、下記の障害・ロールバック考慮へ追記する（想定される失敗モード10）。

なお、既存のworker完了確認（`worker-launch-verify.sh`による`_asc_cli report latest`呼び出しとHEAD SHA照合）は、Agent tool呼び出しの最終応答テキストとは独立した別経路（git commit・`report_status` CLI呼び出し）で行われるため、本問題によってこの完了確認経路自体が損なわれることはない。
...
- 想定される失敗モード:
  ...
  10. workerが定型指示に反しAgent tool呼び出しの最終応答へ成果物本文・diff・引用等の実質的内容を含めてしまい、それが進行役の生成コンテキストへ読み込まれる（design-gate round9指摘agent-tool-return-value-context-leakへの対応、上記「Agent tool戻り値のコンテキスト混入への対応」参照）: 最終応答を完了状態・target_sha・簡潔な1文要約のみに限定する制約は定型指示ベースであり、ツール制限やスキーマ強制のような構造的強制ではないため、上記失敗モード9（`Read`ツール残存）と同種の既知の残存リスクとして受容する。既存のworker完了確認（`worker-launch-verify.sh`による`report latest`とHEAD SHA照合）はこの最終応答テキストとは独立した別経路（git commit・`report_status` CLI呼び出し）で行われるため、本リスクが顕在化しても完了確認経路自体は損なわれない。
