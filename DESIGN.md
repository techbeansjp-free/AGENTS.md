...
**SPEC の非退行保証との関係。** SPEC の要求節は「quick シグナルを解決できない実行環境で実行されるすべての PR」が本変更によって新たに失敗しないことを求め、その例として「本変更を含む CI ワークフローが未展開の配布先」と「API の一時障害・レート制限・権限未付与により Issue ラベルを取得できない環境」を挙げる。上記の残存ケースは、本変更を含むワークフローを展開したうえで配布先が自ら base 取得段を取り除いた環境であり、SPEC が例示した2類型のいずれにも当たらない。ただし文言上の「すべての PR」からは外れない。本設計は SPEC を変更しない。当該残存ケースを SPEC の例外として明記するか、配布先による定義の改変を非退行保証の前提外として扱うかの判断は、進行役へ照会する。
