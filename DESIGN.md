...
| `AC-2`（rejectedは引き続き失敗表現） | D1 | `gate.final`がpending以外に確定していれば（`conformance`/`falsification`が個別にpendingのまま提出されていても）従来通りexit 0で後続の`gate publish`（無変更）へ進む |
...
- `src/commands/verify.ts` の `gateReport()`（D1）: gate-reportの検証結果を「pending以外の理由による違反（otherErrors）」と「pendingであること自体（pendingErrors）」に分離して集計する責務のみを持つ。pendingErrorsへの計上基準は`gate.final === 'pending'`であることのみとする（`gate.final`が単一の権威あるフィールドである点は`.agent-skill-chain/schemas/gate-report.schema.yaml`の`final`定義、および`src/commands/gate.ts`の`deriveFinal()`/`aggregateVerdicts()`が判定確定を`final`のみで表現している既存実装と整合する）。`gate.final === 'pending'`の場合にのみ、`gate.conformance`/`gate.falsification`/`gate.final`各フィールドの案内メッセージを`pendingErrors`へ積む（白紙スキャフォールドでは3フィールドとも同時にpendingであるため、この場合の出力は現状と変わらない）。`gate.final`がpending以外（approved/rejected/human_required）に確定している場合は、`conformance`/`falsification`が個別に`pending`のまま提出されていても`gateReport()`はその2フィールドを一切チェックしない（otherErrorsにもpendingErrorsにも計上しない）。これは、standard profile（レビュア1体）で`falsification='fail'`（blocking finding付き）を提出しつつ`conformance`の網羅チェックを`pending`のまま提出する組み合わせが現実的に起こり得るため、`report.gate.final='rejected'`かつ`report.gate.conformance='pending'`という組み合わせがスキーマ的に正当（`isEvidenceVerdict()`にも`gate-report.schema.yaml`にもconformance/falsification/finalの整合を強制するクロスフィールド制約が無い）だからである。判定基準を`conformance`/`falsification`のいずれかのpendingにまで広げると、この組み合わせを誤って「未レビュー」（exit 2）と扱ってしまい、既に確定した`rejected`判定とblocking findingsがD2でpublishされずに失われる（design-gate指摘: pending-fast-path-swallows-rejected）。`otherErrors`が1件でもあればexit 1（従来通り）。`otherErrors`が0件で`pendingErrors`が1件以上ならexit 2（新設、pending専用）。両方0件ならexit 0。スキーマ検証・target_sha検証・approved_artifacts検証のロジック自体は変更しない。
