...
- 想定される失敗モード（6）: 本修正のマージ前から存在しmainを未取込のissueブランチへ、writer権限を持つ者が旧`on: push`版`agent-skill-chain-reconcile.yml`（当該ブランチ内にファイル名として同一のまま残存、自己参照構成）を改変してpushし、`agent-skill-chain/{spec,design,implementation,validation}-gate`のいずれかの偽Check Run（`conclusion=success`）を発行させても、`resolveApprovedBaseline`のsource identity検証がpathは一致するがevent（`workflow_runs[].event`）は`push`であり`pull_request_target`と不一致であることを検出し、当該候補を棄却する（下記「移行ギャップについて」参照）。
