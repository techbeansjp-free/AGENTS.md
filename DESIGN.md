...
SPEC.mdは「未対応のレビュー・コメント」の検出を要求するが、GitHubのコメントAPI自体には「対応済み/未対応」を表すフラグが無いため、機械的に判定可能な基準を本設計で確定する。本節の基準はADR-0025→ADR-0026→ADR-0027の置き換えを反映した最新版であり、ADR-0025が採用していた「対象ブランチの最新commit時刻より後」というコメント時刻カットオフは廃止済み（理由は下記参照）、ADR-0026が採用していたレビュー判定基準（`latestReviews`の`state === 'CHANGES_REQUESTED'`）はADR-0027でさらに置き換える（理由は次項）。
...
src/commands/segment.ts（start）
  ...
  └─ backend === 'github' → src/lib/review-status.ts: detectGithubReviewStatus()
                              ...
                              └─ PR側（branch解決成功時のみ）
                                  ...
                                  → src/lib/exec.ts: gh()（`api repos/{owner}/{repo}/pulls/<pr番号>/comments`、PR OPEN時のみ、2026-08-05再改定、ADR-0027）
```
...
graph TD
  ...
  PV -->|成功・state!==OPEN| GPS0["prResult=成功（PR無しと同一視、0件）"]
  PV -->|成功・state===OPEN| PVC["gh api pulls/&lt;pr番号&gt;/comments（インラインレビューコメント）"]
...
本Issue専用のADRとして、`docs/adr/ADR-0025-worker-resume-review-feedback-detection.md`（初版、レビュー基準・時刻カットオフ・同名gate report限定の各判定基準を確定、design-gate承認済みで一度 `accepted` になったのち `superseded` へ遷移）、それをsupersedeする `docs/adr/ADR-0026-worker-resume-review-feedback-detection-cross-commit-and-cross-segment.md`（`status: accepted`、コメント時刻カットオフの廃止とローカルモード全segment走査への変更を確定した後 `superseded` へ遷移）、さらにそれをsupersedeする `docs/adr/ADR-0027-worker-resume-review-feedback-detection-full-review-history-and-inline-comments.md`（`status: proposed`、レビュー判定基準を`latestReviews`ベースから全レビュー履歴ベースへ変更し、インラインレビューコメント（review thread comment）の取得を追加）の3件を作成した。ADR本文の不変原則（accepted後は書き換え不可）により、判定基準の変更はADR-0026の本文修正ではなく新ADR（ADR-0027）の作成で反映する。既存の他Issueの `accepted` ADRで本設計が直接 `adopts` するものは無いため、`related_adrs` は空にする（本文中の自然文言及のみ行う）。
