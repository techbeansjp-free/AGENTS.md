...
5. 要求記述節の内容を空行区切りのブロックへ分割し、(a) 空白のみ、(b) HTML コメントのみ、(c) 水平線のみ、(d) **記入省略表現のみ**からなるブロックを捨てる。記入省略表現とは、正規化後に `無し` / `なし` / `特になし` / `未定` / `不明` / `tbd` / `n/a` / `none` / `後日追記` のいずれかへ完全一致する 1 行のブロックをいい、これも同様にコード上の固定列挙とする。ここでの正規化は、前後の空白除去 → **行頭の Markdown リスト記号の除去** → 英字の小文字化 → 末尾の句読点除去の順で適用する。行頭の Markdown リスト記号とは、行頭の `-`・`*`・`+` のいずれか 1 文字、または 1 個以上の半角数字に `.` もしくは `)` が続く並びであって、いずれもその直後に 1 個以上の空白が続くものをいう（直後に空白が無い並びは Markdown のリスト項目を構成しないため除去しない）。リスト記号を除去したら残りの行の前後の空白を再び除去し、行頭にリスト記号がなお現れる限りこの除去と空白除去を繰り返す。
...
**なぜ記入省略表現の判定で行頭のリスト記号を正規化するか。** Issue 本文の記入は箇条書きで行われることが多く、`## 受入基準` の内容が `- TBD` の 1 行だけ、という本文は通常の Markdown の Issue として到達可能である。行頭のリスト記号を正規化しなければ、この行は正規化後に `- tbd` となって固定列挙の `tbd` へ完全一致せず、記入省略表現として捨てられない。結果として実質的な受入基準が 0 件であるにもかかわらず「展開できる」となり、記入省略表現だけが判定軸の実体として判定軸区間へ書き出される。しかし `- TBD` と `TBD` は同一の記入省略表現であり、違いは Markdown のリスト項目としての記号が付いているかどうかだけである。**したがってこれは固定列挙の不足ではなく正規化規則の不足であり、対処は列挙の追加ではなく正規化を 1 段追加することである。** 追加する段は既存の正規化（前後の空白除去・英字の小文字化・末尾の句読点除去）と同じ性質を持ち、行の形だけで決まり、記述の意味も分量も参照しない。またこの段は捨てられるブロックを増やす向きにしか働かないため、判定は「展開できない」＝ `approved` を導出しない側へのみ倒れ、安全側である。リスト記号を除去した結果が空行になった場合も、当該ブロックは空白のみのブロックとして捨てられ、同じ向きへ働く。記入省略表現の固定列挙自体はこの追加によって 1 件も増えず、後掲「本規則が残す誤りと、その射程」が定める列挙の非増加の方針と矛盾しない。
