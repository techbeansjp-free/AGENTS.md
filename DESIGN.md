...
- `agent-skill-chain-gate.yml`（既存、trusted）`detect-segments`ジョブへの追加ステップ`Detect unmigrated reconcile workflow`（design-gate指摘`migration-step-has-no-automated-enforcement-or-detection`対応）: 既存の`Fetch target as read-only Git object`ステップが取得済みのPR head SHAに対し、`git show <head_sha>:.github/workflows/agent-skill-chain-reconcile.yml`をread-only参照し、内容が`on: push`を含む（本Issue以前の自己参照構成のまま残存している）場合、`::warning::`アノテーションを出力し、`gh api`で当該PRへ本文固定の警告コメント（既存の同一マーカー文字列を検索し重複投稿は更新のみに留める）を投稿する。判定結果は既存のいずれのrequired status check（`agent-skill-chain/{spec,design,implementation,validation}-gate`・`verify`）の成否にも影響させない（新規の必須チェックを追加しない。既存PRの必須チェック構成`.agent-skill-chain/templates/github/provisioning/rulesets/main.json`の変更を伴わない）。これは`git worktree list`等に依存する人手中心の確認手順（PLAN.md変更単位#7）を代替するものではなく、開いている全PRについてsynchronizeのたびに自動的に未移行状態を検知・可視化する手段を追加するものである。
...
- 影響を受ける既存機能: `agent-skill-chain/{spec,design,implementation,validation}-gate`のreconcileによる再発行・無効化が主。`agent-skill-chain-gate.yml`（PR gate本体）は`detect-segments`ジョブへの追加ステップ（未移行reconcile検知、上記「コンポーネント構成」参照）のみを変更対象とし、既存のsegment検出・issue_id解決・レビューworker起動ロジック自体は無変更。`agent-skill-chain-ci.yml`（CI検査）は本設計の変更対象に含まれず影響を受けない。`issues/<n>/.agent-skill-chain/reviews/<gate>.yaml`自体の生成・スキーマは無変更（ローカルモードの正本、GitHubモードでもワーカー向けスキャフォールドとして引き続き使用）。
