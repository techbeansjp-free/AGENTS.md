...
|---|---|---|
| `AC-1`（新規配布物からの除外） | D1: `agent-skill-chain-release.yml`のテンプレート側削除 | `init`/`upgrade`は`.github/`自体を一切コピー対象にしない（`NAMESPACED_ENTRIES`定数に`.github`を含まない。`init`は「GitHub workflowは未展開です」と明示出力する）ため、AC-1を実質的に検証できるのは`.github/`を実際に展開する`setup github`（内部で`templates/github/.github`を`copyTreeMirror`する）のみである。テンプレート側の物理削除により、`setup github`が展開する`.github/workflows/`にも当該ファイルは含まれなくなる。AC-1のGiven/Whenが要求する「`setup github`を実際に実行し展開先を確認する」振る舞いは、D4(e)の`test/integration/setup.test.ts`へ追加するintegrationテストが自動検証する（単体テストD4(a)は配布元テンプレートの静的な存在確認に留まり、この振る舞い自体は検証しない） |
| `AC-2`（既存consumerへの`setup github`再実行でも新規配布されない） | D1 | `upgrade`は`.agent-skill-chain/`配下の正本アセットのみを更新し`.github/`には触れない。`.github/`を実際に更新するのは`setup github`のみであり、これが使う`copyTreeMirror`はsource（テンプレート）側の走査のみで動作するため、D1（テンプレート側削除）のみで満たされる。AC-2のGiven/Whenが要求する「既存consumerに対し`upgrade`実行後`setup github`を実行し展開先を確認する」振る舞いは、D4(f)の`test/integration/upgrade.test.ts`へ追加するintegrationテストが自動検証する |
...
- **D4: 回帰テスト・実機検証**: (a) `.agent-skill-chain/templates/github/.github/workflows/`配下に`agent-skill-chain-release.yml`が存在しないことを検査する単体テスト、(b) 本体`.github/workflows/agent-skill-chain-release.yml`が存在し既存のステップ構成を保持していることを検査する単体テスト（AC-3の静的側面）、(c) `computeTemplateSyncDiffs`（またはCLI経由の`verify template-sync`）が「展開先にのみ存在し配布元に無いファイル」を差分として報告しないことを確認する回帰テスト（D3の設計意図を将来の実装変更から保護するためのテスト、AC-4）、(d) `node bin/agents-md.js setup github <tmpdir>`実機実行（`.github/`を実際に展開する処理。`init`単独は`.github/`自体を生成しないため対象にならない）による目視確認（AC-7、VALIDATION.mdに記録）、(e) `setup github`コマンドを`test/integration/setup.test.ts`の既存テスト群（`runCli`ヘルパー経由でCLIをsubprocess実行し、`mkScratch`で作成した一時ディレクトリへ実際に展開して`fs.existsSync`等でファイルシステムを検査する構造）に倣い、新規の一時target_dirへ実際に実行し、展開された`<target_dir>/.github/workflows/`に`agent-skill-chain-release.yml`が含まれないことをファイルシステム上で検証するintegrationテスト（AC-1のGiven/Whenをそのまま自動化するテスト。単体テストの(a)は配布元テンプレート自体の静的確認に留まりCLI実行を伴わないため、AC-1が要求する「`setup github`実行→展開先確認」という振る舞いはこの(e)が担う）、(f) `test/integration/upgrade.test.ts`の既存テスト群（`runCli(['init', targetDir])`で導入した状態を起点に`runCli(['upgrade', targetDir])`を実行しファイルシステムの変化を検証する構造）に倣い、`init`実行済み・`setup github`実行済みで`agent-skill-chain-release.yml`を保持しない既存consumer相当の一時ディレクトリに対し、`upgrade`実行後に`setup github`を再実行し、`<target_dir>/.github/workflows/agent-skill-chain-release.yml`が作成されないことを検証するintegrationテスト（AC-2のGiven/Whenをそのまま自動化するテスト）。PLAN.mdで変更単位として具体化する。
- **D5: `SECURITY_POLICY.md`追記 および `agent-skill-chain-root-cleanup.yml`ヘッダコメントの整合性修正**（いずれも配布物）:
  ...
  - 責務境界: `agent-skill-chain-root-cleanup.yml`のジョブ定義・トリガー・`secrets.RELEASE_MAIN_PAT`という名称・ステップ構成・`permissions`は一切変更しない（ADR-0017 Decision参照、SPEC.md AC-5の「ドキュメント化」選択肢を採用）。SPEC.mdスコープ外条項は`agent-skill-chain-release.yml`以外のワークフロー（`agent-skill-chain-root-cleanup.yml`を含む）のジョブ定義・トリガー・ステップ構成・`permissions`の変更を対象外としつつ、(1) AC-5が要求する`secrets.RELEASE_MAIN_PAT`という名称自体への方針反映と、(2) AC-1による`agent-skill-chain-release.yml`除外の不可避な帰結として、同ファイルをヘッダコメントがファイル名で名指ししている箇所（除外後は配布物内に存在しないファイルを指す記述になる）をファイル名に依存しない表現へ書き換えることの2点を、明示的に対象内の例外として定めている。(b)はこの例外(2)に該当するコメント1文の書き換えのみであり、ジョブの動作には一切影響しない。
