...
1 つの attempt では、レビュープロファイルに応じて 1 体または 2 体のレビュアが起動される。最終判定は attempt 全体でただ 1 つ導出し、導出は次の順序で行う。

| 順序 | 条件 | 最終判定 |
|---|---|---|
| 1 | レビュアが 1 体も起動されず、判定値が 1 件も存在しない | `human_required` |
| 2 | 起動されたレビュアの体数が要求体数に満たない、または起動されたレビュアのうち 1 体以上の判定値を確定できない（タイムアウト・異常終了・スキーマ不適合等の不正応答。起動された全部が返さない場合を含む） | `human_required` |
| 3 | 順序 1・2 に該当せず、いずれかのレビュアのいずれかの観点が `fail` であるか、いずれかのレビュアが blocking finding を挙げている | `rejected` |
| 4 | 順序 1〜3 に該当せず、いずれかのレビュアのいずれかの観点が `pending` である | `human_required` |
| 5 | 順序 1〜4 に該当せず、全レビュアの両観点が `pass` であり blocking finding が 1 件も無い | `approved` |

起動体数が要求体数を上回る場合は、起動された全レビュアの判定値を集約の対象とし、その全員分がそろっていることを順序 3〜5 の共通の前提とする。要求体数が 1 体で、その 1 体が起動されて判定値を返した attempt では、集約結果は当該レビュアの判定値そのものと一致する。
...
**ローカルモードの結線は attempt 単位でまとめてから導出する。** 現行のローカルモードの結線経路は、レビュア 1 体分の判定値を受け取るたびにゲート状態を上書きする。strict プロファイルではこの経路が 2 回実行されるため、最後に結線されたレビュアの判定値だけが最終判定を決め、先行するレビュアの `fail`・blocking finding が失われる。本設計では、同一 attempt に属する判定値を 1 つの集合としてまとめたうえで集約規則を適用し、要求体数に満たない集合からは `approved` を導出しない。要求体数は結線経路が受け取った判定値の件数からではなく、当該 attempt のプロファイルから解決する。

**attempt の識別子と要求体数の供給元。** attempt の識別子・レビュープロファイル・要求体数・各レビュアの slot は、attempt を開始した起動側が attempt の開始時点で 1 度だけ確定させ、起動する各レビュアへ同一の値として渡す。判定値を記録する経路はこの値を受け取って集約に用い、受け取った判定値の件数から要求体数を推定しない。起動側が確定させた値を用いることで、起動に失敗したレビュアの分も母数に残る。

**要求体数を解決できない場合。** attempt のレビュープロファイルを解決できない場合は、要求体数が確定しないため集約結果も確定しない。この場合も `approved` を導出せず `human_required` を導出する。プロファイルを既定値へ落として要求体数を 1 体とみなすと、プロファイル取得の失敗によって strict の要求が消える経路が生じ、母数を要求体数に置いた意味が失われる。
...
- `レビュア起動`（既存のシェル層）: アダプタ起動の前に既に `判定プロンプト生成コマンド` を呼び、その非 0 終了を既存のフェイルセーフ（最終判定を人間判断へ倒す）として扱っている。本設計はこの配線をそのまま用い、中断判定のための新たな分岐もサブコマンドも追加しない。attempt の識別子・プロファイル・要求体数・slot は既に起動側が確定させて各レビュアへ渡しているため、判定値の記録経路がこれらを受け取るための新たな解決処理も設けない。
- `判定値の集約と最終判定の導出`（新規モジュール、純関数のみ）: 要求体数と、attempt に属する判定値の集合から最終判定を導く。要求体数は引数として受け取り、判定値の件数から推定しない。Coordination Backend の別を知らない。
- `ゲート状態への判定値の結線`（既存、変更）: ローカルモードで判定値をゲート状態へ書き込む経路。同一 attempt に属する判定値をまとめ、attempt の識別子・プロファイル・要求体数とともに `判定値の集約と最終判定の導出` へ渡し、返った最終判定を書き出す。判定がそろわない場合も最終判定を書き出してから非 0 で終了する。
...
証跡投稿・検証済みゲート状態の組み立て → 判定値の集約と最終判定の導出（要求体数と判定値集合を引数として渡す）
...
- **Issue 本文からの抽出規則**: 転記区間を除去したうえでの本文ブロック採用とする（判断 D6）。抽出結果が 0 件のときに展開不能として扱うことは `SPEC.md` が確定済みであり、本設計はその判定を変えない。
