...
|---|---|---|
| `AC-1`（pendingでSUCCESS化+action_required発行） | D1: `verify.ts gateReport()` の exit code分離、D2: `agent-skill-chain-gate.yml` の `Verify gate report schema` ステップ改修 | D1がpendingのみを検出可能にし、D2がそれを受けてCheck Runを発行する |
...
- `.github/workflows/agent-skill-chain-gate.yml` の `verify-and-publish`ジョブ内`Verify gate report schema`ステップ（D2）: `set +e`でexit codeを捕捉し、code=2の場合のみ既存の`Verify local-review evidence`ステップと同じ形式（`node -e` + `gh api check-runs`）で`agent-skill-chain/<gate>-gate`という名前のaction_required Check Runを`target_sha`へ発行してからexit 0する。code=2以外（0または1）は現状の挙動をそのまま維持する（0=素通り、1=exit "$CODE"でジョブ失敗）。
...
```text
gateReport()（D1） → verify-gate-report.sh（無変更） → agent-skill-chain-gate.yml「Verify gate report schema」ステップ（D2）
...
D1はD2からのみ呼び出され、D3はD1と独立（detect-segmentsジョブとverify-and-publishジョブは別ジョブであり、循環はない）。D2は既存の`Verify local-review evidence`ステップが持つCheck Run発行ロジック（`node -e`＋`gh api`パターン）を再利用するのみで新規外部依存を導入しない。
