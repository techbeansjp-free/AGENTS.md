...
これに対し、取得深度の切り詰めのように差分算出自体が失敗する改変は、根拠(3) のとおり後続段が同一の実行条件でパイプを介さず同形式の差分算出を行うため現状でも既に赤く、(b) は新たな失敗を生まない。すなわち残存ケースは「S 導出段だけが失敗し、後続段は失敗しない改変」に限られる。
...
この例外の適用範囲は、本節が残存ケースとして特定した状態——配布先が展開結果と配布テンプレートの双方から base 取得段を取り除いた（または同段の実行条件を偽へ固定した）環境——と過不足なく一致する。すなわち残存ケースの PR は SPEC 要求節の「quick シグナルを解決できない実行環境で実行されるすべての PR」から除外され、(b) の適用が SPEC の非退行保証に対する反例を生むことはない。SPEC が非退行保証の例として挙げる「本変更を含む CI ワークフローが未展開の配布先」と「API の一時障害・レート制限・権限未付与により Issue ラベルを取得できない環境」の2類型は、いずれも base 取得段を保つ定義を用いる環境であり、根拠(1)〜(3) により (b) の適用後も従来どおり緑のまま保たれる。本設計は SPEC を変更しない。
