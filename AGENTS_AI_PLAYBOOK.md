...
7. メモや調査メモは、`.workflow/{issue}/memo/` 配下に  
   **必ず** `YYYYMMDD_HHMMSS_タイトル.md` 形式（日時プレフィックス必須）で作成案を出す。

   - 日時プレフィックスは `YYYYMMDD_HHMMSS` 形式（例: `20251125_143022`）
   - **日時の取得方法**: システムの現在日時を取得する機能（例: `date` コマンド、システム API 等）を使用して現在日時を取得し、それを `YYYYMMDD_HHMMSS` 形式にフォーマットしてプレフィックスとして使用する
   - 日時は作成時点の現在日時を使用する（推測や記憶に基づく記載は禁止）
   - 日時プレフィックスなしのファイル名は提案しない
...
- テストファーストを想定した順序で提案する：
  ...
  2. テストコードのドラフト
  3. 実装コードのドラフト
- **テストコード内でのBDD形式コメント必須**: すべてのテストコード内で、BDD形式（Given-When-Then-And）のコメントを記載する
...
- バグや課題のメモ → `memo/YYYYMMDD_HHMMSS_*.md` の新規案（**日時プレフィックス必須**、形式: `YYYYMMDD_HHMMSS_タイトル.md`）
  - 日時はシステムの現在日時を取得する機能（例: `date` コマンド、システム API 等）を使用して取得し、`YYYYMMDD_HHMMSS` 形式にフォーマットする
...
- 必要なら `memo/YYYYMMDD_HHMMSS_タイトル.md` の新規作成案（**日時プレフィックス必須**、日時はシステムの現在日時を取得する機能（例: `date` コマンド、システム API 等）を使用して取得し、`YYYYMMDD_HHMMSS` 形式にフォーマットする）
- 実行コマンド例（※実際の実行はしない）
